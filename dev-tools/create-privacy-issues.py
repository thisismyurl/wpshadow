#!/usr/bin/env python3
"""
Create 12 Privacy Diagnostic GitHub Issues for WPShadow
...
- Database scan: Identify email collection tables and retention periods""",
    ...
    {
        "title": "Diagnostic: Analytics Loading Before Consent Detection (CRITICAL)",
        "body": """## Description
Detect if tracking scripts (Google Analytics, hotjar, Mixpanel, etc.) load before user consent is obtained, which violates GDPR/CCPA. This is one of the most common GDPR violations.
...
## Critical Note
This is one of the most enforced GDPR violations. Many site owners unknowingly load GA before consent and face significant fines.""",
...
- Cron verification: Ensure cleanup jobs scheduled""",
    ...
    {
        "title": "Diagnostic: Deleted User Data Cleanup Verification (Right to Erasure - CRITICAL)",
        "body": """## Description
Verify that when users are deleted, all associated personal data is properly removed from database, including orphaned meta records, comments, and logs. This is a GDPR Article 17 "Right to Erasure" requirement.
...
## Critical Note
If users cannot delete their data, the site violates GDPR Article 17. This affects regulatory compliance in EU and similar jurisdictions worldwide.""",
...
def main():
    ...
    if not token:
        ...
        print("   Set GITHUB_TOKEN environment variable or configure git:")
    ...
    for i, issue in enumerate(ISSUES, 1):
        ...
        if result and "number" in result:
            ...
            created.append((issue_num, issue["title"]))
    ...
    if created:
        print(f"\n🎉 Next step: Visit https://github.com/{OWNER}/{REPO}/issues")
        print("   Your issues are ready for implementation!")
