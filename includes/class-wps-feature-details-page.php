...
class WPSHADOW_Feature_Details_Page {
	...
	public static function set_page_title(): void {
		...
		if ( $feature_id ) {
			...
			if ( $feature ) {
				$title = $feature['name'] ?? __( 'Feature Details', 'plugin-wpshadow' );
				return;
			}
		}
		$title = __( 'Feature Details', 'plugin-wpshadow' );
	}
	...
	public static function filter_admin_title( string $admin_title, string $title ): string {
		...
		if ( $screen && 'admin_page_wpshadow-feature-details' === $screen->id ) {
			...
			return __( 'Feature Details', 'plugin-wpshadow' ) . ' ' . $admin_title;
		}
		return $admin_title;
	}
	...
	public static function render_page(): void {
		...
		add_meta_box(
			...
			__( 'Feature Information', 'plugin-wpshadow' ),
		...
		add_meta_box(
			...
			__( 'Activity Log', 'plugin-wpshadow' ),
	...
	public static function render_feature_info_metabox(): void {
					...
					<td class="check-column" style="width: 60px; padding: 12px; border: none;">
						<label class="wps-feature-toggle-label">
							...
							<span class="screen-reader-text">
								<?php esc_html_e( 'Enable this feature', 'plugin-wpshadow' ); ?>
							</span>
						</label>
					</td>
		...
		<?php if ( ! empty( $sub_features ) ) : ?>
			<div class="feature-sub-features-section <?php echo ! $is_enabled ? 'feature-settings-disabled' : ''; ?>" style="margin-top: 15px;">
				<h3 style="margin-top: 0;"><?php esc_html_e( 'Feature Settings', 'plugin-wpshadow' ); ?></h3>
				<p class="description">
							...
							<tr>
								<td class="check-column" style="width: 60px; padding: 12px;">
									<label class="wps-feature-toggle-label">
										<input 
											type="checkbox" 
											class="wps-feature-toggle-input wpshadow-sub-feature-toggle"
											data-feature-id="<?php echo esc_attr( $feature_id ); ?>"
		...
		<div class="postbox-actions" style="float: right; margin: -10px 0 10px 10px;">
			...
			<?php if ( ! empty( $activity_log ) ) : ?>
				<button type="button" 
					...
					<?php esc_html_e( 'Clear History', 'plugin-wpshadow' ); ?>
				</button>
			<?php endif; ?>
		</div>
	...
	public static function ajax_toggle_feature(): void {
		...
		if ( $result ) {
			...
			wp_send_json_error( array( 'message' => __( 'Couldn\'t toggle that feature.', 'plugin-wpshadow' ), 'debug' => 'set_feature_enabled returned false' ) );
		}
	}
	...
	public static function ajax_clear_feature_log(): void {
		...
		if ( $result ) {
			wp_send_json_success( array(
				'message' => __( 'Activity log cleared successfully.', 'plugin-wpshadow' ),
			) );
		} else {
			wp_send_json_error( array( 'message' => __( 'Failed to clear activity log.', 'plugin-wpshadow' ) ) );
		}
	}
