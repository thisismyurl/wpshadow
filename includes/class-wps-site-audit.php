...
class WPSHADOW_Site_Audit {
	...
	public static function handle_toggle_autoload(): void {
		...
		if ( empty( $option_name ) ) {
			wp_send_json_error( __( 'That option doesn\'t exist', 'wpshadow' ) );
		}
		...
		if ( false === $result ) {
			wp_send_json_error( __( 'Couldn\'t update that option', 'wpshadow' ) );
		}
		...
		wp_send_json_success(
			array(
				'message' => sprintf(
					...
					__( 'Disabled autoload for %s', 'wpshadow' ),
					$option_name
				),
			)
		);
	...
	public static function render_audit_page(): void {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Site Audit', 'wpshadow' ); ?></h1>
			<p><?php esc_html_e( 'Comprehensive analysis of site performance, security, and optimization opportunities.', 'wpshadow' ); ?></p>

			<button id="wps-audit-generate" class="button button-primary">
				<?php esc_html_e( '📋 Generate New Audit', 'wpshadow' ); ?>
			</button>

			<?php if ( ! $latest ) : ?>
				<p style="margin-top: 20px; padding: 15px; background: #f0f0f0; border-left: 4px solid #ffb900;">
					<?php esc_html_e( 'No audits generated yet. Create one to see recommendations.', 'wpshadow' ); ?>
				</p>
			<?php else : ?>
				<div style="margin-top: 30px;">
					<h2><?php esc_html_e( 'Latest Audit Report', 'wpshadow' ); ?></h2>
					...
					<?php if ( ! empty( $latest['security']['issues'] ) ) : ?>
						<div style="margin: 20px 0; padding: 15px; background: #fee; border-left: 4px solid #c00;">
							<h3><?php esc_html_e( '🚨 Security Issues Found', 'wpshadow' ); ?> (<?php echo intval( count( $latest['security']['issues'] ) ); ?>)</h3>
					...
					<?php if ( ! empty( $latest['php_compat']['issues'] ) ) : ?>
						<div style="margin: 20px 0; padding: 15px; background: #fee; border-left: 4px solid #ff6600;">
							<h3><?php esc_html_e( '⚠️ PHP Compatibility Issues', 'wpshadow' ); ?> (<?php echo intval( count( $latest['php_compat']['issues'] ) ); ?>)</h3>
							<p style="margin: 10px 0; font-size: 13px;">
								<?php
								printf(
									esc_html__( 'Current PHP Version: %1$s | Recommended: %2$s', 'wpshadow' ),
					...
					<?php if ( ! empty( $latest['broken_links']['broken_count'] ) ) : ?>
						<div style="margin: 20px 0; padding: 15px; background: #ffe; border-left: 4px solid #ff6600;">
							<h3><?php esc_html_e( '🔗 Broken Links Found', 'wpshadow' ); ?> (<?php echo intval( $latest['broken_links']['broken_count'] ); ?>)</h3>
							<p style="margin: 10px 0; font-size: 13px; color: #666;">
								<?php
								printf(
									esc_html__( 'Checked %1$d links and found %2$d broken.', 'wpshadow' ),
							...
							<ul style="margin: 10px 0; padding-left: 20px; max-height: 300px; overflow-y: auto;">
								...
								<?php if ( count( $latest['broken_links']['links'] ) > 10 ) : ?>
									<li style="font-style: italic; color: #666;"><?php printf( esc_html__( 'And %d more...', 'wpshadow' ), count( $latest['broken_links']['links'] ) - 10 ); ?></li>
								<?php endif; ?>
							</ul>
						</div>
					<?php endif; ?>
					...
					<?php if ( ! empty( $latest['missing_alt']['missing_count'] ) ) : ?>
						<div style="margin: 20px 0; padding: 15px; background: #ffe; border-left: 4px solid #ffb900;">
							<h3><?php esc_html_e( '🖼️ Missing Alt Tags', 'wpshadow' ); ?> (<?php echo intval( $latest['missing_alt']['missing_count'] ); ?>)</h3>
							<p style="margin: 10px 0; font-size: 13px; color: #666;">
								<?php
								printf(
									esc_html__( 'Found %1$d images without alt tags out of %2$d total images checked.', 'wpshadow' ),
							...
							<ul style="margin: 10px 0; padding-left: 20px; max-height: 300px; overflow-y: auto;">
								...
								<?php if ( count( $latest['missing_alt']['images'] ) > 10 ) : ?>
									<li style="font-style: italic; color: #666;"><?php printf( esc_html__( 'And %d more...', 'wpshadow' ), count( $latest['missing_alt']['images'] ) - 10 ); ?></li>
								<?php endif; ?>
							</ul>
						</div>
					<?php endif; ?>
					...
					<?php if ( ! empty( $latest['optimization']['suggestions'] ) ) : ?>
						<div style="margin: 20px 0; padding: 15px; background: #eff; border-left: 4px solid #0073aa;">
							<h3><?php esc_html_e( '⚡ Optimization Opportunities', 'wpshadow' ); ?> (<?php echo intval( count( $latest['optimization']['suggestions'] ) ); ?>)</h3>
							<ul style="margin: 10px 0; padding-left: 20px;">
						...
						<div style="margin: 20px 0; padding: 15px; background: <?php echo ( $latest['autoload']['offenders_500kb'] > 0 ) ? '#fee' : '#ffe'; ?>; border-left: 4px solid <?php echo ( $latest['autoload']['offenders_500kb'] > 0 ) ? '#c00' : '#ffb900'; ?>;">
							<h3>
								...
								if ( $latest['autoload']['offenders_500kb'] > 0 ) {
									esc_html_e( '⚠️ Autoload Audit (wp_options) - Critical Offenders Found', 'wpshadow' );
								} else {
									esc_html_e( '⚡ Autoload Audit (wp_options)', 'wpshadow' );
								}
								?>
							</h3>
							<p style="margin: 10px 0; font-size: 13px; color: #666;">
								...
								printf(
									...
									esc_html__( 'Total autoloaded data: %s. Every row with autoload=yes is loaded into memory on every request.', 'wpshadow' ),
							...
							<?php if ( $latest['autoload']['offenders_500kb'] > 0 ) : ?>
								<p style="margin: 10px 0; font-size: 13px; color: #c00; font-weight: bold;">
									...
									printf(
										...
										esc_html__( '⚠️ %d option(s) over 500KB found! These should be disabled from autoload.', 'wpshadow' ),
							...
							<table style="width: 100%; font-size: 13px; margin-top: 10px; border-collapse: collapse;">
								<thead>
									<tr style="background: rgba(0,0,0,0.05); border-bottom: 2px solid rgba(0,0,0,0.1);">
										<th style="padding: 8px; text-align: left;"><?php esc_html_e( 'Option Name', 'wpshadow' ); ?></th>
										<th style="padding: 8px; text-align: right;"><?php esc_html_e( 'Size', 'wpshadow' ); ?></th>
										<th style="padding: 8px; text-align: center;"><?php esc_html_e( 'Severity', 'wpshadow' ); ?></th>
										<th style="padding: 8px; text-align: center;"><?php esc_html_e( 'Action', 'wpshadow' ); ?></th>
									</tr>
								</thead>
								<tbody>
									<?php foreach ( $latest['autoload']['offenders'] as $offender ) : ?>
										<tr style="border-bottom: 1px solid rgba(0,0,0,0.05);">
											...
											<td style="padding: 8px; text-align: center;">
												<?php
												$badge_color = '#999';
												$badge_text  = __( 'Info', 'wpshadow' );
												if ( 'critical' === $offender['severity'] ) {
													$badge_color = '#c00';
													$badge_text  = __( 'Critical', 'wpshadow' );
												} elseif ( 'warning' === $offender['severity'] ) {
													$badge_color = '#ffb900';
													$badge_text  = __( 'Warning', 'wpshadow' );
												}
												?>
											...
											<td style="padding: 8px; text-align: center;">
												<button 
													...
													<?php esc_html_e( 'Disable Autoload', 'wpshadow' ); ?>
												</button>
											</td>
										</tr>
									<?php endforeach; ?>
							...
							<p style="margin-top: 15px; padding: 10px; background: rgba(0,0,0,0.05); border-radius: 3px; font-size: 12px; color: #666;">
								<strong><?php esc_html_e( 'Tip:', 'wpshadow' ); ?></strong>
								<?php esc_html_e( 'If you find a plugin\'s settings taking up 500kb+ but only used on one admin page, update it to autoload = no. WordPress will still fetch it when requested, but it won\'t clog every frontend page load.', 'wpshadow' ); ?>
							</p>
						</div>
					<?php endif; ?>
					...
					<div style="margin: 20px 0; padding: 15px; background: #eee;">
						<h3><?php esc_html_e( '📊 Database', 'wpshadow' ); ?></h3>
						<table style="width: 100%; font-size: 13px;">
							<tr>
								<td><strong><?php esc_html_e( 'Database Size:', 'wpshadow' ); ?></strong></td>
								<td><?php echo esc_html( $latest['performance']['database_size'] ); ?></td>
							</tr>
							<tr>
								<td><strong><?php esc_html_e( 'Draft Posts:', 'wpshadow' ); ?></strong></td>
								<td><?php echo intval( $latest['performance']['draft_posts'] ); ?></td>
							</tr>
							<tr>
								<td><strong><?php esc_html_e( 'Post Revisions:', 'wpshadow' ); ?></strong></td>
								<td><?php echo intval( $latest['performance']['post_revisions'] ); ?></td>
							</tr>
							<tr>
								<td><strong><?php esc_html_e( 'Spam Comments:', 'wpshadow' ); ?></strong></td>
					...
					<!-- Plugin Stats -->
					<div style="margin: 20px 0; padding: 15px; background: #eee;">
						<h3><?php esc_html_e( '🔌 Plugins', 'wpshadow' ); ?></h3>
						<p><?php printf( esc_html__( 'Active: %1$d | Inactive: %2$d | Total: %3$d', 'wpshadow' ), intval( $latest['plugins']['active_count'] ), intval( $latest['plugins']['inactive_count'] ), intval( $latest['plugins']['total_count'] ) ); ?></p>
					</div>
				</div>
			<?php endif; ?>
		</div>
		...
		document.getElementById('wps-audit-generate')?.addEventListener('click', function() {
			this.disabled = true;
			this.textContent = '<?php esc_html_e( 'Generating Audit...', 'wpshadow' ); ?>';
			fetch(ajaxurl, {
				method: 'POST',
			...
			.then(d => {
				...
				else { alert('Error: ' + d.data); this.disabled = false; this.textContent = '<?php esc_html_e( '📋 Generate New Audit', 'wpshadow' ); ?>'; }
			})
			.catch(e => { alert('Error: ' + e); this.disabled = false; this.textContent = '<?php esc_html_e( '📋 Generate New Audit', 'wpshadow' ); ?>'; });
		});

		// Handle toggle autoload buttons
		document.querySelectorAll('.wps-toggle-autoload').forEach(function(btn) {
			btn.addEventListener('click', function() {
				if (!confirm('<?php echo esc_js( __( 'Are you sure you want to disable autoload for this option? WordPress will still fetch it when needed, but it won\'t be loaded on every request.', 'wpshadow' ) ); ?>')) {
					return;
				}
				...
				button.textContent = '<?php echo esc_js( __( 'Processing...', 'wpshadow' ) ); ?>';
				...
				.then(d => {
					if (d.success) {
						button.textContent = '<?php echo esc_js( __( 'Disabled ✓', 'wpshadow' ) ); ?>';
						button.style.background = '#46b450';
						button.style.color = 'white';
						button.style.borderColor = '#46b450';
						setTimeout(function() { location.reload(); }, 1500);
					} else {
						alert('<?php echo esc_js( __( 'Oops:', 'wpshadow' ) ); ?> ' + d.data);
						button.disabled = false;
						button.textContent = '<?php echo esc_js( __( 'Disable Autoload', 'wpshadow' ) ); ?>';
					}
				})
				.catch(e => {
					alert('<?php echo esc_js( __( 'Oops:', 'wpshadow' ) ); ?> ' + e);
					button.disabled = false;
					button.textContent = '<?php echo esc_js( __( 'Disable Autoload', 'wpshadow' ) ); ?>';
				});
			});
