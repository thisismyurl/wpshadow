...
class WPSHADOW_Debug_Mode {
			...
			<div class="wps-debug-bar-inner">
				...
				<?php if ( $settings['debug_display'] ) : ?>
					<span class="wps-debug-bar-item wps-debug-errors">
						...
						printf( esc_html__( '%d Issues', 'wpshadow' ), (int) $error_count );
						?>
					</span>
				<?php endif; ?>
				<?php if ( $settings['query_info'] ) : ?>
					<span class="wps-debug-bar-item wps-debug-queries">
						...
						printf( esc_html__( '%1$d Queries (%2$ss)', 'wpshadow' ), (int) $query_count, number_format_i18n( $query_time, 4 ) );
						?>
					</span>
				<?php endif; ?>
				<?php if ( $settings['memory_usage'] ) : ?>
					<span class="wps-debug-bar-item wps-debug-memory">
						...
						printf( esc_html__( '%s MB', 'wpshadow' ), number_format_i18n( $memory_usage / 1024 / 1024, 2 ) );
						?>
					</span>
				<?php endif; ?>
			</div>
	...
	public static function ajax_toggle_debug(): void {
		...
		if ( empty( $setting ) ) {
			wp_send_json_error( array( 'message' => __( 'That setting doesn\'t exist', 'wpshadow' ) ) );
		}
		...
		if ( self::update_settings( $settings ) ) {
			wp_send_json_success(
				array(
					'message'  => __( 'Debug setting updated', 'wpshadow' ),
					'settings' => self::get_settings(),
				)
			);
		} else {
			wp_send_json_error( array( 'message' => __( 'Couldn\'t update that debug setting', 'wpshadow' ) ) );
		}
	}
	...
	public static function ajax_get_error_log(): void {
		...
		if ( ! file_exists( $log_file ) ) {
			wp_send_json_success(
				array(
					'content' => __( 'No error log file found.', 'wpshadow' ),
					'size'    => 0,
				)
			);
		}
	...
	public static function ajax_clear_error_log(): void {
		...
		$log_file = WP_CONTENT_DIR . '/debug.log';
		if ( ! file_exists( $log_file ) ) {
			wp_send_json_success( array( 'message' => __( 'No error log file found.', 'wpshadow' ) ) );
		}
		...
		if ( file_put_contents( $log_file, '' ) !== false ) {
			...
			wp_send_json_success( array( 'message' => __( 'Error log cleared', 'wpshadow' ) ) );
		} else {
			wp_send_json_error( array( 'message' => __( 'Couldn\'t clear the error log', 'wpshadow' ) ) );
		}
	}
}
