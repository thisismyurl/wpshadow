...
class TIMU_Dashboard_Layout {
	...
	public static function get_default_order(): array {
		return array(
			'normal' => array(
				'timu_widget_at_a_glance',
				'timu_widget_activity',
				'timu_widget_scheduled_tasks',
				'timu_widget_modules',
			),
			'side'   => array(
				'timu_widget_health',
				'timu_widget_quick_actions',
				'timu_widget_events_and_news',
			),
		);
	}
	...
	private static function get_layout_option_name( string $context, bool $network ): string {
		$prefix = $network ? 'timu_network_' : 'timu_';
	...
	public static function setup_dashboard_screen( string $context = 'core', bool $network = false ): void {
		...
		if ( ! empty( $layout['normal'] ) ) {
			foreach ( $layout['normal'] as $widget_id ) {
				self::register_widget_by_id( $widget_id, $screen->id, 'normal' );
			}
		}
		...
		if ( ! empty( $layout['side'] ) ) {
			foreach ( $layout['side'] as $widget_id ) {
				self::register_widget_by_id( $widget_id, $screen->id, 'side' );
			}
		}
		...
		wp_localize_script(
			'timu-dashboard-layout',
			'timu_dashboard_layout',
			array(
				'ajaxUrl'      => admin_url( 'admin-ajax.php' ),
				'nonce'        => wp_create_nonce( 'timu_dashboard_layout' ),
				'context'      => $context,
				'network'      => $network ? '1' : '0',
	 ...
	 * @return void
	 */
	private static function register_widget_by_id( string $widget_id, string $screen_id, string $context ): void {
	...
	private static function get_available_widgets(): array {
		return array(
			'timu_widget_at_a_glance'      => array(
				'title'    => __( 'At a Glance', 'plugin-wp-support-thisismyurl' ),
				'callback' => array( 'TIMU\CoreSupport\TIMU_Dashboard_Widgets', 'render_metabox_at_a_glance' ),
			),
			'timu_widget_activity'         => array(
				'title'    => __( 'Activity', 'plugin-wp-support-thisismyurl' ),
				'callback' => array( 'TIMU\CoreSupport\TIMU_Dashboard_Widgets', 'render_metabox_activity' ),
			),
			'timu_widget_scheduled_tasks'  => array(
				'title'    => __( 'Scheduled Tasks', 'plugin-wp-support-thisismyurl' ),
				'callback' => array( 'TIMU\CoreSupport\TIMU_Dashboard_Widgets', 'render_metabox_scheduled_tasks' ),
			),
			'timu_widget_modules'          => array(
				'title'    => __( 'Modules', 'plugin-wp-support-thisismyurl' ),
				'callback' => array( 'TIMU\CoreSupport\TIMU_Dashboard_Widgets', 'render_metabox_modules' ),
			),
			'timu_widget_health'           => array(
				'title'    => __( 'Health', 'plugin-wp-support-thisismyurl' ),
				'callback' => array( 'TIMU\CoreSupport\TIMU_Dashboard_Widgets', 'render_metabox_health' ),
			),
			'timu_widget_quick_actions'    => array(
				'title'    => __( 'Quick Actions', 'plugin-wp-support-thisismyurl' ),
				'callback' => array( 'TIMU\CoreSupport\TIMU_Dashboard_Widgets', 'render_metabox_quick_actions' ),
			),
			'timu_widget_events_and_news'  => array(
				'title'    => __( 'Events and News', 'plugin-wp-support-thisismyurl' ),
				'callback' => array( 'TIMU\CoreSupport\TIMU_Dashboard_Widgets', 'render_metabox_events_and_news' ),
			),
			'timu_widget_system_health'    => array(
				'title'    => __( 'System Health', 'plugin-wp-support-thisismyurl' ),
				'callback' => array( 'TIMU\CoreSupport\TIMU_Dashboard_Widgets', 'render_metabox_system_health' ),
			),
			'timu_widget_vault_status'     => array(
				'title'    => __( 'Vault Status', 'plugin-wp-support-thisismyurl' ),
				'callback' => array( 'TIMU\CoreSupport\TIMU_Dashboard_Widgets', 'render_metabox_vault_status' ),
			),
			'timu_widget_media_overview'   => array(
				'title'    => __( 'Media Overview', 'plugin-wp-support-thisismyurl' ),
				'callback' => array( 'TIMU\CoreSupport\TIMU_Dashboard_Widgets', 'render_metabox_media_overview' ),
			),
			'timu_widget_vault_overview'   => array(
				'title'    => __( 'Vault Overview', 'plugin-wp-support-thisismyurl' ),
				'callback' => array( 'TIMU\CoreSupport\TIMU_Dashboard_Widgets', 'render_metabox_vault_overview' ),
			),
		);
	}
	...
	public static function ajax_save_layout(): void {
		check_ajax_referer( 'timu_dashboard_layout', 'nonce' );
