...
require_once WPSHADOW_PATH . 'includes/ui/views/functions-card.php';
...
function wpshadow_render_activity_log( $context = 'general', $limit = 10 ) {
	...
	$context_config = array(
		...
		'achievements' => array(
			...
			'description' => __( 'Your recent achievements and progress milestones.', 'wpshadow' ),
			'filters'     => array( 'category' => 'achievements' ),
	...
	$view_all_url = admin_url( 'admin.php?page=wpshadow' );
				...
				<?php foreach ( $activities_result['activities'] as $activity ) : ?>
					<div class="wps-activity-item wps-flex wps-gap-3 wps-py-3 wps-border-bottom">
						<div class="wps-activity-icon wps-flex-shrink-0">
							...
							} elseif ( strpos( $activity['action'], 'achievement' ) !== false ) {
