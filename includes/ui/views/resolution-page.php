...
if ( isset( $excluded[ $slug ] ) ) { return "skipped"; }
if ( isset( $records[ $slug ] ) && ( $records[ $slug ]["status"] ?? "" ) === "resolved" ) { return "resolved"; }
if ( isset( $findings[ $slug ] ) ) { return "issue"; }
return "passing";
}

function wps_rc_badge( string $status ): string {
$labels = [ "issue" => "Needs Attention", "passing" => "Looks Good", "resolved" => "Resolved", "skipped" => "Skipped" ];
...
echo "<div class=\"wps-res-card__title-row\"><strong class=\"wps-res-card__title\">" . esc_html( $title ) . "</strong>" . wps_rc_badge( $status ) . "</div>";
...
if ( "resolved" !== $status ) echo "<button class=\"button button-primary wps-rc-btn\" data-action=\"resolved\" data-slug=\"" . esc_attr( $slug ) . "\" data-nonce=\"" . esc_attr( $nonce ) . "\">Mark Resolved</button>";
if ( "skipped" !== $status )  echo "<button class=\"button wps-rc-btn\" data-action=\"skipped\" data-slug=\"" . esc_attr( $slug ) . "\" data-nonce=\"" . esc_attr( $nonce ) . "\">Skip in Future Scans</button>";
if ( in_array( $status, [ "resolved", "skipped" ], true ) ) echo "<button class=\"button wps-rc-btn\" data-action=\"pending\" data-slug=\"" . esc_attr( $slug ) . "\" data-nonce=\"" . esc_attr( $nonce ) . "\">Undo</button>";
...
$raw      = get_option( "wpshadow_site_findings", [] );
$findings = [];
if ( is_array( $raw ) ) { foreach ( $raw as $f ) { if ( ! empty( $f["id"] ) ) $findings[ $f["id"] ] = $f; } }
...
$nav_assignments   = (array) get_theme_mod( "nav_menu_locations", [] );
...
$pub_pages  = get_pages( [ "post_status" => "publish", "number" => 200 ] );
$find_page  = static function ( string $kw ) use ( $pub_pages ): ?\WP_Post {
$kw = strtolower( $kw );
foreach ( $pub_pages as $p ) { if ( false !== strpos( strtolower( $p->post_title ), $kw ) ) return $p; }
return null;
};
$about_page   = $find_page( "about" );
$contact_page = $find_page( "contact" );

wp_enqueue_style( "wpshadow-resolution-page", plugin_dir_url( WPSHADOW_FILE ) . "assets/css/resolution-page.css", [], "0.6093" );
...
$counts = [ "issue" => 0, "passing" => 0, "resolved" => 0, "skipped" => 0 ];
foreach ( $all_slugs as $si ) { $st = wps_rc_status( $si, $findings, $excluded, $records ); if ( isset( $counts[ $st ] ) ) $counts[ $st ]++; }
$stat_labels = [ "issue" => "Needs Attention", "passing" => "Looks Good", "resolved" => "Resolved", "skipped" => "Skipped" ];
...
<div class="wps-res-summary"><?php foreach ( $counts as $st => $n ) : ?><?php if ( $n > 0 ) : ?><span class="wps-res-summary-pill <?php echo esc_attr( $st ); ?>"><span class="count"><?php echo esc_html( $n ); ?></span> <?php echo esc_html( $stat_labels[ $st ] ?? $st ); ?></span><?php endif; ?><?php endforeach; ?></div>
...
if($seo_plugin&&$seo_robots_url){$seo_link($seo_robots_url,"robots.txt Editor");}else{wps_rc_admin_link(admin_url("tools.php"),"Tools");}
wps_rc_action_close(); wps_rc_card_close($s,$st,$nonce);
...
<?php
$s="primary-navigation-assigned"; $st=wps_rc_status($s,$findings,$excluded,$records);
...
$plabel=""; $phas=false;
foreach($nav_locations as $lk=>$ll){if(stripos($lk,"primary")!==false||stripos($ll,"primary")!==false||stripos($ll,"main")!==false){$plabel=$ll;$phas=!empty($nav_assignments[$lk]);break;}}
if($plabel){echo "<div class='wps-res-current-state'><strong>Location:</strong> ".esc_html($plabel)." &bull; ".($phas?"Menu assigned":"<strong>No menu assigned</strong>")."</div>";}
wps_rc_action_open("Assign a menu");
wps_rc_admin_link(admin_url("nav-menus.php"),"Appearance Menus"); wps_rc_action_close(); wps_rc_card_close($s,$st,$nonce);

$s="footer-menu"; $st=wps_rc_status($s,$findings,$excluded,$records);
...
wps_rc_admin_link(admin_url("widgets.php"),"Appearance Widgets"); wps_rc_action_close(); wps_rc_card_close($s,$st,$nonce);

$s="mobile-menu"; $st=wps_rc_status($s,$findings,$excluded,$records);
...
$s="site-icon"; $st=wps_rc_status($s,$findings,$excluded,$records);
...
if($site_icon_id){echo "<div class='wps-res-current-state'><strong>Icon set:</strong> ".wp_get_attachment_image($site_icon_id,[32,32])."</div>";}
else{echo "<div class='wps-res-current-state'><strong>No site icon set.</strong></div>";}
wps_rc_action_open("Upload site icon");
wps_rc_admin_link(admin_url("customize.php?autofocus[section]=title_tagline"),"Customiser Site Identity"); wps_rc_action_close(); wps_rc_card_close($s,$st,$nonce);

$s="custom-logo-set"; $st=wps_rc_status($s,$findings,$excluded,$records);
...
if($logo_id){echo "<div class='wps-res-current-state'><strong>Logo set:</strong> ".wp_get_attachment_image($logo_id,[120,40])."</div>";}
else{echo "<div class='wps-res-current-state'><strong>No custom logo set.</strong></div>";}
...
?>
	...
	<div class="wps-res-section">
		...
		<?php
		$s="registration-setting-intentional"; $st=wps_rc_status($s,$findings,$excluded,$records);
		...
		if($registration_open){wps_rc_option_row("users_can_register","Close registration","Open","0","Turn Off Registration",$nonce);}
		else{wps_rc_option_row("users_can_register","Open registration (only if intentional)","Closed","1","Enable Registration",$nonce);}
		wps_rc_admin_link(admin_url("options-general.php"),"Settings - General"); wps_rc_action_close(); wps_rc_card_close($s,$st,$nonce);
		...
		$seo_link($seo_titles_url,"Search Page Noindex"); wps_rc_action_close(); wps_rc_card_close($s,$st,$nonce);

		$s="auto-update-policy"; $st=wps_rc_status($s,$findings,$excluded,$records);
		wps_rc_card_open($s,"WordPress Core Auto-Updates",$st);
		$core_display=match((string)$auto_update_core){"true","1"=>"All updates","false","0"=>"Disabled",default=>"Minor updates only"};
		...
		wps_rc_admin_link(admin_url("update-core.php"),"Dashboard - Updates"); wps_rc_action_close(); wps_rc_card_close($s,$st,$nonce);

		$s="plugin-auto-updates"; $st=wps_rc_status($s,$findings,$excluded,$records);
		...
		echo "<div class='wps-res-current-state'><strong>Comments:</strong> ".($comment_status==="open"?"Open":"Closed")." &bull; <strong>Moderation:</strong> ".($comment_mod?"On":"Off")."</div>";
		wps_rc_action_open("Update comment settings");
		if($comment_status==="open"){wps_rc_option_row("default_comment_status","Close on new posts","Open","closed","Close Comments",$nonce);}
		if(!$comment_mod){wps_rc_option_row("comment_moderation","Require moderation","Off","1","Enable Moderation",$nonce);}
		wps_rc_admin_link(admin_url("options-discussion.php"),"Settings - Discussion"); wps_rc_action_close(); wps_rc_card_close($s,$st,$nonce);
...
<?php
$s="about-page-published"; $st=wps_rc_status($s,$findings,$excluded,$records);
...
if($about_page){echo "<div class='wps-res-current-state'><strong>Found:</strong> <a href='".esc_url(get_permalink($about_page))."' target='_blank'>".esc_html($about_page->post_title)." &#8599;</a></div>";}
else{echo "<div class='wps-res-current-state'><strong>No About page found.</strong></div>";}
wps_rc_action_open("Create or review your About page");
if($about_page){wps_rc_admin_link((string)get_edit_post_link($about_page->ID),"Edit About Page");}
else{wps_rc_admin_link(admin_url("post-new.php?post_type=page"),"Create About Page");}
wps_rc_admin_link(admin_url("edit.php?post_type=page"),"All Pages"); wps_rc_action_close(); wps_rc_card_close($s,$st,$nonce);

$s="contact-page-published"; $st=wps_rc_status($s,$findings,$excluded,$records);
...
if($contact_page){echo "<div class='wps-res-current-state'><strong>Found:</strong> <a href='".esc_url(get_permalink($contact_page))."' target='_blank'>".esc_html($contact_page->post_title)." &#8599;</a></div>";}
else{echo "<div class='wps-res-current-state'><strong>No Contact page found.</strong></div>";}
...
else{wps_rc_admin_link(admin_url("post-new.php?post_type=page"),"Create Contact Page");}
wps_rc_admin_link(admin_url("edit.php?post_type=page"),"All Pages"); wps_rc_action_close(); wps_rc_card_close($s,$st,$nonce);
...
$s="homepage-page-published"; $st=wps_rc_status($s,$findings,$excluded,$records);
...
if($show_on_front==="page"&&$homepage_id){$hp=get_post($homepage_id);echo $hp?"<a href='".(string)get_edit_post_link($hp->ID)."'>".esc_html($hp->post_title)."</a>":"Page (ID not found)";}
elseif($show_on_front==="page"){echo "Static page (no page selected)";}
else{echo "Latest posts (WordPress default)";}
echo "</div>";
wps_rc_action_open("Configure Reading settings");
wps_rc_admin_link(admin_url("options-reading.php"),"Settings - Reading");
wps_rc_admin_link(admin_url("edit.php?post_type=page"),"All Pages"); wps_rc_action_close(); wps_rc_card_close($s,$st,$nonce);

$s="posts-page-published"; $st=wps_rc_status($s,$findings,$excluded,$records);
...
if($posts_page_id){$pp=get_post($posts_page_id);echo $pp?"<a href='".(string)get_edit_post_link($pp->ID)."'>".esc_html($pp->post_title)."</a>":"Set (page not found)";}
else{echo "Not set";}
echo "</div>";
...
function toggle(){var b=document.getElementById(h.getAttribute("aria-controls"));if(!b)return;var o=b.hasAttribute("hidden");b.toggleAttribute("hidden",!o);h.setAttribute("aria-expanded",o?"true":"false");}
h.addEventListener("click",toggle);
h.addEventListener("keydown",function(e){if(e.key==="Enter"||e.key===" "){e.preventDefault();toggle();}});
});
document.querySelectorAll(".wps-rc-btn").forEach(function(btn){
btn.addEventListener("click",function(){
var card=btn.closest(".wps-res-card"),msg=card?card.querySelector(".wps-res-feedback-msg"):null;
btn.disabled=true;
var fd=new FormData();
...
btn.disabled=false;
if(d.success){if(msg){msg.textContent=d.data.message||<?php echo wp_json_encode(__("Saved.","wpshadow")); ?>;msg.style.display="inline";}setTimeout(function(){location.reload();},1200);}
else{alert((d.data&&d.data.message)||<?php echo wp_json_encode(__("Could not save.","wpshadow")); ?>);}
}).catch(function(){btn.disabled=false;alert(<?php echo wp_json_encode(__("Network error.","wpshadow")); ?>);});
});
});
document.querySelectorAll(".wps-rc-save-option").forEach(function(btn){
btn.addEventListener("click",function(){
var row=btn.closest(".wps-res-option-row"),fb=row?row.querySelector(".wps-res-option-feedback"):null;
btn.disabled=true;btn.textContent=<?php echo wp_json_encode(__("Saving...","wpshadow")); ?>;
var fd=new FormData();
...
fetch(ajaxUrl,{method:"POST",body:fd}).then(function(r){return r.json();}).then(function(d){
btn.disabled=false;
if(d.success){btn.textContent=<?php echo wp_json_encode(__("Saved","wpshadow")); ?>;if(fb){fb.textContent=d.data.message||<?php echo wp_json_encode(__("Updated.","wpshadow")); ?>;fb.style.display="inline";}setTimeout(function(){location.reload();},1400);}
else{btn.textContent=<?php echo wp_json_encode(__("Save","wpshadow")); ?>;if(fb){fb.textContent=(d.data&&d.data.message)||<?php echo wp_json_encode(__("Could not save.","wpshadow")); ?>;fb.classList.add("error");fb.style.display="inline";}}
}).catch(function(){btn.disabled=false;btn.textContent=<?php echo wp_json_encode(__("Save","wpshadow")); ?>;if(fb){fb.textContent=<?php echo wp_json_encode(__("Network error.","wpshadow")); ?>;fb.classList.add("error");fb.style.display="inline";}});
});
});
}());
</script>
