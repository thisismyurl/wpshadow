...
class WPSHADOW_White_Screen_Recovery {
	...
	public static function handle_exit_recovery_ajax(): void {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Insufficient permissions.', 'wpshadow' ) ) );
		}
		...
		wp_send_json_success( array( 
			'message' => esc_html__( 'Recovery mode exited successfully. Reloading page...', 'wpshadow' ),
	...
	private static function handle_recovery_endpoint(): void {
		...
		if ( ! empty( $nonce ) && ! wp_verify_nonce( $nonce, 'wpshadow_recovery' ) ) {
			wp_die( esc_html__( 'That recovery link didn\'t work. Please request a new one.', 'wpshadow' ) );
		}
	...
	public static function display_recovery_notice(): void {
		...
		if ( $recovery ) {
			...
			$plugin_name = $recovery['plugin'] ?? __( 'Unknown Plugin', 'wpshadow' );
			$attempt     = $recovery['attempt'] ?? 1;

			?>
			<div class="notice notice-warning is-dismissible">
				<p>
					<strong><?php esc_html_e( 'WPShadow Auto-Recovery:', 'wpshadow' ); ?></strong>
					<?php
					echo esc_html(
						sprintf(
							/* translators: 1: Plugin name, 2: Attempt number */
							__( 'A fatal error was detected and plugin "%1$s" was automatically deactivated (attempt #%2$d). Please review the error details and contact the plugin author.', 'wpshadow' ),
							$plugin_name,
							$attempt
						)
					);
				...
				<p>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=wps-emergency-support' ) ); ?>" class="button">
						<?php esc_html_e( 'View Error Details', 'wpshadow' ); ?>
					</a>
				</p>
			</div>
			<?php
		...
		if ( self::is_recovery_mode_active() ) {
			...
			<div class="notice notice-error">
				<p>
					<strong><?php esc_html_e( '🚨 Recovery Mode Active', 'wpshadow' ); ?></strong>
				</p>
				<p>
					<?php
					printf(
						esc_html__( 'All plugins except WPShadow have been temporarily disabled due to a critical error. Please review the error details in the %s and fix the issue before exiting recovery mode.', 'wpshadow' ),
						'<a href="' . esc_url( admin_url( 'admin.php?page=wps-emergency-support' ) ) . '">' . esc_html__( 'Emergency Dashboard', 'wpshadow' ) . '</a>'
					);
					?>
				</p>
				<p>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=wps-emergency-support' ) ); ?>" class="button button-primary">
						<?php esc_html_e( 'Emergency Dashboard', 'wpshadow' ); ?>
					</a>
					<form id="wpshadow-exit-recovery-notice-form" style="display: inline;">
						...
						<button type="button" class="button" onclick="WPShadowRecovery.exitRecoveryMode()">
							<?php esc_html_e( 'Exit Recovery Mode', 'wpshadow' ); ?>
						</button>
					</form>
				</p>
			</div>
		...
		if ( $manual_recovery ) {
			...
			<div class="notice notice-error">
				<p>
					<strong><?php esc_html_e( '🚨 Critical Error - Manual Recovery Required', 'wpshadow' ); ?></strong>
				</p>
				<p>
					<?php esc_html_e( 'Multiple recovery attempts have been made. Manual intervention is required to fix this issue.', 'wpshadow' ); ?>
				</p>
				<p>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=wps-emergency-support' ) ); ?>" class="button button-primary">
						<?php esc_html_e( 'View Details & Get Help', 'wpshadow' ); ?>
					</a>
				</p>
			</div>
			<?php
	...
	public static function register_recovery_metabox(): void {
		add_meta_box(
			...
			__( 'Auto-Recovery Status', 'wpshadow' ),
	...
	public static function render_recovery_metabox(): void {
		...
		<div class="wps-recovery-status">
			<table class="form-table" role="presentation">
				<tbody>
					<tr>
						<th scope="row"><?php esc_html_e( 'Recovery Mode:', 'wpshadow' ); ?></th>
						<td>
							<?php if ( $recovery_mode ) : ?>
								<span style="color: #c00; font-weight: bold;">⚠️ <?php esc_html_e( 'ACTIVE', 'wpshadow' ); ?></span>
							<?php else : ?>
								<span style="color: #090;">✓ <?php esc_html_e( 'Inactive', 'wpshadow' ); ?></span>
							<?php endif; ?>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( 'Recovery Attempts:', 'wpshadow' ); ?></th>
						<td>
					...
					<tr>
						<th scope="row"><?php esc_html_e( 'Problematic Plugins:', 'wpshadow' ); ?></th>
						<td>
							...
							<p class="description">
								<?php esc_html_e( 'These plugins were automatically deactivated due to fatal errors.', 'wpshadow' ); ?>
							</p>
						</td>
					</tr>
					<?php endif; ?>
			...
			<?php if ( $recovery_mode ) : ?>
				<div style="margin-top: 20px; padding: 15px; background: #fee; border-left: 4px solid #c00;">
					<h4 style="margin-top: 0;"><?php esc_html_e( 'Recovery Mode Instructions', 'wpshadow' ); ?></h4>
					<ol>
						<li><?php esc_html_e( 'Review the error details above', 'wpshadow' ); ?></li>
						<li><?php esc_html_e( 'Fix the problematic plugin or remove it', 'wpshadow' ); ?></li>
						<li><?php esc_html_e( 'Exit recovery mode to restore normal operation', 'wpshadow' ); ?></li>
					</ol>
					<form id="wpshadow-exit-recovery-form" method="post">
						...
						<button type="button" class="button" onclick="WPShadowRecovery.exitRecoveryMode( true )">
							<?php esc_html_e( 'Clear Problematic Plugins List', 'wpshadow' ); ?>
						</button>
					</form>
				</div>
			<?php else : ?>
				<div style="margin-top: 20px;">
					<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=wps-emergency-support' ) ); ?>">
						...
						<button type="submit" class="button">
							<?php esc_html_e( 'Manually Activate Recovery Mode', 'wpshadow' ); ?>
						</button>
						<?php if ( $attempts > 0 ) : ?>
							<button type="submit" name="reset_attempts" value="1" class="button">
								<?php esc_html_e( 'Reset Recovery Counter', 'wpshadow' ); ?>
							</button>
						<?php endif; ?>
					</form>
				</div>
