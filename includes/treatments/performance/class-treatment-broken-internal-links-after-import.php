...
class Treatment_Broken_Internal_Links_After_Import extends Treatment_Base {
	...
	public static function check() {
		...
		$home_url = home_url();
		$home_domain = Treatment_URL_And_Pattern_Helper::get_domain( $home_url );
		...
		foreach ( $recent_posts as $post ) {
			...
			if ( preg_match_all( '/<a[^>]+href=["\']([^"\']+)["\'][^>]*>/i', $post->post_content, $matches ) ) {
				foreach ( $matches[1] as $link_url ) {
					...
					$link_domain = Treatment_URL_And_Pattern_Helper::get_domain( $link_url );
		...
		$site_scheme = Treatment_URL_And_Pattern_Helper::get_scheme( $home_url );
		$protocol_mismatches = 0;

		foreach ( $recent_posts as $post ) {
			if ( preg_match_all( '/href=["\']([^"\']+)["\']/', $post->post_content, $matches ) ) {
				foreach ( $matches[1] as $link_url ) {
					$link_scheme = Treatment_URL_And_Pattern_Helper::get_scheme( $link_url );
