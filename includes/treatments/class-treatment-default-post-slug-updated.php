...
class Treatment_Default_Post_Slug_Updated extends Treatment_Base {
	...
	public static function apply(): array {
		...
		if ( null === $post ) {
			return array(
				'success' => true,
				'message' => __( 'Default starter post not found — slug and title may have already been updated.', 'wpshadow' ),
			);
		}
		...
		if ( str_contains( (string) $post->post_content, 'Welcome to WordPress. This is your first post' ) ) {
			return array(
				'success' => false,
				'message' => __( 'This post still contains the original starter content. Use the "Delete default Hello World post" treatment instead.', 'wpshadow' ),
			);
		}
		...
		if ( ! $has_default_slug && ! $has_default_title ) {
			return array(
				'success' => true,
				'message' => __( 'The post\'s slug and title have already been customised. No changes made.', 'wpshadow' ),
			);
		}
		...
		if ( $has_default_slug ) {
			...
			$changed[]                = __( 'slug updated to "welcome"', 'wpshadow' );
		}

		if ( $has_default_title ) {
			$update_args['post_title'] = 'Welcome';
			$changed[]                 = __( 'title updated to "Welcome"', 'wpshadow' );
		}
		...
		if ( is_wp_error( $result ) ) {
			return array(
				...
				'message' => sprintf(
					...
					__( 'Could not update the post: %s', 'wpshadow' ),
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Post identifiers updated: %s. Edit the post to set a more specific title and permalink if needed.', 'wpshadow' ),
				implode( ', ', $changed )
			),
		);
	}
	...
	public static function undo(): array {
		...
		if ( is_wp_error( $result ) ) {
			return array(
				...
				'message' => sprintf(
					/* translators: %s: WP_Error message */
					__( 'Could not restore the post: %s', 'wpshadow' ),
					$result->get_error_message()
				),
			);
		}
		...
		return array(
			...
			'message' => sprintf(
				...
				__( 'Post restored to slug "%1$s" and title "%2$s".', 'wpshadow' ),
				esc_html( $prev['slug'] ),
				esc_html( $prev['title'] )
			),
		);
