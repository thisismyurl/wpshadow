...
class WPSHADOW_Update_Simulator {
	 ...
	 */
	private const SIMULATION_HISTORY_KEY = 'wpshadow_simulation_history';

	/**
	 * Database option key for pending deployments.
	 */
	private const PENDING_DEPLOYMENTS_KEY = 'wpshadow_pending_deployments';
	...
	public static function init(): void {
		...
		add_action( 'wp_ajax_WPSHADOW_simulate_update', array( __CLASS__, 'ajax_simulate_update' ) );
		add_action( 'wp_ajax_WPSHADOW_deploy_update', array( __CLASS__, 'ajax_deploy_update' ) );
		add_action( 'wp_ajax_WPSHADOW_rollback_update', array( __CLASS__, 'ajax_rollback_update' ) );
	...
	public static function enqueue_assets( string $hook ): void {
		...
		wp_localize_script(
			...
			array(
				...
				'nonce'    => wp_create_nonce( 'wpshadow_update_simulator_nonce' ),
			)
		);
	}
	...
	public static function ajax_simulate_update(): void {
		...
		if ( ! class_exists( '\\WPShadow\\WPSHADOW_Staging_Manager' ) ) {
			wp_send_json_error( array( 'message' => __( 'Staging Manager not available', 'plugin-wpshadow' ) ) );
		}

		// Create snapshot before simulation.
		if ( class_exists( '\\WPShadow\\WPSHADOW_Snapshot_Manager' ) ) {
			$snapshot = WPSHADOW_Snapshot_Manager::create_snapshot( 'pre-simulation-' . sanitize_key( $plugin ) );
		}
	...
	public static function ajax_deploy_update(): void {
		...
		$snapshot_id = null;
		if ( class_exists( '\\WPShadow\\WPSHADOW_Snapshot_Manager' ) ) {
			$snapshot_id = WPSHADOW_Snapshot_Manager::create_snapshot( 'pre-deployment-' . sanitize_key( $plugin ) );
		}
	...
	public static function ajax_rollback_update(): void {
		...
		if ( ! class_exists( '\\WPShadow\\WPSHADOW_Snapshot_Manager' ) ) {
			wp_send_json_error( array( 'message' => __( 'Snapshot Manager not available', 'plugin-wpshadow' ) ) );
		}

		$result = WPSHADOW_Snapshot_Manager::restore_snapshot( $snapshot_id );

		if ( is_wp_error( $result ) ) {
			wp_send_json_error( array( 'message' => $result->get_error_message() ) );
		}

		wp_send_json_success( array( 'message' => __( 'Rollback successful', 'plugin-wpshadow' ) ) );
	}
	...
	public static function render_page(): void {
		...
		<div class="wrap wps-update-simulator-page">
			<h1><?php esc_html_e( 'Update Simulator & Safe Deployment', 'plugin-wpshadow' ); ?></h1>
			<p class="description">
				<?php esc_html_e( 'Test plugin and theme updates in staging before deploying to your live site. Includes instant rollback if issues arise.', 'plugin-wpshadow' ); ?>
			</p>

			<?php if ( empty( $available_updates ) ) : ?>
				<div class="notice notice-success">
					<p><?php esc_html_e( 'No updates available. All plugins and themes are up to date!', 'plugin-wpshadow' ); ?></p>
				</div>
			<?php else : ?>
				<div class="wps-updates-available">
					...
					<table class="widefat wps-updates-table">
						<thead>
							<tr>
								<th><?php esc_html_e( 'Plugin/Theme', 'plugin-wpshadow' ); ?></th>
								<th><?php esc_html_e( 'Current', 'plugin-wpshadow' ); ?></th>
								<th><?php esc_html_e( 'Available', 'plugin-wpshadow' ); ?></th>
								<th><?php esc_html_e( 'Changes', 'plugin-wpshadow' ); ?></th>
									...
									<td>
										<?php if ( ! empty( $item['changelog'] ) ) : ?>
											<a href="#" class="wps-view-changelog" data-changelog="<?php echo esc_attr( wp_json_encode( $item['changelog'] ) ); ?>">
												<?php esc_html_e( 'View Changes', 'plugin-wpshadow' ); ?>
											</a>
										<?php else : ?>
											<span class="description"><?php esc_html_e( 'No changelog available', 'plugin-wpshadow' ); ?></span>
										<?php endif; ?>
									</td>
									<td>
										<button type="button" class="button wps-simulate-btn" data-plugin="<?php echo esc_attr( $item['slug'] ); ?>">
											<?php esc_html_e( 'Simulate Update', 'plugin-wpshadow' ); ?>
										</button>
									</td>
								</tr>
							<?php endforeach; ?>
			...
			<?php if ( ! empty( $simulation_history ) ) : ?>
				<div class="wps-simulation-history">
					<h2><?php esc_html_e( 'Recent Simulations', 'plugin-wpshadow' ); ?></h2>
					<table class="widefat">
						<thead>
							<tr>
								...
								<tr>
									...
									<td>
										<?php if ( 'deployed' === $sim['status'] ) : ?>
											<span class="wps-badge-success"><?php esc_html_e( 'Deployed', 'plugin-wpshadow' ); ?></span>
										<?php elseif ( 'simulated' === $sim['status'] ) : ?>
											<span class="wps-badge-pending"><?php esc_html_e( 'Tested in Staging', 'plugin-wpshadow' ); ?></span>
										<?php else : ?>
		...
		<div id="wps-changelog-modal" style="display:none;">
			...
			<div class="wps-modal-content">
				...
				<h2><?php esc_html_e( 'Update Changes', 'plugin-wpshadow' ); ?></h2>
	...
	private static function get_update_info( string $plugin ): array|\WP_Error {
		...
		if ( ! isset( $updates->response[ $plugin ] ) ) {
			return new \WP_Error( 'no_update', __( 'No update available for this plugin', 'plugin-wpshadow' ) );
		}
	...
	private static function fetch_changelog( object $plugin_data ): string {
		...
		$changelog = get_transient( 'wpshadow_changelog_' . $plugin_data->slug );
		if ( false !== $changelog ) {
			return $changelog;
		}
		...
		if ( preg_match( '/<h4>(.+?)<\/h4>.*?<ul>(.+?)<\/ul>/s', $body, $matches ) ) {
			...
			set_transient( 'wpshadow_changelog_' . $plugin_data->slug, $changelog, DAY_IN_SECONDS );
			return $changelog;
		}

		return __( 'Changelog not available', 'plugin-wpshadow' );
	}
	...
	private static function check_compatibility( object $plugin_data ): array {
		...
		if ( isset( $plugin_data->requires ) && version_compare( $wp_version, $plugin_data->requires, '<' ) ) {
			...
			$compatibility['warnings'][] = sprintf(
				...
				__( 'Requires WordPress %s or higher', 'plugin-wpshadow' ),
		...
		if ( isset( $plugin_data->requires_php ) && version_compare( phpversion(), $plugin_data->requires_php, '<' ) ) {
			...
			$compatibility['warnings'][] = sprintf(
				...
				__( 'Requires PHP %s or higher', 'plugin-wpshadow' ),
