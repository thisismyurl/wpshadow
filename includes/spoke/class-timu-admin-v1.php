<?php
/**
 * TIMU Admin UI Component - WordPress Settings API
...
class TIMU_Admin_v1 {
	...
	public function register_settings_api(): void {
		...
		$instance_count = \count( TIMU_Spoke_Base::$instances ?? array() );
		$first_slug     = \array_key_first( TIMU_Spoke_Base::$instances ?? array() );
	...
	private function propagate_global_settings_network_wide( array $global_settings ): void {
		...
		\update_site_option( 'timu_network_global_options', $global_settings );
	...
	public function render_master_page(): void {
		...
		echo '<div class="timu-main-content">';
	...
	private function render_global_settings_form(): void {
		...
		if ( \is_multisite() && \current_user_can( 'manage_network_options' ) ) {
			...
			$global_options = (array) \get_option( 'timu_global_options', $default_options );
		}

		$dynamic_fields = $this->get_dynamic_global_fields();
		echo '<div class="notice notice-info inline" style="margin:20px 0;"><p><strong>' . \esc_html__( 'Global Settings:', 'timu-core' ) . '</strong> ' . \esc_html__( 'Changes made here apply to all active modules.', 'timu-core' ) . '</p></div>';
		echo '<form method="post" action="options.php">';
		...
		foreach ( $dynamic_fields as $field_id => $field_config ) {
			...
			$field_name           = 'timu_global_options[' . $field_id . ']';
			$field_config['id']   = $field_id;
			$field_config['name'] = $field_name;
			echo '<tr><th scope="row"><label for="timu-global-' . \esc_attr( $field_id ) . '">' . \esc_html( $field_config['label'] ?? \ucfirst( \str_replace( '_', ' ', $field_id ) ) ) . '</label></th><td>';
	...
	public function render_default_sidebar_actions( string $plugin_slug ): void {
		echo '<div class="timu-sidebar-actions" style="margin-top:1rem; padding-top:1rem; border-top:1px solid #e5e7eb;">';
