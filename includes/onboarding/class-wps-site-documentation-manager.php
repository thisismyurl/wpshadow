...
class WPSHADOW_Site_Documentation_Manager {
	...
	public static function register_admin_pages(): void {
		...
		add_submenu_page(
			null, // Hidden from menu - accessible via QuickLinks
			__( 'Export Documentation', 'wpshadow' ),
			__( 'Export Documentation', 'wpshadow' ),
			'manage_options',
	...
	public static function add_plugin_action_links( array $actions, string $plugin_file ): array {
		...
		$link_text = $is_protected ? __( 'Unprotect', 'wpshadow' ) : __( 'Protect', 'wpshadow' );
	...
	public static function ajax_toggle_protection(): void {
		...
		if ( empty( $plugin_file ) ) {
			wp_send_json_error( array( 'message' => __( 'Please select a valid plugin', 'wpshadow' ) ) );
		}
		...
		if ( $is_protected ) {
			...
			$message = __( 'Plugin protection removed', 'wpshadow' );
		} else {
			...
			$message                           = __( 'Plugin marked as protected', 'wpshadow' );
		}
	...
	public static function render_blueprint_page(): void {
		...
		<div class="wrap wps-blueprint-page">
			<h1><?php esc_html_e( 'Site Blueprint', 'wpshadow' ); ?></h1>
			<p class="description">
				<?php esc_html_e( 'Comprehensive site documentation in plain English. Understand what you have and how it works.', 'wpshadow' ); ?>
			</p>

			<div class="wps-blueprint-actions">
				<button type="button" class="button button-primary" id="wps-export-blueprint">
					<?php esc_html_e( 'Export Blueprint', 'wpshadow' ); ?>
				</button>
	...
	public static function render_protected_plugins_page(): void {
		...
		<div class="wrap wps-protected-plugins-page">
			<h1><?php esc_html_e( 'Protected Plugins', 'wpshadow' ); ?></h1>
			<p class="description">
				<?php esc_html_e( 'Mark critical plugins for protection. Prevent accidental removal and receive update notifications.', 'wpshadow' ); ?>
			</p>

			<div class="wps-protected-summary">
				<span class="dashicons dashicons-shield"></span>
				<strong><?php echo esc_html( count( $protected_plugins ) ); ?></strong>
				<?php esc_html_e( 'plugins protected', 'wpshadow' ); ?>
			</div>

			<table class="widefat wps-protected-plugins-table">
				<thead>
					<tr>
						<th><?php esc_html_e( 'Plugin', 'wpshadow' ); ?></th>
						<th><?php esc_html_e( 'Purpose', 'wpshadow' ); ?></th>
						<th><?php esc_html_e( 'If Removed', 'wpshadow' ); ?></th>
						<th><?php esc_html_e( 'Protected', 'wpshadow' ); ?></th>
	...
	public static function render_export_page(): void {
		...
		<div class="wrap wps-export-page">
			<h1><?php esc_html_e( 'Export Site Documentation', 'wpshadow' ); ?></h1>
			<p class="description">
				<?php esc_html_e( 'Export complete site documentation for handoff to developers or for personal records.', 'wpshadow' ); ?>
			</p>

			<div class="wps-export-options">
				<h2><?php esc_html_e( 'Export Format', 'wpshadow' ); ?></h2>
				<ul class="wps-export-formats">
					<li>
						<label>
							<input type="radio" name="export_format" value="html" checked>
							<strong><?php esc_html_e( 'HTML', 'wpshadow' ); ?></strong>
							<br>
							<small><?php esc_html_e( 'Web viewable, can be opened in any browser', 'wpshadow' ); ?></small>
						</label>
					</li>
					<li>
						<label>
							<input type="radio" name="export_format" value="markdown">
							<strong><?php esc_html_e( 'Markdown', 'wpshadow' ); ?></strong>
							<br>
							<small><?php esc_html_e( 'Developer friendly, works with GitHub and documentation systems', 'wpshadow' ); ?></small>
						</label>
					</li>
					<li>
						<label>
							<input type="radio" name="export_format" value="text">
							<strong><?php esc_html_e( 'Plain Text', 'wpshadow' ); ?></strong>
							<br>
							<small><?php esc_html_e( 'Universal format, readable anywhere', 'wpshadow' ); ?></small>
						</label>
					</li>
				</ul>

				<h2><?php esc_html_e( 'Include Sections', 'wpshadow' ); ?></h2>
				<ul class="wps-export-sections">
					<li><label><input type="checkbox" name="include_summary" checked> <?php esc_html_e( 'Executive Summary', 'wpshadow' ); ?></label></li>
					<li><label><input type="checkbox" name="include_plugins" checked> <?php esc_html_e( 'Plugin Inventory', 'wpshadow' ); ?></label></li>
					<li><label><input type="checkbox" name="include_customizations" checked> <?php esc_html_e( 'Customization Audit', 'wpshadow' ); ?></label></li>
					<li><label><input type="checkbox" name="include_technical" checked> <?php esc_html_e( 'Technical Specifications', 'wpshadow' ); ?></label></li>
					<li><label><input type="checkbox" name="include_maintenance" checked> <?php esc_html_e( 'Maintenance Schedule', 'wpshadow' ); ?></label></li>
					<li><label><input type="checkbox" name="include_emergency" checked> <?php esc_html_e( 'Emergency Contacts', 'wpshadow' ); ?></label></li>
					<li><label><input type="checkbox" name="include_recovery" checked> <?php esc_html_e( 'Recovery Procedures', 'wpshadow' ); ?></label></li>
				</ul>

				<button type="button" class="button button-primary button-hero" id="wps-export-now">
					<?php esc_html_e( 'Export Documentation', 'wpshadow' ); ?>
				</button>
			</div>

			<div class="wps-export-preview" style="display: none;">
				<h2><?php esc_html_e( 'Preview', 'wpshadow' ); ?></h2>
	...
	private static function generate_blueprint(): string {
		...
		$output .= '<h2>' . esc_html__( 'Executive Summary', 'wpshadow' ) . '</h2>';
		$output .= self::generate_executive_summary();
		$output .= '</div>';

		// Plugin Inventory.
		$output .= '<div class="wps-blueprint-section">';
		$output .= '<h2>' . esc_html__( 'Your Plugins Explained', 'wpshadow' ) . '</h2>';
		$output .= self::generate_plugin_inventory();
		$output .= '</div>';

		// Customization Audit.
		$output .= '<div class="wps-blueprint-section">';
		$output .= '<h2>' . esc_html__( 'What\'s Custom on Your Site', 'wpshadow' ) . '</h2>';
		$output .= self::generate_customization_audit();
		$output .= '</div>';

		// Technical Specifications.
		$output .= '<div class="wps-blueprint-section">';
		$output .= '<h2>' . esc_html__( 'Technical Specifications', 'wpshadow' ) . '</h2>';
	...
	private static function generate_executive_summary(): string {
		...
		$output .= '<li><strong>' . esc_html__( 'Site URL:', 'wpshadow' ) . '</strong> ' . esc_html( get_site_url() ) . '</li>';
		$output .= '<li><strong>' . esc_html__( 'Last Updated:', 'wpshadow' ) . '</strong> ' . esc_html( date_i18n( get_option( 'date_format' ), time() ) ) . '</li>';
		$output .= '<li><strong>' . esc_html__( 'Total Plugins:', 'wpshadow' ) . '</strong> ' . esc_html( (string) count( $active_plugins ) ) . ' active, ' . esc_html( (string) ( count( $all_plugins ) - count( $active_plugins ) ) ) . ' inactive</li>';
		$output .= '<li><strong>' . esc_html__( 'Theme:', 'wpshadow' ) . '</strong> ' . esc_html( $theme->get( 'Name' ) ) . ' v' . esc_html( $theme->get( 'Version' ) ) . '</li>';
		$output .= '<li><strong>' . esc_html__( 'Multisite:', 'wpshadow' ) . '</strong> ' . ( is_multisite() ? esc_html__( 'Yes', 'wpshadow' ) : esc_html__( 'No', 'wpshadow' ) ) . '</li>';
		$output .= '<li><strong>' . esc_html__( 'WooCommerce:', 'wpshadow' ) . '</strong> ' . ( class_exists( 'WooCommerce' ) ? esc_html__( 'Yes', 'wpshadow' ) : esc_html__( 'No', 'wpshadow' ) ) . '</li>';
		$output .= '</ul>';
	...
	private static function generate_plugin_inventory(): string {
		...
		foreach ( $active_plugins as $plugin_file => $plugin_data ) {
			...
			$output .= '<p><strong>' . esc_html__( 'Purpose:', 'wpshadow' ) . '</strong> ' . esc_html( $purpose ) . '</p>';
			$output .= '<p><strong>' . esc_html__( 'What it does:', 'wpshadow' ) . '</strong> ' . esc_html( $what_it_does ) . '</p>';
			$output .= '<p><strong>' . esc_html__( 'Is it essential?', 'wpshadow' ) . '</strong> ' . ( $essential ? '<span class="wps-badge-yes">' . esc_html__( 'YES', 'wpshadow' ) . '</span>' : '<span class="wps-badge-no">' . esc_html__( 'NO', 'wpshadow' ) . '</span>' ) . '</p>';
			$output .= '<p><strong>' . esc_html__( 'If removed:', 'wpshadow' ) . '</strong> ' . esc_html( $consequences ) . '</p>';
			$output .= '</div>';
		}
	...
	private static function generate_customization_audit(): string {
		...
		if ( file_exists( $functions_php ) ) {
			...
			$output .= '<br><small>' . esc_html__( 'Purpose: Adds custom features to your theme', 'wpshadow' ) . '</small>';
			$output .= '</div>';
		}
		...
		if ( ! empty( $custom_plugins ) ) {
			foreach ( $custom_plugins as $plugin_file => $plugin_data ) {
				...
				$output .= '<br><small>' . esc_html__( 'Purpose: Custom functionality', 'wpshadow' ) . '</small>';
				$output .= '<br><small>' . esc_html__( 'Maintenance: May require developer for updates', 'wpshadow' ) . '</small>';
				$output .= '</div>';
			}
		}

		// Standard setup items.
		$output .= '<h4>' . esc_html__( 'Standard Setup (No Customization)', 'wpshadow' ) . '</h4>';
		$output .= '<ul>';
		$output .= '<li><span class="dashicons dashicons-yes-alt"></span> ' . esc_html__( 'Menus', 'wpshadow' ) . '</li>';
		$output .= '<li><span class="dashicons dashicons-yes-alt"></span> ' . esc_html__( 'Widgets', 'wpshadow' ) . '</li>';
		$output .= '<li><span class="dashicons dashicons-yes-alt"></span> ' . esc_html__( 'Pages/Posts', 'wpshadow' ) . '</li>';
		$output .= '</ul>';
	...
	private static function generate_technical_specs(): string {
		...
		$output .= '<li><strong>' . esc_html__( 'WordPress Version:', 'wpshadow' ) . '</strong> ' . esc_html( get_bloginfo( 'version' ) ) . '</li>';
		$output .= '<li><strong>' . esc_html__( 'PHP Version:', 'wpshadow' ) . '</strong> ' . esc_html( phpversion() ) . '</li>';
		$output .= '<li><strong>' . esc_html__( 'MySQL Version:', 'wpshadow' ) . '</strong> ' . esc_html( $wpdb->db_version() ) . '</li>';
		$output .= '<li><strong>' . esc_html__( 'Theme:', 'wpshadow' ) . '</strong> ' . esc_html( $theme->get( 'Name' ) ) . '</li>';
		$output .= '<li><strong>' . esc_html__( 'Plugins:', 'wpshadow' ) . '</strong> ' . esc_html( (string) $active_count ) . ' active, ' . esc_html( (string) ( count( $all_plugins ) - $active_count ) ) . ' inactive</li>';
		$output .= '<li><strong>' . esc_html__( 'Database Tables:', 'wpshadow' ) . '</strong> ' . esc_html( (string) $table_count ) . '</li>';
		$output .= '<li><strong>' . esc_html__( 'Uploads Disk Space:', 'wpshadow' ) . '</strong> ' . esc_html( size_format( $disk_space ) ) . '</li>';
		$output .= '</ul>';
	...
	private static function generate_export( string $format ): array|\WP_Error {
		if ( ! in_array( $format, array( 'html', 'markdown', 'text' ), true ) ) {
			return new \WP_Error( 'invalid_format', __( 'That export format isn\'t supported', 'wpshadow' ) );
		}
	...
	private static function generate_html_export(): string {
		...
		$output .= '<p><em>' . esc_html__( 'Generated:', 'wpshadow' ) . ' ' . esc_html( $date ) . '</em></p>';
		$output .= self::generate_blueprint();
		$output .= '<hr><h2>' . esc_html__( 'Maintenance Schedule', 'wpshadow' ) . '</h2>';
		$output .= self::generate_maintenance_schedule();
		$output .= '<h2>' . esc_html__( 'Emergency Contacts', 'wpshadow' ) . '</h2>';
	...
	private static function generate_maintenance_schedule(): string {
		...
		$output .= '<h3>' . esc_html__( 'Monthly (First Monday)', 'wpshadow' ) . '</h3>';
		$output .= '<ul>';
		$output .= '<li>' . esc_html__( 'Check for plugin updates', 'wpshadow' ) . '</li>';
		$output .= '<li>' . esc_html__( 'Run site audit', 'wpshadow' ) . '</li>';
		$output .= '<li>' . esc_html__( 'Check activity logs', 'wpshadow' ) . '</li>';
		$output .= '<li>' . esc_html__( 'Verify backups working', 'wpshadow' ) . '</li>';
		$output .= '</ul>';
		$output .= '<p><em>' . esc_html__( 'Expected time: 1-2 hours', 'wpshadow' ) . '</em></p>';

		$output .= '<h3>' . esc_html__( 'Quarterly', 'wpshadow' ) . '</h3>';
		$output .= '<ul>';
		$output .= '<li>' . esc_html__( 'Full security audit', 'wpshadow' ) . '</li>';
		$output .= '<li>' . esc_html__( 'Performance optimization', 'wpshadow' ) . '</li>';
		$output .= '<li>' . esc_html__( 'Database cleanup', 'wpshadow' ) . '</li>';
		$output .= '</ul>';
		$output .= '<p><em>' . esc_html__( 'Expected time: 3-4 hours', 'wpshadow' ) . '</em></p>';
		$output .= '</div>';
	...
	private static function generate_emergency_contacts(): string {
		...
		$output .= '<h3>' . esc_html__( 'Emergency Support', 'wpshadow' ) . '</h3>';
		$output .= '<p><strong>' . esc_html__( 'WPS Dashboard Support', 'wpshadow' ) . '</strong><br>';
		$output .= esc_html__( 'Email:', 'wpshadow' ) . ' christopher@wpshadow.com<br>';
		$output .= esc_html__( 'Support Portal:', 'wpshadow' ) . ' <a href="https://wpshadow.com/support" target="_blank">https://wpshadow.com/support</a><br>';
		$output .= esc_html__( '24/7 Emergency SOS available', 'wpshadow' ) . '</p>';

		$output .= '<h3>' . esc_html__( 'Community Support', 'wpshadow' ) . '</h3>';
		$output .= '<p>' . esc_html__( 'WordPress.org Forums:', 'wpshadow' ) . ' <a href="https://wordpress.org/support/" target="_blank">https://wordpress.org/support/</a></p>';
		$output .= '</div>';
	...
	private static function get_plugin_purpose( string $plugin_file ): string {
		...
		$purposes = array(
			'contact-form-7/'                => __( 'Contact form management', 'wpshadow' ),
			'woocommerce/'                   => __( 'E-commerce shop functionality', 'wpshadow' ),
			'wordpress-seo/'                 => __( 'SEO optimization and ranking', 'wpshadow' ),
			'akismet/'                       => __( 'Spam protection', 'wpshadow' ),
			'elementor/'                     => __( 'Page builder and design', 'wpshadow' ),
			'jetpack/'                       => __( 'Security, performance, and marketing', 'wpshadow' ),
			'wordfence/'                     => __( 'Security and firewall', 'wpshadow' ),
			'wpshadow/'        => __( 'Site management and support', 'wpshadow' ),
			'plugin-wpshadow/' => __( 'Site management and support', 'wpshadow' ),
		);
		...
		return __( 'General plugin functionality', 'wpshadow' );
	}
	...
	private static function get_plugin_function( string $plugin_file ): string {
		$functions = array(
			'contact-form-7/'                => __( 'Collects form submissions and sends email notifications', 'wpshadow' ),
			'woocommerce/'                   => __( 'Manages products, orders, and payments', 'wpshadow' ),
			'wordpress-seo/'                 => __( 'Analyzes content and suggests SEO improvements', 'wpshadow' ),
			'akismet/'                       => __( 'Automatically filters spam comments', 'wpshadow' ),
			'elementor/'                     => __( 'Provides drag-and-drop page design tools', 'wpshadow' ),
			'jetpack/'                       => __( 'Adds security features, backups, and traffic tools', 'wpshadow' ),
			'wordfence/'                     => __( 'Scans for malware and blocks attacks', 'wpshadow' ),
			'wpshadow/'        => __( 'Provides dashboard insights and maintenance tools', 'wpshadow' ),
			'plugin-wpshadow/' => __( 'Provides dashboard insights and maintenance tools', 'wpshadow' ),
		);
		...
		return __( 'Adds additional features to your site', 'wpshadow' );
	}
	...
	private static function get_removal_consequences( string $plugin_file ): string {
		$consequences = array(
			'contact-form-7/'                => __( 'Contact forms will stop working', 'wpshadow' ),
			'woocommerce/'                   => __( 'Your entire shop will be disabled', 'wpshadow' ),
			'wordpress-seo/'                 => __( 'You lose SEO guidance (site still works)', 'wpshadow' ),
			'akismet/'                       => __( 'Spam comments will appear on your site', 'wpshadow' ),
			'elementor/'                     => __( 'Page layouts may break or render incorrectly', 'wpshadow' ),
			'jetpack/'                       => __( 'Security features and backups will be disabled', 'wpshadow' ),
			'wordfence/'                     => __( 'Site security monitoring will stop', 'wpshadow' ),
			'wpshadow/'        => __( 'Dashboard management tools will be unavailable', 'wpshadow' ),
			'plugin-wpshadow/' => __( 'Dashboard management tools will be unavailable', 'wpshadow' ),
		);
		...
		return __( 'Some site features may stop working', 'wpshadow' );
	}
