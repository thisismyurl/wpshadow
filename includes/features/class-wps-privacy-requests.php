...
class WPSHADOW_Privacy_Requests {
		...
		if ( class_exists( '\\WPShadow\\WPSHADOW_Activity_Logger' ) ) {
			WPSHADOW_Activity_Logger::log(
				...
				sprintf(
					...
					__( 'Privacy %1$s request submitted by user #%2$d', 'wpshadow' ),
					$request_type,
					$user_id
				),
				array(
			...
			if ( class_exists( '\\WPShadow\\WPSHADOW_Activity_Logger' ) && $request ) {
				WPSHADOW_Activity_Logger::log(
					...
					sprintf(
						...
						__( 'Privacy %1$s request status changed to %2$s', 'wpshadow' ),
	...
	private static function send_request_notification( int $request_id, string $event ): bool {
		...
		switch ( $event ) {
			case 'submitted':
				$subject = __( 'Privacy Request Submitted', 'wpshadow' );
				$message = sprintf(
					/* translators: 1: request type */
					__( 'Your privacy %1$s request has been submitted and is pending review.', 'wpshadow' ),
					$request->request_type
				);
				break;

			case 'approved':
				$subject = __( 'Privacy Request Approved', 'wpshadow' );
				$message = sprintf(
					/* translators: 1: request type */
					__( 'Your privacy %1$s request has been approved and is being processed.', 'wpshadow' ),
					$request->request_type
				);
				break;

			case 'completed':
				$subject = __( 'Privacy Request Completed', 'wpshadow' );
				if ( 'export' === $request->request_type ) {
					...
					$message      = sprintf(
						...
						__( 'Your data export is ready. Download it here: %1$s', 'wpshadow' ),
						$download_url
					);
				} else {
					$message = __( 'Your data erasure request has been completed.', 'wpshadow' );
				}
				break;

			case 'denied':
				$subject = __( 'Privacy Request Denied', 'wpshadow' );
				$message = sprintf(
					/* translators: 1: request type, 2: admin notes */
					__( 'Your privacy %1$s request has been denied. Reason: %2$s', 'wpshadow' ),
					$request->request_type,
					$request->admin_notes ?: __( 'No reason provided', 'wpshadow' )
				);
				break;
		}
	...
	private static function send_admin_notification( int $request_id ): bool {
		...
		$subject     = __( 'New Privacy Request', 'wpshadow' );
		$message     = sprintf(
			/* translators: 1: request type, 2: user ID, 3: admin URL */
			__( 'A new privacy %1$s request has been submitted by user #%2$d. Review it here: %3$s', 'wpshadow' ),
			$request->request_type,
			$request->user_id,
	...
	public static function ajax_submit_request(): void {
		...
		if ( ! is_user_logged_in() ) {
			wp_send_json_error( array( 'message' => __( 'You must be logged in to submit a request.', 'wpshadow' ) ) );
		}
		...
		if ( ! in_array( $request_type, array( 'export', 'erase' ), true ) ) {
			wp_send_json_error( array( 'message' => __( 'That request type isn\'t recognized.', 'wpshadow' ) ) );
		}

		$request_id = self::submit_request( $user_id, $request_type );

		if ( ! $request_id ) {
			wp_send_json_error( array( 'message' => __( 'Couldn\'t submit that request.', 'wpshadow' ) ) );
		}

		wp_send_json_success(
			array(
				'message'    => __( 'Request submitted.', 'wpshadow' ),
				'request_id' => $request_id,
			)
		);
	}
	...
	public static function ajax_process_request(): void {
		...
		if ( ! $request ) {
			wp_send_json_error( array( 'message' => __( 'Request not found.', 'wpshadow' ) ) );
		}
		...
		switch ( $action ) {
			case 'approve':
				...
				wp_send_json_success( array( 'message' => __( 'Request approved and processed.', 'wpshadow' ) ) );
				break;
			...
			case 'deny':
				...
				wp_send_json_success( array( 'message' => __( 'Request denied.', 'wpshadow' ) ) );
				break;

			default:
				wp_send_json_error( array( 'message' => __( 'That action doesn\'t exist.', 'wpshadow' ) ) );
		}
	}
	...
	public static function ajax_download_export(): void {
		if ( ! is_user_logged_in() ) {
			wp_die( esc_html__( 'Access denied.', 'wpshadow' ) );
		}
		...
		if ( ! $request || $request->user_id !== get_current_user_id() ) {
			wp_die( esc_html__( 'You don\'t have access to that request.', 'wpshadow' ) );
		}

		if ( empty( $request->export_file ) ) {
			wp_die( esc_html__( 'Export file not available.', 'wpshadow' ) );
		}
		...
		if ( ! file_exists( $filepath ) ) {
			wp_die( esc_html__( 'Export file not found.', 'wpshadow' ) );
		}
