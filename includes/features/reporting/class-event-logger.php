...
class Event_Logger {
	 ...
	 */
	public static function log_event( string $event_type, string $category, array $data = array() ): string {
	 ...
	 */
	public static function get_events( array $filter = array(), int $limit = 100 ): array {
		$events = get_option( 'wpshadow_events', array() );
	...
	public static function get_statistics( string $category = '' ): array {
		$stats = get_option( 'wpshadow_event_stats', array() );

		if ( empty( $category ) ) {
			return $stats;
		}

		return $stats[ $category ] ?? array();
	}
	 ...
	 * @param array $filter Filters to apply
	 */
	public static function clear_events( array $filter = array() ): void {
		if ( empty( $filter ) ) {
	...
	public static function cleanup_old_events( int $retention_days = self::DEFAULT_RETENTION_DAYS ): void {
		self::clear_events( array( 'older_than_days' => $retention_days ) );
	...
	public static function get_summary(): array {
		...
		return array(
			...
			'events_today' => count( self::get_events( array(), PHP_INT_MAX ) ), // Filter in get_events
