...
final class WPSHADOW_Feature_External_Fonts_Disabler extends WPSHADOW_Abstract_Feature {
	 ...
	 *
	 * @param string $input Whitelist input.
	 * @return string Sanitized whitelist.
	 */
	public function sanitize_whitelist( string $input ): string {
		// Split by newlines, sanitize each URL/domain, remove empty lines
	...
	public function register(): void {
		...
		// Remove external fonts from wp_head.
		...
		// Register AJAX handler for settings.

		// Suppress theme font-face blocks in admin if enabled.
	...
	/**
	 * Remove external fonts from style URLs.
	 *
	 ...
	 * @return bool Whether URL is whitelisted.
	 */
	...
	/**
	 * Log blocked font to browser console if logging is enabled.
	 *
	 * @param string $url Blocked font URL.
	 * @return void
	 */
	private function log_blocked_font( string $url ): void {
		...
		// Add inline script to log to console
	...
	/**
	 * Get list of blocked font domains based on enabled sub-features.
	 *
	 * @return array<string> List of blocked domains.
	 */
	...
	/**
	 * Start output buffering to remove external fonts from HTML.
	 *
	 * @return void
	 */
	...
	/**
	 * Remove external fonts from HTML output.
	 *
	...
	public function remove_external_fonts_from_html( string $html ): string {
		...
		foreach ( $blocked_domains as $domain ) {
			...
			// Remove preconnect and dns-prefetch hints.
			$html = preg_replace(
	...
	/**
	 * Site Health test for External Fonts disabler.
	 *
	 * @return array<string, mixed>
	 */
	...
	/**
	 * Suppress theme font-face styles in admin area.
	 *
	 * Filters the should_print_font_face_styles hook to prevent
	 * theme font-face blocks (wp-fonts-local) from being printed
	 * in the admin area when the setting is enabled.
	 *
	 * @param bool $should_print Whether to print font-face styles.
	 * @return bool False to suppress font-face styles in admin, original value otherwise.
	 */
	public function suppress_admin_font_face_styles( bool $should_print ): bool {
		// Only suppress if we're in admin and the setting is enabled
		if ( is_admin() && get_option( 'wpshadow_external_fonts_suppress_theme_fonts_admin', false ) ) {
			return false;
		}
