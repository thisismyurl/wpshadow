...
class Apply_Family_Fix_Handler extends AJAX_Handler_Base {
    ...
    public static function register() : void {
        add_action( 'wp_ajax_wpshadow_apply_family_fix', [ __CLASS__, 'handle' ] );
    }
    ...
    public static function handle() : void {
        ...
        $family_ids = self::get_post_param( 'family_ids', 'raw', [] );

        // Ensure family_ids is array
        if ( ! is_array( $family_ids ) ) {
            $family_ids = [];
        }

        // Always include the primary finding
        $finding_ids_to_fix = [ $finding_id ];
        ...
        if ( $fix_all_family && ! empty( $family_ids ) ) {
            ...
            $finding_ids_to_fix = array_unique( $finding_ids_to_fix );
        }
        ...
        if ( $successful_count === count( $finding_ids_to_fix ) ) {
            ...
            self::send_error(
                sprintf(
                    __( 'Fixed %d of %d issues', 'wpshadow' ),
                    $successful_count,
