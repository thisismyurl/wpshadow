...
class Save_Workflow_Handler extends AJAX_Handler_Base {
	...
	public static function handle(): void {
		...
		$blocks      = self::get_post_param( 'blocks', 'json', array(), false );
	...
	private static function handle_wizard_format( string $workflow_json ): void {
		...
		// Save workflow
		$workflows = Options_Manager::get_array( 'wpshadow_workflows', array() );
		...
		Activity_Logger::log(
			...
			array(
				...
				'blocks_count' => count( $workflow['blocks'] ?? array() ),
			)
		);
	...
	private static function handle_block_format( string $name, array $blocks, $workflow_id ): void {
		...
		Activity_Logger::log(
			'workflow_saved',
			sprintf( __( 'Workflow saved: "%1$s" with %2$d blocks', 'wpshadow' ), $workflow['name'], count( $blocks ) ),
			'workflows',
			array(
				'workflow_id'  => $workflow['id'],
