...
class Quick_Scan_Handler extends AJAX_Handler_Base
{
	...
	private static function run_quick_scan(): array
	{
					...
					if (null !== $result && is_array($result)) {
						...
						if (class_exists('\\WPShadow\\Core\\Activity_Logger')) {
							Activity_Logger::log(
								...
								$category,
								array('diagnostic' => $diagnostic_class, 'finding_id' => $result['id'] ?? '')
							);
						}
		...
		return array(
			...
			'findings_by_category' => $findings_by_category,
			'message' => sprintf(
				__('Quick Scan completed. Found %d findings from %d diagnostics (skipped: %d).', 'wpshadow'),
				count($findings),
				$completed,
				$skipped
			)
	...
	private static function schedule_quick_scan(): array
	{
		...
		if (class_exists('\\WPShadow\\Core\\Activity_Logger')) {
			Activity_Logger::log(
				...
				'automation',
				array('scan_type' => 'quick_scan', 'frequency' => 'daily')
			);
		}
