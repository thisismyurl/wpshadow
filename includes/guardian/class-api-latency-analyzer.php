...
class API_Latency_Analyzer {
    ...
    public static function init(): void {
        ...
        add_action('http_api_debug', [__CLASS__, 'track_http_request'], 10, 5);
        
        // Save data on shutdown
        add_action('shutdown', [__CLASS__, 'save_api_data']);
        
        // Run hourly analysis
        if (!wp_next_scheduled('wpshadow_analyze_api_latency')) {
            wp_schedule_event(time(), 'hourly', 'wpshadow_analyze_api_latency');
        }
        add_action('wpshadow_analyze_api_latency', [__CLASS__, 'analyze']);
    }
