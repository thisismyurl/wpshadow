...
class Block_Rendering_Performance_Analyzer {
    ...
    public static function init(): void {
        ...
        add_filter('render_block', [__CLASS__, 'track_block_render'], 10, 2);
        
        // Track page render time
        add_action('template_redirect', [__CLASS__, 'start_page_timer']);
        add_action('wp_footer', [__CLASS__, 'save_render_data'], 999);
        
        // Run hourly analysis
        if (!wp_next_scheduled('wpshadow_analyze_block_rendering')) {
            wp_schedule_event(time(), 'hourly', 'wpshadow_analyze_block_rendering');
        }
        add_action('wpshadow_analyze_block_rendering', [__CLASS__, 'analyze']);
    }
    ...
    public static function save_render_data(): void {
        ...
        $stored['blocks'] = array_filter($stored['blocks'], function($item) use ($one_day_ago) {
            return $item['timestamp'] > $one_day_ago;
        });
        $stored['pages'] = array_filter($stored['pages'], function($item) use ($one_day_ago) {
            return $item['timestamp'] > $one_day_ago;
        });
