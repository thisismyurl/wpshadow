...
class Baseline_Manager {
	...
	public static function get_baseline(): array {
		return get_option( 'wpshadow_site_baseline', [] );
	}
	...
	public static function update_baseline( array $findings ): void {
		...
		$baseline['active_plugins_current'] = count( get_option( 'active_plugins', [] ) );
	...
	public static function detect_anomalies(): array {
		...
		$current_active = count( get_option( 'active_plugins', [] ) );
		...
		if ( $current_theme !== $baseline_theme ) {
			$anomalies[] = [
				...
				'severity' => 'info',
				'message'  => sprintf(
					__( 'Theme changed from "%s" to "%s"', 'wpshadow' ),
					$baseline_theme,
					$current_theme
				),
		...
		if ( version_compare( $current_php, $baseline_php, '!=' ) ) {
			$anomalies[] = [
				...
				'severity' => 'warning',
				'message'  => sprintf(
					__( 'PHP version changed from %s to %s', 'wpshadow' ),
					$baseline_php,
					$current_php
				),
		...
		if ( version_compare( $current_wp, $baseline_wp, '!=' ) ) {
			$anomalies[] = [
				...
				'severity' => 'info',
				'message'  => sprintf(
					__( 'WordPress updated from %s to %s', 'wpshadow' ),
					$baseline_wp,
					$current_wp
				),
	...
	public static function get_history( int $limit = 30 ): array {
		$history = get_option( 'wpshadow_baseline_history', [] );
