...
class Cache_Invalidation_Analyzer {
    ...
    public static function init(): void {
        ...
        add_action('clean_post_cache', [__CLASS__, 'track_post_cache_clear']);
        add_action('clean_term_cache', [__CLASS__, 'track_term_cache_clear']);
        add_action('clean_user_cache', [__CLASS__, 'track_user_cache_clear']);
        add_action('clean_comment_cache', [__CLASS__, 'track_comment_cache_clear']);
        
        // Track object cache flushes
        add_action('wp_cache_flush_group', [__CLASS__, 'track_cache_group_flush'], 10, 1);
        
        // Track transient deletions
        add_action('deleted_transient', [__CLASS__, 'track_transient_deletion'], 10, 1);
        ...
        add_action('wpshadow_analyze_cache_invalidation', [__CLASS__, 'analyze']);
    }
    ...
    private static function record_cache_event(string $type, int $object_id = 0, string $extra = ''): void {
        ...
        $events['hourly'] = array_filter($events['hourly'], function($e) use ($one_hour_ago) {
            return $e['timestamp'] > $one_hour_ago;
        });
        
        // Keep only last 24 hours for daily
        $one_day_ago = time() - DAY_IN_SECONDS;
        $events['daily'] = array_filter($events['daily'], function($e) use ($one_day_ago) {
