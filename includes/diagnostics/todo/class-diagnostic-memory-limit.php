...
class Diagnostic_Memory_Limit extends Diagnostic_Base
{
	 ...
	 *
	 * Tests PHP memory limit configuration (64MB recommended, 256MB ideal).
	 *
	 * @return array {
	...
	public static function test_live__memory_limit(): array
	{
		...
		if ($memory_limit < 64) {
			return array(
				'passed'  => false,
				'message' => '✗ Memory limit too low: ' . $memory_limit . 'MB (needs 64MB+)',
			);
		}
		...
		if ($memory_limit < 256) {
			return array(
				'passed'  => true,
				'message' => '✓ Memory limit adequate: ' . $memory_limit . 'MB (ideal: 256MB+)',
			);
		}

		return array(
			'passed'  => true,
			'message' => '✓ Memory limit ideal: ' . $memory_limit . 'MB',
		);
	}
}
