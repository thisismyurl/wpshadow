...
class Diagnostic_Theme_Direct_Edits extends Diagnostic_Base {
	 ...
	 *
	 * Diagnostic: Theme Direct Edits
	 * Slug: theme-direct-edits
	 * 
	 * Test Purpose:
	 * - Verify that check() method returns the correct result based on site state
	 * - PASS: check() returns NULL when diagnostic condition is NOT met (site is healthy)
	 * - FAIL: check() returns array when diagnostic condition IS met (issue found)
	 * - Description: Automatically initialized lean diagnostic for Theme Direct Edits. Optimized for minimal overhead while surfacing high-value signals.
	 *
	 * @return array {
	...
	public static function test_live_theme_direct_edits(): array {
		$result = self::check();
		if ($result === null) {
			return ['passed' => true, 'message' => 'Theme customizations use proper child theme or hooks'];
		}
		$message = $result['description'] ?? 'Direct theme file edits detected';
