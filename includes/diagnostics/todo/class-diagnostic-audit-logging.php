...
class Diagnostic_Audit_Logging extends Diagnostic_Base
{
     ...
     * - Description: Verifies user action logging is enabled.
     *
     * @return array {
     ...
     * }
     */
    public static function test_live_audit_logging(): array
    {
        $result = self::check();
        ...
        $should_pass = $has_audit_logging;
        $diagnostic_passed = is_null($result);
        $test_passes = ($should_pass === $diagnostic_passed);

        return array(
            'passed' => $test_passes,
            'message' => $test_passes ? 'Audit logging check matches site state' :
