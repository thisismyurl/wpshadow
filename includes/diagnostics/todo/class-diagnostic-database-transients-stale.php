...
class Diagnostic_Database_Transients_Stale extends Diagnostic_Base {
	 ...
	 *
	 * Diagnostic: Database Transients Stale
	 * Slug: database-transients-stale
	 * 
	 * Test Purpose:
	 * - Verify that check() method returns the correct result based on site state
	 * - PASS: check() returns NULL when diagnostic condition is NOT met (site is healthy)
	 * - FAIL: check() returns array when diagnostic condition IS met (issue found)
	 * - Description: Automatically initialized lean diagnostic for Database Transients Stale. Optimized for minimal overhead while surfacing high-value signals.
	 *
	 * @return array {
	 ...
	 * }
	 */
	public static function test_live_database_transients_stale(): array {
		$result = self::check();
		if ($result === null) {
			return ['passed' => true, 'message' => 'No excessive stale transients in database'];
		}
		$message = $result['description'] ?? 'Stale transients buildup detected';
