...
class Diagnostic_WCAG_Accessibility extends Diagnostic_Base
{
    ...
    public static function check(): ?array
    {
        ...
        foreach ($theme_files as $file) {
            if (
                strpos(file_get_contents($file), 'skip-link') !== false ||
    ...
    public static function test_live_wcag_accessibility(): array
    {
        ...
        foreach ($theme_files as $file) {
            ...
            if ($file_content && (strpos($file_content, 'skip-link') !== false ||
                strpos($file_content, 'skip to content') !== false)) {
                $has_skip_link = true;
                break;
            }
        }

        $should_pass = ($has_a11y_plugin || $has_skip_link);
        $diagnostic_passed = is_null($result);
        $test_passes = ($should_pass === $diagnostic_passed);

        return array(
            'passed' => $test_passes,
            'message' => $test_passes ? 'WCAG accessibility check matches site state' :
