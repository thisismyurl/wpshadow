...
class Diagnostic_Terms_Of_Service_Exists extends Diagnostic_Base {
	 ...
	 *
	 * Diagnostic: Terms Of Service Exists
	 * Slug: terms-of-service-exists
	 * 
	 * Test Purpose:
	 * - Verify that check() method returns the correct result based on site state
	 * - PASS: check() returns NULL when diagnostic condition is NOT met (site is healthy)
	 * - FAIL: check() returns array when diagnostic condition IS met (issue found)
	 * - Description: Automatically initialized lean diagnostic for Terms Of Service Exists. Optimized for minimal overhead while surfacing high-value signals.
	 *
	 * @return array {
	...
	public static function test_live_terms_of_service_exists(): array {
		$result = self::check();
		if ($result === null) {
			return ['passed' => true, 'message' => 'Terms of service are published and accessible'];
		}
		$message = $result['description'] ?? 'Terms of service availability issue detected';
