...
class Diagnostic_Tap_Target_Size extends Diagnostic_Base {
	...
	private static function get_page_html(): ?string {
		// Get cached or current page HTML
		$html = get_transient( 'wpshadow_page_html_analysis' );

		if ( ! $html ) {
			// Use WordPress admin page capture
			ob_start();
			?>
			<!DOCTYPE html>
			<html>
			...
			<?php
			$html = ob_get_clean();
			set_transient( 'wpshadow_page_html_analysis', $html, HOUR_IN_SECONDS );
		}
