...
class Diagnostic_VIP_Go_Compatibility extends Diagnostic_Base
{
	 ...
	 */
	public static function test_live_vip_go_compatibility(): array
	{
		$is_vip = (defined('WPCOM_IS_VIP_ENV') && WPCOM_IS_VIP_ENV === true) ||
			(defined('VIP_GO_ENV') && VIP_GO_ENV === 'production');
		...
		$expected_issue = $is_vip && (count($found_incompatible) > 0);
		$diagnostic_result    = self::check();
		$diagnostic_has_issue = (null !== $diagnostic_result);
		$test_passes          = ($expected_issue === $diagnostic_has_issue);

		$message = sprintf(
			'VIP environment: %s. Incompatible plugins: %d (%s). Expected diagnostic to %s issue. Diagnostic %s issue. Test: %s',
			$is_vip ? 'YES' : 'NO',
			count($found_incompatible),
			empty($found_incompatible) ? 'none' : implode('; ', $found_incompatible),
