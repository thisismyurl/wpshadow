...
class Diagnostic_Monitor_Social extends Diagnostic_Base {
	public static function check(): ?array {
		return array(
			'id'            => 'monitor-social_proof_presence',
			'title'         => __( 'Social Proof Elements Check', 'wpshadow' ),
			'description'   => __( 'Verifies reviews, testimonials, user counts. Missing = trust signal gap.', 'wpshadow' ),
			'severity'      => 'medium',
			'category'      => 'monitoring',
	...
	public static function test_live__monitor_social_proof_presence(): array {
		...
		if ( $result === null ) {
			return array(
				'passed'  => true,
				'message' => 'Social proof elements are prominently displayed',
			);
		}
