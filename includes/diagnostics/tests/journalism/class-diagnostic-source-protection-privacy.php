...
class Diagnostic_Source_Protection_Privacy extends Diagnostic_Base {
	...
	public static function check() {
		...
		$has_anonymous_system = false;
		$anonymous_plugins    = array(
		...
		foreach ( $active_plugins as $plugin ) {
			foreach ( $anonymous_plugins as $anon_plugin ) {
				if ( stripos( $plugin, $anon_plugin ) !== false ) {
					$has_anonymous_system = true;
					break 2;
				}
			}
		}

		if ( ! $has_anonymous_system ) {
			$issues[] = __( 'No anonymous tip submission system found', 'wpshadow' );
		}
