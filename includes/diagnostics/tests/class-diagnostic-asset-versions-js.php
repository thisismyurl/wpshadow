...
class Diagnostic_Asset_Versions_JS extends Diagnostic_Base {
	...
	public static function check(): ?array {
		...
		return array(
			...
			'title'        => self::$title,
			'description'  => sprintf(
				__( 'Found %1$d JavaScript files with version query strings (?ver=) that could be removed. Examples: %2$s', 'wpshadow' ),
				$versioned_assets,
	...
	public static function test_live_asset_versions_js(): array {
		...
		if ( $result === null ) {
			return array(
				'passed'  => true,
				'message' => 'JavaScript assets have proper version cache-busting configured',
			);
		}
