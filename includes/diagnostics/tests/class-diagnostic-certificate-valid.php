...
class Diagnostic_Certificate_Valid extends Diagnostic_Base {
	 ...
	 *
	 * Diagnostic: Certificate Valid
	 * Slug: certificate-valid
	 * 
	 * Test Purpose:
	 * - Verify that check() method returns the correct result based on site state
	 * - PASS: check() returns NULL when diagnostic condition is NOT met (site is healthy)
	 * - FAIL: check() returns array when diagnostic condition IS met (issue found)
	 * - Description: Automatically initialized lean diagnostic for Certificate Valid. Optimized for minimal overhead while surfacing high-value signals.
	 *
	 * @return array {
	 ...
	 * }
	 */
	public static function test_live_certificate_valid(): array {
		$result = self::check();
		if ($result === null) {
			return ['passed' => true, 'message' => 'SSL certificate is valid and properly configured'];
		}
		$message = $result['description'] ?? 'SSL certificate issue detected';
