...
class Test_SEO_Keyword_Density extends Diagnostic_Base
{
	...
	protected static function analyze_html(string $html, string $checked_url): ?array
	{
		...
		// Remove common words
		$stop_words = ['the', 'be', 'to', 'of', 'and', 'a', 'in', 'that', 'have', 'i', 'it', 'for', 'not', 'on', 'with', 'he', 'as', 'you', 'do', 'at', 'this', 'but', 'his', 'by', 'from'];
		...
		foreach ($top_words as $word => $count) {
			...
			if ($density > 5 && $count > 10) { // More than 5% density
				return [
					...
					'module' => 'SEO',
					'priority' => 3,
					'meta' => ['keyword' => $word, 'count' => $count, 'density' => round($density, 1), 'total_words' => $total_words],
				];
			}
		}
