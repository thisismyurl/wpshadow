	...
	public static function check() {
		...
		if ( $posts_checked > 0 ) {
			...
			if ( $generic_percentage < 20 && $specific_percentage > 30 ) {
				...
				$score = 2;
				$score_details[]   = sprintf( __( '◐ %d%% of content has generic CTAs', 'wpshadow' ), round( $generic_percentage ) );
				$recommendations[] = __( 'Replace generic CTAs with specific ones: "Download Free SEO Checklist" vs "Click Here"', 'wpshadow' );
			} else {
				$score = 0;
				$score_details[]   = sprintf( __( '✗ %d%% of content uses generic CTAs', 'wpshadow' ), round( $generic_percentage ) );
