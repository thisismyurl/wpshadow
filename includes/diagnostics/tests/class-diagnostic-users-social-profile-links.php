 ...
 * ==============================
 *
 * Question to Answer: Social Profile Links
 *
 * Category: Users & Team
 ...
 * Automatically initialized lean diagnostic for Users Social Profile Links. Optimized for minimal over...
 */
...
class Diagnostic_Users_Social_Profile_Links extends Diagnostic_Base {
	...
	public static function check(): ?array {
		// Check how many users have social profiles configured
		global $wpdb;
		$users_with_social = $wpdb->get_var(
			"SELECT COUNT(*) FROM {$wpdb->usermeta}
			 WHERE meta_key IN ('facebook', 'twitter', 'instagram', 'linkedin', 'youtube')
			 AND meta_value != ''"
		);
		...
		// Flag if low social profile adoption
		if ( $users_with_social < ( $total_count * 0.3 ) ) {
			return \WPShadow\Core\Diagnostic_Lean_Checks::build_finding(
				'users-social-profile-links',
				'Users Social Profile Links',
				'Low adoption of social profiles. Encourage team to add social profiles for improved author credibility.',
				'users',
				'low',
				15,
				'users-social-profile-links'
