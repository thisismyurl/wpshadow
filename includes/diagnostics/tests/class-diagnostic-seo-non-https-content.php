...
class Diagnostic_SEO_Non_HTTPS_Content extends Diagnostic_Base
{
	...
	public static function test_live__seo_non_https_content(): array
	{
		...
		$should_find_issue = (! $is_ssl) || ($is_ssl && $mixed > 0);
		$diagnostic_has_issue = (null !== $diagnostic_result);
		$test_passes = ($should_find_issue === $diagnostic_has_issue);

		$message = sprintf(
			'SSL active: %s, mixed-content pages: %d. Expected diagnostic to %s issue. Diagnostic %s issue. Test: %s',
			$is_ssl ? 'YES' : 'NO',
			$mixed,
