...
class Diagnostic_Phishing_Page_Detection extends Diagnostic_Base
{
	...
	public static function test_live__phishing_page_detection(): array
	{
		...
		$flagged_post    = '';
		if (! empty($suspects)) {
			foreach ($suspects as $post) {
				$content = get_post_field('post_content', $post->ID);
				if (preg_match('/password|credit.?card|ssn|account.?number/i', $content)) {
					$found_sensitive = true;
					$flagged_post    = $post->post_title;
					break;
				}
			}
		}
		...
		$should_find_issue    = $found_sensitive;
		$diagnostic_has_issue = (null !== $diagnostic_result);
		$test_passes          = ($should_find_issue === $diagnostic_has_issue);

		$message = sprintf(
			'Suspect posts: %d, sensitive detected: %s%s. Expected diagnostic to %s issue. Diagnostic %s issue. Test: %s',
			is_array($suspects) ? count($suspects) : 0,
			$found_sensitive ? 'YES' : 'NO',
			$flagged_post ? ' (e.g., ' . $flagged_post . ')' : '',
