...
class Test_Plugin_Conflicts extends Diagnostic_Base
{
					...
					if (! empty($matches[1])) {
						foreach ($matches[1] as $class) {
							if (isset($classes_found[$class])) {
								$collisions[] = ['class' => $class, 'plugins' => [$classes_found[$class], $plugin]];
							} else {
			...
			if (file_exists($plugin_file)) {
				...
				foreach ($deprecated_funcs as $func) {
					if (strpos($content, $func) !== false) {
						$usage[] = ['plugin' => $plugin, 'function' => $func];
					}
				}
			}
		}
	...
	private static function detect_conflicts(): array
	{
		...
		$conflict_pairs = [
			['jetpack/jetpack.php', 'vaultpress/vaultpress.php'],
			['yoast-seo/wp-seo.php', 'rank-math/rank-math.php'],
			['elementor/elementor.php', 'beaver-builder-lite-version/fl-builder.php'],
			['wpforms-lite/wpforms.php', 'ninja-forms/ninja-forms.php'],
			['wordfence/wordfence.php', 'sucuri-scanner/sucuri.php'],
		];

		$active_plugins = array_map('strtolower', get_option('active_plugins', []));
