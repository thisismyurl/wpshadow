...
class Diagnostic_RedirectionSecurityAccess extends Diagnostic_Base {
	...
	public static function check() {
		if ( ! class_exists( 'Red_Item' ) ) { if ( isset( $issues ) && ! empty( $issues ) ) {
		return array(
			'id' => self::$slug,
			'title' => self::$title,
			'description' => sprintf(
			...
			'threat_level' => 45,
			'auto_fixable' => false,
			'kb_link' => 'https://wpshadow.com/kb/redirection-security-access',
		);
	}
	return null; }
	...
	if ( ! (! empty( get_option( "redirection_security_access_settings" ) )) ) {
		if ( ! isset( $issues ) ) {
			$issues = array();
		}
