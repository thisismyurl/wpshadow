...
class Test_Unnecessary_Plugins extends Diagnostic_Base
{
	...
	public static function test_plugin_load(): array
	{
		...
		return [
			...
			'description'    => sprintf('%d plugins currently active', count(get_option('active_plugins', []))),
		];
	}
	...
	private static function find_duplicate_plugins(): array
	{
		...
		$duplicate_pairs = [
			['jetpack', 'akismet'],           // Similar spam/security features
			['yoast', 'rank-math'],           // Both SEO plugins
			['woocommerce', 'easy-digital-downloads'], // E-commerce
			['elementor', 'divi'],            // Page builders
			['wpcf7', 'ninja-forms'],         // Contact forms
		];

		$active_plugins = array_map(fn($p) => strtolower($p), get_option('active_plugins', []));
