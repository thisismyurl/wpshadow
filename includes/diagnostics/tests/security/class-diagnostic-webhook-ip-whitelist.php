	...
	public static function check() {
		...
		if ( empty( $github_ips ) ) {
			return array(
				...
				'threat_level' => 70,
				'auto_fixable' => false,
				'kb_link'     => 'https://wpshadow.com/kb/webhook-ip-whitelist?utm_source=wpshadow&utm_medium=plugin&utm_campaign=kb_diagnostics',
			);
		}
		...
		if ( class_exists( '\WPShadow\Core\Secret_Manager' ) ) {
			...
			if ( empty( $webhook_secret ) ) {
				return array(
					...
					'threat_level' => 90,
					'auto_fixable' => false,
					'kb_link'     => 'https://wpshadow.com/kb/webhook-secret-configuration?utm_source=wpshadow&utm_medium=plugin&utm_campaign=kb_diagnostics',
				);
			}
		}
