...
class Deep_Scanner {
	...
	public static function initiate_scan(): array {
		...
		if ( ! Registration_Manager::is_registered() ) {
			return [ 'error' => __( 'Site not registered. Register for free to enable cloud scans.', 'wpshadow' ) ];
		}
		...
		if ( $status['scans_remaining'] <= 0 ) {
			return [
				'error' => sprintf(
					__( 'Scan quota exceeded (%d used this month). %s to continue.', 'wpshadow' ),
					$status['scans_used'],
		...
		if ( empty( $scan_id ) ) {
			return [ 'error' => __( 'Failed to create scan (no ID returned).', 'wpshadow' ) ];
		}
