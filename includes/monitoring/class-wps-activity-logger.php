...
class WPSHADOW_Activity_Logger {
	...
	public static function render_dashboard_activity(): void {
		...
		echo '<h3 style="margin: 0;">' . esc_html__( 'WPS Suite Activity', 'wpshadow' ) . '</h3>';
		...
		echo '<option value="">' . esc_html__( 'All Types', 'wpshadow' ) . '</option>';
		...
		echo '<option value="">' . esc_html__( 'All Modules', 'wpshadow' ) . '</option>';
		...
		echo '<button id="wps-clear-filters" class="button button-small" style="font-size: 12px; display: none;">' . esc_html__( 'Clear', 'wpshadow' ) . '</button>';
		...
		echo '</ul>';
		echo '<div id="wps-no-results" style="display: none; padding: 12px; color: #666; font-style: italic;">' . esc_html__( 'No activity found matching the selected filters.', 'wpshadow' ) . '</div>';

		$dashboard_url = admin_url( 'admin.php?page=wps-dashboard&tab=activity' );
		printf(
			'<p><a href="%s">%s →</a></p>',
			esc_url( $dashboard_url ),
			esc_html__( 'View all activity', 'wpshadow' )
		);
	...
	private static function render_activity_item( array $event ): void {
		...
		$username      = $user ? $user->display_name : __( 'Unknown', 'wpshadow' );
	...
	private static function get_event_type_label( string $event_type ): string {
		$labels = array(
			self::EVENT_MODULE_ACTIVATED   => __( 'Module Activated', 'wpshadow' ),
			self::EVENT_MODULE_DEACTIVATED => __( 'Module Deactivated', 'wpshadow' ),
			self::EVENT_MODULE_INSTALLED   => __( 'Module Installed', 'wpshadow' ),
			self::EVENT_MODULE_UPDATED     => __( 'Module Updated', 'wpshadow' ),
			self::EVENT_VAULT_FILE_ADDED   => __( 'Vault File Added', 'wpshadow' ),
			self::EVENT_VAULT_FILE_REMOVED => __( 'Vault File Removed', 'wpshadow' ),
			self::EVENT_VAULT_VERIFIED     => __( 'Vault Verified', 'wpshadow' ),
			self::EVENT_VAULT_RESTORED     => __( 'Vault Restored', 'wpshadow' ),
			self::EVENT_LICENSE_REGISTERED => __( 'License Registered', 'wpshadow' ),
			self::EVENT_LICENSE_VERIFIED   => __( 'License Verified', 'wpshadow' ),
			self::EVENT_LICENSE_EXPIRED    => __( 'License Expired', 'wpshadow' ),
			self::EVENT_SETTINGS_CHANGED   => __( 'Settings Changed', 'wpshadow' ),
			self::EVENT_ENCRYPTION_CHANGED => __( 'Encryption Changed', 'wpshadow' ),
			self::EVENT_MEDIA_FILE_ADDED   => __( 'Media File Uploaded', 'wpshadow' ),
			self::EVENT_MEDIA_FILE_EDITED  => __( 'Media File Edited', 'wpshadow' ),
			self::EVENT_MEDIA_FILE_DELETED => __( 'Media File Deleted', 'wpshadow' ),
			self::EVENT_ERROR              => __( 'Issue', 'wpshadow' ),
		);
	...
	public static function render_standalone_widget(): void {
		...
		if ( empty( $events ) ) {
			echo '<p>' . esc_html__( 'No recent WPS activity.', 'wpshadow' ) . '</p>';
			return;
		}
	...
	public static function log_vault_verified( int $files_verified, int $files_failed = 0, string $module_source = 'vault' ): bool {
		if ( $files_failed > 0 ) {
			$description = sprintf(
				...
				__( 'Vault check: %1$d files confirmed, %2$d need review', 'wpshadow' ),
				$files_verified,
				$files_failed
			);
		} else {
			$description = sprintf(
				...
				__( 'Vault verified: %d files passed', 'wpshadow' ),
				$files_verified
			);
		}
		...
		if ( $is_valid ) {
			$description = $license_type
				? sprintf(
					...
					__( 'License verified: %s', 'wpshadow' ),
					$license_type
				)
				: __( 'License verified', 'wpshadow' );
		} else {
			$description = __( 'Couldn\'t verify your license. Let\'s check that again.', 'wpshadow' );
		}
