...
class WPSHADOW_Features_Discovery_Widget {
	...
	public static function register_widget(): void {
		...
		wp_add_dashboard_widget(
			...
			__( '🚀 WPShadow Features - Discover What\'s Available', 'wpshadow' ),
		...
		<div class="wps-features-widget">
			...
			<div class="wps-features-stats">
				<div class="wps-stat-box" style="background: #f0f9f1; border: 1px solid #46b450;">
					...
					<div class="wps-stat-label"><?php esc_html_e( 'Features You\'re Using', 'wpshadow' ); ?></div>
				</div>
				<div class="wps-stat-box" style="background: #fef8e7; border: 1px solid #dba617;">
					...
					<div class="wps-stat-label"><?php esc_html_e( 'Ready to Add', 'wpshadow' ); ?></div>
				</div>
			</div>
			...
			<?php if ( $ghost_count > 0 ) : ?>
				<div class="wps-cta-box">
					<h3><?php esc_html_e( '🎁 Free Features Waiting!', 'wpshadow' ); ?></h3>
					<p style="margin: 0 0 15px; opacity: 0.9;">
						<?php
						/* translators: %d: Number of ghost features */
						printf( esc_html__( 'Install free modules to unlock %d additional features. No credit card required.', 'wpshadow' ), absint( $ghost_count ) );
						?>
					</p>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=wpshadow&tab=modules' ) ); ?>" class="wps-cta-button">
						<?php esc_html_e( 'Browse All Modules', 'wpshadow' ); ?>
					</a>
				</div>
			<?php endif; ?>
		</div>
	...
	private static function render_features_list( array $features, int $limit = 5 ): void {
		if ( empty( $features ) ) {
			...
			esc_html_e( 'No features found in this category.', 'wpshadow' );
			echo '</p>';
			return;
		}
				...
				<div class="wps-mini-feature-card <?php echo esc_attr( $card_class ); ?>">
					<div class="wps-feature-title">
						...
						<?php if ( $is_available ) : ?>
							...
							<?php esc_html_e( 'YOU HAVE THIS', 'wpshadow' ); ?>
							</span>
						<?php else : ?>
							...
							<?php esc_html_e( 'GET THIS', 'wpshadow' ); ?>
							</span>
						<?php endif; ?>
					</div>
					...
					<div class="wps-feature-module">
						...
						printf( esc_html__( 'from %s module', 'wpshadow' ), '<strong>' . esc_html( $feature['module_name'] ) . '</strong>' );
						?>
					</div>
					<?php if ( ! $is_available ) : ?>
						<a href="<?php echo esc_url( $feature['install_url'] ); ?>" class="wps-install-link">
							...
							<?php
							/* translators: %s: Module name */
							printf( esc_html__( 'Install %s →', 'wpshadow' ), esc_html( $feature['module_name'] ) );
							?>
						</a>
					<?php endif; ?>
				</div>
		...
		if ( $total_count > $limit ) {
			...
			<p style="text-align: center; margin-top: 15px;">
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=wpshadow&tab=modules' ) ); ?>" style="color: #2271b1; text-decoration: none; font-weight: 600;">
					...
					printf( esc_html__( 'View %d more features →', 'wpshadow' ), absint( $total_count - $limit ) );
					?>
				</a>
			</p>
			<?php
