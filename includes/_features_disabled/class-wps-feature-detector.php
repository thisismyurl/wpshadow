...
class WPSHADOW_Feature_Detector {
	...
	public static function get_capabilities(): array {
		...
		$core_capabilities = array(
			'backup_verification'   => array(
				'available'   => true,
				'level'       => 'core',
				'description' => __( 'Snapshot-based backup verification and restore testing', 'wpshadow' ),
			),
			'automated_scheduling'  => array(
				'available'   => true,
				'level'       => 'core',
				'description' => __( 'Schedule automatic backup verification tests', 'wpshadow' ),
			),
			'integrity_monitoring'  => array(
				'available'   => true,
				'level'       => 'core',
				'description' => __( 'Database and file integrity checks', 'wpshadow' ),
			),
			'restore_simulation'    => array(
				'available'   => true,
				'level'       => 'core',
				'description' => __( 'Test restore procedures in staging environment', 'wpshadow' ),
			),
		);
		...
		$vault_capabilities = array(
			'encrypted_backups'     => array(
				...
				'description' => __( 'AES-256 encrypted backup storage for sensitive data', 'wpshadow' ),
				'upgrade_url' => admin_url( 'admin.php?page=wpshadow&tab=modules&install=vault-wpshadow' ),
			),
			'cloud_offload'         => array(
				'available'   => $has_vault,
				'level'       => $has_vault ? 'vault' : 'upgrade',
				'description' => __( 'Automatic offsite backup to cloud storage providers', 'wpshadow' ),
				'upgrade_url' => admin_url( 'admin.php?page=wpshadow&tab=modules&install=vault-wpshadow' ),
			),
			'file_versioning'       => array(
				'available'   => $has_vault,
				'level'       => $has_vault ? 'vault' : 'upgrade',
				'description' => __( 'Keep multiple versions of files with one-click rollback', 'wpshadow' ),
				'upgrade_url' => admin_url( 'admin.php?page=wpshadow&tab=modules&install=vault-wpshadow' ),
			),
			'compression'           => array(
				'available'   => $has_vault,
				'level'       => $has_vault ? 'vault' : 'upgrade',
				'description' => __( 'Intelligent compression reduces storage by up to 70%', 'wpshadow' ),
				'upgrade_url' => admin_url( 'admin.php?page=wpshadow&tab=modules&install=vault-wpshadow' ),
			),
			'deduplication'         => array(
				'available'   => $has_vault,
				'level'       => $has_vault ? 'vault' : 'upgrade',
				'description' => __( 'Eliminate duplicate files to save space', 'wpshadow' ),
				'upgrade_url' => admin_url( 'admin.php?page=wpshadow&tab=modules&install=vault-wpshadow' ),
			),
			'broken_link_guardian'  => array(
				'available'   => $has_vault,
				'level'       => $has_vault ? 'vault' : 'upgrade',
				'description' => __( 'Prevent broken links when media files are deleted', 'wpshadow' ),
				'upgrade_url' => admin_url( 'admin.php?page=wpshadow&tab=modules&install=vault-wpshadow' ),
			),
		);
	...
	public static function render_upgrade_prompt( string $context = 'backup' ): void {
		...
		<div class="notice notice-info wps-vault-upgrade-prompt" style="border-left-color: #2271b1; padding: 15px;">
			<h3 style="margin-top: 0;">
				...
				<?php esc_html_e( 'Unlock Advanced Backup Features', 'wpshadow' ); ?>
			</h3>
			
			<p>
				<?php esc_html_e( 'Install the free Vault module to enhance your backup capabilities:', 'wpshadow' ); ?>
			</p>
			...
			<p>
				<a href="<?php echo esc_url( $install_url ); ?>" class="button button-primary">
					...
					<?php esc_html_e( 'Install Vault Module (Free)', 'wpshadow' ); ?>
				</a>
			...
			<p class="description">
				<?php esc_html_e( '✓ Free forever  •  ✓ No credit card needed  •  ✓ Install in one click', 'wpshadow' ); ?>
			</p>
		</div>
		<?php
	}
	...
	public static function render_feature_badge( string $level ): void {
		$badges = array(
			'core'    => array(
				'label' => __( 'Core', 'wpshadow' ),
				'color' => '#46b450',
				'icon'  => 'dashicons-yes-alt',
			),
			'vault'   => array(
				'label' => __( 'Vault', 'wpshadow' ),
				'color' => '#2271b1',
				'icon'  => 'dashicons-vault',
			),
			'upgrade' => array(
				'label' => __( 'Upgrade', 'wpshadow' ),
				'color' => '#dba617',
				'icon'  => 'dashicons-lock',
			),
			'pro'     => array(
				'label' => __( 'Pro', 'wpshadow' ),
				'color' => '#9b51e0',
