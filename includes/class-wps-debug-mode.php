 ...
 *
 * Manages WordPress debug constants via TIMU-managed configuration file.
 */
class WPS_Debug_Mode {
	 ...
	 */
	private const DISPLAY_COOKIE = 'wps_debug_display';

	/**
	 * Option key for debug settings.
	 */
	private const OPTION_KEY = 'wps_debug_settings';

	/**
	 * Option key for debug mode timestamp.
	 */
	private const TIMESTAMP_KEY = 'wps_debug_timestamp';
	...
	public static function init(): void {
		...
		// AJAX handlers.
		add_action( 'wp_ajax_wps_toggle_debug', array( __CLASS__, 'ajax_toggle_debug' ) );
		add_action( 'wp_ajax_wps_get_error_log', array( __CLASS__, 'ajax_get_error_log' ) );
		add_action( 'wp_ajax_wps_clear_error_log', array( __CLASS__, 'ajax_clear_error_log' ) );
	...
	public static function add_admin_menu(): void {
		add_submenu_page(
			'wp-support',
			__( 'Debug Tools', 'plugin-wp-support-thisismyurl' ),
			__( 'Debug Tools', 'plugin-wp-support-thisismyurl' ),
			'manage_options',
			'wps-debug-tools',
	...
	public static function render_page(): void {
		...
		require wp_support_PATH . 'includes/views/debug-tools.php';
	}
	...
	private static function write_config_file( array $settings ): bool {
		...
		$content .= " * Auto-generated by WordPress Support Plugin\n";
	...
	private static function create_mu_plugin_loader(): bool {
		...
		$content .= "\$wps_debug_config = WP_CONTENT_DIR . '/" . self::CONFIG_FILE . "';\n";
		$content .= "if ( file_exists( \$wps_debug_config ) ) {\n";
		$content .= "\trequire_once \$wps_debug_config;\n";
		$content .= "}\n";
	...
	private static function set_display_cookie(): void {
		...
		$value = wp_create_nonce( 'wps_debug_display_' . get_current_user_id() );
			...
			<div class="wps-debug-bar-inner">
				...
				<?php if ( $settings['debug_display'] ) : ?>
					<span class="wps-debug-bar-item wps-debug-errors">
						...
						printf( esc_html__( '%d Errors', 'plugin-wp-support-thisismyurl' ), (int) $error_count );
						?>
					</span>
				<?php endif; ?>
				<?php if ( $settings['query_info'] ) : ?>
					<span class="wps-debug-bar-item wps-debug-queries">
						...
						printf( esc_html__( '%1$d Queries (%2$ss)', 'plugin-wp-support-thisismyurl' ), (int) $query_count, number_format_i18n( $query_time, 4 ) );
						?>
					</span>
				<?php endif; ?>
				<?php if ( $settings['memory_usage'] ) : ?>
					<span class="wps-debug-bar-item wps-debug-memory">
						...
						printf( esc_html__( '%s MB', 'plugin-wp-support-thisismyurl' ), number_format_i18n( $memory_usage / 1024 / 1024, 2 ) );
						?>
					</span>
				<?php endif; ?>
			</div>
	...
	public static function ajax_toggle_debug(): void {
		check_ajax_referer( 'wps_debug_tools', 'nonce' );
		...
		if ( empty( $setting ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid setting', 'plugin-wp-support-thisismyurl' ) ) );
		}
		...
		if ( self::update_settings( $settings ) ) {
			wp_send_json_success( array(
				'message' => __( 'Debug setting updated', 'plugin-wp-support-thisismyurl' ),
				'settings' => self::get_settings(),
			) );
		} else {
			wp_send_json_error( array( 'message' => __( 'Failed to update debug setting', 'plugin-wp-support-thisismyurl' ) ) );
		}
	}
	...
	public static function ajax_get_error_log(): void {
		...
		if ( ! file_exists( $log_file ) ) {
			wp_send_json_success( array(
				'content' => __( 'No error log file found.', 'plugin-wp-support-thisismyurl' ),
				'size' => 0,
			) );
		}
	...
	public static function ajax_clear_error_log(): void {
		...
		$log_file = WP_CONTENT_DIR . '/debug.log';
		if ( ! file_exists( $log_file ) ) {
			wp_send_json_success( array( 'message' => __( 'No error log file found.', 'plugin-wp-support-thisismyurl' ) ) );
		}
		...
		if ( file_put_contents( $log_file, '' ) !== false ) {
			...
			wp_send_json_error( array( 'message' => __( 'Failed to clear error log', 'plugin-wp-support-thisismyurl' ) ) );
		}
	}
}
