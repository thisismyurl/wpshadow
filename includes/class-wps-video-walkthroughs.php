...
class WPSHADOW_Video_Walkthroughs {
	...
	private static function register_video_types(): void {
		self::$video_types = array(
			'foundation' => array(
				'label'  => __( 'Foundation Videos', 'wpshadow' ),
				'desc'   => __( 'Essential videos for all WordPress sites', 'wpshadow' ),
				'videos' => array(
					'dashboard-overview' => array(
						'title'       => __( 'Dashboard Overview', 'wpshadow' ),
						'description' => __( 'WordPress dashboard overview, navigation, and basic interface', 'wpshadow' ),
						'duration'    => '3:42',
						'steps'       => array(
					...
					'add-page'           => array(
						'title'       => __( 'Adding a Page', 'wpshadow' ),
						'description' => __( 'Step-by-step guide to creating a new WordPress page', 'wpshadow' ),
						'duration'    => '2:15',
						'steps'       => array(
							'Click Pages → Add New',
							'Enter page title',
					...
					'add-post'           => array(
						'title'       => __( 'Adding a Blog Post', 'wpshadow' ),
						'description' => __( 'Creating and publishing a blog post', 'wpshadow' ),
						'duration'    => '2:08',
						'steps'       => array(
							'Click Posts → Add New',
					...
					'manage-media'       => array(
						'title'       => __( 'Managing Media', 'wpshadow' ),
						'description' => __( 'Uploading and managing media files in WordPress', 'wpshadow' ),
						'duration'    => '1:54',
						'steps'       => array(
			...
			'custom'     => array(
				'label'  => __( 'Your Custom Videos', 'wpshadow' ),
				'desc'   => __( 'Videos based on installed plugins and your site configuration', 'wpshadow' ),
				'videos' => array(),
			),
		);
	...
	private static function detect_plugin_videos(): void {
		...
		if ( is_plugin_active( 'contact-form-7/wp-contact-form-7.php' ) ) {
			self::$video_types['custom']['videos']['contact-form-7'] = array(
				'title'       => __( 'Using Contact Forms', 'wpshadow' ),
				'description' => __( 'How Contact Form 7 captures and manages form submissions', 'wpshadow' ),
				'duration'    => '1:47',
				'steps'       => array(
		...
		if ( class_exists( 'WooCommerce' ) ) {
			self::$video_types['custom']['videos']['woocommerce-products'] = array(
				'title'       => __( 'Managing Products', 'wpshadow' ),
				'description' => __( 'Adding products, setting prices, and managing inventory', 'wpshadow' ),
				'duration'    => '4:23',
				'steps'       => array(
			...
			self::$video_types['custom']['videos']['woocommerce-orders'] = array(
				'title'       => __( 'Processing Orders', 'wpshadow' ),
				'description' => __( 'Managing customer orders and fulfillment', 'wpshadow' ),
				'duration'    => '3:12',
				'steps'       => array(
					'View order list',
					'Open order details',
		...
		if ( class_exists( 'GFForms' ) ) {
			self::$video_types['custom']['videos']['gravity-forms'] = array(
				'title'       => __( 'Working with Gravity Forms', 'wpshadow' ),
				'description' => __( 'Creating forms and processing submissions', 'wpshadow' ),
				'duration'    => '2:34',
				'steps'       => array(
					'Create new form',
					'Add form fields',
		...
		if ( defined( 'ELEMENTOR_VERSION' ) ) {
			self::$video_types['custom']['videos']['elementor-editing'] = array(
				'title'       => __( 'Editing with Elementor', 'wpshadow' ),
				'description' => __( 'Using Elementor page builder for custom layouts', 'wpshadow' ),
				'duration'    => '3:45',
				'steps'       => array(
	...
	public static function enqueue_assets( string $hook ): void {
		...
		wp_localize_script(
			...
			array(
				...
				'strings'  => array(
					'generating'      => __( 'Generating video...', 'wpshadow' ),
					'generationError' => __( 'Video generation failed. Please check service configuration.', 'wpshadow' ),
					'serviceOffline'  => __( 'Video generation service is not configured. Please configure in settings.', 'wpshadow' ),
				),
			)
		);
	}
	...
	public static function ajax_generate_video(): void {
		...
		if ( empty( $video_id ) ) {
			wp_send_json_error( array( 'message' => __( 'That video isn\'t available', 'wpshadow' ) ) );
		}
		...
		if ( empty( $service_url ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'Video generation service not configured. Please configure the service URL in settings.', 'wpshadow' ),
					'type'    => 'not_configured',
				)
			);
		}
		...
		if ( ! $video_config ) {
			wp_send_json_error( array( 'message' => __( 'Video type not found', 'wpshadow' ) ) );
		}
		...
		wp_send_json_success(
			array(
				'message' => __( 'Video generation started. This may take a few minutes.', 'wpshadow' ),
				'video'   => $response,
			)
		);
	}
	...
	private static function request_video_generation( string $service_url, string $video_id, array $config ) {
		...
		if ( 200 !== $status_code ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Video generation service returned error (status %d)', 'wpshadow' ),
					$status_code
				)
			);
		}
		...
		if ( ! $data ) {
			return new \WP_Error( 'invalid_response', __( 'We had trouble generating your video', 'wpshadow' ) );
		}
	...
	public static function ajax_download_video(): void {
		...
		if ( ! isset( $library[ $video_id ] ) ) {
			wp_send_json_error( array( 'message' => __( 'Video not found in library', 'wpshadow' ) ) );
		}
	...
	public static function ajax_check_video_service(): void {
		...
		if ( empty( $service_url ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'Service URL not configured', 'wpshadow' ),
		...
		if ( is_wp_error( $response ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'Service is offline or unreachable', 'wpshadow' ),
					'status'  => 'offline',
				)
			);
		}
		...
		wp_send_json_success(
			array(
				'message' => __( 'Service is online and ready', 'wpshadow' ),
				'status'  => 'online',
			)
		);
	}
	...
	public static function render_page(): void {
		...
		<div class="wrap wps-video-library-page">
			<h1><?php esc_html_e( 'Video Library', 'wpshadow' ); ?></h1>
			<p class="description">
				<?php esc_html_e( 'Auto-generated video walkthroughs of your site functionality. Videos are created on-demand and can be downloaded, shared, or embedded on your help pages.', 'wpshadow' ); ?>
			</p>

			<?php if ( ! $service_configured ) : ?>
				<div class="notice notice-warning">
					<p>
						<strong><?php esc_html_e( 'Video Generation Service Not Configured', 'wpshadow' ); ?></strong>
					</p>
					<p>
						<?php
						echo wp_kses_post(
							sprintf(
								...
								__( 'To generate videos, you need to configure a video generation service. This requires an external Puppeteer/Playwright-based microservice for screen recording and video encoding. <a href="%s">Configure in Settings</a>', 'wpshadow' ),
					...
					<p class="description">
						<?php esc_html_e( 'Recommended: Deploy a dedicated video generation microservice or use a third-party SaaS solution.', 'wpshadow' ); ?>
					</p>
				</div>
			<?php endif; ?>
									...
									<?php if ( $in_library && 'completed' === $video_status ) : ?>
										<p class="wps-video-generated">
											...
											printf(
												...
												esc_html__( 'Generated %s ago', 'wpshadow' ),
								...
								<div class="wps-video-actions">
									...
									<?php else : ?>
										<button type="button" class="button button-primary wps-generate-video" data-video-id="<?php echo esc_attr( $video_id ); ?>" <?php disabled( ! $service_configured ); ?>>
											<span class="dashicons dashicons-video-alt3"></span>
											<?php esc_html_e( 'Generate Video', 'wpshadow' ); ?>
										</button>
									<?php endif; ?>
								</div>
							</div>
		...
		<div id="wps-embed-modal" class="wps-modal" style="display: none;">
			<div class="wps-modal-content">
				<span class="wps-modal-close">&times;</span>
				<div class="wps-modal-body">
					<h2><?php esc_html_e( 'Embed Code', 'wpshadow' ); ?></h2>
					<p><?php esc_html_e( 'Copy this code to embed the video on your site:', 'wpshadow' ); ?></p>
