...
class TIMU_Plugin_Upgrader {
	...
	public function install_plugin( string $download_url, bool $activate = true, bool $network = false ) {
		...
		if ( empty( $download_url ) ) {
			return new WP_Error( 'empty_url', __( 'Download URL is required.', 'core-support-thisismyurl' ) );
		}

		// Initialize filesystem.
		if ( ! $this->init_filesystem() ) {
			return new WP_Error( 'fs_unavailable', __( 'Could not access filesystem.', 'core-support-thisismyurl' ) );
		}
		...
		if ( $wp_filesystem->exists( $destination ) ) {
			if ( ! $wp_filesystem->delete( $destination, true ) ) {
				$this->cleanup( $working_dir );
				return new WP_Error( 'delete_failed', __( 'Could not remove old plugin version.', 'core-support-thisismyurl' ) );
			}
		}
		...
		if ( ! $move_result ) {
			$this->cleanup( $working_dir );
			return new WP_Error( 'move_failed', __( 'Could not move plugin files to destination.', 'core-support-thisismyurl' ) );
		}
		...
		if ( ! file_exists( $plugin_path ) ) {
			return new WP_Error( 'plugin_not_found', __( 'Plugin file not found after extraction.', 'core-support-thisismyurl' ) );
		}
		...
		TIMU_Module_Registry::refresh_modules();
	...
	public function update_plugin( string $plugin_file, string $download_url ) {
		...
		if ( empty( $plugin_file ) || empty( $download_url ) ) {
			return new WP_Error( 'invalid_params', __( 'Plugin file and download URL are required.', 'core-support-thisismyurl' ) );
		}

		// Check plugin exists.
		$plugin_path = WP_PLUGIN_DIR . '/' . $plugin_file;
		if ( ! file_exists( $plugin_path ) ) {
			return new WP_Error( 'plugin_not_found', __( 'Plugin not found.', 'core-support-thisismyurl' ) );
		}
		...
		if ( ! $wp_filesystem->move( $destination, $backup_dir, true ) ) {
			$this->reactivate_plugin( $plugin_file, $was_active_network, $was_active_single );
			$this->cleanup( $working_dir );
			return new WP_Error( 'backup_failed', __( 'Could not backup current plugin version.', 'core-support-thisismyurl' ) );
		}
		...
		if ( ! $move_result ) {
			...
			$this->reactivate_plugin( $plugin_file, $was_active_network, $was_active_single );
			$this->cleanup( $working_dir );
			return new WP_Error( 'move_failed', __( 'Could not move updated plugin files.', 'core-support-thisismyurl' ) );
		}
	...
	private function get_working_directory() {
		...
		if ( is_wp_error( $upload_dir ) || empty( $upload_dir['basedir'] ) ) {
			return new WP_Error( 'upload_dir_error', __( 'Could not determine upload directory.', 'core-support-thisismyurl' ) );
		}

		$working_dir = $upload_dir['basedir'] . '/timu-temp-' . time();

		if ( ! wp_mkdir_p( $working_dir ) ) {
			return new WP_Error( 'mkdir_failed', __( 'Could not create temporary directory.', 'core-support-thisismyurl' ) );
		}
	...
	private function download_package( string $url, string $working_dir ) {
		...
		if ( ! filter_var( $url, FILTER_VALIDATE_URL ) ) {
			return new WP_Error( 'invalid_url', __( 'Invalid download URL.', 'core-support-thisismyurl' ) );
		}
		...
		if ( 200 !== $http_code ) {
			return new WP_Error( 'http_error', sprintf( __( 'HTTP error: %s', 'core-support-thisismyurl' ), intval( $http_code ) ) );
		}

		$zip_file = $working_dir . '/plugin.zip';
		if ( ! file_exists( $zip_file ) ) {
			return new WP_Error( 'download_failed', __( 'Could not download plugin ZIP.', 'core-support-thisismyurl' ) );
		}
	...
	private function unpack_package( string $zip_file, string $working_dir ) {
		...
		if ( ! class_exists( '\ZipArchive' ) ) {
			return new WP_Error( 'no_zip_ext', __( 'ZIP extension not available.', 'core-support-thisismyurl' ) );
		}
		...
		if ( true !== $res ) {
			return new WP_Error( 'bad_zip', __( 'Could not open ZIP file.', 'core-support-thisismyurl' ) );
		}

		if ( ! $zip->extractTo( $working_dir ) ) {
			$zip->close();
			return new WP_Error( 'extract_failed', __( 'Could not extract ZIP file.', 'core-support-thisismyurl' ) );
		}
		...
		if ( false === $files ) {
			return new WP_Error( 'scan_failed', __( 'Could not scan extracted files.', 'core-support-thisismyurl' ) );
		}
		...
		return new WP_Error( 'no_dir_found', __( 'Could not find extracted plugin directory.', 'core-support-thisismyurl' ) );
	}
	...
	private function find_plugin_file( string $working_dir, string $extracted_dir ) {
		...
		if ( false === $files ) {
			return new WP_Error( 'scan_failed', __( 'Could not scan plugin directory.', 'core-support-thisismyurl' ) );
		}
		...
		return new WP_Error( 'no_plugin_file', __( 'Could not find plugin file in extracted directory.', 'core-support-thisismyurl' ) );
	}
	...
	private function get_destination_path( string $plugin_file, string $working_dir ) {
		...
		if ( empty( $dir_name ) ) {
			return new WP_Error( 'invalid_dir_name', __( 'Could not determine plugin directory name.', 'core-support-thisismyurl' ) );
		}
...
/* @changelog TIMU_Plugin_Upgrader class created for install/update/activate flows. */
