...
class WPSHADOW_Magic_Link_Support {
	...
	public static function create_magic_link(
		...
		if ( ! is_email( $developer_email ) || ! is_email( $owner_email ) ) {
			return array(
				'success' => false,
				'error'   => __( 'Please enter a valid email address', 'wpshadow' ),
			);
		}
		...
		if ( class_exists( '\WPShadow\WPSHADOW_Activity_Logger' ) ) {
			WPSHADOW_Activity_Logger::log(
				...
				sprintf(
					...
					__( 'Magic link created for %1$s, expires %2$s', 'wpshadow' ),
					$developer_name,
	...
	public static function handle_magic_link_login(): void {
		...
		if ( ! isset( $links[ $token ] ) ) {
			wp_die(
				esc_html__( 'That link didn\'t work. Please request a new one.', 'wpshadow' ),
				esc_html__( 'Access Denied', 'wpshadow' ),
				array( 'response' => 403 )
			);
		}
		...
		if ( $link_data['expires'] < time() ) {
			wp_die(
				esc_html__( 'This magic link has expired.', 'wpshadow' ),
				esc_html__( 'Link Expired', 'wpshadow' ),
				array( 'response' => 403 )
			);
		}
		...
		if ( ! $user ) {
			...
			if ( is_wp_error( $user_id ) ) {
				wp_die(
					...
					esc_html__( 'Login Issue', 'wpshadow' ),
					array( 'response' => 500 )
				);
			}
		...
		if ( class_exists( '\WPShadow\WPSHADOW_Activity_Logger' ) ) {
			WPSHADOW_Activity_Logger::log(
				...
				sprintf(
					...
					__( 'Magic link login: %s', 'wpshadow' ),
	...
	private static function end_session( string $session_id ): void {
		...
		if ( class_exists( '\WPShadow\WPSHADOW_Activity_Logger' ) ) {
			WPSHADOW_Activity_Logger::log(
				...
				sprintf(
					/* translators: %s: developer name */
					__( 'Magic link session ended: %s', 'wpshadow' ),
	...
	private static function send_summary_email( array $session ): void {
		...
		$subject = sprintf(
			...
			__( '[%1$s] Support Access Summary - %2$s', 'wpshadow' ),
			get_bloginfo( 'name' ),
			$developer_name
		);
		...
		$message = sprintf(
			...
			__( 'Support access by %1$s on %2$s has ended.', 'wpshadow' ) . "\n\n",
			$developer_name,
			get_bloginfo( 'name' )
		);

		$message .= __( 'Summary of changes made:', 'wpshadow' ) . "\n\n";

		if ( empty( $changes ) ) {
			$message .= __( 'No changes were made during this session.', 'wpshadow' ) . "\n";
		} else {
			$message .= sprintf(
				/* translators: %d: number of changes */
				__( 'Total changes: %d', 'wpshadow' ) . "\n\n",
				count( $changes )
			);
				...
				switch ( $type ) {
					...
					case 'plugin_activated':
						$message .= sprintf(
							...
							__( 'Activated plugin: %s', 'wpshadow' ),
							$change['plugin']
						);
						break;
					...
					case 'plugin_deactivated':
						$message .= sprintf(
							/* translators: %s: plugin name */
							__( 'Deactivated plugin: %s', 'wpshadow' ),
							$change['plugin']
						);
						break;
					...
					case 'admin_action':
						if ( ! empty( $change['action'] ) ) {
							$message .= sprintf(
								...
								__( 'Performed action: %s', 'wpshadow' ),
								$change['action']
							);
						} elseif ( ! empty( $change['page'] ) ) {
							$message .= sprintf(
								/* translators: %s: page name */
								__( 'Accessed page: %s', 'wpshadow' ),
								$change['page']
							);
						}
						break;
	...
	public static function revoke_magic_link( string $token ): bool {
		...
		if ( class_exists( '\WPShadow\WPSHADOW_Activity_Logger' ) ) {
			WPSHADOW_Activity_Logger::log(
				'magic_link_revoked',
				__( 'Magic link revoked', 'wpshadow' ),
	...
	public static function ajax_create_magic_link(): void {
		...
		if ( empty( $developer_name ) || empty( $developer_email ) || empty( $owner_email ) ) {
			wp_send_json_error( __( 'All fields are required', 'wpshadow' ) );
		}
		...
		if ( $result['success'] ) {
			...
			wp_send_json_error( $result['error'] ?? __( 'Couldn\'t create that magic link', 'wpshadow' ) );
		}
	}
	...
	public static function ajax_revoke_magic_link(): void {
		...
		if ( empty( $token ) ) {
			wp_send_json_error( __( 'Token is required', 'wpshadow' ) );
		}
		...
		if ( $result ) {
			...
			wp_send_json_error( __( 'Couldn\'t revoke that magic link', 'wpshadow' ) );
		}
	}
	...
	public static function render_admin_page(): void {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Magic Link Support Access', 'wpshadow' ); ?></h1>
			<p class="description">
				<?php esc_html_e( 'Generate secure, time-limited login URLs for developers. All changes made during the session are tracked and a summary is emailed when the link expires.', 'wpshadow' ); ?>
			</p>

			<div style="background: #fff; border: 1px solid #ccd0d4; padding: 20px; margin: 20px 0; max-width: 800px;">
				<h2><?php esc_html_e( 'Create New Magic Link', 'wpshadow' ); ?></h2>
				<form id="wps-create-magic-link-form">
					<table class="form-table">
						<tr>
							<th scope="row">
								<label for="developer_name"><?php esc_html_e( 'Developer Name', 'wpshadow' ); ?></label>
							</th>
							<td>
								<input type="text" id="developer_name" name="developer_name" class="regular-text" required placeholder="<?php esc_attr_e( 'e.g., John Smith', 'wpshadow' ); ?>">
								<p class="description"><?php esc_html_e( 'Name of the developer who will receive access', 'wpshadow' ); ?></p>
							</td>
						</tr>
						<tr>
							<th scope="row">
								<label for="developer_email"><?php esc_html_e( 'Developer Email', 'wpshadow' ); ?></label>
							</th>
							<td>
								<input type="email" id="developer_email" name="developer_email" class="regular-text" required placeholder="<?php esc_attr_e( 'developer@example.com', 'wpshadow' ); ?>">
								<p class="description"><?php esc_html_e( 'Email address for the temporary admin account', 'wpshadow' ); ?></p>
							</td>
						</tr>
						<tr>
							<th scope="row">
								<label for="owner_email"><?php esc_html_e( 'Site Owner Email', 'wpshadow' ); ?></label>
							</th>
							<td>
								<input type="email" id="owner_email" name="owner_email" class="regular-text" required value="<?php echo esc_attr( $admin_email ); ?>">
								<p class="description"><?php esc_html_e( 'Email address to receive the activity summary', 'wpshadow' ); ?></p>
							</td>
						</tr>
						<tr>
							<th scope="row">
								<label for="reason"><?php esc_html_e( 'Reason for Access', 'wpshadow' ); ?></label>
							</th>
							<td>
								<input type="text" id="reason" name="reason" class="regular-text" placeholder="<?php esc_attr_e( 'e.g., Debugging performance issue', 'wpshadow' ); ?>">
								<p class="description"><?php esc_html_e( 'Optional description of why access is needed', 'wpshadow' ); ?></p>
							</td>
						</tr>
					</table>
					<p class="submit">
						<button type="submit" class="button button-primary button-large">
							<?php esc_html_e( '🔐 Generate Magic Link (24-hour expiry)', 'wpshadow' ); ?>
						</button>
					</p>
				</form>
				...
				<div id="wps-magic-link-result" style="display: none; margin-top: 20px; padding: 15px; background: #d4edda; border: 1px solid #c3e6cb; border-radius: 4px;">
					<h3><?php esc_html_e( 'Magic Link Created!', 'wpshadow' ); ?></h3>
					<p><?php esc_html_e( 'Send this link to the developer:', 'wpshadow' ); ?></p>
					...
					<p class="description">
						<?php esc_html_e( 'This link expires in 24 hours and grants full admin access. A summary of changes will be emailed when the link expires or is revoked.', 'wpshadow' ); ?>
					</p>
				</div>
			</div>

			<?php if ( ! empty( $active_links ) ) : ?>
				<h2><?php esc_html_e( 'Active Magic Links', 'wpshadow' ); ?></h2>
				<table class="wp-list-table widefat fixed striped">
					<thead>
						<tr>
							<th><?php esc_html_e( 'Developer', 'wpshadow' ); ?></th>
							<th><?php esc_html_e( 'Email', 'wpshadow' ); ?></th>
							<th><?php esc_html_e( 'Reason', 'wpshadow' ); ?></th>
							<th><?php esc_html_e( 'Created', 'wpshadow' ); ?></th>
							<th><?php esc_html_e( 'Expires', 'wpshadow' ); ?></th>
							<th><?php esc_html_e( 'Uses', 'wpshadow' ); ?></th>
						...
						<?php foreach ( array_reverse( $active_links, true ) as $token => $link_data ) : ?>
							...
							<tr>
								...
								<td>
									...
									<br><small><?php echo esc_html( $time_left ); ?> <?php esc_html_e( 'remaining', 'wpshadow' ); ?></small>
								</td>
								<td><?php echo intval( $link_data['uses'] ); ?></td>
								<td>
									<?php if ( $is_active ) : ?>
										<span style="color: #2ecc71; font-weight: bold;">● <?php esc_html_e( 'Active Session', 'wpshadow' ); ?></span>
									<?php elseif ( $link_data['uses'] > 0 ) : ?>
										<span style="color: #f39c12;">○ <?php esc_html_e( 'Used', 'wpshadow' ); ?></span>
									<?php else : ?>
										<span style="color: #95a5a6;">○ <?php esc_html_e( 'Unused', 'wpshadow' ); ?></span>
									<?php endif; ?>
								</td>
								<td>
			...
			<?php else : ?>
				<div style="background: #f9f9f9; border: 1px solid #ddd; padding: 20px; margin: 20px 0; text-align: center;">
					<p><?php esc_html_e( 'No active magic links. Create one above to grant temporary support access.', 'wpshadow' ); ?></p>
				</div>
			<?php endif; ?>
		</div>
		...
		jQuery(document).ready(function($) {
			...
			$('#wps-copy-magic-link').on('click', function() {
				...
				navigator.clipboard.writeText(link).then(function() {
					...
					btn.text('<?php esc_html_e( 'Copied!', 'wpshadow' ); ?>');
					setTimeout(function() {
						btn.text('<?php esc_html_e( 'Copy Link', 'wpshadow' ); ?>');
					}, 2000);
				});
			});

			$('.wps-revoke-link').on('click', function() {
				if (!confirm('<?php esc_html_e( 'Are you sure you want to revoke this magic link? Any active session will be terminated and a summary email will be sent.', 'wpshadow' ); ?>')) {
					return;
				}
