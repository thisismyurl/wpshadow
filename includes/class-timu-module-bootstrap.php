...
class TIMU_Module_Bootstrap {
	...
	public static function show_missing_notices(): void {
		...
		foreach ( $missing as $slug => $module ) {
			...
			$action_label = $is_installed ? __( 'Activate', 'core-support-thisismyurl' ) : __( 'Install & Activate', 'core-support-thisismyurl' );
				...
				<p>
					...
					echo wp_kses_post(
						sprintf(
							...
							__( '<strong>Core Support:</strong> %1$s is recommended. %2$s', 'core-support-thisismyurl' ),
	...
	public static function handle_install_module(): void {
		if ( ! current_user_can( 'manage_options' ) && ! current_user_can( 'manage_network_options' ) ) {
			wp_die( esc_html__( 'Insufficient permissions.', 'core-support-thisismyurl' ) );
		}
		...
		if ( ! wp_verify_nonce( $nonce, 'timu_module_action' ) ) {
			wp_die( esc_html__( 'Invalid nonce.', 'core-support-thisismyurl' ) );
		}

		$all_modules = array_merge( self::REQUIRED_MODULES, self::OPTIONAL_MODULES );
		if ( ! isset( $all_modules[ $module ] ) ) {
			wp_die( esc_html__( 'Invalid module.', 'core-support-thisismyurl' ) );
		}
	...
	public static function handle_activate_module(): void {
		...
		if ( ! self::is_plugin_installed( $module ) ) {
			wp_die( esc_html__( 'Plugin not installed.', 'core-support-thisismyurl' ) );
		}

		// Activate plugin.
		$plugin_file = self::get_plugin_file( $module );
		if ( ! $plugin_file ) {
			wp_die( esc_html__( 'Plugin file not found.', 'core-support-thisismyurl' ) );
		}
