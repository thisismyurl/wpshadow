...
<div class="wizard-step action-selection">
	<div class="step-header">
		<a href="<?php echo esc_url( admin_url( 'admin.php?page=wpshadow-automation&action=create&step=trigger-config&trigger=' . $trigger_id ) ); ?>" class="back-button">
			<span class="dashicons dashicons-arrow-left-alt2"></span>
			<?php esc_html_e( 'Back', 'wpshadow' ); ?>
		</a>
		<h2><?php esc_html_e( 'Then Do This...', 'wpshadow' ); ?></h2>
	</div>

	<p class="description">
		<?php esc_html_e( 'Choose what actions to perform when your trigger fires. You can add multiple actions.', 'wpshadow' ); ?>
	</p>

	<!-- Selected Actions -->
	<div id="selected-actions" class="selected-actions" style="display: none;">
		<h3><?php esc_html_e( 'Selected Actions', 'wpshadow' ); ?></h3>
			...
			<div class="action-category">
				...
				<div class="action-options">
					<?php foreach ( $category['actions'] as $action_id => $action ) : ?>
						<button class="action-option" data-action-id="<?php echo esc_attr( $action_id ); ?>">
...
jQuery(document).ready(function($) {
	...
	$('#continue-to-review').on('click', function() {
		...
		// Navigate to action config (first action)
		window.location.href = '<?php echo admin_url( 'admin.php?page=wpshadow-automation&action=create&step=action-config' ); ?>&trigger=' + triggerId + '&action_index=0';
	});
});
</script>
