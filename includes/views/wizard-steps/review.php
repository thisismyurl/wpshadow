...
<div class="wizard-step review-step">
	...
	<form id="save-workflow-form" class="save-form">
		...
		<div class="form-actions">
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=wpshadow-automation&action=create&step=action&trigger=' . $trigger_id ) ); ?>" class="button button-large">
...
jQuery(document).ready(function($) {
	...
	function buildSummary() {
		...
		$triggerSection.append('<h3><?php esc_html_e( 'IF This Happens', 'wpshadow' ); ?></h3>');
		...
		$actionsSection.append('<h3><?php esc_html_e( 'THEN Do This', 'wpshadow' ); ?></h3>');
	...
	$('#save-workflow-form').on('submit', function(e) {
		...
		$.post(ajaxurl, {
			...
			if (response.success) {
				...
				setTimeout(function() {
					window.location.href = '<?php echo admin_url( 'admin.php?page=wpshadow-automation' ); ?>';
				}, 1000);
			} else {
