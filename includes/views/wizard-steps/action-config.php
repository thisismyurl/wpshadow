...
if ( empty( $trigger_id ) ) {
	wp_safe_redirect( admin_url( 'admin.php?page=wpshadow-automation&action=create' ) );
	exit;
}
?>
...
<div class="wizard-step action-config">
	<div class="step-header">
		<a href="<?php echo esc_url( admin_url( 'admin.php?page=wpshadow-automation&action=create&step=action&trigger=' . $trigger_id ) ); ?>" class="back-button">
...
jQuery(document).ready(function($) {
	...
	let actions = JSON.parse(sessionStorage.getItem('workflow_actions') || '[]');
	
	if (actions.length === 0 || !actions[actionIndex]) {
		window.location.href = '<?php echo admin_url( 'admin.php?page=wpshadow-automation&action=create&step=action' ); ?>&trigger=' + triggerId;
		return;
	}
	...
	function moveToNextStep() {
		if (actionIndex < actions.length - 1) {
			// Next action config
			window.location.href = '<?php echo admin_url( 'admin.php?page=wpshadow-automation&action=create&step=action-config' ); ?>&trigger=' + triggerId + '&action_index=' + (actionIndex + 1);
		} else {
			// Review
			window.location.href = '<?php echo admin_url( 'admin.php?page=wpshadow-automation&action=create&step=review' ); ?>&trigger=' + triggerId;
		}
	}
});
</script>
