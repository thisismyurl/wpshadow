 ...
 */
class TIMU_License {
	private const OPTION_KEY      = 'timu_suite_license_key';
	private const OPTION_STATUS   = 'timu_suite_license_status';
	private const OPTION_MESSAGE  = 'timu_suite_license_message';
	private const OPTION_CHECKED  = 'timu_suite_license_checked_at';
	private const OPTION_REMINDER = 'timu_suite_license_reminder_at';
	private const REMINDER_MIN    = 6 * MONTH_IN_SECONDS;
	private const REMINDER_MAX    = 12 * MONTH_IN_SECONDS;
	private const ENDPOINT        = 'https://thisismyurl.com/wp-json/timu/v1/license';
	...
	public static function maybe_handle_submission( bool $network ): void {
		if ( empty( $_POST['timu_license_action'] ) ) {
			return;
		}

		$capability = $network && is_multisite() ? 'manage_network_options' : 'manage_options';
		if ( ! current_user_can( $capability ) ) {
			wp_die( esc_html__( 'You do not have permission to manage the license.', 'core-support-thisismyurl' ) );
		}

		$action = sanitize_text_field( wp_unslash( $_POST['timu_license_action'] ) );

		if ( 'broadcast' === $action && $network && is_multisite() ) {
			check_admin_referer( 'timu_license_broadcast', 'timu_license_broadcast_nonce' );

			$key      = isset( $_POST['timu_license_key'] ) ? sanitize_text_field( wp_unslash( $_POST['timu_license_key'] ) ) : '';
			...
			$redirect = add_query_arg(
				array(
					'timu_license_status'  => rawurlencode( $status ),
					'timu_license_message' => rawurlencode( $message ),
				),
				$redirect
			);
		...
		check_admin_referer( 'timu_license_settings', 'timu_license_nonce' );
	...
	public static function validate_key( string $key, bool $network ): array {
		...
		if ( '' === $key ) {
			...
			call_user_func( $update_fn, self::OPTION_MESSAGE, __( 'No key provided.', 'core-support-thisismyurl' ) );
			call_user_func( $update_fn, self::OPTION_CHECKED, time() );
			self::handle_reminder_scheduling( false, $use_network );

			return array(
				'status'  => 'none',
				'message' => __( 'No key provided.', 'core-support-thisismyurl' ),
			);
		}
		...
		$message = is_array( $data ) && isset( $data['message'] ) ? (string) $data['message'] : __( 'License verified.', 'core-support-thisismyurl' );
	...
	private static function maybe_display_notice( bool $use_network ): void {
		...
		$message = sprintf(
			...
			__( 'This site is not registered. Please register to receive updates and support. Visit the <a href="%s">license settings</a>.', 'core-support-thisismyurl' ),
			esc_url( $url )
		);
	...
	public static function broadcast_network_key( string $key, array $site_ids = array(), bool $auto_new = true ): array {
		if ( ! is_multisite() || ! current_user_can( 'manage_network_options' ) ) {
			return array(
				...
				'errors'       => array( __( 'Network admin context required.', 'core-support-thisismyurl' ) ),
				'broadcast_id' => '',
			);
		}
		...
		if ( '' === $key ) {
			return array(
				'success'      => 0,
				'failed'       => 0,
				'errors'       => array( __( 'License key cannot be empty.', 'core-support-thisismyurl' ) ),
				'broadcast_id' => '',
			);
		}
		...
		if ( $auto_new ) {
			update_site_option( 'timu_auto_broadcast_network_key', 'yes' );
		}
	...
	private static function log_broadcast_event( int $site_id, string $broadcast_id, int $user_id, bool $success, string $message ): void {
		...
		update_option( 'timu_license_broadcast_log', $audit_log );
	}
