...
final class WPSHADOW_Site_Health_Integration {
	...
	public static function test_security_score(): array {
		...
		$status = 'good';
		$label  = __( 'Your security configuration is excellent', 'wpshadow' );

		if ( $score < self::STATUS_GOOD ) {
			$status = 'recommended';
			$label  = __( 'Your security could be improved', 'wpshadow' );
		}

		if ( $score < self::STATUS_CRITICAL ) {
			$status = 'critical';
			$label  = __( 'Your security needs immediate attention', 'wpshadow' );
		}
		...
		return array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					__( 'WPShadow Security Score: %d/100', 'wpshadow' ),
					$score
				)
			),
			'actions'     => sprintf(
				...
				__( 'Review Security Settings', 'wpshadow' )
			),
	...
	public static function test_performance_score(): array {
		...
		$status = 'good';
		$label  = __( 'Your performance optimization is excellent', 'wpshadow' );

		if ( $score < self::STATUS_GOOD ) {
			$status = 'recommended';
			$label  = __( 'Your performance could be improved', 'wpshadow' );
		}

		if ( $score < self::STATUS_CRITICAL ) {
			$status = 'critical';
			$label  = __( 'Your performance needs attention', 'wpshadow' );
		}
		...
		return array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					__( 'WPShadow Performance Score: %d/100', 'wpshadow' ),
					$score
				)
			),
			'actions'     => sprintf(
				...
				__( 'Review Performance Settings', 'wpshadow' )
			),
	...
	public static function test_overall_health(): array {
		...
		$status = 'good';
		$label  = __( 'WPShadow is optimally configured', 'wpshadow' );

		if ( $score < self::STATUS_GOOD ) {
			$status = 'recommended';
			$label  = __( 'WPShadow configuration could be improved', 'wpshadow' );
		}

		if ( $score < self::STATUS_CRITICAL ) {
			$status = 'critical';
			$label  = __( 'WPShadow needs configuration', 'wpshadow' );
		}
		...
		return array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					__( 'Overall Health Score: %d/100', 'wpshadow' ),
					$score
				)
			),
			'actions'     => sprintf(
				...
				__( 'View WPShadow Dashboard', 'wpshadow' )
			),
	...
	public static function test_feature_status(): array {
		...
		$label  = $enabled_count > 0
			? __( 'WPShadow features are active', 'wpshadow' )
			: __( 'No WPShadow features are enabled', 'wpshadow' );
		...
		return array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					__( '%1$d of %2$d features enabled', 'wpshadow' ),
					$enabled_count,
					$total_count
				)
			),
	...
	public static function add_debug_information( array $info ): array {
		...
		$info['wpshadow'] = array(
			...
			'fields' => array(
				'version' => array(
					'label' => __( 'Plugin Version', 'wpshadow' ),
					'value' => defined( 'WPSHADOW_VERSION' ) ? WPSHADOW_VERSION : 'Unknown',
				),
				'overall_health' => array(
					'label' => __( 'Overall Health Score', 'wpshadow' ),
					'value' => $overall_score . '/100',
				),
				'enabled_features' => array(
					'label' => __( 'Enabled Features', 'wpshadow' ),
					'value' => count( $enabled_features ),
				),
				'feature_list' => array(
					'label' => __( 'Active Features', 'wpshadow' ),
					'value' => ! empty( $enabled_features ) ? implode( ', ', $enabled_features ) : __( 'None', 'wpshadow' ),
				),
			),
		);
