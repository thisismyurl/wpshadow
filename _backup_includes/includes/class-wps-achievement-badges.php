...
class WPSHADOW_Achievement_Badges {
	...
	public static function get_badge_definitions(): array {
		return array(
			self::BADGE_FIRST_FIX         => array(
				'title'       => __( 'First Fix', 'wpshadow' ),
				'description' => __( 'Activated your first feature or module', 'wpshadow' ),
				'icon'        => 'dashicons-admin-tools',
				'color'       => '#4CAF50',
			),
			self::BADGE_SECURITY_HARDENED => array(
				'title'       => __( 'Security Hardened', 'wpshadow' ),
				'description' => __( 'Enabled security features to protect your site', 'wpshadow' ),
				'icon'        => 'dashicons-lock',
				'color'       => '#F44336',
			),
			self::BADGE_PERFORMANCE_BOOST => array(
				'title'       => __( 'Performance Boosted', 'wpshadow' ),
				'description' => __( 'Optimized your site for better performance', 'wpshadow' ),
				'icon'        => 'dashicons-performance',
				'color'       => '#2196F3',
			),
			self::BADGE_MODULE_MASTER     => array(
				'title'       => __( 'Module Master', 'wpshadow' ),
				'description' => __( 'Activated 5 or more modules', 'wpshadow' ),
				'icon'        => 'dashicons-admin-plugins',
				'color'       => '#9C27B0',
			),
			self::BADGE_VAULT_GUARDIAN    => array(
				'title'       => __( 'Vault Guardian', 'wpshadow' ),
				'description' => __( 'Protected your first file in the vault', 'wpshadow' ),
				'icon'        => 'dashicons-vault',
				'color'       => '#FF9800',
			),
			self::BADGE_EARLY_ADOPTER     => array(
				'title'       => __( 'Early Adopter', 'wpshadow' ),
				'description' => __( 'One of the first to use WPShadow', 'wpshadow' ),
	...
	private static function send_badge_notification( int $user_id, string $badge_id ): void {
		...
		$subject = sprintf(
			...
			__( '[%1$s] You Did It! You Earned: %2$s', 'wpshadow' ),
			get_bloginfo( 'name' ),
			$badge['title']
		);
...
-- The WPShadow Team',
	...
	public static function register_dashboard_widget(): void {
		wp_add_dashboard_widget(
			...
			__( 'Achievement Badges', 'wpshadow' ),
	...
	public static function render_dashboard_widget(): void {
		...
		if ( empty( $badges ) ) {
			echo '<p>' . esc_html__( 'Start using WPShadow features to earn achievement badges!', 'wpshadow' ) . '</p>';
		} else {
			...
			foreach ( $badges as $badge_id => $badge_data ) {
				...
				printf(
					...
					esc_attr( $def['color'] ),
					esc_html( $def['title'] ),
					esc_html( human_time_diff( $badge_data['timestamp'], time() ) . ' ' . __( 'ago', 'wpshadow' ) )
				);
			}
			echo '</div>';
		}
		...
		if ( ! empty( $locked_badges ) ) {
			...
			echo '<h4>' . esc_html__( 'Locked Badges', 'wpshadow' ) . '</h4>';
		...
		echo '<p><a href="' . esc_url( admin_url( 'admin.php?page=wps-achievements' ) ) . '" class="button">' . esc_html__( 'View All Achievements', 'wpshadow' ) . '</a></p>';
		echo '</div>';
	}
	...
	public static function render_achievements_page(): void {
		...
		echo '<h1>' . esc_html__( 'Your Achievements', 'wpshadow' ) . '</h1>';
		...
		printf(
			...
			sprintf(
				...
				esc_html__( 'You have earned %1$d out of %2$d badges (%3$d%%)', 'wpshadow' ),
				(int) $earned_count,
				(int) $total_count,
				(int) $percentage
			)
		...
		// Earned badges.
		echo '<h2>' . esc_html__( 'Earned Badges', 'wpshadow' ) . '</h2>';
			...
			printf(
				...
				esc_html(
					...
					/* translators: %s: Time ago */
						__( 'Earned %s', 'wpshadow' ),
						human_time_diff( $badge_data['timestamp'], time() ) . ' ' . __( 'ago', 'wpshadow' )
					)
				)
			);
		}
		...
		$locked_badges = array_diff_key( $definitions, $badges );
		if ( ! empty( $locked_badges ) ) {
			echo '<h2>' . esc_html__( 'Locked Badges', 'wpshadow' ) . '</h2>';
			echo '<div class="wps-badges-grid">';
			foreach ( $locked_badges as $badge_id => $def ) {
				printf(
