...
class WPSHADOW_Environment_Checker {
	...
	public static function get_php_version_status(): array {
		...
		return array(
			...
			'message'           => $meets_requirement
				? __( 'PHP version is compatible.', 'wpshadow' )
				: sprintf(
					/* translators: 1: Current PHP version, 2: Minimum required PHP version */
					__( 'PHP version %1$s is below the minimum required version %2$s.', 'wpshadow' ),
					$current,
	...
	public static function get_wp_version_status(): array {
		...
		return array(
			...
			'meets_requirement' => $meets_requirement,
			'message'           => $meets_requirement
				? __( 'WordPress version is compatible.', 'wpshadow' )
				: sprintf(
					/* translators: 1: Current WordPress version, 2: Minimum required WordPress version */
					__( 'WordPress version %1$s is below the minimum required version %2$s.', 'wpshadow' ),
					$current,
					self::MIN_WP_VERSION
				),
		);
	...
	private static function get_memory_limit_message( bool $meets_minimum, bool $meets_recommended ): string {
		if ( $meets_recommended ) {
			return __( 'Memory limit meets recommended requirements.', 'wpshadow' );
		}
		if ( $meets_minimum ) {
			return sprintf(
				/* translators: %s: Recommended memory limit */
				__( 'Memory limit meets minimum requirements but %s is recommended for optimal performance.', 'wpshadow' ),
				self::format_bytes( self::RECOMMENDED_MEMORY_LIMIT )
			);
		}
		return sprintf(
			/* translators: %s: Minimum memory limit */
			__( 'Memory limit is below the minimum required (%s). Heavy operations will be disabled.', 'wpshadow' ),
	...
	public static function get_execution_time_status(): array {
		...
		if ( 0 === $max_execution_time ) {
			return array(
				...
				'level'             => 'good',
				'message'           => __( 'Execution time is unlimited.', 'wpshadow' ),
			);
		}
	...
	private static function get_execution_time_message( int $current, bool $meets_minimum, bool $meets_recommended ): string {
		if ( $meets_recommended ) {
			return __( 'Execution time meets recommended requirements.', 'wpshadow' );
		}
		if ( $meets_minimum ) {
			return sprintf(
				/* translators: %d: Recommended execution time in seconds */
				__( 'Execution time meets minimum requirements but %d seconds is recommended.', 'wpshadow' ),
				self::RECOMMENDED_EXECUTION_TIME
			);
		}
		return sprintf(
			/* translators: 1: Current execution time, 2: Minimum execution time */
			__( 'Execution time (%1$ds) is below minimum (%2$ds). Long-running tasks will be batched.', 'wpshadow' ),
			$current,
	...
	public static function get_upload_limit_status(): array {
		...
		return array(
			...
			'message'             => sprintf(
				...
				__( 'Upload limit: %1$s (post limit: %2$s)', 'wpshadow' ),
				$upload_max,
				$post_max
			),
		);
	...
	private static function get_extensions_message( array $required_missing, array $recommended_missing ): string {
		if ( ! empty( $required_missing ) ) {
			return sprintf(
				...
				__( 'Required PHP extensions missing: %s', 'wpshadow' ),
		...
		if ( ! empty( $recommended_missing ) ) {
			return sprintf(
				/* translators: %s: Comma-separated list of missing extensions */
				__( 'Recommended PHP extensions missing: %s. Some features may be limited.', 'wpshadow' ),
				implode( ', ', $recommended_missing )
			);
		}
		return __( 'All required and recommended PHP extensions are loaded.', 'wpshadow' );
	}
	...
	private static function display_incompatibility_notice( array $status ): void {
		...
		if ( ! $status['php_version']['meets_requirement'] ) {
			$issues[] = sprintf(
				/* translators: 1: Current PHP version, 2: Minimum required PHP version */
				__( 'PHP version %1$s (minimum: %2$s required)', 'wpshadow' ),
		...
		if ( ! $status['wp_version']['meets_requirement'] ) {
			$issues[] = sprintf(
				/* translators: 1: Current WordPress version, 2: Minimum required WordPress version */
				__( 'WordPress version %1$s (minimum: %2$s required)', 'wpshadow' ),
		...
		if ( ! $status['memory_limit']['meets_minimum'] ) {
			$issues[] = sprintf(
				...
				__( 'Memory limit %1$s (minimum: %2$s required)', 'wpshadow' ),
		...
		if ( ! $status['execution_time']['meets_minimum'] ) {
			$issues[] = sprintf(
				...
				__( 'Execution time %1$ds (minimum: %2$ds required)', 'wpshadow' ),
		...
		if ( ! empty( $status['extensions']['required_missing'] ) ) {
			$issues[] = sprintf(
				/* translators: %s: Comma-separated list of missing extensions */
				__( 'Missing required PHP extensions: %s', 'wpshadow' ),
		...
		<div class="notice notice-error">
			<p><strong><?php esc_html_e( 'WPShadow: Environment Incompatibility Detected', 'wpshadow' ); ?></strong></p>
			<p><?php esc_html_e( 'Your server environment does not meet the minimum requirements. Heavy operations have been disabled to prevent errors.', 'wpshadow' ); ?></p>
			...
			<p>
				<a href="<?php echo esc_url( admin_url( 'site-health.php' ) ); ?>" class="button">
					<?php esc_html_e( 'View Site Health', 'wpshadow' ); ?>
				</a>
			</p>
		</div>
		<?php
	...
	private static function display_constraint_notice( array $status ): void {
		...
		if ( ! $status['memory_limit']['meets_recommended'] ) {
			$warnings[] = sprintf(
				...
				__( 'Memory limit is %1$s (recommended: %2$s)', 'wpshadow' ),
				$status['memory_limit']['current'],
		...
		if ( ! $status['execution_time']['meets_recommended'] && 0 !== $status['execution_time']['current'] ) {
			$warnings[] = sprintf(
				...
				__( 'Execution time is %1$ds (recommended: %2$ds)', 'wpshadow' ),
				$status['execution_time']['current'],
		...
		if ( ! empty( $status['extensions']['recommended_missing'] ) ) {
			$warnings[] = sprintf(
				/* translators: %s: Comma-separated list of missing extensions */
				__( 'Recommended extensions missing: %s', 'wpshadow' ),
		...
		<div class="notice notice-warning is-dismissible" data-notice-key="wpshadow_environment_constraints" data-dismiss-duration="3600">
			<p><strong><?php esc_html_e( 'WPShadow: Resource Constraints Detected', 'wpshadow' ); ?></strong></p>
			<p><?php esc_html_e( 'Your server is running below recommended specifications. Operations will be batched for optimal performance.', 'wpshadow' ); ?></p>
			<ul style="list-style: disc; margin-left: 20px;">
			...
			<button type="button" class="notice-dismiss">
				<span class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'wpshadow' ); ?></span>
			</button>
		</div>
	...
	public static function get_module_requirements_status( string $module ): array {
		...
		if ( empty( $requirements ) ) {
			return array(
				'supported' => true,
				'missing'   => array(),
				'message'   => __( 'No specific requirements for this module.', 'wpshadow' ),
			);
		}
		...
		return array(
			...
			'message'   => $supported
				? sprintf(
					...
					__( '%s: All requirements met.', 'wpshadow' ),
					ucfirst( $module )
				)
				: sprintf(
					/* translators: 1: Module name, 2: Comma-separated list of missing extensions */
					__( '%1$s: Missing extensions: %2$s', 'wpshadow' ),
					ucfirst( $module ),
					implode( ', ', $missing )
				),
		);
