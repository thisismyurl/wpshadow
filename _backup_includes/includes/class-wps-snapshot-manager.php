...
class WPSHADOW_Snapshot_Manager {
	...
	public static function register_menu(): void {
		add_submenu_page(
			...
			__( 'Site Snapshots', 'wpshadow' ),
			__( 'Snapshots', 'wpshadow' ),
			'manage_options',
			'wps-snapshots',
	...
	public static function render_snapshots_page(): void {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Site Snapshots', 'wpshadow' ); ?></h1>
			<p><?php esc_html_e( 'Snapshots capture your site state for rollback capability.', 'wpshadow' ); ?></p>

			<button id="wps-snapshot-create" class="button button-primary">
				<?php esc_html_e( '📸 Create Snapshot Now', 'wpshadow' ); ?>
			</button>

			<?php if ( empty( $snapshots ) ) : ?>
				<p style="margin-top: 20px; padding: 15px; background: #f0f0f0; border-left: 4px solid #ffb900;">
					<?php esc_html_e( 'No snapshots yet. Create one to enable rollback capability.', 'wpshadow' ); ?>
				</p>
			<?php else : ?>
				<table class="wp-list-table widefat striped" style="margin-top: 20px;">
					<thead>
						<tr>
							...
							<th><?php esc_html_e( 'Plugins', 'wpshadow' ); ?></th>
							<th><?php esc_html_e( 'Theme', 'wpshadow' ); ?></th>
							<th><?php esc_html_e( 'Database', 'wpshadow' ); ?></th>
