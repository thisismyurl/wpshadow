...
class WPSHADOW_System_Report_Generator {
		...
		wp_localize_script(
			...
			array(
				...
				'strings' => array(
					...
					'generated'    => __( 'Report generated', 'wpshadow' ),
					'copied'       => __( 'Copied to clipboard', 'wpshadow' ),
					'copyFailed'   => __( 'Couldn\'t copy', 'wpshadow' ),
					'creatingLink' => __( 'Creating shareable link...', 'wpshadow' ),
					'linkCreated'  => __( 'Link created', 'wpshadow' ),
					'error'        => __( 'An error occurred', 'wpshadow' ),
				),
			)
		);
	...
	public static function handle_shareable_report(): void {
		...
		if ( ! $link_data || ! is_array( $link_data ) ) {
			wp_die( esc_html__( 'That report link didn\'t work. Please request a new one.', 'wpshadow' ) );
		}
		...
		if ( ! empty( $link_data['password'] ) ) {
			...
			if ( ! wp_check_password( $provided_password, $link_data['password'] ) ) {
				wp_die( esc_html__( 'Incorrect password.', 'wpshadow' ) );
			}
		}
	...
	private static function render_password_form( string $token ): void {
		...
		<head>
			...
			<title><?php esc_html_e( 'Protected Report', 'wpshadow' ); ?></title>
			<style>
				body {
		...
		<body>
			<div class="password-form-container">
				<h1><?php esc_html_e( 'Protected Report', 'wpshadow' ); ?></h1>
				<p><?php esc_html_e( 'This report is password protected. Please enter the password to view.', 'wpshadow' ); ?></p>
				<form method="post">
					<label for="report_password"><?php esc_html_e( 'Password:', 'wpshadow' ); ?></label>
					<input type="password" id="report_password" name="report_password" required />
					<button type="submit"><?php esc_html_e( 'View Report', 'wpshadow' ); ?></button>
				</form>
			</div>
		</body>
		</html>
	...
	private static function render_shareable_report( array $data, int $expires_at ): void {
		...
		<head>
			<meta charset="<?php bloginfo( 'charset' ); ?>">
			<meta name="viewport" content="width=device-width, initial-scale=1">
			<title><?php esc_html_e( 'System Report', 'wpshadow' ); ?></title>
			<style>
				body {
					font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Oxygen-Sans, Ubuntu, Cantarell, "Helvetica Neue", sans-serif;
					background: #f0f0f1;
		...
		<body>
			<div class="report-container">
				<h1><?php esc_html_e( 'System Report', 'wpshadow' ); ?></h1>
				<div class="expiry-notice">
					<?php
					printf(
						/* translators: %s: Expiry date */
						esc_html__( 'This report link expires on: %s', 'wpshadow' ),
						'<strong>' . esc_html( date( 'Y-m-d H:i:s', $expires_at ) ) . '</strong>'
					);
					?>
				</div>
				<div class="actions">
					<button onclick="copyToClipboard()"><?php esc_html_e( 'Copy to Clipboard', 'wpshadow' ); ?></button>
					<button onclick="downloadReport()"><?php esc_html_e( 'Download', 'wpshadow' ); ?></button>
				</div>
			...
			<script>
				function copyToClipboard() {
					...
					navigator.clipboard.writeText(content).then(function() {
						alert('<?php esc_html_e( 'Report copied to clipboard', 'wpshadow' ); ?>');
					}).catch(function() {
						alert('<?php esc_html_e( 'Failed to copy', 'wpshadow' ); ?>');
					});
				}
