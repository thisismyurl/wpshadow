...
use WP_REST_Server;
use WP_Error;
use WPShadow\WPSHADOW_Settings;
...
class WPSHADOW_REST_Settings_Controller extends WPSHADOW_REST_Controller_Base {
	...
	public function get_settings( WP_REST_Request $request ) {
		if ( ! class_exists( '\\WPShadow\\WPSHADOW_Settings' ) ) {
			return $this->error_response(
				...
				__( 'Settings system is not available.', 'wpshadow' ),
				503
			);
		}
	...
	public function update_settings( WP_REST_Request $request ) {
		...
		if ( empty( $module ) ) {
			return $this->error_response(
				'invalid_module',
				__( 'Module parameter is required.', 'wpshadow' ),
				400
			);
		}
		...
		if ( ! is_array( $settings ) ) {
			return $this->error_response(
				'invalid_settings',
				__( 'Settings must be an object.', 'wpshadow' ),
				400
			);
		}
		...
		if ( 0 === $updated_count ) {
			return $this->error_response(
				'update_failed',
				__( 'Couldn\'t save settings.', 'wpshadow' ),
				500
			);
		}
		...
		return $this->success_response(
			...
			sprintf(
				...
				__( '%d settings updated successfully.', 'wpshadow' ),
				$updated_count
			)
		);
	}
	...
	public function reset_settings( WP_REST_Request $request ) {
		...
		if ( $module ) {
			...
			if ( ! $result ) {
				return $this->error_response(
					'reset_failed',
					__( 'Couldn\'t reset module settings.', 'wpshadow' ),
					500
				);
			}
			...
			return $this->success_response(
				...
				sprintf(
					...
					__( 'Settings for %s reset to defaults.', 'wpshadow' ),
					$module
				)
			);
		}
		...
		if ( ! $result ) {
			return $this->error_response(
				'reset_failed',
				__( 'Couldn\'t reset all settings.', 'wpshadow' ),
				500
			);
		}
		...
		return $this->success_response(
			array( 'reset' => 'all' ),
			__( 'All settings reset to defaults.', 'wpshadow' )
		);
	}
