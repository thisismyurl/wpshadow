...
class WPSHADOW_Performance_Monitor {
	...
	public static function get_recommendations(): array {
		...
		if ( isset( $metrics['query_count'] ) && $metrics['query_count'] > 50 ) {
			$recommendations[] = array(
				'type'        => 'warning',
				'title'       => __( 'High Query Count', 'wpshadow' ),
				'description' => sprintf(
					/* translators: %d: number of queries */
					__( 'Query count is high (%d queries) - consider object caching or query optimization.', 'wpshadow' ),
					$metrics['query_count']
				),
			);
		}
		...
		if ( $db_stats['expired_transients'] > 100 ) {
			$recommendations[] = array(
				'type'        => 'warning',
				'title'       => __( 'Expired Transients', 'wpshadow' ),
				'description' => sprintf(
					/* translators: %d: number of expired transients */
					__( 'Database has %d expired transients - cleanup recommended.', 'wpshadow' ),
		...
		if ( isset( $metrics['memory_mb'] ) && $metrics['memory_mb'] > 100 ) {
			...
			$recommendations[] = array(
				'type'        => 'critical',
				'title'       => __( 'High Memory Usage', 'wpshadow' ),
				'description' => sprintf(
					/* translators: 1: current memory usage, 2: memory limit */
					__( 'Memory usage is high (%1$s MB). Current limit: %2$s. Consider increasing WP_MEMORY_LIMIT.', 'wpshadow' ),
					$metrics['memory_mb'],
					$memory_limit
				),
			);
		...
		if ( $db_stats['orphaned_postmeta'] > 0 ) {
			$recommendations[] = array(
				'type'        => 'info',
				'title'       => __( 'Orphaned Data', 'wpshadow' ),
				'description' => sprintf(
					/* translators: %d: number of orphaned postmeta records */
					__( 'Found %d orphaned postmeta records - database cleanup recommended.', 'wpshadow' ),
		...
		if ( isset( $metrics['load_time'] ) && $metrics['load_time'] > 2 ) {
			$recommendations[] = array(
				'type'        => 'warning',
				'title'       => __( 'Slow Page Load', 'wpshadow' ),
				'description' => sprintf(
					/* translators: %s: load time in seconds */
					__( 'Page load time is %s seconds - consider enabling caching and optimization features.', 'wpshadow' ),
					$metrics['load_time']
				),
			);
		}
		...
		if ( $metrics['query_count'] > $thresholds['query_count'] ) {
			self::trigger_alert(
				...
				sprintf(
					...
					__( 'Query count (%1$d) exceeds threshold (%2$d)', 'wpshadow' ),
					$metrics['query_count'],
		...
		if ( $metrics['load_time'] > $thresholds['load_time'] ) {
			self::trigger_alert(
				...
				sprintf(
					...
					__( 'Load time (%1$s s) exceeds threshold (%2$s s)', 'wpshadow' ),
					$metrics['load_time'],
		...
		if ( $metrics['memory_mb'] > $memory_threshold ) {
			self::trigger_alert(
				...
				sprintf(
					...
					__( 'Memory usage (%1$s MB) exceeds 80%% of limit (%2$s MB)', 'wpshadow' ),
					$metrics['memory_mb'],
	...
	public static function ajax_clear_history(): void {
		...
		wp_send_json_success( array( 'message' => __( 'Historical data cleared', 'wpshadow' ) ) );
	}
}
