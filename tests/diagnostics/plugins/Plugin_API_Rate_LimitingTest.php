<?php
/**
 * Test for Plugin API Rate Limiting
 *
...
use WPShadow\Diagnostics\Diagnostic_Plugin_API_Rate_Limiting;

/**
 * Plugin_API_Rate_LimitingTest Class
 */
class Plugin_API_Rate_LimitingTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_Plugin_API_Rate_Limiting' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_Plugin_API_Rate_Limiting', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_Plugin_API_Rate_Limiting', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_Plugin_API_Rate_Limiting::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_Plugin_API_Rate_Limiting' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'plugin-api-rate-limiting', $slug );
	}
