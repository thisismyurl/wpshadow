<?php
/**
 * Test for Classic Editor Tinymce Performance
 *
...
use WPShadow\Diagnostics\Diagnostic_ClassicEditorTinymcePerformance;

/**
 * ClassicEditorTinymcePerformanceTest Class
 */
class ClassicEditorTinymcePerformanceTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_ClassicEditorTinymcePerformance' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_ClassicEditorTinymcePerformance', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_ClassicEditorTinymcePerformance', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_ClassicEditorTinymcePerformance::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_ClassicEditorTinymcePerformance' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'classic-editor-tinymce-performance', $slug );
	}
