<?php
/**
 * Test for ACF Options Page Caching
 *
...
use WPShadow\Diagnostics\Diagnostic_AcfOptionsPageCaching;

/**
 * AcfOptionsPageCachingTest Class
 */
class AcfOptionsPageCachingTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_AcfOptionsPageCaching' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_AcfOptionsPageCaching', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_AcfOptionsPageCaching', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_AcfOptionsPageCaching::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_AcfOptionsPageCaching' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'acf-options-page-caching', $slug );
	}
