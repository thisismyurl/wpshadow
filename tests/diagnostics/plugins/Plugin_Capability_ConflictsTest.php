<?php
/**
 * Test for Plugin Capability Conflicts
 *
...
use WPShadow\Diagnostics\Diagnostic_Plugin_Capability_Conflicts;

/**
 * Plugin_Capability_ConflictsTest Class
 */
class Plugin_Capability_ConflictsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_Plugin_Capability_Conflicts' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_Plugin_Capability_Conflicts', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_Plugin_Capability_Conflicts', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_Plugin_Capability_Conflicts::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_Plugin_Capability_Conflicts' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'plugin-capability-conflicts', $slug );
	}
