<?php
/**
 * Test for Duplicator Database Encryption
 *
...
use WPShadow\Diagnostics\Diagnostic_DuplicatorDatabaseEncryption;

/**
 * DuplicatorDatabaseEncryptionTest Class
 */
class DuplicatorDatabaseEncryptionTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_DuplicatorDatabaseEncryption' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_DuplicatorDatabaseEncryption', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_DuplicatorDatabaseEncryption', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_DuplicatorDatabaseEncryption::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_DuplicatorDatabaseEncryption' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'duplicator-database-encryption', $slug );
	}
