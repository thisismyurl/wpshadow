<?php
/**
 * Test for Kadence Theme Header Builder Queries
 *
...
use WPShadow\Diagnostics\Diagnostic_KadenceThemeHeaderBuilderQueries;

/**
 * KadenceThemeHeaderBuilderQueriesTest Class
 */
class KadenceThemeHeaderBuilderQueriesTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_KadenceThemeHeaderBuilderQueries' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_KadenceThemeHeaderBuilderQueries', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_KadenceThemeHeaderBuilderQueries', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_KadenceThemeHeaderBuilderQueries::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_KadenceThemeHeaderBuilderQueries' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'kadence-theme-header-builder-queries', $slug );
	}
