<?php
/**
 * Test for Wp Super Cache Cdn Support
 *
...
use WPShadow\Diagnostics\Diagnostic_WpSuperCacheCdnSupport;

/**
 * WpSuperCacheCdnSupportTest Class
 */
class WpSuperCacheCdnSupportTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheCdnSupport' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheCdnSupport', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheCdnSupport', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WpSuperCacheCdnSupport::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheCdnSupport' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wp-super-cache-cdn-support', $slug );
	}
