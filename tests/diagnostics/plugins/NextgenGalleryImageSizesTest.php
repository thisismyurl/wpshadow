<?php
/**
 * Test for NextGEN Gallery Image Sizes
 *
...
use WPShadow\Diagnostics\Diagnostic_NextgenGalleryImageSizes;

/**
 * NextgenGalleryImageSizesTest Class
 */
class NextgenGalleryImageSizesTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_NextgenGalleryImageSizes' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_NextgenGalleryImageSizes', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_NextgenGalleryImageSizes', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'class_exists' )
			->with( 'C_NextGEN_Bootstrap' )
			->andReturn( false );
		
		$result = Diagnostic_NextgenGalleryImageSizes::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_NextgenGalleryImageSizes' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'nextgen-gallery-image-sizes', $slug );
	}
