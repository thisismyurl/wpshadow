<?php
/**
 * Test for Plugin Custom Post Type Orphans
 *
...
use WPShadow\Diagnostics\Diagnostic_Plugin_CPT_Orphans;

/**
 * Plugin_CPT_OrphansTest Class
 */
class Plugin_CPT_OrphansTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_Plugin_CPT_Orphans' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_Plugin_CPT_Orphans', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_Plugin_CPT_Orphans', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_Plugin_CPT_Orphans::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_Plugin_CPT_Orphans' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'plugin-cpt-orphans', $slug );
	}
