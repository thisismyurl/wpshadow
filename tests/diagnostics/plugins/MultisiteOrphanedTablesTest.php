<?php
/**
 * Test for Multisite Orphaned Tables
 *
...
use WPShadow\Diagnostics\Diagnostic_MultisiteOrphanedTables;

/**
 * MultisiteOrphanedTablesTest Class
 */
class MultisiteOrphanedTablesTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_MultisiteOrphanedTables' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_MultisiteOrphanedTables', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_MultisiteOrphanedTables', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_MultisiteOrphanedTables::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_MultisiteOrphanedTables' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'multisite-orphaned-tables', $slug );
	}
