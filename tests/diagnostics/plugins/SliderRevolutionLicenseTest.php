<?php
/**
 * Test for Slider Revolution License
 *
...
use WPShadow\Diagnostics\Diagnostic_SliderRevolutionLicense;

/**
 * SliderRevolutionLicenseTest Class
 */
class SliderRevolutionLicenseTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_SliderRevolutionLicense' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_SliderRevolutionLicense', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_SliderRevolutionLicense', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_SliderRevolutionLicense::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_SliderRevolutionLicense' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'slider-revolution-license', $slug );
	}
