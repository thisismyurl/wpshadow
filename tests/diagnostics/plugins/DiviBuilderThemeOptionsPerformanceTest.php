<?php
/**
 * Test for Divi Theme Options Performance
 *
...
use WPShadow\Diagnostics\Diagnostic_DiviBuilderThemeOptionsPerformance;

/**
 * DiviBuilderThemeOptionsPerformanceTest Class
 */
class DiviBuilderThemeOptionsPerformanceTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_DiviBuilderThemeOptionsPerformance' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_DiviBuilderThemeOptionsPerformance', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_DiviBuilderThemeOptionsPerformance', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'function_exists' )
			->with( 'et_divi_fonts_url' )
			->andReturn( false );
		
		$result = Diagnostic_DiviBuilderThemeOptionsPerformance::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_DiviBuilderThemeOptionsPerformance' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'divi-builder-theme-options-performance', $slug );
	}
