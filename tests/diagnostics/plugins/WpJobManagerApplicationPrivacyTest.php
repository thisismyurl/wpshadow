<?php
/**
 * Test for WP Job Manager Application Privacy
 *
...
use WPShadow\Diagnostics\Diagnostic_WpJobManagerApplicationPrivacy;

/**
 * WpJobManagerApplicationPrivacyTest Class
 */
class WpJobManagerApplicationPrivacyTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerApplicationPrivacy' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerApplicationPrivacy', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerApplicationPrivacy', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WpJobManagerApplicationPrivacy::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerApplicationPrivacy' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wp-job-manager-application-privacy', $slug );
	}
