<?php
/**
 * Test for Gallery Categories Taxonomy
 *
...
use WPShadow\Diagnostics\Diagnostic_GalleryCategoriesTaxonomy;

/**
 * GalleryCategoriesTaxonomyTest Class
 */
class GalleryCategoriesTaxonomyTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_GalleryCategoriesTaxonomy' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_GalleryCategoriesTaxonomy', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_GalleryCategoriesTaxonomy', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_GalleryCategoriesTaxonomy::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_GalleryCategoriesTaxonomy' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'gallery-categories-taxonomy', $slug );
	}
