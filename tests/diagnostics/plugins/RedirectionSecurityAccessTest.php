<?php
/**
 * Test for Redirection Access Controls
 *
...
use WPShadow\Diagnostics\Diagnostic_RedirectionSecurityAccess;

/**
 * RedirectionSecurityAccessTest Class
 */
class RedirectionSecurityAccessTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_RedirectionSecurityAccess' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_RedirectionSecurityAccess', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_RedirectionSecurityAccess', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_RedirectionSecurityAccess::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_RedirectionSecurityAccess' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'redirection-security-access', $slug );
	}
