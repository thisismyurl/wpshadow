<?php
/**
 * Test for Paypal Subscription Webhook Verification
 *
...
use WPShadow\Diagnostics\Diagnostic_PaypalSubscriptionWebhookVerification;

/**
 * PaypalSubscriptionWebhookVerificationTest Class
 */
class PaypalSubscriptionWebhookVerificationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_PaypalSubscriptionWebhookVerification' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_PaypalSubscriptionWebhookVerification', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_PaypalSubscriptionWebhookVerification', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'class_exists' )
			->with( 'WC_Gateway_Paypal' )
			->andReturn( false );
		
		$result = Diagnostic_PaypalSubscriptionWebhookVerification::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_PaypalSubscriptionWebhookVerification' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'paypal-subscription-webhook-verification', $slug );
	}
