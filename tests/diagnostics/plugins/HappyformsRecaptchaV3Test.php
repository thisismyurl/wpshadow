<?php
/**
 * Test for Happyforms Recaptcha V3
 *
...
use WPShadow\Diagnostics\Diagnostic_HappyformsRecaptchaV3;

/**
 * HappyformsRecaptchaV3Test Class
 */
class HappyformsRecaptchaV3Test extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_HappyformsRecaptchaV3' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_HappyformsRecaptchaV3', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_HappyformsRecaptchaV3', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_HappyformsRecaptchaV3::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_HappyformsRecaptchaV3' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'happyforms-recaptcha-v3', $slug );
	}
