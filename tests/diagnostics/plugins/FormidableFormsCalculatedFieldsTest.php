<?php
/**
 * Test for Formidable Forms Calculated Fields
 *
...
use WPShadow\Diagnostics\Diagnostic_FormidableFormsCalculatedFields;

/**
 * FormidableFormsCalculatedFieldsTest Class
 */
class FormidableFormsCalculatedFieldsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_FormidableFormsCalculatedFields' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_FormidableFormsCalculatedFields', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_FormidableFormsCalculatedFields', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_FormidableFormsCalculatedFields::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_FormidableFormsCalculatedFields' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'formidable-forms-calculated-fields', $slug );
	}
