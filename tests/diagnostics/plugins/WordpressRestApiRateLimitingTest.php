<?php
/**
 * Test for Wordpress Rest Api Rate Limiting
 *
...
use WPShadow\Diagnostics\Diagnostic_WordpressRestApiRateLimiting;

/**
 * WordpressRestApiRateLimitingTest Class
 */
class WordpressRestApiRateLimitingTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WordpressRestApiRateLimiting' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WordpressRestApiRateLimiting', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WordpressRestApiRateLimiting', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_WordpressRestApiRateLimiting::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WordpressRestApiRateLimiting' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wordpress-rest-api-rate-limiting', $slug );
	}
