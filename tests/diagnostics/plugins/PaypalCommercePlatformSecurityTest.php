<?php
/**
 * Test for Paypal Commerce Platform Security
 *
...
use WPShadow\Diagnostics\Diagnostic_PaypalCommercePlatformSecurity;

/**
 * PaypalCommercePlatformSecurityTest Class
 */
class PaypalCommercePlatformSecurityTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_PaypalCommercePlatformSecurity' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_PaypalCommercePlatformSecurity', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_PaypalCommercePlatformSecurity', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_PaypalCommercePlatformSecurity::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_PaypalCommercePlatformSecurity' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'paypal-commerce-platform-security', $slug );
	}
