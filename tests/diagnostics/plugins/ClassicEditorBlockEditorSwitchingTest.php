<?php
/**
 * Test for Classic Editor Block Editor Switching
 *
...
use WPShadow\Diagnostics\Diagnostic_ClassicEditorBlockEditorSwitching;

/**
 * ClassicEditorBlockEditorSwitchingTest Class
 */
class ClassicEditorBlockEditorSwitchingTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_ClassicEditorBlockEditorSwitching' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_ClassicEditorBlockEditorSwitching', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_ClassicEditorBlockEditorSwitching', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'function_exists' )
			->with( 'classic_editor_init_actions' )
			->andReturn( false );
		
		$result = Diagnostic_ClassicEditorBlockEditorSwitching::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_ClassicEditorBlockEditorSwitching' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'classic-editor-block-editor-switching', $slug );
	}
