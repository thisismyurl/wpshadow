<?php
/**
 * Test for Formidable Forms Api Hooks
 *
...
use WPShadow\Diagnostics\Diagnostic_FormidableFormsApiHooks;

/**
 * FormidableFormsApiHooksTest Class
 */
class FormidableFormsApiHooksTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_FormidableFormsApiHooks' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_FormidableFormsApiHooks', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_FormidableFormsApiHooks', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_FormidableFormsApiHooks::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_FormidableFormsApiHooks' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'formidable-forms-api-hooks', $slug );
	}
