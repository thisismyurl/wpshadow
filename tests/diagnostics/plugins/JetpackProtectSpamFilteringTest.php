<?php
/**
 * Test for Jetpack Protect Spam Filtering
 *
...
use WPShadow\Diagnostics\Diagnostic_JetpackProtectSpamFiltering;

/**
 * JetpackProtectSpamFilteringTest Class
 */
class JetpackProtectSpamFilteringTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_JetpackProtectSpamFiltering' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_JetpackProtectSpamFiltering', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_JetpackProtectSpamFiltering', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_JetpackProtectSpamFiltering::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_JetpackProtectSpamFiltering' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'jetpack-protect-spam-filtering', $slug );
	}
