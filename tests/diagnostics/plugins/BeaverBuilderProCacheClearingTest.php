<?php
/**
 * Test for Beaver Builder Pro Cache Clearing
 *
...
use WPShadow\Diagnostics\Diagnostic_BeaverBuilderProCacheClearing;

/**
 * BeaverBuilderProCacheClearingTest Class
 */
class BeaverBuilderProCacheClearingTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderProCacheClearing' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderProCacheClearing', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderProCacheClearing', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_BeaverBuilderProCacheClearing::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderProCacheClearing' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'beaver-builder-pro-cache-clearing', $slug );
	}
