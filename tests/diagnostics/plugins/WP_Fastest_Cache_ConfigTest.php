<?php
/**
 * Test for WP Fastest Cache Configuration
 *
...
use WPShadow\Diagnostics\Diagnostic_WP_Fastest_Cache_Config;

/**
 * WP_Fastest_Cache_ConfigTest Class
 */
class WP_Fastest_Cache_ConfigTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WP_Fastest_Cache_Config' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WP_Fastest_Cache_Config', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WP_Fastest_Cache_Config', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WP_Fastest_Cache_Config::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WP_Fastest_Cache_Config' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wp-fastest-cache-config', $slug );
	}
