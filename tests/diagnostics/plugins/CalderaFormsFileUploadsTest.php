<?php
/**
 * Test for Caldera Forms File Uploads
 *
...
use WPShadow\Diagnostics\Diagnostic_CalderaFormsFileUploads;

/**
 * CalderaFormsFileUploadsTest Class
 */
class CalderaFormsFileUploadsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsFileUploads' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsFileUploads', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsFileUploads', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_CalderaFormsFileUploads::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsFileUploads' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'caldera-forms-file-uploads', $slug );
	}
