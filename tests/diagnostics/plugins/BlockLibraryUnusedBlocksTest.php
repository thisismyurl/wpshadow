<?php
/**
 * Test for Block Library Unused Blocks
 *
...
use WPShadow\Diagnostics\Diagnostic_BlockLibraryUnusedBlocks;

/**
 * BlockLibraryUnusedBlocksTest Class
 */
class BlockLibraryUnusedBlocksTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BlockLibraryUnusedBlocks' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BlockLibraryUnusedBlocks', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BlockLibraryUnusedBlocks', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_BlockLibraryUnusedBlocks::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BlockLibraryUnusedBlocks' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'block-library-unused-blocks', $slug );
	}
