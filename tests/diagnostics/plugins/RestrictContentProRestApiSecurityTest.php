<?php
/**
 * Test for Restrict Content Pro REST API Security
 *
...
use WPShadow\Diagnostics\Diagnostic_RestrictContentProRestApiSecurity;

/**
 * RestrictContentProRestApiSecurityTest Class
 */
class RestrictContentProRestApiSecurityTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_RestrictContentProRestApiSecurity' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_RestrictContentProRestApiSecurity', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_RestrictContentProRestApiSecurity', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_RestrictContentProRestApiSecurity::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_RestrictContentProRestApiSecurity' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'restrict-content-pro-rest-api-security', $slug );
	}
