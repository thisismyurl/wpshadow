<?php
/**
 * Test for Woocommerce Deposits Payment
 *
...
use WPShadow\Diagnostics\Diagnostic_WoocommerceDepositsPayment;

/**
 * WoocommerceDepositsPaymentTest Class
 */
class WoocommerceDepositsPaymentTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WoocommerceDepositsPayment' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WoocommerceDepositsPayment', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WoocommerceDepositsPayment', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WoocommerceDepositsPayment::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WoocommerceDepositsPayment' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'woocommerce-deposits-payment', $slug );
	}
