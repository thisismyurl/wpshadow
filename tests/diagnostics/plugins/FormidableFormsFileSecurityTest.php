<?php
/**
 * Test for Formidable Forms File Security
 *
...
use WPShadow\Diagnostics\Diagnostic_FormidableFormsFileSecurity;

/**
 * FormidableFormsFileSecurityTest Class
 */
class FormidableFormsFileSecurityTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_FormidableFormsFileSecurity' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_FormidableFormsFileSecurity', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_FormidableFormsFileSecurity', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_FormidableFormsFileSecurity::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_FormidableFormsFileSecurity' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'formidable-forms-file-security', $slug );
	}
