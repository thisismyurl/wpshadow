<?php
/**
 * Test for Qtranslate X Widget Translation
 *
...
use WPShadow\Diagnostics\Diagnostic_QtranslateXWidgetTranslation;

/**
 * QtranslateXWidgetTranslationTest Class
 */
class QtranslateXWidgetTranslationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_QtranslateXWidgetTranslation' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_QtranslateXWidgetTranslation', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_QtranslateXWidgetTranslation', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_QtranslateXWidgetTranslation::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_QtranslateXWidgetTranslation' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'qtranslate-x-widget-translation', $slug );
	}
