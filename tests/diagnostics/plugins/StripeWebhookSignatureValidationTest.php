<?php
/**
 * Test for Stripe Webhook Signature Validation
 *
...
use WPShadow\Diagnostics\Diagnostic_StripeWebhookSignatureValidation;

/**
 * StripeWebhookSignatureValidationTest Class
 */
class StripeWebhookSignatureValidationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_StripeWebhookSignatureValidation' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_StripeWebhookSignatureValidation', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_StripeWebhookSignatureValidation', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_StripeWebhookSignatureValidation::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_StripeWebhookSignatureValidation' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'stripe-webhook-signature-validation', $slug );
	}
