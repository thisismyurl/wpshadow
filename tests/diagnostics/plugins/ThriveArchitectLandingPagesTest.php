<?php
/**
 * Test for Thrive Architect Landing Pages
 *
...
use WPShadow\Diagnostics\Diagnostic_ThriveArchitectLandingPages;

/**
 * ThriveArchitectLandingPagesTest Class
 */
class ThriveArchitectLandingPagesTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_ThriveArchitectLandingPages' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_ThriveArchitectLandingPages', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_ThriveArchitectLandingPages', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_ThriveArchitectLandingPages::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_ThriveArchitectLandingPages' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'thrive-architect-landing-pages', $slug );
	}
