<?php
/**
 * Test for bbPress Subscription Notifications
 *
...
use WPShadow\Diagnostics\Diagnostic_BbpressSubscriptionNotifications;

/**
 * BbpressSubscriptionNotificationsTest Class
 */
class BbpressSubscriptionNotificationsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BbpressSubscriptionNotifications' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BbpressSubscriptionNotifications', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BbpressSubscriptionNotifications', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_BbpressSubscriptionNotifications::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BbpressSubscriptionNotifications' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'bbpress-subscription-notifications', $slug );
	}
