<?php
/**
 * Test for LiteSpeed Server Compatibility
 *
...
use WPShadow\Diagnostics\Diagnostic_LitespeedServerCompatibility;

/**
 * LitespeedServerCompatibilityTest Class
 */
class LitespeedServerCompatibilityTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_LitespeedServerCompatibility' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_LitespeedServerCompatibility', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_LitespeedServerCompatibility', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'function_exists' )
			->with( 'litespeed_server_type' )
			->andReturn( false );
		
		$result = Diagnostic_LitespeedServerCompatibility::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_LitespeedServerCompatibility' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'litespeed-server-compatibility', $slug );
	}
