<?php
/**
 * Test for Contact Form 7 Database Logging
 *
...
use WPShadow\Diagnostics\Diagnostic_ContactForm7DatabaseLogging;

/**
 * ContactForm7DatabaseLoggingTest Class
 */
class ContactForm7DatabaseLoggingTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_ContactForm7DatabaseLogging' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_ContactForm7DatabaseLogging', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_ContactForm7DatabaseLogging', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'class_exists' )
			->with( 'WPCF7_ContactFormDB' )
			->andReturn( false );
		
		$result = Diagnostic_ContactForm7DatabaseLogging::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_ContactForm7DatabaseLogging' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'contact-form-7-database-logging', $slug );
	}
