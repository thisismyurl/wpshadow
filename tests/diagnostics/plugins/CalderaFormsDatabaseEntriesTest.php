<?php
/**
 * Test for Caldera Forms Database Entries
 *
...
use WPShadow\Diagnostics\Diagnostic_CalderaFormsDatabaseEntries;

/**
 * CalderaFormsDatabaseEntriesTest Class
 */
class CalderaFormsDatabaseEntriesTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsDatabaseEntries' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsDatabaseEntries', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsDatabaseEntries', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_CalderaFormsDatabaseEntries::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsDatabaseEntries' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'caldera-forms-database-entries', $slug );
	}
