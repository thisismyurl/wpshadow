<?php
/**
 * Test for Gravity Forms Conditional Logic
 *
...
use WPShadow\Diagnostics\Diagnostic_GravityFormsConditionalLogic;

/**
 * GravityFormsConditionalLogicTest Class
 */
class GravityFormsConditionalLogicTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_GravityFormsConditionalLogic' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_GravityFormsConditionalLogic', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_GravityFormsConditionalLogic', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_GravityFormsConditionalLogic::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_GravityFormsConditionalLogic' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'gravity-forms-conditional-logic', $slug );
	}
