<?php
/**
 * Test for Sucuri File Integrity Monitoring
 *
...
use WPShadow\Diagnostics\Diagnostic_SucuriFileIntegrityMonitoring;

/**
 * SucuriFileIntegrityMonitoringTest Class
 */
class SucuriFileIntegrityMonitoringTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_SucuriFileIntegrityMonitoring' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_SucuriFileIntegrityMonitoring', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_SucuriFileIntegrityMonitoring', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_SucuriFileIntegrityMonitoring::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_SucuriFileIntegrityMonitoring' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'sucuri-file-integrity-monitoring', $slug );
	}
