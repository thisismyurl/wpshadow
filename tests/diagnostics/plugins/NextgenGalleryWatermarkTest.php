<?php
/**
 * Test for NextGEN Gallery Watermark
 *
...
use WPShadow\Diagnostics\Diagnostic_NextgenGalleryWatermark;

/**
 * NextgenGalleryWatermarkTest Class
 */
class NextgenGalleryWatermarkTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_NextgenGalleryWatermark' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_NextgenGalleryWatermark', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_NextgenGalleryWatermark', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_NextgenGalleryWatermark::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_NextgenGalleryWatermark' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'nextgen-gallery-watermark', $slug );
	}
