<?php
/**
 * Test for MonsterInsights Performance
 *
...
use WPShadow\Diagnostics\Diagnostic_MonsterInsights_Performance;

/**
 * MonsterInsights_PerformanceTest Class
 */
class MonsterInsights_PerformanceTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_MonsterInsights_Performance' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_MonsterInsights_Performance', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_MonsterInsights_Performance', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'class_exists' )
			->with( 'MonsterInsights_Lite' )
			->andReturn( false );
		
		$result = Diagnostic_MonsterInsights_Performance::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_MonsterInsights_Performance' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'monsterinsights-performance', $slug );
	}
