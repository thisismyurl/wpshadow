<?php
/**
 * Test for Woocommerce One Page Checkout Validation
 *
...
use WPShadow\Diagnostics\Diagnostic_WoocommerceOnePageCheckoutValidation;

/**
 * WoocommerceOnePageCheckoutValidationTest Class
 */
class WoocommerceOnePageCheckoutValidationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WoocommerceOnePageCheckoutValidation' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WoocommerceOnePageCheckoutValidation', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WoocommerceOnePageCheckoutValidation', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WoocommerceOnePageCheckoutValidation::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WoocommerceOnePageCheckoutValidation' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'woocommerce-one-page-checkout-validation', $slug );
	}
