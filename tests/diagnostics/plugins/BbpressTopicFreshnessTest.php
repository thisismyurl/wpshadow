<?php
/**
 * Test for bbPress Topic Freshness Caching
 *
...
use WPShadow\Diagnostics\Diagnostic_BbpressTopicFreshness;

/**
 * BbpressTopicFreshnessTest Class
 */
class BbpressTopicFreshnessTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BbpressTopicFreshness' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BbpressTopicFreshness', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BbpressTopicFreshness', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_BbpressTopicFreshness::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BbpressTopicFreshness' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'bbpress-topic-freshness', $slug );
	}
