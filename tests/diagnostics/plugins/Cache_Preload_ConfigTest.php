<?php
/**
 * Test for Cache Preload Configuration
 *
...
use WPShadow\Diagnostics\Diagnostic_Cache_Preload_Config;

/**
 * Cache_Preload_ConfigTest Class
 */
class Cache_Preload_ConfigTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_Cache_Preload_Config' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_Cache_Preload_Config', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_Cache_Preload_Config', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_Cache_Preload_Config::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_Cache_Preload_Config' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'cache-preload-config', $slug );
	}
