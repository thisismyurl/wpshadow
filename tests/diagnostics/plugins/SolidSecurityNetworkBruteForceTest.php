<?php
/**
 * Test for Solid Security Network Brute Force
 *
...
use WPShadow\Diagnostics\Diagnostic_SolidSecurityNetworkBruteForce;

/**
 * SolidSecurityNetworkBruteForceTest Class
 */
class SolidSecurityNetworkBruteForceTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_SolidSecurityNetworkBruteForce' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_SolidSecurityNetworkBruteForce', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_SolidSecurityNetworkBruteForce', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_SolidSecurityNetworkBruteForce::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_SolidSecurityNetworkBruteForce' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'solid-security-network-brute-force', $slug );
	}
