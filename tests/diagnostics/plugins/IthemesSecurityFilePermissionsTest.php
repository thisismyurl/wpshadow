<?php
/**
 * Test for Ithemes Security File Permissions
 *
...
use WPShadow\Diagnostics\Diagnostic_IthemesSecurityFilePermissions;

/**
 * IthemesSecurityFilePermissionsTest Class
 */
class IthemesSecurityFilePermissionsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityFilePermissions' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityFilePermissions', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityFilePermissions', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_IthemesSecurityFilePermissions::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityFilePermissions' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'ithemes-security-file-permissions', $slug );
	}
