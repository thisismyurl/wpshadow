<?php
/**
 * Test for Elementor Pro Custom Fonts Loading
 *
...
use WPShadow\Diagnostics\Diagnostic_ElementorProCustomFontsLoading;

/**
 * ElementorProCustomFontsLoadingTest Class
 */
class ElementorProCustomFontsLoadingTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_ElementorProCustomFontsLoading' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_ElementorProCustomFontsLoading', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_ElementorProCustomFontsLoading', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_ElementorProCustomFontsLoading::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_ElementorProCustomFontsLoading' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'elementor-pro-custom-fonts-loading', $slug );
	}
