<?php
/**
 * Test for The Events Calendar Recurring Events
 *
...
use WPShadow\Diagnostics\Diagnostic_EventsCalendarRecurringPerformance;

/**
 * EventsCalendarRecurringPerformanceTest Class
 */
class EventsCalendarRecurringPerformanceTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_EventsCalendarRecurringPerformance' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_EventsCalendarRecurringPerformance', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_EventsCalendarRecurringPerformance', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'class_exists' )
			->with( 'Tribe__Events__Main' )
			->andReturn( false );
		
		$result = Diagnostic_EventsCalendarRecurringPerformance::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_EventsCalendarRecurringPerformance' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'events-calendar-recurring-performance', $slug );
	}
