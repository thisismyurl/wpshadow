<?php
/**
 * Test for Beaver Builder Database Cleanup
 *
...
use WPShadow\Diagnostics\Diagnostic_BeaverBuilderDatabaseCleanup;

/**
 * BeaverBuilderDatabaseCleanupTest Class
 */
class BeaverBuilderDatabaseCleanupTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderDatabaseCleanup' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderDatabaseCleanup', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderDatabaseCleanup', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_BeaverBuilderDatabaseCleanup::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderDatabaseCleanup' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'beaver-builder-database-cleanup', $slug );
	}
