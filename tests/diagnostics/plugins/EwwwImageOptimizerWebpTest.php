<?php
/**
 * Test for Ewww Image Optimizer Webp
 *
...
use WPShadow\Diagnostics\Diagnostic_EwwwImageOptimizerWebp;

/**
 * EwwwImageOptimizerWebpTest Class
 */
class EwwwImageOptimizerWebpTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_EwwwImageOptimizerWebp' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_EwwwImageOptimizerWebp', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_EwwwImageOptimizerWebp', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_EwwwImageOptimizerWebp::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_EwwwImageOptimizerWebp' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'ewww-image-optimizer-webp', $slug );
	}
