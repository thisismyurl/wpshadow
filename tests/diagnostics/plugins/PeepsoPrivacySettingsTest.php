<?php
/**
 * Test for PeepSo Privacy Settings
 *
...
use WPShadow\Diagnostics\Diagnostic_PeepsoPrivacySettings;

/**
 * PeepsoPrivacySettingsTest Class
 */
class PeepsoPrivacySettingsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_PeepsoPrivacySettings' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_PeepsoPrivacySettings', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_PeepsoPrivacySettings', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_PeepsoPrivacySettings::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_PeepsoPrivacySettings' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'peepso-privacy-settings', $slug );
	}
