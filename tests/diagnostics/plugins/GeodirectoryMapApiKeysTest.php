<?php
/**
 * Test for GeoDirectory Map API Keys
 *
...
use WPShadow\Diagnostics\Diagnostic_GeodirectoryMapApiKeys;

/**
 * GeodirectoryMapApiKeysTest Class
 */
class GeodirectoryMapApiKeysTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_GeodirectoryMapApiKeys' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_GeodirectoryMapApiKeys', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_GeodirectoryMapApiKeys', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_GeodirectoryMapApiKeys::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_GeodirectoryMapApiKeys' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'geodirectory-map-api-keys', $slug );
	}
