<?php
/**
 * Test for Gallery Custom Links Security
 *
...
use WPShadow\Diagnostics\Diagnostic_GalleryCustomLinksSecurity;

/**
 * GalleryCustomLinksSecurityTest Class
 */
class GalleryCustomLinksSecurityTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_GalleryCustomLinksSecurity' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_GalleryCustomLinksSecurity', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_GalleryCustomLinksSecurity', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_GalleryCustomLinksSecurity::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_GalleryCustomLinksSecurity' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'gallery-custom-links-security', $slug );
	}
