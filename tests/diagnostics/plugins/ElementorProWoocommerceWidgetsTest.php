<?php
/**
 * Test for Elementor Pro Woocommerce Widgets
 *
...
use WPShadow\Diagnostics\Diagnostic_ElementorProWoocommerceWidgets;

/**
 * ElementorProWoocommerceWidgetsTest Class
 */
class ElementorProWoocommerceWidgetsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_ElementorProWoocommerceWidgets' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_ElementorProWoocommerceWidgets', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_ElementorProWoocommerceWidgets', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_ElementorProWoocommerceWidgets::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_ElementorProWoocommerceWidgets' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'elementor-pro-woocommerce-widgets', $slug );
	}
