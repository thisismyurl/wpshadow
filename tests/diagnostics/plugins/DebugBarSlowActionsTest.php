<?php
/**
 * Test for Debug Bar Slow Actions
 *
...
use WPShadow\Diagnostics\Diagnostic_DebugBarSlowActions;

/**
 * DebugBarSlowActionsTest Class
 */
class DebugBarSlowActionsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_DebugBarSlowActions' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_DebugBarSlowActions', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_DebugBarSlowActions', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_DebugBarSlowActions::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_DebugBarSlowActions' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'debug-bar-slow-actions', $slug );
	}
