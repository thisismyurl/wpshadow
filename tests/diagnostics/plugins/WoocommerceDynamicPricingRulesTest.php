<?php
/**
 * Test for Woocommerce Dynamic Pricing Rules
 *
...
use WPShadow\Diagnostics\Diagnostic_WoocommerceDynamicPricingRules;

/**
 * WoocommerceDynamicPricingRulesTest Class
 */
class WoocommerceDynamicPricingRulesTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WoocommerceDynamicPricingRules' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WoocommerceDynamicPricingRules', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WoocommerceDynamicPricingRules', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WoocommerceDynamicPricingRules::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WoocommerceDynamicPricingRules' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'woocommerce-dynamic-pricing-rules', $slug );
	}
