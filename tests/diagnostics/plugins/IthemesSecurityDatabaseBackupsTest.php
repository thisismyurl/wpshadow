<?php
/**
 * Test for Ithemes Security Database Backups
 *
...
use WPShadow\Diagnostics\Diagnostic_IthemesSecurityDatabaseBackups;

/**
 * IthemesSecurityDatabaseBackupsTest Class
 */
class IthemesSecurityDatabaseBackupsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityDatabaseBackups' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityDatabaseBackups', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityDatabaseBackups', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_IthemesSecurityDatabaseBackups::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityDatabaseBackups' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'ithemes-security-database-backups', $slug );
	}
