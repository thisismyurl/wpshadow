<?php
/**
 * Test for Wp Super Cache Expert Settings
 *
...
use WPShadow\Diagnostics\Diagnostic_WpSuperCacheExpertSettings;

/**
 * WpSuperCacheExpertSettingsTest Class
 */
class WpSuperCacheExpertSettingsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheExpertSettings' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheExpertSettings', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheExpertSettings', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WpSuperCacheExpertSettings::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheExpertSettings' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wp-super-cache-expert-settings', $slug );
	}
