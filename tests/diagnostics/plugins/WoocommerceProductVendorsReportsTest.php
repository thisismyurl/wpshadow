<?php
/**
 * Test for Woocommerce Product Vendors Reports
 *
...
use WPShadow\Diagnostics\Diagnostic_WoocommerceProductVendorsReports;

/**
 * WoocommerceProductVendorsReportsTest Class
 */
class WoocommerceProductVendorsReportsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WoocommerceProductVendorsReports' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WoocommerceProductVendorsReports', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WoocommerceProductVendorsReports', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WoocommerceProductVendorsReports::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WoocommerceProductVendorsReports' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'woocommerce-product-vendors-reports', $slug );
	}
