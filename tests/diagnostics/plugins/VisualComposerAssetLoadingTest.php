<?php
/**
 * Test for Visual Composer Asset Loading
 *
...
use WPShadow\Diagnostics\Diagnostic_VisualComposerAssetLoading;

/**
 * VisualComposerAssetLoadingTest Class
 */
class VisualComposerAssetLoadingTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_VisualComposerAssetLoading' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_VisualComposerAssetLoading', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_VisualComposerAssetLoading', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_VisualComposerAssetLoading::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_VisualComposerAssetLoading' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'visual-composer-asset-loading', $slug );
	}
