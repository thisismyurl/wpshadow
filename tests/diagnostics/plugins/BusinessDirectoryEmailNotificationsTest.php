<?php
/**
 * Test for Business Directory Email Notifications
 *
...
use WPShadow\Diagnostics\Diagnostic_BusinessDirectoryEmailNotifications;

/**
 * BusinessDirectoryEmailNotificationsTest Class
 */
class BusinessDirectoryEmailNotificationsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryEmailNotifications' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryEmailNotifications', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryEmailNotifications', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_BusinessDirectoryEmailNotifications::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryEmailNotifications' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'business-directory-email-notifications', $slug );
	}
