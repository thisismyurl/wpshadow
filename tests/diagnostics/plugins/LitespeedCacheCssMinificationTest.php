<?php
/**
 * Test for Litespeed Cache Css Minification
 *
...
use WPShadow\Diagnostics\Diagnostic_LitespeedCacheCssMinification;

/**
 * LitespeedCacheCssMinificationTest Class
 */
class LitespeedCacheCssMinificationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_LitespeedCacheCssMinification' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_LitespeedCacheCssMinification', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_LitespeedCacheCssMinification', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_LitespeedCacheCssMinification::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_LitespeedCacheCssMinification' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'litespeed-cache-css-minification', $slug );
	}
