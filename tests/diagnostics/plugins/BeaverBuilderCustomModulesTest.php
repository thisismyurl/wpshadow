<?php
/**
 * Test for Beaver Builder Custom Modules
 *
...
use WPShadow\Diagnostics\Diagnostic_BeaverBuilderCustomModules;

/**
 * BeaverBuilderCustomModulesTest Class
 */
class BeaverBuilderCustomModulesTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderCustomModules' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderCustomModules', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderCustomModules', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_BeaverBuilderCustomModules::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderCustomModules' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'beaver-builder-custom-modules', $slug );
	}
