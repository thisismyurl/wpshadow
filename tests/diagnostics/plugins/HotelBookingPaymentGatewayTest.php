<?php
/**
 * Test for Hotel Booking Payment Gateway
 *
...
use WPShadow\Diagnostics\Diagnostic_HotelBookingPaymentGateway;

/**
 * HotelBookingPaymentGatewayTest Class
 */
class HotelBookingPaymentGatewayTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_HotelBookingPaymentGateway' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_HotelBookingPaymentGateway', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_HotelBookingPaymentGateway', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_HotelBookingPaymentGateway::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_HotelBookingPaymentGateway' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'hotel-booking-payment-gateway', $slug );
	}
