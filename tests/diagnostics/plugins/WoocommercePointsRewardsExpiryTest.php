<?php
/**
 * Test for Woocommerce Points Rewards Expiry
 *
...
use WPShadow\Diagnostics\Diagnostic_WoocommercePointsRewardsExpiry;

/**
 * WoocommercePointsRewardsExpiryTest Class
 */
class WoocommercePointsRewardsExpiryTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WoocommercePointsRewardsExpiry' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WoocommercePointsRewardsExpiry', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WoocommercePointsRewardsExpiry', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WoocommercePointsRewardsExpiry::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WoocommercePointsRewardsExpiry' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'woocommerce-points-rewards-expiry', $slug );
	}
