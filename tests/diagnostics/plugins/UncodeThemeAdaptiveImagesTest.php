<?php
/**
 * Test for Uncode Theme Adaptive Images
 *
...
use WPShadow\Diagnostics\Diagnostic_UncodeThemeAdaptiveImages;

/**
 * UncodeThemeAdaptiveImagesTest Class
 */
class UncodeThemeAdaptiveImagesTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_UncodeThemeAdaptiveImages' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_UncodeThemeAdaptiveImages', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_UncodeThemeAdaptiveImages', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_UncodeThemeAdaptiveImages::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_UncodeThemeAdaptiveImages' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'uncode-theme-adaptive-images', $slug );
	}
