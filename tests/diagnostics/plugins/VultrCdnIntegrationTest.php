<?php
/**
 * Test for Vultr Cdn Integration
 *
...
use WPShadow\Diagnostics\Diagnostic_VultrCdnIntegration;

/**
 * VultrCdnIntegrationTest Class
 */
class VultrCdnIntegrationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_VultrCdnIntegration' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_VultrCdnIntegration', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_VultrCdnIntegration', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_VultrCdnIntegration::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_VultrCdnIntegration' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'vultr-cdn-integration', $slug );
	}
