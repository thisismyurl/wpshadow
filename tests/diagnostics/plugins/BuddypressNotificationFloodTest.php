<?php
/**
 * Test for BuddyPress Notification Flood
 *
...
use WPShadow\Diagnostics\Diagnostic_BuddypressNotificationFlood;

/**
 * BuddypressNotificationFloodTest Class
 */
class BuddypressNotificationFloodTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BuddypressNotificationFlood' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BuddypressNotificationFlood', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BuddypressNotificationFlood', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_BuddypressNotificationFlood::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BuddypressNotificationFlood' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'buddypress-notification-flood', $slug );
	}
