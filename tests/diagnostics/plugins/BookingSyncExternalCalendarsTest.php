<?php
/**
 * Test for Booking Sync External Calendars
 *
...
use WPShadow\Diagnostics\Diagnostic_BookingSyncExternalCalendars;

/**
 * BookingSyncExternalCalendarsTest Class
 */
class BookingSyncExternalCalendarsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BookingSyncExternalCalendars' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BookingSyncExternalCalendars', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BookingSyncExternalCalendars', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_BookingSyncExternalCalendars::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BookingSyncExternalCalendars' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'booking-sync-external-calendars', $slug );
	}
