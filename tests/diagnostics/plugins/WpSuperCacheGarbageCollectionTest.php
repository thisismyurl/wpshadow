<?php
/**
 * Test for Wp Super Cache Garbage Collection
 *
...
use WPShadow\Diagnostics\Diagnostic_WpSuperCacheGarbageCollection;

/**
 * WpSuperCacheGarbageCollectionTest Class
 */
class WpSuperCacheGarbageCollectionTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheGarbageCollection' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheGarbageCollection', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheGarbageCollection', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'function_exists' )
			->with( 'wp_cache_postload' )
			->andReturn( false );
		
		$result = Diagnostic_WpSuperCacheGarbageCollection::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheGarbageCollection' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wp-super-cache-garbage-collection', $slug );
	}
