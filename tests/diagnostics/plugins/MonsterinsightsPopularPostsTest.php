<?php
/**
 * Test for MonsterInsights Popular Posts
 *
...
use WPShadow\Diagnostics\Diagnostic_MonsterinsightsPopularPosts;

/**
 * MonsterinsightsPopularPostsTest Class
 */
class MonsterinsightsPopularPostsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_MonsterinsightsPopularPosts' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_MonsterinsightsPopularPosts', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_MonsterinsightsPopularPosts', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_MonsterinsightsPopularPosts::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_MonsterinsightsPopularPosts' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'monsterinsights-popular-posts', $slug );
	}
