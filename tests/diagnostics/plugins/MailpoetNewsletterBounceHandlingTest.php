<?php
/**
 * Test for Mailpoet Newsletter Bounce Handling
 *
...
use WPShadow\Diagnostics\Diagnostic_MailpoetNewsletterBounceHandling;

/**
 * MailpoetNewsletterBounceHandlingTest Class
 */
class MailpoetNewsletterBounceHandlingTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_MailpoetNewsletterBounceHandling' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_MailpoetNewsletterBounceHandling', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_MailpoetNewsletterBounceHandling', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_MailpoetNewsletterBounceHandling::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_MailpoetNewsletterBounceHandling' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'mailpoet-newsletter-bounce-handling', $slug );
	}
