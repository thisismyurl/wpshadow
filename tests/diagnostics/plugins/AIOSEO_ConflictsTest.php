<?php
/**
 * Test for AIOSEO Plugin Conflicts
 *
...
use WPShadow\Diagnostics\Diagnostic_AIOSEO_Conflicts;

/**
 * AIOSEO_ConflictsTest Class
 */
class AIOSEO_ConflictsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_AIOSEO_Conflicts' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_AIOSEO_Conflicts', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_AIOSEO_Conflicts', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_AIOSEO_Conflicts::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_AIOSEO_Conflicts' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'aioseo-conflicts', $slug );
	}
