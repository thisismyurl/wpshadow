<?php
/**
 * Test for Ithemes Security Brute Force
 *
...
use WPShadow\Diagnostics\Diagnostic_IthemesSecurityBruteForce;

/**
 * IthemesSecurityBruteForceTest Class
 */
class IthemesSecurityBruteForceTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityBruteForce' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityBruteForce', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityBruteForce', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_IthemesSecurityBruteForce::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityBruteForce' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'ithemes-security-brute-force', $slug );
	}
