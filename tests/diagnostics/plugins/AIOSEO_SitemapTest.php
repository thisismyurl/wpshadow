<?php
/**
 * Test for AIOSEO Sitemap Configuration
 *
...
use WPShadow\Diagnostics\Diagnostic_AIOSEO_Sitemap;

/**
 * AIOSEO_SitemapTest Class
 */
class AIOSEO_SitemapTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_AIOSEO_Sitemap' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_AIOSEO_Sitemap', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_AIOSEO_Sitemap', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_AIOSEO_Sitemap::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_AIOSEO_Sitemap' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'aioseo-sitemap', $slug );
	}
