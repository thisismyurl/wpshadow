<?php
/**
 * Test for NextGEN Gallery Cache
 *
...
use WPShadow\Diagnostics\Diagnostic_NextgenGalleryCache;

/**
 * NextgenGalleryCacheTest Class
 */
class NextgenGalleryCacheTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_NextgenGalleryCache' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_NextgenGalleryCache', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_NextgenGalleryCache', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_NextgenGalleryCache::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_NextgenGalleryCache' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'nextgen-gallery-cache', $slug );
	}
