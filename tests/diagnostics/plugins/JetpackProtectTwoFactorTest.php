<?php
/**
 * Test for Jetpack Protect Two Factor
 *
...
use WPShadow\Diagnostics\Diagnostic_JetpackProtectTwoFactor;

/**
 * JetpackProtectTwoFactorTest Class
 */
class JetpackProtectTwoFactorTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_JetpackProtectTwoFactor' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_JetpackProtectTwoFactor', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_JetpackProtectTwoFactor', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_JetpackProtectTwoFactor::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_JetpackProtectTwoFactor' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'jetpack-protect-two-factor', $slug );
	}
