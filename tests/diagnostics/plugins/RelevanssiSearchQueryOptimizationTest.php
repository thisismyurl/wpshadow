<?php
/**
 * Test for Relevanssi Search Query Optimization
 *
...
use WPShadow\Diagnostics\Diagnostic_RelevanssiSearchQueryOptimization;

/**
 * RelevanssiSearchQueryOptimizationTest Class
 */
class RelevanssiSearchQueryOptimizationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_RelevanssiSearchQueryOptimization' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_RelevanssiSearchQueryOptimization', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_RelevanssiSearchQueryOptimization', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'function_exists' )
			->with( 'relevanssi_search' )
			->andReturn( false );
		
		$result = Diagnostic_RelevanssiSearchQueryOptimization::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_RelevanssiSearchQueryOptimization' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'relevanssi-search-query-optimization', $slug );
	}
