<?php
/**
 * Test for Beaver Builder SEO Optimization
 *
...
use WPShadow\Diagnostics\Diagnostic_BeaverBuilderSeoOptimization;

/**
 * BeaverBuilderSeoOptimizationTest Class
 */
class BeaverBuilderSeoOptimizationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderSeoOptimization' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderSeoOptimization', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderSeoOptimization', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_BeaverBuilderSeoOptimization::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BeaverBuilderSeoOptimization' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'beaver-builder-seo-optimization', $slug );
	}
