<?php
/**
 * Test for Gutenberg Block Patterns Database
 *
...
use WPShadow\Diagnostics\Diagnostic_GutenbergBlockPatternsDatabase;

/**
 * GutenbergBlockPatternsDatabaseTest Class
 */
class GutenbergBlockPatternsDatabaseTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_GutenbergBlockPatternsDatabase' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_GutenbergBlockPatternsDatabase', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_GutenbergBlockPatternsDatabase', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_GutenbergBlockPatternsDatabase::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_GutenbergBlockPatternsDatabase' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'gutenberg-block-patterns-database', $slug );
	}
