<?php
/**
 * Test for Caldera Forms Conditional Logic
 *
...
use WPShadow\Diagnostics\Diagnostic_CalderaFormsConditionalLogic;

/**
 * CalderaFormsConditionalLogicTest Class
 */
class CalderaFormsConditionalLogicTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsConditionalLogic' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsConditionalLogic', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsConditionalLogic', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_CalderaFormsConditionalLogic::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsConditionalLogic' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'caldera-forms-conditional-logic', $slug );
	}
