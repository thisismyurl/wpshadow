<?php
/**
 * Test for Google Analytics Event Tracking Performance
 *
...
use WPShadow\Diagnostics\Diagnostic_GoogleAnalyticsEventTrackingPerformance;

/**
 * GoogleAnalyticsEventTrackingPerformanceTest Class
 */
class GoogleAnalyticsEventTrackingPerformanceTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsEventTrackingPerformance' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsEventTrackingPerformance', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsEventTrackingPerformance', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'function_exists' )
			->with( 'ga_load_options' )
			->andReturn( false );
		
		$result = Diagnostic_GoogleAnalyticsEventTrackingPerformance::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsEventTrackingPerformance' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'google-analytics-event-tracking-performance', $slug );
	}
