<?php
/**
 * Test for Wordpress Rest Api Exposed Endpoints
 *
...
use WPShadow\Diagnostics\Diagnostic_WordpressRestApiExposedEndpoints;

/**
 * WordpressRestApiExposedEndpointsTest Class
 */
class WordpressRestApiExposedEndpointsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WordpressRestApiExposedEndpoints' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WordpressRestApiExposedEndpoints', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WordpressRestApiExposedEndpoints', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_WordpressRestApiExposedEndpoints::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WordpressRestApiExposedEndpoints' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wordpress-rest-api-exposed-endpoints', $slug );
	}
