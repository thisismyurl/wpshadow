<?php
/**
 * Test for Duplicator Storage Location
 *
...
use WPShadow\Diagnostics\Diagnostic_DuplicatorStorageLocation;

/**
 * DuplicatorStorageLocationTest Class
 */
class DuplicatorStorageLocationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_DuplicatorStorageLocation' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_DuplicatorStorageLocation', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_DuplicatorStorageLocation', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_DuplicatorStorageLocation::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_DuplicatorStorageLocation' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'duplicator-storage-location', $slug );
	}
