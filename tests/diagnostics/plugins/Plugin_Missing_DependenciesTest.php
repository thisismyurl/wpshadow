<?php
/**
 * Test for Plugin Missing Dependencies
 *
...
/**
 * Plugin_Missing_DependenciesTest Class
 */
class Plugin_Missing_DependenciesTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_Plugin_Missing_Dependencies' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_Plugin_Missing_Dependencies', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_Plugin_Missing_Dependencies', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_Plugin_Missing_Dependencies::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_Plugin_Missing_Dependencies' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'plugin-missing-dependencies', $slug );
	}
