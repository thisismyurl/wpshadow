<?php
/**
 * Test for Divi Builder Pro Layout Library
 *
...
use WPShadow\Diagnostics\Diagnostic_DiviBuilderProLayoutLibrary;

/**
 * DiviBuilderProLayoutLibraryTest Class
 */
class DiviBuilderProLayoutLibraryTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_DiviBuilderProLayoutLibrary' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_DiviBuilderProLayoutLibrary', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_DiviBuilderProLayoutLibrary', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_DiviBuilderProLayoutLibrary::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_DiviBuilderProLayoutLibrary' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'divi-builder-pro-layout-library', $slug );
	}
