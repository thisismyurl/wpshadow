<?php
/**
 * Test for Astra Theme Dynamic Css Generation
 *
...
use WPShadow\Diagnostics\Diagnostic_AstraThemeDynamicCssGeneration;

/**
 * AstraThemeDynamicCssGenerationTest Class
 */
class AstraThemeDynamicCssGenerationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_AstraThemeDynamicCssGeneration' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_AstraThemeDynamicCssGeneration', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_AstraThemeDynamicCssGeneration', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_AstraThemeDynamicCssGeneration::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_AstraThemeDynamicCssGeneration' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'astra-theme-dynamic-css-generation', $slug );
	}
