<?php
/**
 * Test for Multi Language Widget Performance
 *
...
use WPShadow\Diagnostics\Diagnostic_MultiLanguageWidgetPerformance;

/**
 * MultiLanguageWidgetPerformanceTest Class
 */
class MultiLanguageWidgetPerformanceTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_MultiLanguageWidgetPerformance' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_MultiLanguageWidgetPerformance', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_MultiLanguageWidgetPerformance', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_MultiLanguageWidgetPerformance::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_MultiLanguageWidgetPerformance' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'multi-language-widget-performance', $slug );
	}
