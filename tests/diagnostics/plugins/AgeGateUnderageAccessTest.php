<?php
/**
 * Test for Age Gate Underage Access
 *
...
use WPShadow\Diagnostics\Diagnostic_AgeGateUnderageAccess;

/**
 * AgeGateUnderageAccessTest Class
 */
class AgeGateUnderageAccessTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_AgeGateUnderageAccess' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_AgeGateUnderageAccess', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_AgeGateUnderageAccess', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_AgeGateUnderageAccess::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_AgeGateUnderageAccess' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'age-gate-underage-access', $slug );
	}
