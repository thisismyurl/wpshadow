<?php
/**
 * Test for Wpforms Lite Captcha Integration
 *
...
use WPShadow\Diagnostics\Diagnostic_WpformsLiteCaptchaIntegration;

/**
 * WpformsLiteCaptchaIntegrationTest Class
 */
class WpformsLiteCaptchaIntegrationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpformsLiteCaptchaIntegration' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpformsLiteCaptchaIntegration', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpformsLiteCaptchaIntegration', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WpformsLiteCaptchaIntegration::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpformsLiteCaptchaIntegration' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wpforms-lite-captcha-integration', $slug );
	}
