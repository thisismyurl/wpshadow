<?php
/**
 * Test for Local By Flywheel Add Ons
 *
...
use WPShadow\Diagnostics\Diagnostic_LocalByFlywheelAddOns;

/**
 * LocalByFlywheelAddOnsTest Class
 */
class LocalByFlywheelAddOnsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_LocalByFlywheelAddOns' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_LocalByFlywheelAddOns', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_LocalByFlywheelAddOns', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_LocalByFlywheelAddOns::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_LocalByFlywheelAddOns' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'local-by-flywheel-add-ons', $slug );
	}
