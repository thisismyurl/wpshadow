<?php
/**
 * Test for Jetpack Protect Downtime Monitoring
 *
...
use WPShadow\Diagnostics\Diagnostic_JetpackProtectDowntimeMonitoring;

/**
 * JetpackProtectDowntimeMonitoringTest Class
 */
class JetpackProtectDowntimeMonitoringTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_JetpackProtectDowntimeMonitoring' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_JetpackProtectDowntimeMonitoring', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_JetpackProtectDowntimeMonitoring', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_JetpackProtectDowntimeMonitoring::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_JetpackProtectDowntimeMonitoring' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'jetpack-protect-downtime-monitoring', $slug );
	}
