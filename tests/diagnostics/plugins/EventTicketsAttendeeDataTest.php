<?php
/**
 * Test for Event Tickets Attendee Data
 *
...
use WPShadow\Diagnostics\Diagnostic_EventTicketsAttendeeData;

/**
 * EventTicketsAttendeeDataTest Class
 */
class EventTicketsAttendeeDataTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_EventTicketsAttendeeData' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_EventTicketsAttendeeData', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_EventTicketsAttendeeData', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_EventTicketsAttendeeData::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_EventTicketsAttendeeData' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'event-tickets-attendee-data', $slug );
	}
