<?php
/**
 * Test for Directory Listing Categories
 *
...
use WPShadow\Diagnostics\Diagnostic_DirectoryListingCategories;

/**
 * DirectoryListingCategoriesTest Class
 */
class DirectoryListingCategoriesTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_DirectoryListingCategories' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_DirectoryListingCategories', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_DirectoryListingCategories', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_DirectoryListingCategories::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_DirectoryListingCategories' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'directory-listing-categories', $slug );
	}
