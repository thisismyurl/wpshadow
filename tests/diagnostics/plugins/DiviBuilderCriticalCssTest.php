<?php
/**
 * Test for Divi Builder Critical CSS
 *
...
use WPShadow\Diagnostics\Diagnostic_DiviBuilderCriticalCss;

/**
 * DiviBuilderCriticalCssTest Class
 */
class DiviBuilderCriticalCssTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_DiviBuilderCriticalCss' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_DiviBuilderCriticalCss', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_DiviBuilderCriticalCss', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_DiviBuilderCriticalCss::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_DiviBuilderCriticalCss' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'divi-builder-critical-css', $slug );
	}
