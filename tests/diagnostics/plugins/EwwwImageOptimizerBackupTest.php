<?php
/**
 * Test for Ewww Image Optimizer Backup
 *
...
use WPShadow\Diagnostics\Diagnostic_EwwwImageOptimizerBackup;

/**
 * EwwwImageOptimizerBackupTest Class
 */
class EwwwImageOptimizerBackupTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_EwwwImageOptimizerBackup' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_EwwwImageOptimizerBackup', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_EwwwImageOptimizerBackup', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_EwwwImageOptimizerBackup::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_EwwwImageOptimizerBackup' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'ewww-image-optimizer-backup', $slug );
	}
