<?php
/**
 * Test for Mailpoet Newsletter Subscription Security
 *
...
use WPShadow\Diagnostics\Diagnostic_MailpoetNewsletterSubscriptionSecurity;

/**
 * MailpoetNewsletterSubscriptionSecurityTest Class
 */
class MailpoetNewsletterSubscriptionSecurityTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_MailpoetNewsletterSubscriptionSecurity' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_MailpoetNewsletterSubscriptionSecurity', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_MailpoetNewsletterSubscriptionSecurity', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'class_exists' )
			->with( 'MailPoet\Config\Initializer' )
			->andReturn( false );
		
		$result = Diagnostic_MailpoetNewsletterSubscriptionSecurity::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_MailpoetNewsletterSubscriptionSecurity' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'mailpoet-newsletter-subscription-security', $slug );
	}
