<?php
/**
 * Test for bbPress Subscription Emails
 *
...
use WPShadow\Diagnostics\Diagnostic_BbpressSubscriptionEmails;

/**
 * BbpressSubscriptionEmailsTest Class
 */
class BbpressSubscriptionEmailsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BbpressSubscriptionEmails' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BbpressSubscriptionEmails', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BbpressSubscriptionEmails', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_BbpressSubscriptionEmails::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BbpressSubscriptionEmails' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'bbpress-subscription-emails', $slug );
	}
