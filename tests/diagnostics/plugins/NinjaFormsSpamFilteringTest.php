<?php
/**
 * Test for Ninja Forms Spam Filtering
 *
...
use WPShadow\Diagnostics\Diagnostic_NinjaFormsSpamFiltering;

/**
 * NinjaFormsSpamFilteringTest Class
 */
class NinjaFormsSpamFilteringTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_NinjaFormsSpamFiltering' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_NinjaFormsSpamFiltering', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_NinjaFormsSpamFiltering', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_NinjaFormsSpamFiltering::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_NinjaFormsSpamFiltering' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'ninja-forms-spam-filtering', $slug );
	}
