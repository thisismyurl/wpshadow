<?php
/**
 * Test for Wpforms Lite Upgrade Prompts
 *
...
use WPShadow\Diagnostics\Diagnostic_WpformsLiteUpgradePrompts;

/**
 * WpformsLiteUpgradePromptsTest Class
 */
class WpformsLiteUpgradePromptsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpformsLiteUpgradePrompts' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpformsLiteUpgradePrompts', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpformsLiteUpgradePrompts', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WpformsLiteUpgradePrompts::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpformsLiteUpgradePrompts' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wpforms-lite-upgrade-prompts', $slug );
	}
