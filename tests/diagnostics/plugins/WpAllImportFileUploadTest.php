<?php
/**
 * Test for WP All Import File Upload Security
 *
...
use WPShadow\Diagnostics\Diagnostic_WpAllImportFileUpload;

/**
 * WpAllImportFileUploadTest Class
 */
class WpAllImportFileUploadTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpAllImportFileUpload' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpAllImportFileUpload', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpAllImportFileUpload', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WpAllImportFileUpload::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpAllImportFileUpload' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wp-all-import-file-upload', $slug );
	}
