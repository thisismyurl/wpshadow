<?php
/**
 * Test for OptinMonster Campaign Publishing
 *
...
use WPShadow\Diagnostics\Diagnostic_OptinmonsterCampaignPublishing;

/**
 * OptinmonsterCampaignPublishingTest Class
 */
class OptinmonsterCampaignPublishingTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_OptinmonsterCampaignPublishing' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_OptinmonsterCampaignPublishing', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_OptinmonsterCampaignPublishing', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_OptinmonsterCampaignPublishing::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_OptinmonsterCampaignPublishing' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'optinmonster-campaign-publishing', $slug );
	}
