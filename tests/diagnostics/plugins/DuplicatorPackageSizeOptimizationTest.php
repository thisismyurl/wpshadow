<?php
/**
 * Test for Duplicator Package Size
 *
...
use WPShadow\Diagnostics\Diagnostic_DuplicatorPackageSizeOptimization;

/**
 * DuplicatorPackageSizeOptimizationTest Class
 */
class DuplicatorPackageSizeOptimizationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_DuplicatorPackageSizeOptimization' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_DuplicatorPackageSizeOptimization', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_DuplicatorPackageSizeOptimization', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'class_exists' )
			->with( 'DUP_PRO_Package' )
			->andReturn( false );
		
		$result = Diagnostic_DuplicatorPackageSizeOptimization::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_DuplicatorPackageSizeOptimization' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'duplicator-package-size-optimization', $slug );
	}
