<?php
/**
 * Test for Stripe Subscription Webhook Events
 *
...
use WPShadow\Diagnostics\Diagnostic_StripeSubscriptionWebhookEvents;

/**
 * StripeSubscriptionWebhookEventsTest Class
 */
class StripeSubscriptionWebhookEventsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_StripeSubscriptionWebhookEvents' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_StripeSubscriptionWebhookEvents', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_StripeSubscriptionWebhookEvents', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_StripeSubscriptionWebhookEvents::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_StripeSubscriptionWebhookEvents' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'stripe-subscription-webhook-events', $slug );
	}
