<?php
/**
 * Test for Google Analytics Site Speed Sample
 *
...
use WPShadow\Diagnostics\Diagnostic_GoogleAnalyticsSiteSpeedSample;

/**
 * GoogleAnalyticsSiteSpeedSampleTest Class
 */
class GoogleAnalyticsSiteSpeedSampleTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsSiteSpeedSample' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsSiteSpeedSample', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsSiteSpeedSample', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_GoogleAnalyticsSiteSpeedSample::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsSiteSpeedSample' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'google-analytics-site-speed-sample', $slug );
	}
