<?php
/**
 * Test for Multisite Cross Site Forms
 *
...
use WPShadow\Diagnostics\Diagnostic_MultisiteCrossSiteForms;

/**
 * MultisiteCrossSiteFormsTest Class
 */
class MultisiteCrossSiteFormsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_MultisiteCrossSiteForms' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_MultisiteCrossSiteForms', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_MultisiteCrossSiteForms', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_MultisiteCrossSiteForms::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_MultisiteCrossSiteForms' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'multisite-cross-site-forms', $slug );
	}
