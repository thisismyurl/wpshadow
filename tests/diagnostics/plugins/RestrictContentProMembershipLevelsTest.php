<?php
/**
 * Test for Restrict Content Pro Membership Levels
 *
...
use WPShadow\Diagnostics\Diagnostic_RestrictContentProMembershipLevels;

/**
 * RestrictContentProMembershipLevelsTest Class
 */
class RestrictContentProMembershipLevelsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_RestrictContentProMembershipLevels' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_RestrictContentProMembershipLevels', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_RestrictContentProMembershipLevels', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_RestrictContentProMembershipLevels::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_RestrictContentProMembershipLevels' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'restrict-content-pro-membership-levels', $slug );
	}
