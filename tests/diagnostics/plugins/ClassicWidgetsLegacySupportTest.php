<?php
/**
 * Test for Classic Widgets Legacy Support
 *
...
use WPShadow\Diagnostics\Diagnostic_ClassicWidgetsLegacySupport;

/**
 * ClassicWidgetsLegacySupportTest Class
 */
class ClassicWidgetsLegacySupportTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_ClassicWidgetsLegacySupport' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_ClassicWidgetsLegacySupport', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_ClassicWidgetsLegacySupport', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_ClassicWidgetsLegacySupport::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_ClassicWidgetsLegacySupport' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'classic-widgets-legacy-support', $slug );
	}
