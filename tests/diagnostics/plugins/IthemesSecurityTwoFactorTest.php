<?php
/**
 * Test for Ithemes Security Two Factor
 *
...
use WPShadow\Diagnostics\Diagnostic_IthemesSecurityTwoFactor;

/**
 * IthemesSecurityTwoFactorTest Class
 */
class IthemesSecurityTwoFactorTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityTwoFactor' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityTwoFactor', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityTwoFactor', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_IthemesSecurityTwoFactor::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_IthemesSecurityTwoFactor' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'ithemes-security-two-factor', $slug );
	}
