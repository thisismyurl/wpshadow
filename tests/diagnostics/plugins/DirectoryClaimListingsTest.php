<?php
/**
 * Test for Directory Claim Listings
 *
...
use WPShadow\Diagnostics\Diagnostic_DirectoryClaimListings;

/**
 * DirectoryClaimListingsTest Class
 */
class DirectoryClaimListingsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_DirectoryClaimListings' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_DirectoryClaimListings', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_DirectoryClaimListings', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_DirectoryClaimListings::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_DirectoryClaimListings' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'directory-claim-listings', $slug );
	}
