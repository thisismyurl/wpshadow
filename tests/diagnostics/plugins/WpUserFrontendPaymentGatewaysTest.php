<?php
/**
 * Test for Wp User Frontend Payment Gateways
 *
...
use WPShadow\Diagnostics\Diagnostic_WpUserFrontendPaymentGateways;

/**
 * WpUserFrontendPaymentGatewaysTest Class
 */
class WpUserFrontendPaymentGatewaysTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpUserFrontendPaymentGateways' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpUserFrontendPaymentGateways', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpUserFrontendPaymentGateways', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_WpUserFrontendPaymentGateways::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpUserFrontendPaymentGateways' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wp-user-frontend-payment-gateways', $slug );
	}
