<?php
/**
 * Test for Event Tickets Payment Gateway
 *
...
use WPShadow\Diagnostics\Diagnostic_EventTicketsPaymentGateway;

/**
 * EventTicketsPaymentGatewayTest Class
 */
class EventTicketsPaymentGatewayTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_EventTicketsPaymentGateway' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_EventTicketsPaymentGateway', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_EventTicketsPaymentGateway', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'class_exists' )
			->with( 'Tribe__Tickets__Main' )
			->andReturn( false );
		
		$result = Diagnostic_EventTicketsPaymentGateway::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_EventTicketsPaymentGateway' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'event-tickets-payment-gateway', $slug );
	}
