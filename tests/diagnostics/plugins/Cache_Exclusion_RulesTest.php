<?php
/**
 * Test for Cache Exclusion Rules
 *
...
use WPShadow\Diagnostics\Diagnostic_Cache_Exclusion_Rules;

/**
 * Cache_Exclusion_RulesTest Class
 */
class Cache_Exclusion_RulesTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_Cache_Exclusion_Rules' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_Cache_Exclusion_Rules', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_Cache_Exclusion_Rules', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_Cache_Exclusion_Rules::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_Cache_Exclusion_Rules' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'cache-exclusion-rules', $slug );
	}
