<?php
/**
 * Test for Hotjar Survey Triggers
 *
...
use WPShadow\Diagnostics\Diagnostic_HotjarSurveyTriggers;

/**
 * HotjarSurveyTriggersTest Class
 */
class HotjarSurveyTriggersTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_HotjarSurveyTriggers' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_HotjarSurveyTriggers', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_HotjarSurveyTriggers', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_HotjarSurveyTriggers::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_HotjarSurveyTriggers' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'hotjar-survey-triggers', $slug );
	}
