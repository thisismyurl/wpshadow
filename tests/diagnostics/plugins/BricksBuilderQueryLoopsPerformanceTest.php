<?php
/**
 * Test for Bricks Builder Query Loops Performance
 *
...
use WPShadow\Diagnostics\Diagnostic_BricksBuilderQueryLoopsPerformance;

/**
 * BricksBuilderQueryLoopsPerformanceTest Class
 */
class BricksBuilderQueryLoopsPerformanceTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BricksBuilderQueryLoopsPerformance' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BricksBuilderQueryLoopsPerformance', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BricksBuilderQueryLoopsPerformance', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_BricksBuilderQueryLoopsPerformance::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BricksBuilderQueryLoopsPerformance' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'bricks-builder-query-loops-performance', $slug );
	}
