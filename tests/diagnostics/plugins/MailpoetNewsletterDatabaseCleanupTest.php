<?php
/**
 * Test for Mailpoet Newsletter Database Cleanup
 *
...
use WPShadow\Diagnostics\Diagnostic_MailpoetNewsletterDatabaseCleanup;

/**
 * MailpoetNewsletterDatabaseCleanupTest Class
 */
class MailpoetNewsletterDatabaseCleanupTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_MailpoetNewsletterDatabaseCleanup' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_MailpoetNewsletterDatabaseCleanup', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_MailpoetNewsletterDatabaseCleanup', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_MailpoetNewsletterDatabaseCleanup::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_MailpoetNewsletterDatabaseCleanup' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'mailpoet-newsletter-database-cleanup', $slug );
	}
