...
use WPShadow\Diagnostics\Diagnostic_AIOSEO_Setup;

/**
 * AIOSEO_SetupTest Class
 */
class AIOSEO_SetupTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_AIOSEO_Setup' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_AIOSEO_Setup', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_AIOSEO_Setup', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'class_exists' )
			->with( 'AIOSEO\Plugin\AIOSEO' )
			->andReturn( false );
		
		$result = Diagnostic_AIOSEO_Setup::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_AIOSEO_Setup' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'aioseo-setup', $slug );
	}
