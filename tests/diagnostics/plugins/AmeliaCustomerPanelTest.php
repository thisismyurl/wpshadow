<?php
/**
 * Test for Amelia Customer Panel
 *
...
use WPShadow\Diagnostics\Diagnostic_AmeliaCustomerPanel;

/**
 * AmeliaCustomerPanelTest Class
 */
class AmeliaCustomerPanelTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_AmeliaCustomerPanel' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_AmeliaCustomerPanel', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_AmeliaCustomerPanel', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_AmeliaCustomerPanel::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_AmeliaCustomerPanel' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'amelia-customer-panel', $slug );
	}
