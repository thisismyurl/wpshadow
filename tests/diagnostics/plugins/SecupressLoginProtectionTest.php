<?php
/**
 * Test for Secupress Login Protection
 *
...
use WPShadow\Diagnostics\Diagnostic_SecupressLoginProtection;

/**
 * SecupressLoginProtectionTest Class
 */
class SecupressLoginProtectionTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_SecupressLoginProtection' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_SecupressLoginProtection', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_SecupressLoginProtection', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_SecupressLoginProtection::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_SecupressLoginProtection' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'secupress-login-protection', $slug );
	}
