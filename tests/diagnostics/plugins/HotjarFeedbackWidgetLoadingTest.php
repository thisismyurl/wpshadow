<?php
/**
 * Test for Hotjar Feedback Widget Loading
 *
...
use WPShadow\Diagnostics\Diagnostic_HotjarFeedbackWidgetLoading;

/**
 * HotjarFeedbackWidgetLoadingTest Class
 */
class HotjarFeedbackWidgetLoadingTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_HotjarFeedbackWidgetLoading' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_HotjarFeedbackWidgetLoading', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_HotjarFeedbackWidgetLoading', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_HotjarFeedbackWidgetLoading::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_HotjarFeedbackWidgetLoading' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'hotjar-feedback-widget-loading', $slug );
	}
