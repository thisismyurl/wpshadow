<?php
/**
 * Test for WP Job Manager Application Limits
 *
...
use WPShadow\Diagnostics\Diagnostic_WpJobManagerApplicationLimits;

/**
 * WpJobManagerApplicationLimitsTest Class
 */
class WpJobManagerApplicationLimitsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerApplicationLimits' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerApplicationLimits', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerApplicationLimits', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WpJobManagerApplicationLimits::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerApplicationLimits' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wp-job-manager-application-limits', $slug );
	}
