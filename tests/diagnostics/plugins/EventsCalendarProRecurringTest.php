<?php
/**
 * Test for Events Calendar Pro Recurring
 *
...
use WPShadow\Diagnostics\Diagnostic_EventsCalendarProRecurring;

/**
 * EventsCalendarProRecurringTest Class
 */
class EventsCalendarProRecurringTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_EventsCalendarProRecurring' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_EventsCalendarProRecurring', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_EventsCalendarProRecurring', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_EventsCalendarProRecurring::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_EventsCalendarProRecurring' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'events-calendar-pro-recurring', $slug );
	}
