<?php
/**
 * Test for The Events Calendar Google Maps
 *
...
use WPShadow\Diagnostics\Diagnostic_EventsCalendarGoogleMapsApi;

/**
 * EventsCalendarGoogleMapsApiTest Class
 */
class EventsCalendarGoogleMapsApiTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_EventsCalendarGoogleMapsApi' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_EventsCalendarGoogleMapsApi', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_EventsCalendarGoogleMapsApi', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_EventsCalendarGoogleMapsApi::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_EventsCalendarGoogleMapsApi' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'events-calendar-google-maps-api', $slug );
	}
