<?php
/**
 * Test for WooCommerce Booking Cost Calculation
 *
...
use WPShadow\Diagnostics\Diagnostic_WoocommerceBookingCostCalculation;

/**
 * WoocommerceBookingCostCalculationTest Class
 */
class WoocommerceBookingCostCalculationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WoocommerceBookingCostCalculation' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WoocommerceBookingCostCalculation', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WoocommerceBookingCostCalculation', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WoocommerceBookingCostCalculation::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WoocommerceBookingCostCalculation' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'woocommerce-booking-cost-calculation', $slug );
	}
