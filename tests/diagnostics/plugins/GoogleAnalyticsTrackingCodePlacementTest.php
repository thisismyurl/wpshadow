<?php
/**
 * Test for Google Analytics Tracking Code Placement
 *
...
use WPShadow\Diagnostics\Diagnostic_GoogleAnalyticsTrackingCodePlacement;

/**
 * GoogleAnalyticsTrackingCodePlacementTest Class
 */
class GoogleAnalyticsTrackingCodePlacementTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsTrackingCodePlacement' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsTrackingCodePlacement', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsTrackingCodePlacement', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_GoogleAnalyticsTrackingCodePlacement::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsTrackingCodePlacement' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'google-analytics-tracking-code-placement', $slug );
	}
