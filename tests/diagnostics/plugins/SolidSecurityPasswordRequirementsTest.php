<?php
/**
 * Test for Solid Security Password Requirements
 *
...
use WPShadow\Diagnostics\Diagnostic_SolidSecurityPasswordRequirements;

/**
 * SolidSecurityPasswordRequirementsTest Class
 */
class SolidSecurityPasswordRequirementsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_SolidSecurityPasswordRequirements' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_SolidSecurityPasswordRequirements', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_SolidSecurityPasswordRequirements', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'function_exists' )
			->with( 'itsec_load_textdomain' )
			->andReturn( false );
		
		$result = Diagnostic_SolidSecurityPasswordRequirements::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_SolidSecurityPasswordRequirements' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'solid-security-password-requirements', $slug );
	}
