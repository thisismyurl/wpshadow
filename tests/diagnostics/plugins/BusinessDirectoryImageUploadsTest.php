<?php
/**
 * Test for Business Directory Image Uploads
 *
...
use WPShadow\Diagnostics\Diagnostic_BusinessDirectoryImageUploads;

/**
 * BusinessDirectoryImageUploadsTest Class
 */
class BusinessDirectoryImageUploadsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryImageUploads' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryImageUploads', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryImageUploads', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_BusinessDirectoryImageUploads::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryImageUploads' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'business-directory-image-uploads', $slug );
	}
