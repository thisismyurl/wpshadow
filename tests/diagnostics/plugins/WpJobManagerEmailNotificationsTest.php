<?php
/**
 * Test for WP Job Manager Email Notifications
 *
...
use WPShadow\Diagnostics\Diagnostic_WpJobManagerEmailNotifications;

/**
 * WpJobManagerEmailNotificationsTest Class
 */
class WpJobManagerEmailNotificationsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerEmailNotifications' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerEmailNotifications', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerEmailNotifications', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WpJobManagerEmailNotifications::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerEmailNotifications' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wp-job-manager-email-notifications', $slug );
	}
