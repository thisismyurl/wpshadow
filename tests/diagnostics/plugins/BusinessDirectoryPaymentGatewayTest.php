<?php
/**
 * Test for Business Directory Payment Gateway
 *
...
use WPShadow\Diagnostics\Diagnostic_BusinessDirectoryPaymentGateway;

/**
 * BusinessDirectoryPaymentGatewayTest Class
 */
class BusinessDirectoryPaymentGatewayTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryPaymentGateway' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryPaymentGateway', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryPaymentGateway', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_BusinessDirectoryPaymentGateway::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryPaymentGateway' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'business-directory-payment-gateway', $slug );
	}
