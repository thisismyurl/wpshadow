<?php
/**
 * Test for Seo Framework Meta Tags
 *
...
use WPShadow\Diagnostics\Diagnostic_SeoFrameworkMetaTags;

/**
 * SeoFrameworkMetaTagsTest Class
 */
class SeoFrameworkMetaTagsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_SeoFrameworkMetaTags' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_SeoFrameworkMetaTags', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_SeoFrameworkMetaTags', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_SeoFrameworkMetaTags::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_SeoFrameworkMetaTags' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'seo-framework-meta-tags', $slug );
	}
