<?php
/**
 * Test for Wpengine Staging Environment
 *
...
use WPShadow\Diagnostics\Diagnostic_WpengineStagingEnvironment;

/**
 * WpengineStagingEnvironmentTest Class
 */
class WpengineStagingEnvironmentTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpengineStagingEnvironment' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpengineStagingEnvironment', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpengineStagingEnvironment', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_WpengineStagingEnvironment::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpengineStagingEnvironment' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wpengine-staging-environment', $slug );
	}
