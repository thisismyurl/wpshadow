<?php
/**
 * Test for Fluent Forms Conditional Shortcodes
 *
...
use WPShadow\Diagnostics\Diagnostic_FluentFormsConditionalShortcodes;

/**
 * FluentFormsConditionalShortcodesTest Class
 */
class FluentFormsConditionalShortcodesTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_FluentFormsConditionalShortcodes' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_FluentFormsConditionalShortcodes', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_FluentFormsConditionalShortcodes', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_FluentFormsConditionalShortcodes::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_FluentFormsConditionalShortcodes' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'fluent-forms-conditional-shortcodes', $slug );
	}
