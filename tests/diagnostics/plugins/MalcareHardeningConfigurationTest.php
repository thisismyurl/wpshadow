<?php
/**
 * Test for Malcare Hardening Configuration
 *
...
use WPShadow\Diagnostics\Diagnostic_MalcareHardeningConfiguration;

/**
 * MalcareHardeningConfigurationTest Class
 */
class MalcareHardeningConfigurationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_MalcareHardeningConfiguration' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_MalcareHardeningConfiguration', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_MalcareHardeningConfiguration', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_MalcareHardeningConfiguration::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_MalcareHardeningConfiguration' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'malcare-hardening-configuration', $slug );
	}
