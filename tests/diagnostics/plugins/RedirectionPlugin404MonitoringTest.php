<?php
/**
 * Test for Redirection Plugin 404 Monitoring
 *
...
use WPShadow\Diagnostics\Diagnostic_RedirectionPlugin404Monitoring;

/**
 * RedirectionPlugin404MonitoringTest Class
 */
class RedirectionPlugin404MonitoringTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_RedirectionPlugin404Monitoring' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_RedirectionPlugin404Monitoring', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_RedirectionPlugin404Monitoring', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_RedirectionPlugin404Monitoring::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_RedirectionPlugin404Monitoring' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'redirection-plugin-404-monitoring', $slug );
	}
