<?php
/**
 * Test for Oceanwp Theme Demo Import
 *
...
use WPShadow\Diagnostics\Diagnostic_OceanwpThemeDemoImport;

/**
 * OceanwpThemeDemoImportTest Class
 */
class OceanwpThemeDemoImportTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_OceanwpThemeDemoImport' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_OceanwpThemeDemoImport', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_OceanwpThemeDemoImport', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_OceanwpThemeDemoImport::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_OceanwpThemeDemoImport' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'oceanwp-theme-demo-import', $slug );
	}
