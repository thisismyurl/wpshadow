<?php
/**
 * Test for WP Job Manager Application Data
 *
...
use WPShadow\Diagnostics\Diagnostic_WpJobManagerApplicationData;

/**
 * WpJobManagerApplicationDataTest Class
 */
class WpJobManagerApplicationDataTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerApplicationData' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerApplicationData', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerApplicationData', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WpJobManagerApplicationData::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpJobManagerApplicationData' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wp-job-manager-application-data', $slug );
	}
