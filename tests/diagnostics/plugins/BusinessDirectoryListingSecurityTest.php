<?php
/**
 * Test for Business Directory Listing Security
 *
...
use WPShadow\Diagnostics\Diagnostic_BusinessDirectoryListingSecurity;

/**
 * BusinessDirectoryListingSecurityTest Class
 */
class BusinessDirectoryListingSecurityTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryListingSecurity' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryListingSecurity', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryListingSecurity', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_BusinessDirectoryListingSecurity::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectoryListingSecurity' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'business-directory-listing-security', $slug );
	}
