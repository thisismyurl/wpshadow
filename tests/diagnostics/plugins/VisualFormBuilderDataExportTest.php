<?php
/**
 * Test for Visual Form Builder Data Export
 *
...
use WPShadow\Diagnostics\Diagnostic_VisualFormBuilderDataExport;

/**
 * VisualFormBuilderDataExportTest Class
 */
class VisualFormBuilderDataExportTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_VisualFormBuilderDataExport' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_VisualFormBuilderDataExport', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_VisualFormBuilderDataExport', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_VisualFormBuilderDataExport::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_VisualFormBuilderDataExport' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'visual-form-builder-data-export', $slug );
	}
