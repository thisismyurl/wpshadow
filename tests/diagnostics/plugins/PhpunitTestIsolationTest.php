<?php
/**
 * Test for Phpunit Test Isolation
 *
...
use WPShadow\Diagnostics\Diagnostic_PhpunitTestIsolation;

/**
 * PhpunitTestIsolationTest Class
 */
class PhpunitTestIsolationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_PhpunitTestIsolation' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_PhpunitTestIsolation', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_PhpunitTestIsolation', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_PhpunitTestIsolation::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_PhpunitTestIsolation' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'phpunit-test-isolation', $slug );
	}
