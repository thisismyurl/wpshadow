<?php
/**
 * Test for Caldera Forms GDPR Compliance
 *
...
use WPShadow\Diagnostics\Diagnostic_CalderaFormsGdprCompliance;

/**
 * CalderaFormsGdprComplianceTest Class
 */
class CalderaFormsGdprComplianceTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsGdprCompliance' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsGdprCompliance', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsGdprCompliance', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_CalderaFormsGdprCompliance::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsGdprCompliance' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'caldera-forms-gdpr-compliance', $slug );
	}
