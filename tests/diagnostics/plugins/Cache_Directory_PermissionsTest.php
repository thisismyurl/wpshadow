<?php
/**
 * Test for Cache Directory Permissions
 *
...
use WPShadow\Diagnostics\Diagnostic_Cache_Directory_Permissions;

/**
 * Cache_Directory_PermissionsTest Class
 */
class Cache_Directory_PermissionsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_Cache_Directory_Permissions' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_Cache_Directory_Permissions', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_Cache_Directory_Permissions', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_Cache_Directory_Permissions::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_Cache_Directory_Permissions' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'cache-directory-permissions', $slug );
	}
