<?php
/**
 * Test for Paypal Express Api Credentials
 *
...
use WPShadow\Diagnostics\Diagnostic_PaypalExpressApiCredentials;

/**
 * PaypalExpressApiCredentialsTest Class
 */
class PaypalExpressApiCredentialsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_PaypalExpressApiCredentials' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_PaypalExpressApiCredentials', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_PaypalExpressApiCredentials', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_PaypalExpressApiCredentials::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_PaypalExpressApiCredentials' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'paypal-express-api-credentials', $slug );
	}
