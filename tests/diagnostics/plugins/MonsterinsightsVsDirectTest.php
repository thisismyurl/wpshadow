<?php
/**
 * Test for MonsterInsights vs Direct GA
 *
...
use WPShadow\Diagnostics\Diagnostic_MonsterinsightsVsDirect;

/**
 * MonsterinsightsVsDirectTest Class
 */
class MonsterinsightsVsDirectTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_MonsterinsightsVsDirect' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_MonsterinsightsVsDirect', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_MonsterinsightsVsDirect', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_MonsterinsightsVsDirect::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_MonsterinsightsVsDirect' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'monsterinsights-vs-direct', $slug );
	}
