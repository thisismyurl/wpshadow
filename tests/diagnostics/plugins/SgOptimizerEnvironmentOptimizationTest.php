<?php
/**
 * Test for Sg Optimizer Environment Optimization
 *
...
use WPShadow\Diagnostics\Diagnostic_SgOptimizerEnvironmentOptimization;

/**
 * SgOptimizerEnvironmentOptimizationTest Class
 */
class SgOptimizerEnvironmentOptimizationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_SgOptimizerEnvironmentOptimization' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_SgOptimizerEnvironmentOptimization', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_SgOptimizerEnvironmentOptimization', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_SgOptimizerEnvironmentOptimization::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_SgOptimizerEnvironmentOptimization' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'sg-optimizer-environment-optimization', $slug );
	}
