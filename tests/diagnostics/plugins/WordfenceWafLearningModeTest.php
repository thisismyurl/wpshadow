<?php
/**
 * Test for Wordfence Waf Learning Mode
 *
...
use WPShadow\Diagnostics\Diagnostic_WordfenceWafLearningMode;

/**
 * WordfenceWafLearningModeTest Class
 */
class WordfenceWafLearningModeTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WordfenceWafLearningMode' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WordfenceWafLearningMode', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WordfenceWafLearningMode', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_WordfenceWafLearningMode::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WordfenceWafLearningMode' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wordfence-waf-learning-mode', $slug );
	}
