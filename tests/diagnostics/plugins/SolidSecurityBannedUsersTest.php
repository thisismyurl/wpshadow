<?php
/**
 * Test for Solid Security Banned Users
 *
...
use WPShadow\Diagnostics\Diagnostic_SolidSecurityBannedUsers;

/**
 * SolidSecurityBannedUsersTest Class
 */
class SolidSecurityBannedUsersTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_SolidSecurityBannedUsers' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_SolidSecurityBannedUsers', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_SolidSecurityBannedUsers', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_SolidSecurityBannedUsers::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_SolidSecurityBannedUsers' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'solid-security-banned-users', $slug );
	}
