<?php
/**
 * Test for Envira Gallery Lazy Loading
 *
...
use WPShadow\Diagnostics\Diagnostic_EnviraGalleryLazyLoading;

/**
 * EnviraGalleryLazyLoadingTest Class
 */
class EnviraGalleryLazyLoadingTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_EnviraGalleryLazyLoading' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_EnviraGalleryLazyLoading', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_EnviraGalleryLazyLoading', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_EnviraGalleryLazyLoading::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_EnviraGalleryLazyLoading' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'envira-gallery-lazy-loading', $slug );
	}
