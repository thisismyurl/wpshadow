<?php
/**
 * Test for Wp Super Cache Compression
 *
...
use WPShadow\Diagnostics\Diagnostic_WpSuperCacheCompression;

/**
 * WpSuperCacheCompressionTest Class
 */
class WpSuperCacheCompressionTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheCompression' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheCompression', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheCompression', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WpSuperCacheCompression::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpSuperCacheCompression' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wp-super-cache-compression', $slug );
	}
