<?php
/**
 * Test for LifterLMS Database Queries
 *
...
use WPShadow\Diagnostics\Diagnostic_LifterlmsDatabaseQueries;

/**
 * LifterlmsDatabaseQueriesTest Class
 */
class LifterlmsDatabaseQueriesTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_LifterlmsDatabaseQueries' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_LifterlmsDatabaseQueries', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_LifterlmsDatabaseQueries', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_LifterlmsDatabaseQueries::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_LifterlmsDatabaseQueries' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'lifterlms-database-queries', $slug );
	}
