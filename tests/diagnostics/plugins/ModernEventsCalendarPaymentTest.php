<?php
/**
 * Test for Modern Events Calendar Payment
 *
...
use WPShadow\Diagnostics\Diagnostic_ModernEventsCalendarPayment;

/**
 * ModernEventsCalendarPaymentTest Class
 */
class ModernEventsCalendarPaymentTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_ModernEventsCalendarPayment' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_ModernEventsCalendarPayment', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_ModernEventsCalendarPayment', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_ModernEventsCalendarPayment::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_ModernEventsCalendarPayment' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'modern-events-calendar-payment', $slug );
	}
