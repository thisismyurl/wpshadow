<?php
/**
 * Test for Rank Math 404 Monitoring
 *
...
use WPShadow\Diagnostics\Diagnostic_RankMath404Monitoring;

/**
 * RankMath404MonitoringTest Class
 */
class RankMath404MonitoringTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_RankMath404Monitoring' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_RankMath404Monitoring', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_RankMath404Monitoring', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_RankMath404Monitoring::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_RankMath404Monitoring' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'rank-math-404-monitoring', $slug );
	}
