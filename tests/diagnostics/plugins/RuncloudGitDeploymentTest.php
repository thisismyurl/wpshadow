<?php
/**
 * Test for Runcloud Git Deployment
 *
...
use WPShadow\Diagnostics\Diagnostic_RuncloudGitDeployment;

/**
 * RuncloudGitDeploymentTest Class
 */
class RuncloudGitDeploymentTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_RuncloudGitDeployment' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_RuncloudGitDeployment', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_RuncloudGitDeployment', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_RuncloudGitDeployment::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_RuncloudGitDeployment' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'runcloud-git-deployment', $slug );
	}
