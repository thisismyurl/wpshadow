<?php
/**
 * Test for Wp Gdpr Compliance Data Requests
 *
...
use WPShadow\Diagnostics\Diagnostic_WpGdprComplianceDataRequests;

/**
 * WpGdprComplianceDataRequestsTest Class
 */
class WpGdprComplianceDataRequestsTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpGdprComplianceDataRequests' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpGdprComplianceDataRequests', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpGdprComplianceDataRequests', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_WpGdprComplianceDataRequests::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpGdprComplianceDataRequests' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wp-gdpr-compliance-data-requests', $slug );
	}
