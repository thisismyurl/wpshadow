<?php
/**
 * Test for Caldera Forms Email Delivery
 *
...
use WPShadow\Diagnostics\Diagnostic_CalderaFormsEmailDelivery;

/**
 * CalderaFormsEmailDeliveryTest Class
 */
class CalderaFormsEmailDeliveryTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsEmailDelivery' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsEmailDelivery', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsEmailDelivery', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_CalderaFormsEmailDelivery::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_CalderaFormsEmailDelivery' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'caldera-forms-email-delivery', $slug );
	}
