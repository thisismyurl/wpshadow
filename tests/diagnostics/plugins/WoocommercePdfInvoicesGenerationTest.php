<?php
/**
 * Test for Woocommerce Pdf Invoices Generation
 *
...
use WPShadow\Diagnostics\Diagnostic_WoocommercePdfInvoicesGeneration;

/**
 * WoocommercePdfInvoicesGenerationTest Class
 */
class WoocommercePdfInvoicesGenerationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WoocommercePdfInvoicesGeneration' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WoocommercePdfInvoicesGeneration', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WoocommercePdfInvoicesGeneration', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WoocommercePdfInvoicesGeneration::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WoocommercePdfInvoicesGeneration' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'woocommerce-pdf-invoices-generation', $slug );
	}
