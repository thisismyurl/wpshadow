<?php
/**
 * Test for LiteSpeed Cache Crawler
 *
...
use WPShadow\Diagnostics\Diagnostic_LitespeedCrawler;

/**
 * LitespeedCrawlerTest Class
 */
class LitespeedCrawlerTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_LitespeedCrawler' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_LitespeedCrawler', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_LitespeedCrawler', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_LitespeedCrawler::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_LitespeedCrawler' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'litespeed-crawler', $slug );
	}
