<?php
/**
 * Test for Google Analytics Anonymize Ip
 *
...
use WPShadow\Diagnostics\Diagnostic_GoogleAnalyticsAnonymizeIp;

/**
 * GoogleAnalyticsAnonymizeIpTest Class
 */
class GoogleAnalyticsAnonymizeIpTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsAnonymizeIp' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsAnonymizeIp', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsAnonymizeIp', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_GoogleAnalyticsAnonymizeIp::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_GoogleAnalyticsAnonymizeIp' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'google-analytics-anonymize-ip', $slug );
	}
