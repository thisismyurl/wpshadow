<?php
/**
 * Test for Business Directory Spam Protection
 *
...
use WPShadow\Diagnostics\Diagnostic_BusinessDirectorySpamProtection;

/**
 * BusinessDirectorySpamProtectionTest Class
 */
class BusinessDirectorySpamProtectionTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectorySpamProtection' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectorySpamProtection', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectorySpamProtection', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_BusinessDirectorySpamProtection::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_BusinessDirectorySpamProtection' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'business-directory-spam-protection', $slug );
	}
