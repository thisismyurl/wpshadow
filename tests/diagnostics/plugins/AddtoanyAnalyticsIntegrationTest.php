...
use WPShadow\Diagnostics\Diagnostic_AddtoanyAnalyticsIntegration;

/**
 * AddtoanyAnalyticsIntegrationTest Class
 */
class AddtoanyAnalyticsIntegrationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_AddtoanyAnalyticsIntegration' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_AddtoanyAnalyticsIntegration', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_AddtoanyAnalyticsIntegration', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'function_exists' )
			->with( 'A2A_SHARE_SAVE_init' )
			->andReturn( false );
		
		$result = Diagnostic_AddtoanyAnalyticsIntegration::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_AddtoanyAnalyticsIntegration' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'addtoany-analytics-integration', $slug );
	}
