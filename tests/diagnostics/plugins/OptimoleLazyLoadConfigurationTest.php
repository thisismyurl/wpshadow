<?php
/**
 * Test for Optimole Lazy Load Configuration
 *
...
use WPShadow\Diagnostics\Diagnostic_OptimoleLazyLoadConfiguration;

/**
 * OptimoleLazyLoadConfigurationTest Class
 */
class OptimoleLazyLoadConfigurationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_OptimoleLazyLoadConfiguration' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_OptimoleLazyLoadConfiguration', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_OptimoleLazyLoadConfiguration', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_OptimoleLazyLoadConfiguration::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_OptimoleLazyLoadConfiguration' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'optimole-lazy-load-configuration', $slug );
	}
