<?php
/**
 * Test for WP Rocket File Optimization
 *
...
use WPShadow\Diagnostics\Diagnostic_WpRocketFileOptimization;

/**
 * WpRocketFileOptimizationTest Class
 */
class WpRocketFileOptimizationTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpRocketFileOptimization' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpRocketFileOptimization', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpRocketFileOptimization', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		\WP_Mock::userFunction( 'function_exists' )
			->with( 'rocket_direct_filesystem' )
			->andReturn( false );
		
		$result = Diagnostic_WpRocketFileOptimization::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpRocketFileOptimization' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wp-rocket-file-optimization', $slug );
	}
