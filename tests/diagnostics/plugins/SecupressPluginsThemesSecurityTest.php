<?php
/**
 * Test for Secupress Plugins Themes Security
 *
...
use WPShadow\Diagnostics\Diagnostic_SecupressPluginsThemesSecurity;

/**
 * SecupressPluginsThemesSecurityTest Class
 */
class SecupressPluginsThemesSecurityTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_SecupressPluginsThemesSecurity' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_SecupressPluginsThemesSecurity', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_SecupressPluginsThemesSecurity', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_SecupressPluginsThemesSecurity::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_SecupressPluginsThemesSecurity' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'secupress-plugins-themes-security', $slug );
	}
