<?php
/**
 * Test for SearchWP Minimum Word Length
 *
...
use WPShadow\Diagnostics\Diagnostic_SearchwpMinimumWordLength;

/**
 * SearchwpMinimumWordLengthTest Class
 */
class SearchwpMinimumWordLengthTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_SearchwpMinimumWordLength' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_SearchwpMinimumWordLength', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_SearchwpMinimumWordLength', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_SearchwpMinimumWordLength::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_SearchwpMinimumWordLength' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'searchwp-minimum-word-length', $slug );
	}
