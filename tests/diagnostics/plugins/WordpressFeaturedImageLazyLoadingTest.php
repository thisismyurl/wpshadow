<?php
/**
 * Test for Wordpress Featured Image Lazy Loading
 *
...
use WPShadow\Diagnostics\Diagnostic_WordpressFeaturedImageLazyLoading;

/**
 * WordpressFeaturedImageLazyLoadingTest Class
 */
class WordpressFeaturedImageLazyLoadingTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WordpressFeaturedImageLazyLoading' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WordpressFeaturedImageLazyLoading', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WordpressFeaturedImageLazyLoading', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_WordpressFeaturedImageLazyLoading::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WordpressFeaturedImageLazyLoading' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wordpress-featured-image-lazy-loading', $slug );
	}
