<?php
/**
 * Test for WP All Import Custom Field Mapping
 *
...
use WPShadow\Diagnostics\Diagnostic_WpAllImportCustomFieldMapping;

/**
 * WpAllImportCustomFieldMappingTest Class
 */
class WpAllImportCustomFieldMappingTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_WpAllImportCustomFieldMapping' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_WpAllImportCustomFieldMapping', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_WpAllImportCustomFieldMapping', 'check' ] ) );
	}
	...
	public function test_returns_null_when_plugin_not_active() {
		...
		$result = Diagnostic_WpAllImportCustomFieldMapping::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_WpAllImportCustomFieldMapping' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'wp-all-import-custom-field-mapping', $slug );
	}
