<?php
/**
 * Test for Plesk Wordpress Toolkit
 *
...
use WPShadow\Diagnostics\Diagnostic_PleskWordpressToolkit;

/**
 * PleskWordpressToolkitTest Class
 */
class PleskWordpressToolkitTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_PleskWordpressToolkit' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_PleskWordpressToolkit', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_PleskWordpressToolkit', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_PleskWordpressToolkit::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_PleskWordpressToolkit' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'plesk-wordpress-toolkit', $slug );
	}
