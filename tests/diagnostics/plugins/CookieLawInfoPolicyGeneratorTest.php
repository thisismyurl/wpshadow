<?php
/**
 * Test for Cookie Law Info Policy Generator
 *
...
use WPShadow\Diagnostics\Diagnostic_CookieLawInfoPolicyGenerator;

/**
 * CookieLawInfoPolicyGeneratorTest Class
 */
class CookieLawInfoPolicyGeneratorTest extends TestCase {
	...
	public function test_diagnostic_class_exists() {
		$this->assertTrue( class_exists( 'WPShadow\Diagnostics\\Diagnostic_CookieLawInfoPolicyGenerator' ) );
	}

	public function test_check_method_is_callable() {
		$this->assertTrue( method_exists( 'WPShadow\Diagnostics\\Diagnostic_CookieLawInfoPolicyGenerator', 'check' ) );
		$this->assertTrue( is_callable( [ 'WPShadow\Diagnostics\\Diagnostic_CookieLawInfoPolicyGenerator', 'check' ] ) );
	}

	public function test_returns_null_when_plugin_not_active() {
		// No specific plugin activation check
		
		$result = Diagnostic_CookieLawInfoPolicyGenerator::check();
	...
	public function test_slug_property_matches_expected() {
		$reflection = new \ReflectionClass( 'WPShadow\Diagnostics\\Diagnostic_CookieLawInfoPolicyGenerator' );
		$property = $reflection->getProperty( 'slug' );
		$property->setAccessible( true );
		$slug = $property->getValue();
		
		$this->assertEquals( 'cookie-law-info-policy-generator', $slug );
	}
