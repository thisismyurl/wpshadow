...
class Diagnostic_Login_URL_Obfuscation extends Diagnostic_Base
{
	...
	public static function test_live__login_url_obfuscation(): array
	{
		...
		$active = get_option('active_plugins', array());
		$login_url_changed = false;
		foreach ($login_plugins as $plugin) {
			if (in_array($plugin, $active, true)) {
				$login_url_changed = true;
				break;
			}
		}

		$diagnostic_passed = is_null($result);
		$test_passes = ($login_url_changed === $diagnostic_passed);

		return array(
			'passed' => $test_passes,
			'message' => $test_passes ? 'Login URL obfuscation check matches site state' :
				"Mismatch: expected " . ($login_url_changed ? 'obfuscated login' : 'default login') . " but got " .
				($diagnostic_passed ? 'pass' : 'issue'),
		);
	}
}
