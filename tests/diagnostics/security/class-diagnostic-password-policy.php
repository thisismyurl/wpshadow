...
class Diagnostic_Password_Policy extends Diagnostic_Base
{
	...
	public static function test_live__password_policy(): array
	{
		...
		$test_passes = ($has_policy === !$diagnostic_found_issue);

		return array(
			'passed' => $test_passes,
			'message' => $test_passes ? 'Password policy check matches site state' :
				"Mismatch: expected " . ($has_policy ? 'no issue' : 'issue') . " but got " .
