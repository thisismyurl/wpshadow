...
class Diagnostic_Weak_Password_Hashing extends Diagnostic_Base
{
	...
	public static function test_live__weak_password_hashing(): array
	{
		...
		$has_weak_hashes = !empty($results[0]->count) && $results[0]->count > 0;
		$diagnostic_found_issue = !is_null($result);
		$test_passes = ($has_weak_hashes === $diagnostic_found_issue);

		return array(
			'passed' => $test_passes,
			'message' => $test_passes ? 'Weak password hashing check matches site state' :
				"Mismatch: expected " . ($has_weak_hashes ? 'issue' : 'no issue') . " but got " .
