...
class Diagnostic_Code_CODE_SECURITY_NONCE_ENFORCEMENT extends Diagnostic_Base
{
	...
	public static function test_live__code_code_security_nonce_enforcement(): array
	{
		...
		$result = self::check();
		if (is_array($result) && isset($result['id']) && 'code-security-nonce-enforcement' === $result['id']) {
			return array(
				'passed' => true,
				'message' => 'Nonce enforcement advisory returned with correct structure.',
			);
		}
		return array(
			'passed' => false,
			'message' => 'Expected advisory structure for nonce enforcement check.',
		);
	}
}
