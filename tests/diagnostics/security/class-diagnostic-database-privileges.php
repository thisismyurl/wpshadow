...
class Diagnostic_Database_Privileges extends Diagnostic_Base
{
	...
	public static function test_live__database_privileges(): array
	{
		...
		if (empty($grants)) {
			...
			return array(
				'passed' => null === $result,
				'message' => 'Cannot check database privileges',
			);
		}
		...
		$test_passes = ($has_dangerous === $diagnostic_found_issue);

		return array(
			'passed' => $test_passes,
			'message' => $test_passes ? 'Database privileges check matches site state' :
				"Mismatch: expected " . ($has_dangerous ? 'issue' : 'no issue') . " but got " .
