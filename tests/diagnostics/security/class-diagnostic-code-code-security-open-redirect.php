...
class Diagnostic_Code_CODE_SECURITY_OPEN_REDIRECT extends Diagnostic_Base
{
	...
	public static function test_live__code_code_security_open_redirect(): array
	{
		...
		$result = self::check();
		if (is_array($result) && isset($result['id']) && 'code-security-open-redirect' === $result['id']) {
			return array(
				'passed' => true,
				'message' => 'Open redirect advisory returned with correct structure.',
			);
		}
		return array(
			'passed' => false,
			'message' => 'Expected advisory structure for open redirect check.',
		);
	}
}
