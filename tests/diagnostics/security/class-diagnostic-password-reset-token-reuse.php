...
class Diagnostic_Password_Reset_Token_Reuse extends Diagnostic_Base
{
	...
	public static function test_live__password_reset_token_reuse(): array
	{
		...
		$reset_handlers = array();
		if (isset($wp_filter['retrieve_password_message']) && isset($wp_filter['retrieve_password_message']->callbacks)) {
			foreach ($wp_filter['retrieve_password_message']->callbacks as $callbacks) {
				$reset_handlers = array_merge($reset_handlers, array_keys($callbacks));
			}
		}

		$expected_issue = count($reset_handlers) > 1;

		$result = self::check();
		$has_finding = is_array($result);

		if ($expected_issue === $has_finding) {
			$message = $expected_issue ? 'Finding returned when multiple password reset handlers exist.' : 'No finding returned when reset handler setup is default.';
			return array(
				'passed'  => true,
				'message' => $message,
			);
		...
		$message = $expected_issue
			? 'Expected a finding with multiple reset handlers, but got none.'
			: 'Expected no finding with default reset handlers, but got a finding.';
