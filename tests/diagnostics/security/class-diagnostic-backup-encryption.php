...
class Diagnostic_Backup_Encryption extends Diagnostic_Base
{
	...
	public static function test_live__backup_encryption(): array
	{
		...
		$test_passes = ($has_unencrypted === $diagnostic_found_issue);

		return array(
			'passed' => $test_passes,
			'message' => $test_passes ? 'Backup encryption check matches site state' :
				"Mismatch: expected " . ($has_unencrypted ? 'issue' : 'no issue') . " but got " .
