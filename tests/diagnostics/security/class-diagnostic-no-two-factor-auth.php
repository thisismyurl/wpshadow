...
class Diagnostic_No_Two_Factor_Auth extends Diagnostic_Base
{
    ...
    public static function test_live_no_two_factor_auth(): array
    {
        ...
        if (empty($privileged_users)) {
            ...
            return array(
                'passed'  => null === $result,
                'message' => null === $result ? 'No privileged users; diagnostic correctly returned no finding.' : 'No privileged users but diagnostic returned a finding.',
            );
        }
        ...
        $total_users    = count($privileged_users);
        $adoption_ratio = $total_users > 0 ? $with_2fa / $total_users : 0;
        $expected_issue = ! $has_2fa_plugin || $adoption_ratio < 0.5;

        $result = self::check();
        $has_finding = is_array($result);

        if ($expected_issue === $has_finding) {
            $message = $expected_issue ? 'Finding returned because 2FA adoption is low or missing.' : 'No finding returned when 2FA adoption meets threshold.';
            return array(
                'passed'  => true,
                'message' => $message,
            );
        ...
        $message = $expected_issue
            ? 'Expected a finding for low 2FA adoption, but got none.'
            : 'Expected no finding for adequate 2FA adoption, but got a finding.';
