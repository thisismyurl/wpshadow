...
class Diagnostic_Admin_Font_Bloat extends Diagnostic_Base {
	 ...
	 *
	 * Diagnostic: Admin Font Bloat
	 * Slug: -admin-font-bloat
	 * File: class-diagnostic-admin-font-bloat.php
	 * 
	 * Test Purpose:
	 ...
	 * }
	 */
	public static function test_live__admin_font_bloat(): array {
		global $wp_styles;
		...
		if ( is_admin() && isset( $wp_styles ) && ! empty( $wp_styles->queue ) ) {
			foreach ( $wp_styles->queue as $handle ) {
				if ( ! isset( $wp_styles->registered[ $handle ] ) ) {
					continue;
				}
		...
		$has_issue = ( $font_handle_ct > 3 || $font_face_blocks > 6 );
		if ( ! is_admin() ) {
			$has_issue = false; // Diagnostic is admin-only
		}
		...
		$message = $test_passes
			? 'Admin font bloat check matches site state'
			: sprintf(
				'Mismatch: expected %s but diagnostic returned %s (fonts: %d handles, %d @font-face blocks)',
