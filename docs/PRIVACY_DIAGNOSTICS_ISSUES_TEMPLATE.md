# Privacy & Plugin-Focused Diagnostics - Issue Generation List

**Status:** Ready for GitHub Issue Creation  
**Total Issues:** 100+ diagnostic ideas  
**Phases:** 6 implementation phases (8 weeks)  
**Priority:** Phase 1 starts immediately

---

## 📋 How to Create Issues

These issues are organized by phase. Create them using GitHub web interface, CLI, or API:

```bash
# Using gh CLI (if available)
gh issue create --title "Title" --body "Body" --label "privacy,gdpr"
```
...
### Issue #1: Google Analytics Tracking Detection & GDPR Compliance
...
```
## Description
Detect and analyze Google Analytics installations, verify GDPR compliance settings (anonymizeIp), 
and check for consent mechanism integration.
...
### Issue #2: Facebook Pixel Installation & Privacy Compliance
...
```
## Description
Detect Facebook Pixel implementations and verify privacy compliance, including consent requirements 
and server-side API usage (more private alternative).
...
### Issue #3: Third-Party Cookie Audit & Consent Management
...
```
## Description
Scan for all cookies set by external domains, categorize them (essential/analytics/marketing), 
and verify consent is required before setting.
...
### Issue #4: External API Calls Inventory & Data Flow Mapping
...
```
## Description
Detect all wp_remote_get/post calls to external services and map data flow, identifying 
...
### Issue #5: Email Marketing Service Detection & Consent Verification
...
```
## Description
Identify email marketing service integrations (MailChimp, ConvertKit, etc.) and verify 
GDPR-compliant consent mechanisms, including double opt-in.
...
### Issue #6: Analytics Loading Before Consent Detection (CRITICAL)
...
```
## Description
Detect if tracking scripts (Google Analytics, hotjar, Mixpanel, etc.) load before user consent 
is obtained, which violates GDPR/CCPA. This is one of the most common violations.
...
## Note
This is a critical GDPR compliance issue. Many site owners don't realize loading GA before 
consent violates regulations.
```

---

### Issue #7: User IP Address Logging & Anonymization Verification
...
```
## Description
Detect plugins logging full IP addresses, verify if IP anonymization is enabled, 
and check retention policies comply with GDPR.
...
### Issue #8: Geolocation Data Collection Detection & Privacy Verification
...
```
## Description
Identify plugins collecting geolocation data (GPS, IP-based location, maps) and verify 
user notification, consent, and legitimate purpose.
...
## 📊 PHASE 2: Data Storage & Retention Diagnostics

### Issue #9: Expired Transients Not Being Deleted (Data Bloat)
...
```
## Description
Detect transients that have expired but not been automatically deleted from wp_options table, 
indicating data bloat and potential privacy issue.
...
### Issue #10: User Activity Log Retention Policy Verification
...
```
## Description
Verify plugins storing user activity logs (WooCommerce orders, comment history, admin actions) 
have GDPR-compliant retention policies.
...
### Issue #11: Deleted User Data Cleanup Verification (Right to Erasure)
...
```
## Description
Verify that when users are deleted, all associated personal data is properly removed from 
database, including orphaned meta records, comments, and logs. This is a GDPR "Right to Erasure" requirement.
...
## Critical Note
If users cannot delete their data, the site violates GDPR Article 17. This affects regulatory 
compliance in EU and similar jurisdictions worldwide.
```

---

### Issue #12: Database Table Encryption for Sensitive Data
...
```
## Description
Identify database tables storing sensitive data and verify encryption is enabled at rest. 
Flag plaintext storage of passwords, credit cards, or PII.
...
The document at `/workspaces/wpshadow/docs/DIAGNOSTIC_PRIVACY_EXPANSION_PLAN.md` contains:

- **Phase 3:** GDPR Rights Implementation (7 diagnostics)
- **Phase 4:** External Services Privacy (7 diagnostics)
- **Phase 5:** Plugin-Specific Diagnostics (40+ diagnostics for top WordPress plugins)
- **Phase 6:** Additional Plugin Coverage (50+ remaining plugins)

---

## 🚀 Next Steps

1. ✅ **Plan created** (`DIAGNOSTIC_PRIVACY_EXPANSION_PLAN.md`)
2. ⏳ **Create these 12 initial issues in GitHub**
3. ⏳ **Implement Phase 1 diagnostics (8 issues)**
4. ⏳ **Create remaining phase 2-6 issues**
5. ⏳ **Implement full 100+ diagnostic suite**

---

## 📌 Tags/Labels to Create

Make sure these labels exist in GitHub:

- `diagnostic` - All diagnostic issues
- `privacy` - Privacy-focused
- `tracking` - Tracking/analytics
- `gdpr` - GDPR compliance
- `compliance` - Legal compliance
- `phase-1` through `phase-6` - Implementation phases
- `high-priority` - Quick wins
- `critical` - Urgent
- `auto-fixable` - Has auto-fix treatment

---

**Ready to create issues! Let me know if you'd like me to help with the GitHub API integration.** 🔐
