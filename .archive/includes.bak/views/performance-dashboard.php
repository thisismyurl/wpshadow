...
$current_tab = isset( $_GET['perf_tab'] ) ? sanitize_key( $_GET['perf_tab'] ) : 'overview'; 
...
<div class="wrap wps-performance-dashboard">
	<h1><?php esc_html_e( 'Performance Monitoring Dashboard', 'wpshadow' ); ?></h1>
	...
	<h2 class="nav-tab-wrapper">
		...
		<a href="?page=wpshadow&WPSHADOW_tab=performance&perf_tab=database" class="nav-tab <?php echo 'database' === $current_tab ? 'nav-tab-active' : ''; ?>">
			<?php esc_html_e( 'Database', 'wpshadow' ); ?>
		</a>
	...
	<div class="wps-tab-content">
		<?php if ( 'overview' === $current_tab ) : ?>
			...
			<div class="wps-perf-overview">
				...
				<div class="wps-perf-card" style="max-width: 500px; margin: 20px auto;">
					<h3 style="text-align: center; font-size: 16px;"><?php esc_html_e( 'Performance Score', 'wpshadow' ); ?></h3>
					...
					<div style="text-align: center; padding: 15px; background: #f8f9fa; border-radius: 4px; margin-top: 20px;">
						<p style="margin: 0; font-size: 14px; color: #666;">
							...
							if ( $score_data['score'] >= 90 ) {
								esc_html_e( '🎉 Excellent! Your site is performing very well.', 'wpshadow' );
							} elseif ( $score_data['score'] >= 70 ) {
								esc_html_e( '✅ Good performance. Check recommendations for improvements.', 'wpshadow' );
							} elseif ( $score_data['score'] >= 50 ) {
								esc_html_e( '⚠️ Performance needs attention. Review recommendations below.', 'wpshadow' );
							} else {
								esc_html_e( '🚨 Critical performance issues detected. Immediate action recommended.', 'wpshadow' );
							}
							?>
						</p>
					</div>
				...
				<!-- Current Metrics -->
				<h2><?php esc_html_e( '📊 Current Metrics', 'wpshadow' ); ?></h2>
				<div class="wps-perf-grid">
					<div class="wps-perf-card">
						<h3><?php esc_html_e( 'Database Queries', 'wpshadow' ); ?></h3>
						<span class="metric-value"><?php echo esc_html( $metrics['query_count'] ?? 0 ); ?></span>
						<span class="metric-label">
							<?php

							echo esc_html( sprintf( __( '%s sec total time', 'wpshadow' ), number_format( (float) ( $metrics['query_time'] ?? 0 ), 3 ) ) );
							?>
						</span>
					</div>

					<div class="wps-perf-card">
						<h3><?php esc_html_e( 'Page Load Time', 'wpshadow' ); ?></h3>
						<span class="metric-value"><?php echo esc_html( number_format( (float) ( $metrics['load_time'] ?? 0 ), 3 ) ); ?>s</span>
						<span class="metric-label"><?php esc_html_e( 'Server generation time', 'wpshadow' ); ?></span>
					</div>

					<div class="wps-perf-card">
						<h3><?php esc_html_e( 'Memory Usage', 'wpshadow' ); ?></h3>
						<span class="metric-value"><?php echo esc_html( $metrics['memory_mb'] ?? 0 ); ?> MB</span>
						<span class="metric-label">
							<?php
							$memory_limit = ini_get( 'memory_limit' );

							echo esc_html( sprintf( __( 'Limit: %s', 'wpshadow' ), $memory_limit ) );
							?>
						</span>
					</div>

					<div class="wps-perf-card">
						<h3><?php esc_html_e( 'Database Size', 'wpshadow' ); ?></h3>
						<span class="metric-value"><?php echo esc_html( $metrics['db_size'] ?? 0 ); ?> MB</span>
						<span class="metric-label"><?php esc_html_e( 'Total database size', 'wpshadow' ); ?></span>
					</div>

					<div class="wps-perf-card">
						<h3><?php esc_html_e( 'Active Plugins', 'wpshadow' ); ?></h3>
						<span class="metric-value"><?php echo esc_html( $metrics['active_plugins'] ?? 0 ); ?></span>
						<span class="metric-label"><?php esc_html_e( 'Currently active', 'wpshadow' ); ?></span>
					</div>
				</div>

				<!-- Recommendations -->
				<?php if ( ! empty( $recommendations ) ) : ?>
				<h2><?php esc_html_e( '💡 Optimization Recommendations', 'wpshadow' ); ?></h2>
				...
				<div class="notice notice-success inline">
					<p><?php esc_html_e( '✅ No performance issues detected! Your site is running well.', 'wpshadow' ); ?></p>
				</div>
				<?php endif; ?>
			</div>
		...
		<?php elseif ( 'database' === $current_tab ) : ?>
			<!-- Database Tab -->
			<h2><?php esc_html_e( '🗄️ Database Statistics', 'wpshadow' ); ?></h2>

			<div class="wps-perf-grid">
				<div class="wps-perf-card">
					<h3><?php esc_html_e( 'Total Size', 'wpshadow' ); ?></h3>
					<span class="metric-value"><?php echo esc_html( $db_stats['total_size'] ); ?> MB</span>
					<span class="metric-label"><?php esc_html_e( 'All tables combined', 'wpshadow' ); ?></span>
				</div>

				<div class="wps-perf-card">
					<h3><?php esc_html_e( 'Transients', 'wpshadow' ); ?></h3>
					<span class="metric-value"><?php echo esc_html( $db_stats['transient_count'] ); ?></span>
					<span class="metric-label">
						<?php

						echo esc_html( sprintf( __( '%d expired', 'wpshadow' ), $db_stats['expired_transients'] ) );
						?>
					</span>
				</div>

				<div class="wps-perf-card">
					<h3><?php esc_html_e( 'Orphaned Data', 'wpshadow' ); ?></h3>
					<span class="metric-value"><?php echo esc_html( $db_stats['orphaned_postmeta'] ); ?></span>
					<span class="metric-label"><?php esc_html_e( 'Orphaned postmeta records', 'wpshadow' ); ?></span>
				</div>
			</div>

			<?php if ( ! empty( $db_stats['largest_tables'] ) ) : ?>
			<h3><?php esc_html_e( 'Largest Tables (Top 10)', 'wpshadow' ); ?></h3>
			<table class="wps-db-table">
				<thead>
					<tr>
						<th><?php esc_html_e( 'Table Name', 'wpshadow' ); ?></th>
						<th><?php esc_html_e( 'Size (MB)', 'wpshadow' ); ?></th>
					</tr>
				</thead>
				<tbody>
			...
			<h3><?php esc_html_e( 'Slow Queries (&gt;100ms)', 'wpshadow' ); ?></h3>
			<table class="wps-db-table">
				<thead>
					<tr>
						<th><?php esc_html_e( 'Query', 'wpshadow' ); ?></th>
						<th><?php esc_html_e( 'Time (s)', 'wpshadow' ); ?></th>
						<th><?php esc_html_e( 'Caller', 'wpshadow' ); ?></th>
					</tr>
				</thead>
				<tbody>
			...
			<h3><?php esc_html_e( 'Query Breakdown by Type', 'wpshadow' ); ?></h3>
			<div class="wps-perf-grid">
		...
		<?php elseif ( 'history' === $current_tab ) : ?>
			<!-- History Tab -->
			<h2><?php esc_html_e( '📈 Historical Performance Data', 'wpshadow' ); ?></h2>
			...
			<div class="wps-perf-card">
				<h3><?php esc_html_e( 'Performance Trends', 'wpshadow' ); ?></h3>
				<p><?php esc_html_e( 'Historical data tracking coming soon. This will show:' ); ?></p>
				<ul>
					<li><?php esc_html_e( 'Average page load time (7/30/90 days)', 'wpshadow' ); ?></li>
					<li><?php esc_html_e( 'Query count per request trends', 'wpshadow' ); ?></li>
					<li><?php esc_html_e( 'Memory usage trends', 'wpshadow' ); ?></li>
					<li><?php esc_html_e( 'Database size growth', 'wpshadow' ); ?></li>
				</ul>

				<p style="margin-top: 20px;">
					<?php

					echo esc_html( sprintf( __( 'Currently tracking %d data points over the last 90 days.', 'wpshadow' ), count( WPSHADOW_Performance_Monitor::get_historical_metrics( 90 ) ) ) );
					?>
				</p>

				<p>
					<a href="#" class="button" id="wps-export-csv"><?php esc_html_e( 'Export as CSV', 'wpshadow' ); ?></a>
					<a href="#" class="button" id="wps-export-json"><?php esc_html_e( 'Export as JSON', 'wpshadow' ); ?></a>
				</p>
			</div>
