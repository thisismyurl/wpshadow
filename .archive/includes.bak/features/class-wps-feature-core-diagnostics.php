...
final class WPSHADOW_Feature_Core_Diagnostics extends WPSHADOW_Abstract_Feature {
		...
		parent::__construct(
			array(
				...
				'sub_features'       => array(
					...
					'description_long'   => __( 'Validates SSL/HTTPS configuration and consistency. Checks if site URLs (home_url and site_url) use https protocol and verifies force SSL admin setting. Essential for security and SEO as mixed content warnings or inconsistent SSL configuration can break site functionality and damage search rankings. Identifies if SSL is properly configured at server level versus just WordPress settings. Critical for GDPR compliance and PCI-DSS requirements. Includes recommendations for fixing SSL issues and implementing forced HTTPS redirects.', 'wpshadow' ),
	...
	private function check_ssl_configuration(): array {
		$home_url = home_url();
		$site_url = site_url();
		if ( ! is_ssl() && ( strpos( $home_url, 'https://' ) !== false || strpos( $site_url, 'https://' ) !== false ) ) {
			return array(
				'status'   => 'critical',
