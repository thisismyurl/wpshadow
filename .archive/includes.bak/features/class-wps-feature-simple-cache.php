...
final class WPSHADOW_Feature_Simple_Cache extends WPSHADOW_Abstract_Feature {
	...
	private function should_cache_request(): bool {
		...
		if ( $this->is_sub_feature_enabled( 'skip_query_strings', true ) && ! empty( $_SERVER['QUERY_STRING'] ) ) { 
			return false;
		}
	...
	public function cache_output_buffer( string $buffer ): string {
		...
		$saved = file_put_contents( $cache_file, $buffer_to_save ); 
	...
	public function display_admin_notices(): void {
		if ( isset( $_GET['wpshadow-cache-cleared'] ) ) { 
			$deleted = intval( $_GET['wpshadow-cache-cleared'] ); 
			?>
