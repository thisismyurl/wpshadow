...
(function ($) {
	...
	/**
	 * Persist dismissal for 24 hours when the notice is closed.
	 */
	function initDismissHandler() {
		$( document ).on( 'click', '.wpshadow-phone-home-indicator .notice-dismiss', function () {
			$.ajax(
				{
					url: wpshadowPhoneHome.ajax_url,
					type: 'POST',
					data: {
						action: 'wpshadow_dismiss_phone_home_indicator',
						_wpnonce: wpshadowPhoneHome.nonce
					}
				}
			);
