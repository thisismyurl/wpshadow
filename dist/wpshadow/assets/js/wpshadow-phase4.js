...
(function ($) {
	...
	const WPShadowPhase4 = {
		...
		initAutoSnapshot: function () {
			...
			$( document ).on(
				'wpshadow:report:complete',
				function (e, reportId, data) {
		...
		displayComparison: function (comparison) {
			...
			if (comparison.resolved_issues.length > 0) {
				...
				comparison.resolved_issues.forEach(
					function (issue) {
		...
		bindAnalyticsButtons: function () {
			$( document ).on(
				'click',
				'.wpshadow-calculate-analytics-btn',
				function (e) {
					e.preventDefault();
		...
		formatExecutiveSummary: function (summary) {
			...
			if (summary.priority_actions.length > 0) {
				...
				summary.priority_actions.forEach(
					function (action) {
		...
		formatRegressions: function (regressions) {
			...
			if ( ! regressions.detected) {
				...
				regressions.regressions.forEach(
					function (reg) {
						html += '<li>';
