...
class Phone_Home_Indicator extends Hook_Subscriber_Base {
	...
	protected static function get_hooks(): array {
		return array(
			...
			'wp_ajax_wpshadow_dismiss_phone_home_indicator'   => 'ajax_dismiss_indicator',
		);
	}
	...
	public static function show_indicator() {
		...
		$dismissed_until = (int) get_user_meta( get_current_user_id(), 'wpshadow_phone_home_dismissed_until', true );
	...
	/**
	 * AJAX handler to dismiss indicator for 24 hours.
	 *
	 * @since 1.6093.1200
	 * @return void Dies after sending JSON response.
	 */
	public static function ajax_dismiss_indicator() {
		if ( ! \WPShadow\Core\Security_Validator::verify_nonce( 'wpshadow_phone_home', '_wpnonce', false ) ||
			! \WPShadow\Core\Security_Validator::verify_capability( 'manage_options', false ) ) {
			wp_send_json_error( array( 'message' => \WPShadow\Core\Security_Validator::get_permission_error() ) );
		}

		update_user_meta( get_current_user_id(), 'wpshadow_phone_home_dismissed_until', time() + DAY_IN_SECONDS );

		wp_send_json_success(
			array(
				'message' => __( 'Network activity indicator hidden for 24 hours.', 'wpshadow' ),
			)
		);
	}
