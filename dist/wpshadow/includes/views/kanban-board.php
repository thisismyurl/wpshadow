...
// Ensure Finding_Status_Manager is loaded
if ( ! class_exists( '\WPShadow\Core\Finding_Status_Manager' ) ) {
	$class_file = WPSHADOW_PATH . 'includes/systems/core/class-finding-status-manager.php';
...
$all_findings        = method_exists( $diagnostic_registry, 'run_enabled_scans' ) ? $diagnostic_registry::run_enabled_scans() : array();

// Apply category filter if present (Issue #564)
$kanban_category = isset( $_GET['kanban_category'] ) ? sanitize_key( wp_unslash( $_GET['kanban_category'] ) ) : '';
...
<!-- Kanban interactions are handled by assets/js/kanban-board.js -->