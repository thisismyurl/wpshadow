...
Most WordPress sites are **missing critical database indexes** that make queries 50-100x slower. Adding them takes 5 minutes and can reduce page load times by 50-80%. This is one of the highest-impact optimizations you can do.
...
WPShadow detects missing indexes automatically and adds them with one click—no technical knowledge needed. Your site will be 50-80% faster in 5 minutes.

### Install WPShadow (Free)

If you don't have WPShadow installed yet:

1. **Login to your WordPress admin** → Plugins → Add New
...
### Apply "Add Missing Indexes" Treatment

1. **Open WPShadow Dashboard**
2. **Go to Diagnostics & Treatments**
3. **Find "Missing Database Indexes"** → Shows exactly which indexes are missing and impact
4. **Click "Apply Treatment"** → Adds all missing indexes in seconds

Done! Your database is optimized. Check your admin pages—they should feel noticeably faster.
...
### Index Strategies for Custom Plugins

**Single Column Index (Simple):**
```php
...
// Look for:
// - type: "ref" or "range" (good), not "ALL" (bad)
// - key: should show index name
// - rows: fewer rows examined = better
```
