...
(function($) {
	...
	$(document).ready(function() {
		console.log('[WPShadow] Feature Details JS loaded and initializing...');
		initializeFeatureToggles();
		initializeSubFeatureToggles();
		initializeParentChildToggles();
		console.log('[WPShadow] Feature Details JS initialization complete');
	});
	 ...
	 * When toggled back on, children are restored to their previous state or default.
	 */
	function initializeParentChildToggles() {
		// Storage key prefix for saved child states
		var storagePrefix = 'wpshadow_child_state_';
		...
		$(document).on('change', '.wpshadow-parent-toggle', function() {
			...
			if (!isParentEnabled) {
				// Parent is being turned OFF - save child states and disable them
				$childToggles.each(function() {
					var $childToggle = $(this);
					var childId = $childToggle.data('feature-id');
					var wasEnabled = $childToggle.is(':checked');
					
					// Save the previous state for this child (only if it was enabled)
					if (childId && wasEnabled) {
						localStorage.setItem(storagePrefix + childId, 'true');
					}
					
					// Turn off the child WITHOUT triggering change event
					// (we don't want to save the forced "off" state to backend)
					if ($childToggle.is(':checked')) {
						$childToggle.prop('checked', false);
					}
					
					// Disable the child toggle visually
				...
				// Parent is being turned ON - restore children to their previous state or default
				$childToggles.each(function() {
					var $childToggle = $(this);
					var childId = $childToggle.data('feature-id');
					
					// Re-enable the child toggle visually
					$childToggle.prop('disabled', false).css('opacity', '1');
					
					// Restore previous state if available
					if (childId) {
						var savedState = localStorage.getItem(storagePrefix + childId);
						
						if (savedState !== null && savedState === 'true') {
							// Restore to previous enabled state
							if (!$childToggle.is(':checked')) {
								$childToggle.prop('checked', true).trigger('change');
							}
							// Clean up the saved state
							localStorage.removeItem(storagePrefix + childId);
						}
						// If saved state was false or not available, child stays off (no action needed)
					}
				});
	...
	function saveFeatureState(featureId, featureName, isEnabled) {
		...
		console.log('[WPShadow] Sending AJAX to save feature:', data);
		...
		$.ajax({
			...
			success: function(response) {
				console.log('[WPShadow] AJAX success response:', response);
				if (response.success) {
