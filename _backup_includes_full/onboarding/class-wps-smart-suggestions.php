...
class WPSHADOW_Smart_Suggestions {
	...
	public static function add_dashboard_widget(): void {
		...
		wp_add_dashboard_widget(
			...
			__( 'WPShadow Smart Suggestions', 'wpshadow' ),
	...
	public static function render_dashboard_widget(): void {
		...
		if ( empty( $suggestions ) ) {
			echo '<p><em>' . esc_html__( 'No suggestions at this time. Your site is running smoothly!', 'wpshadow' ) . '</em></p>';
			return;
		}
				...
				<div style="flex: 1;">
					...
					<?php if ( ! empty( $suggestion['action_url'] ) ) : ?>
						<a href="<?php echo esc_url( $suggestion['action_url'] ); ?>" class="button button-small button-primary">
							<?php echo esc_html( $suggestion['action_text'] ?? __( 'Take Action', 'wpshadow' ) ); ?>
						</a>
					<?php endif; ?>
					...
					<?php if ( ! empty( $suggestion['evidence'] ) ) : ?>
						<p style="margin: 10px 0 0 0; font-size: 12px; color: #999;">
							<strong><?php esc_html_e( 'Why?', 'wpshadow' ); ?></strong> <?php echo esc_html( $suggestion['evidence'] ); ?>
						</p>
					<?php endif; ?>
				</div>
			</div>
		...
		if ( $recent_count >= 40 ) {
			...
			if ( ! $cache_enabled ) {
				$suggestions[] = array(
					...
					'title'       => __( 'Enable Page Caching', 'wpshadow' ),
					'description' => __( 'Your site is processing many page requests. Enabling page caching can significantly improve performance and reduce server load.', 'wpshadow' ),
					'action_text' => __( 'Enable Page Cache', 'wpshadow' ),
					'action_url'  => admin_url( 'admin.php?page=wpshadow-features&feature=page-cache' ),
					'evidence'    => sprintf(
						/* translators: %d: number of cleanup events */
						__( 'Head cleanup ran %d times in the last 24 hours, indicating high page request volume.', 'wpshadow' ),
						$recent_count
					),
					'priority'    => 8,
					'created_at'  => time(),
			...
			if ( $total > 50 ) {
				...
				if ( $hit_rate < 50 ) {
					$suggestions[] = array(
						...
						'title'       => __( 'Optimize Cache Settings', 'wpshadow' ),
						'description' => __( 'Your cache hit rate is low. Consider increasing cache expiration time or excluding dynamic pages from caching.', 'wpshadow' ),
						'action_text' => __( 'Configure Cache', 'wpshadow' ),
						'action_url'  => admin_url( 'admin.php?page=wpshadow-features&feature=page-cache' ),
						'evidence'    => sprintf(
							/* translators: %s: cache hit rate percentage */
							__( 'Cache hit rate is only %s%%, which means most requests are not being served from cache.', 'wpshadow' ),
		...
		if ( $failed_logins >= 10 ) {
			...
			if ( ! $brute_force_enabled ) {
				$suggestions[] = array(
					...
					'title'       => __( 'Enable Brute Force Protection', 'wpshadow' ),
					'description' => __( 'Multiple failed login attempts detected. Enable brute force protection to block malicious login attempts.', 'wpshadow' ),
					'action_text' => __( 'Enable Protection', 'wpshadow' ),
					...
					'evidence'    => sprintf(
						...
						__( '%d failed login attempts detected in recent activity.', 'wpshadow' ),
						$failed_logins
					),
					'priority'    => 9,
					'created_at'  => time(),
		...
		if ( ! $optimizer_enabled ) {
			...
			if ( $large_images > 10 ) {
				$suggestions[] = array(
					...
					'type'        => self::TYPE_OPTIMIZATION,
					'title'       => __( 'Optimize Your Images', 'wpshadow' ),
					'description' => __( 'Large unoptimized images can slow down your site. Enable image optimization to reduce file sizes without losing quality.', 'wpshadow' ),
					'action_text' => __( 'Enable Image Optimizer', 'wpshadow' ),
					'action_url'  => admin_url( 'admin.php?page=wpshadow-features&feature=image-optimizer' ),
					'evidence'    => sprintf(
						/* translators: %d: number of large images */
						__( 'Found %d images larger than 500KB in your media library.', 'wpshadow' ),
						$large_images
					),
					'priority'    => 6,
					'created_at'  => time(),
		...
		if ( $revision_count > 100 || $trash_count > 50 ) {
			...
			if ( ! $cleanup_enabled ) {
				$suggestions[] = array(
					...
					'title'       => __( 'Clean Up Your Database', 'wpshadow' ),
					'description' => __( 'Your database contains many old revisions and trashed items. Regular cleanup can improve performance.', 'wpshadow' ),
					'action_text' => __( 'Enable Database Cleanup', 'wpshadow' ),
					'action_url'  => admin_url( 'admin.php?page=wpshadow-features&feature=database-cleanup' ),
					'evidence'    => sprintf(
						/* translators: 1: revision count, 2: trash count */
						__( 'Found %1$d old revisions and %2$d trashed items in your database.', 'wpshadow' ),
						$revision_count,
						$trash_count
					),
					'priority'    => 5,
	...
	public static function ajax_dismiss_suggestion(): void {
		...
		if ( empty( $suggestion_id ) ) {
			wp_send_json_error( array( 'message' => __( 'That suggestion doesn\'t exist', 'wpshadow' ) ) );
		}
		...
		wp_send_json_success( array( 'message' => __( 'Suggestion dismissed', 'wpshadow' ) ) );
	}
					...
					success: function() {
						$button.closest('.wpshadow-suggestion').fadeOut(300, function() {
							...
							if ($('.wpshadow-suggestion').length === 0) {
								$('.wpshadow-suggestions-list').html('<p><em><?php echo esc_js( __( 'No suggestions at this time.', 'wpshadow' ) ); ?></em></p>');
							}
						});
					}
				});
