...
class WPSHADOW_SOS_Support {
	...
	public static function register_dashboard_widget(): void {
		...
		wp_add_dashboard_widget(
			...
			sprintf( __( '🚨 Emergency SOS: %d Pending', 'wpshadow' ), $pending ),
	...
	public static function render_dashboard_widget(): void {
		...
		if ( empty( $incidents ) ) {
			echo '<p>' . esc_html__( 'No pending incidents.', 'wpshadow' ) . '</p>';
			return;
		}

		echo '<div style="margin-bottom: 10px;">';
		echo '<strong>' . esc_html( sprintf( __( '%d urgent incident(s) need attention:', 'wpshadow' ), count( $incidents ) ) ) . '</strong>';
		echo '</div>';
		...
		foreach ( $incidents as $incident ) {
			...
			echo '<p style="margin: 0; font-size: 13px;"><strong>#' . esc_html( $incident['id'] ?? '' ) . '</strong> - ' . esc_html( $incident['subject'] ?? __( 'No subject', 'wpshadow' ) ) . '</p>';
			echo '<p style="margin: 5px 0 0 0; font-size: 11px; color: #666;">';
			echo esc_html( sprintf( __( 'Severity: %1$s | %2$s ago', 'wpshadow' ), ucfirst( $incident['severity'] ?? 'high' ), $time_ago ) );
			echo '</p>';
			echo '</div>';
		}
		...
		echo esc_html__( 'View All Incidents', 'wpshadow' );
		echo '</a>';
		echo '</div>';
	}
	...
	private static function render_incident_details( string $incident_id ): void {
		...
		if ( ! $incident ) {
			wp_die( esc_html__( 'Incident not found.', 'wpshadow' ) );
		}
	...
	public static function handle_sos_submission(): void {
		...
		if ( empty( $_POST['wpshadow_sos_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['wpshadow_sos_nonce'] ) ), 'wpshadow_sos_submit' ) ) {
			wp_die( esc_html__( 'Your session expired. Please refresh and try again.', 'wpshadow' ) );
		}
		...
		if ( empty( $data['email'] ) || empty( $data['subject'] ) || empty( $data['description'] ) ) {
			wp_die( esc_html__( 'Please fill in all required fields.', 'wpshadow' ) );
		}
		...
		if ( ! $incident_id ) {
			wp_die( esc_html__( 'Couldn\'t create your support request. Let\'s try again.', 'wpshadow' ) );
		}
	...
	private static function auto_triage( array $data ): array {
		...
		if ( preg_match( '/\b(down|offline|not loading|white screen|500 error|fatal error)\b/i', $combined ) ) {
			...
			$immediate_action = array(
				__( 'Check if site loads in incognito/private browsing', 'wpshadow' ),
				__( 'Disable plugins via FTP/cPanel', 'wpshadow' ),
				__( 'Check server error logs', 'wpshadow' ),
			);
		...
		if ( preg_match( '/\b(database|mysql|connection|error establishing)\b/i', $combined ) ) {
			...
			$immediate_action = array(
				__( 'Check database credentials in wp-config.php', 'wpshadow' ),
				__( 'Verify database server is running', 'wpshadow' ),
				__( 'Repair database tables', 'wpshadow' ),
			);
		...
		if ( preg_match( '/\b(hacked|breach|malware|virus|injected|compromised)\b/i', $combined ) ) {
			...
			$immediate_action = array(
				__( 'Change all passwords immediately', 'wpshadow' ),
				__( 'Scan for malware', 'wpshadow' ),
				__( 'Check for unauthorized admin users', 'wpshadow' ),
			);
		...
		if ( preg_match( '/\b(slow|performance|timeout|memory|cpu)\b/i', $combined ) ) {
			...
			$immediate_action = array(
				__( 'Enable caching plugin', 'wpshadow' ),
				__( 'Disable resource-heavy plugins', 'wpshadow' ),
				__( 'Contact hosting provider about limits', 'wpshadow' ),
			);
			$estimated_time   = '2-4 hours';
		}
		...
		if ( preg_match( '/\b(plugin|theme|conflict|compatibility)\b/i', $combined ) ) {
			...
			$immediate_action = array(
				__( 'Deactivate recently installed plugins', 'wpshadow' ),
				__( 'Switch to default theme temporarily', 'wpshadow' ),
				__( 'Enable WordPress debug mode', 'wpshadow' ),
			);
			$estimated_time   = '1-2 hours';
		}
	...
	private static function send_user_confirmation( array $incident ): void {
		$to      = $incident['email'] ?? '';
		$subject = sprintf( __( 'We Got Your Support Request #%s - Help Is On The Way', 'wpshadow' ), $incident['id'] ?? '' );
	...
	private static function send_admin_notification( array $incident ): void {
		...
		$subject = sprintf( __( '🚨 New Emergency SOS: #%1$s (%2$s)', 'wpshadow' ), $incident['id'] ?? '', ucfirst( $incident['severity'] ?? 'high' ) );
	...
	public static function add_incident_note( string $incident_id, string $note, int $user_id = 0 ): bool {
		...
		$incidents[ $incident_id ]['notes'][] = array(
			'note'      => $note,
			'user'      => $user ? $user->display_name : __( 'System', 'wpshadow' ),
			'timestamp' => time(),
		);
	...
	public static function ajax_update_incident_status(): void {
		...
		if ( empty( $incident_id ) || empty( $status ) ) {
			wp_send_json_error( array( 'message' => __( 'Something went wrong with your request', 'wpshadow' ) ) );
		}

		$updated = self::update_incident_status( $incident_id, $status );

		if ( $updated ) {
			wp_send_json_success( array( 'message' => __( 'Status updated', 'wpshadow' ) ) );
		} else {
			wp_send_json_error( array( 'message' => __( 'Couldn\'t update the status', 'wpshadow' ) ) );
		}
	}
	...
	public static function ajax_add_incident_note(): void {
		...
		if ( $added ) {
			wp_send_json_success( array( 'message' => __( 'Note added', 'wpshadow' ) ) );
		} else {
			wp_send_json_error( array( 'message' => __( 'Couldn\'t add that note', 'wpshadow' ) ) );
		}
	}
