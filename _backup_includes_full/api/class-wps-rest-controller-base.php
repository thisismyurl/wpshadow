...
abstract class WPSHADOW_REST_Controller_Base extends WP_REST_Controller {
	...
	protected function check_permission( string $capability ) {
		if ( ! current_user_can( $capability ) ) {
			return new WP_Error(
				'rest_forbidden',
				__( 'You do not have permission to access this resource.', 'wpshadow' ),
				array( 'status' => 403 )
			);
		}
	...
	protected function validate_slug( string $slug ) {
		...
		if ( empty( $sanitized ) ) {
			return $this->error_response(
				'invalid_slug',
				__( 'That slug isn\'t valid.', 'wpshadow' ),
				400
			);
		}
		...
		if ( $count >= $limit ) {
			return $this->error_response(
				...
				sprintf(
					...
					__( 'Rate limit exceeded for %1$s. Maximum %2$d requests per %3$d minutes.', 'wpshadow' ),
					$operation,
					$limit,
					$period / 60
				),
