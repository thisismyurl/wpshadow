...
class WPSHADOW_Server_Limits {
	...
	private static function get_recommendations( array $status ): array {
		...
		if ( 'critical' === $status['memory']['level'] ) {
			$recommendations[] = __( 'Memory usage is critical. Consider increasing PHP memory_limit.', 'wpshadow' );
		} elseif ( 'warning' === $status['memory']['level'] ) {
			$recommendations[] = __( 'Memory usage is high. Operations will be batched automatically.', 'wpshadow' );
		}

		if ( 'critical' === $status['time']['level'] ) {
			$recommendations[] = __( 'Execution time is critical. Consider increasing max_execution_time.', 'wpshadow' );
		} elseif ( 'warning' === $status['time']['level'] ) {
			$recommendations[] = __( 'Execution time is high. Long operations will be batched.', 'wpshadow' );
		}

		if ( empty( $recommendations ) ) {
			$recommendations[] = __( 'Resource usage is optimal.', 'wpshadow' );
		}
