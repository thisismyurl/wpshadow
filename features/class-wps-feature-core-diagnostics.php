...
class WPSHADOW_Feature_Core_Diagnostics extends WPSHADOW_Abstract_Feature {
	...
	public function __construct() {
		parent::__construct(
			array(
				...
				'priority'           => 30,
				'sub_features'       => array(
					'core_updates'      => __( 'WordPress Core Update Checks', 'plugin-wpshadow' ),
					'php_version'       => __( 'PHP Version Compatibility', 'plugin-wpshadow' ),
					'database_health'   => __( 'Database Health Monitoring', 'plugin-wpshadow' ),
					'file_permissions'  => __( 'File Permission Verification', 'plugin-wpshadow' ),
					'security_headers'  => __( 'Security Headers Check', 'plugin-wpshadow' ),
					'debug_mode'        => __( 'Debug Mode Detection', 'plugin-wpshadow' ),
					'error_log'         => __( 'Error Log Monitoring', 'plugin-wpshadow' ),
					'ssl_check'         => __( 'SSL/HTTPS Configuration', 'plugin-wpshadow' ),
				),
				'settings'           => array(
					'check_interval'         => array(
						'type'        => 'select',
						'label'       => __( 'Check Interval', 'plugin-wpshadow' ),
						'description' => __( 'How often to run diagnostic checks', 'plugin-wpshadow' ),
						'default'     => 'daily',
						'options'     => array(
							'hourly'     => __( 'Hourly', 'plugin-wpshadow' ),
							'twicedaily' => __( 'Twice Daily', 'plugin-wpshadow' ),
							'daily'      => __( 'Daily', 'plugin-wpshadow' ),
							'weekly'     => __( 'Weekly', 'plugin-wpshadow' ),
						),
					),
					'autoload_threshold_mb'  => array(
						'type'        => 'number',
						'label'       => __( 'Autoload Data Threshold (MB)', 'plugin-wpshadow' ),
						'description' => __( 'Warn when autoloaded options exceed this size', 'plugin-wpshadow' ),
						'default'     => 1,
						'min'         => 0.5,
						'max'         => 10,
						'step'        => 0.5,
					...
					'log_size_threshold_mb'  => array(
						'type'        => 'number',
						'label'       => __( 'Error Log Size Threshold (MB)', 'plugin-wpshadow' ),
						'description' => __( 'Warn when error log exceeds this size', 'plugin-wpshadow' ),
						'default'     => 10,
						'min'         => 1,
						'max'         => 100,
						'step'        => 1,
					...
					'transient_cleanup_count' => array(
						'type'        => 'number',
						'label'       => __( 'Expired Transients Threshold', 'plugin-wpshadow' ),
						'description' => __( 'Warn when expired transients exceed this count', 'plugin-wpshadow' ),
						'default'     => 100,
						'min'         => 10,
						'max'         => 1000,
						'step'        => 10,
					...
					'show_admin_notices'     => array(
						'type'        => 'checkbox',
						'label'       => __( 'Show Admin Notices', 'plugin-wpshadow' ),
						'description' => __( 'Display admin notices for critical issues', 'plugin-wpshadow' ),
						'default'     => true,
					),
					'log_all_checks'         => array(
						'type'        => 'checkbox',
						'label'       => __( 'Log All Checks', 'plugin-wpshadow' ),
						'description' => __( 'Log passing checks in addition to warnings and errors', 'plugin-wpshadow' ),
						'default'     => false,
					),
				),
			)
	...
	private function check_core_updates(): array {
		...
		if ( ! is_array( $updates ) || empty( $updates ) ) {
			return array( 'status' => 'pass', 'message' => __( 'WordPress core is up to date.', 'plugin-wpshadow' ) );
		}
		$update = reset( $updates );
		if ( 'upgrade' === $update->response ) {
			return array(
				'status'   => 'critical',
				'message'  => sprintf( __( 'WordPress core update available: %s', 'plugin-wpshadow' ), $update->version ),
				'severity' => 'high',
			);
		}
		return array( 'status' => 'pass', 'message' => __( 'WordPress core is up to date.', 'plugin-wpshadow' ) );
	...
	private function check_php_version(): array {
		...
		if ( version_compare( $current_version, $minimum_version, '<' ) ) {
			return array(
				'status'   => 'critical',
				'message'  => sprintf( __( 'PHP version %1$s is outdated. Minimum required: %2$s', 'plugin-wpshadow' ), $current_version, $minimum_version ),
				'severity' => 'critical',
			);
		}
		if ( version_compare( $current_version, $recommended_version, '<' ) ) {
			return array(
				'status'   => 'warning',
				'message'  => sprintf( __( 'PHP version %1$s works but %2$s+ is recommended for better performance and security.', 'plugin-wpshadow' ), $current_version, $recommended_version ),
				'severity' => 'medium',
			);
		}
		return array( 'status' => 'pass', 'message' => sprintf( __( 'PHP version %s is up to date.', 'plugin-wpshadow' ), $current_version ) );
	}
	...
	private function check_database_health(): array {
		...
		if ( ! $wpdb->check_connection( false ) ) {
			return array( 'status' => 'critical', 'message' => __( 'Database connection failed.', 'plugin-wpshadow' ), 'severity' => 'critical' );
		}
		...
		if ( $autoload_size > $autoload_threshold ) {
			$issues[] = sprintf( __( 'Large autoloaded data: %1$s MB (threshold: %2$s MB)', 'plugin-wpshadow' ), round( $autoload_size / 1048576, 2 ), $this->get_setting( 'autoload_threshold_mb', 1, false ) );
		}
		...
		if ( $expired_transients > $transient_threshold ) {
			$issues[] = sprintf( __( '%1$d expired transients should be cleaned up (threshold: %2$d).', 'plugin-wpshadow' ), $expired_transients, $transient_threshold );
		}
		if ( ! empty( $issues ) ) {
			return array( 'status' => 'warning', 'message' => implode( ' ', $issues ), 'severity' => 'medium' );
		}
		return array( 'status' => 'pass', 'message' => __( 'Database is healthy.', 'plugin-wpshadow' ) );
	}
	...
	private function check_file_permissions(): array {
		...
		if ( file_exists( $wp_config ) ) {
			...
			if ( $perms & 0020 || $perms & 0002 ) {
				$issues[] = __( 'wp-config.php has insecure permissions (should be 400 or 440).', 'plugin-wpshadow' );
			}
		}
		if ( ! is_writable( WP_CONTENT_DIR ) ) {
			$issues[] = __( 'wp-content directory is not writable.', 'plugin-wpshadow' );
		}
		if ( ! empty( $issues ) ) {
			return array( 'status' => 'warning', 'message' => implode( ' ', $issues ), 'severity' => 'medium' );
		}
		return array( 'status' => 'pass', 'message' => __( 'File permissions are correctly configured.', 'plugin-wpshadow' ) );
	}
	...
	private function check_security_headers(): array {
		...
		if ( is_wp_error( $response ) ) {
			return array( 'status' => 'warning', 'message' => __( 'Unable to check security headers.', 'plugin-wpshadow' ) );
		}
		$headers = wp_remote_retrieve_headers( $response );
		$issues = array();
		if ( empty( $headers['x-frame-options'] ) ) {
			$issues[] = __( 'X-Frame-Options header missing (clickjacking protection).', 'plugin-wpshadow' );
		}
		if ( empty( $headers['x-content-type-options'] ) ) {
			$issues[] = __( 'X-Content-Type-Options header missing (MIME sniffing protection).', 'plugin-wpshadow' );
		}
		if ( ! empty( $issues ) ) {
			return array( 'status' => 'warning', 'message' => implode( ' ', $issues ), 'severity' => 'low' );
		}
		return array( 'status' => 'pass', 'message' => __( 'Security headers are properly configured.', 'plugin-wpshadow' ) );
	}
	...
	private function check_debug_mode(): array {
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			$message = __( 'WP_DEBUG is enabled.', 'plugin-wpshadow' );
			if ( defined( 'WP_DEBUG_DISPLAY' ) && WP_DEBUG_DISPLAY ) {
				$message .= ' ' . __( 'Errors are being displayed publicly (security risk).', 'plugin-wpshadow' );
				return array( 'status' => 'critical', 'message' => $message, 'severity' => 'high' );
			}
			return array( 'status' => 'warning', 'message' => $message . ' ' . __( 'Consider disabling in production.', 'plugin-wpshadow' ), 'severity' => 'medium' );
		}
		return array( 'status' => 'pass', 'message' => __( 'Debug mode is not enabled.', 'plugin-wpshadow' ) );
	}
	...
	private function check_error_log(): array {
		...
		if ( ! file_exists( $log_file ) || ! is_readable( $log_file ) ) {
			return array( 'status' => 'info', 'message' => __( 'No error log file found or not readable.', 'plugin-wpshadow' ) );
		}
		$log_size = filesize( $log_file );
		$log_threshold = $this->get_setting( 'log_size_threshold_mb', 10, false ) * 1048576;
		if ( $log_size > $log_threshold ) {
			return array( 'status' => 'warning', 'message' => sprintf( __( 'Error log is large (%1$s MB) and should be reviewed/cleared (threshold: %2$s MB).', 'plugin-wpshadow' ), round( $log_size / 1048576, 2 ), $this->get_setting( 'log_size_threshold_mb', 10, false ) ), 'severity' => 'medium' );
		}
		$handle = fopen( $log_file, 'r' );
		if ( false === $handle ) {
			return array( 'status' => 'info', 'message' => __( 'Unable to read error log.', 'plugin-wpshadow' ) );
		}
		...
		if ( $fatal_count > 0 ) {
			return array( 'status' => 'critical', 'message' => sprintf( __( '%d fatal error(s) detected in recent logs.', 'plugin-wpshadow' ), $fatal_count ), 'severity' => 'high' );
		}
		return array( 'status' => 'pass', 'message' => __( 'No critical errors in recent logs.', 'plugin-wpshadow' ) );
	}
	...
	private function check_ssl_configuration(): array {
		...
		if ( ! is_ssl() && ( strpos( $home_url, 'https://' ) === 0 || strpos( $site_url, 'https://' ) === 0 ) ) {
			return array( 'status' => 'critical', 'message' => __( 'Site URLs are configured for HTTPS but SSL is not detected.', 'plugin-wpshadow' ), 'severity' => 'high' );
		}
		if ( ! defined( 'FORCE_SSL_ADMIN' ) ) {
			return array( 'status' => 'warning', 'message' => __( 'FORCE_SSL_ADMIN is not defined. Consider forcing SSL for admin area.', 'plugin-wpshadow' ), 'severity' => 'low' );
		}
		if ( is_ssl() ) {
			return array( 'status' => 'pass', 'message' => __( 'SSL/HTTPS is properly configured.', 'plugin-wpshadow' ) );
		}
		return array( 'status' => 'info', 'message' => __( 'Site is not using HTTPS. Consider enabling SSL for security.', 'plugin-wpshadow' ), 'severity' => 'low' );
	}
	...
	public function display_diagnostic_notices(): void {
		...
		if ( ! empty( $critical_issues ) ) {
			...
			echo '<p><strong>' . esc_html__( 'WPShadow: Critical Issues Detected', 'plugin-wpshadow' ) . '</strong></p>';
	...
	public function ajax_run_diagnostics(): void {
		...
		wp_send_json_success( array(
			'results' => $results,
			'message' => __( 'Diagnostics completed successfully.', 'plugin-wpshadow' ),
		) );
	}
