<?php declare(strict_types=1);
/**
 * Core Integrity Feature
 *
 * Verifies WordPress core files against official checksums from WordPress.org API,
 * detects unauthorized modifications, and provides automatic repair capabilities.
 *
 * @package    WPShadow
 * @subpackage Features
 * @since      1.0.0
 */

namespace WPShadow\CoreSupport;

if ( ! defined( 'ABSPATH' ) ) {
	die( 'Direct access not permitted.' );
}

/**
 * Core Integrity Feature Class
 *
 * Scans WordPress core files and compares them against official checksums from
 * WordPress.org to detect tampering, malware, or accidental modifications.
 *
 * @since 1.0.0
 */
final class WPSHADOW_Feature_Core_Integrity extends WPSHADOW_Abstract_Feature {

	const CHECKSUMS_API = 'https://api.wordpress.org/core/checksums/1.0/';

	/**
	 * Feature constructor.
	 *
	 * @since 1.0.0
	 */
	public function __construct() {
		parent::__construct(
			array(
				'id'                 => 'core-integrity',
				'name'               => __( 'Core File Integrity Verification', 'wpshadow' ),
				'description_short'  => __( 'Verify WordPress core files against official checksums', 'wpshadow' ),
				'description_long'   => __( 'Comprehensive WordPress core file integrity verification system that compares every core file against official checksums from WordPress.org. Detects modified, missing, or unknown files in wp-admin and wp-includes directories. Identifies potential security compromises, malware injections, or accidental modifications. Includes automatic repair capabilities that download and restore official versions of modified files while creating safety backups. Scheduled scanning, email alerts, and Site Health integration provide ongoing protection and monitoring.', 'wpshadow' ),
				'description_wizard' => __( 'Enable WordPress core file verification to detect unauthorized modifications, potential malware, or corrupted files. Automatically scans and compares your installation against official WordPress.org checksums.', 'wpshadow' ),
				'aliases'            => array( 'checksums', 'file verification', 'core files', 'malware detection', 'file integrity', 'security scan' ),
				'sub_features'       => array(
					'enable_auto_repair'    => array(
						'name'               => __( 'Automatic Repair', 'wpshadow' ),
						'description_short'  => __( 'Auto-repair modified core files', 'wpshadow' ),
						'description_long'   => __( 'Automatically downloads and repairs modified WordPress core files when integrity violations are detected. Creates timestamped backups of existing files before replacement, downloads fresh copies from WordPress.org SVN repository, verifies checksums after repair, and maintains rollback capability for 7 days. Includes safety checks to prevent filesystem errors and detailed logging of all repair actions. Only repairs files in wp-admin and wp-includes directories - never touches wp-content, plugins, themes, or user uploads.', 'wpshadow' ),
						'description_wizard' => __( 'Automatically repair any modified core files by downloading fresh copies from WordPress.org. Creates backups before making changes.', 'wpshadow' ),
						'default_enabled'    => false,
					),
					'email_alerts'          => array(
						'name'               => __( 'Email Alerts', 'wpshadow' ),
						'description_short'  => __( 'Send email when issues detected', 'wpshadow' ),
						'description_long'   => __( 'Sends detailed email notifications to site administrators when core file integrity issues are detected. Emails include summary of modified files, list of unknown files found, timestamps of detection, severity levels, and direct links to the repair interface. Configurable alert frequency prevents email flooding while ensuring timely notification of critical security issues. Email templates are customizable and support HTML formatting for better readability.', 'wpshadow' ),
						'description_wizard' => __( 'Get email notifications when modified or suspicious core files are detected so you can take immediate action.', 'wpshadow' ),
						'default_enabled'    => false,
					),
					'exclude_non_critical'  => array(
						'name'               => __( 'Exclude Non-Critical Files', 'wpshadow' ),
						'description_short'  => __( 'Skip non-critical file checks', 'wpshadow' ),
						'description_long'   => __( 'Excludes non-critical files from integrity scanning to reduce false positives and focus on security-sensitive files. Skips readme files, license.txt, sample configuration files, and other documentation that may be legitimately modified or removed. Maintains focus on PHP, JavaScript, and CSS files that could pose security risks if tampered with. Includes configurable exclusion patterns and ability to whitelist specific files or directories. Improves scan performance by reducing the number of files checked.', 'wpshadow' ),
						'description_wizard' => __( 'Skip non-critical files like readme.txt and sample configurations to reduce false positive alerts and focus on security-critical files.', 'wpshadow' ),
						'default_enabled'    => true,
					),
				),
			)
		);
	}

	/**
	 * Check if feature has details page.
	 *
	 * @since 1.0.0
	 * @return bool
	 */
	public function has_details_page(): bool {
		return true;
	}

	/**
	 * Register feature hooks.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function register(): void {
		// Schedule integrity scans
		if ( ! wp_next_scheduled( 'wpshadow_core_integrity_scan' ) ) {
			wp_schedule_event( time(), 'daily', 'wpshadow_core_integrity_scan' );
		}

		add_action( 'wpshadow_core_integrity_scan', array( $this, 'run_scheduled_scan' ) );

		// AJAX handlers
		add_action( 'wp_ajax_wpshadow_scan_core_files', array( $this, 'ajax_scan_core_files' ) );
		add_action( 'wp_ajax_wpshadow_repair_core_file', array( $this, 'ajax_repair_core_file' ) );
		add_action( 'wp_ajax_wpshadow_repair_all_core_files', array( $this, 'ajax_repair_all_core_files' ) );

		// Site Health integration
		add_filter( 'site_status_tests', array( $this, 'add_site_health_test' ) );

		// Admin notices
		add_action( 'admin_notices', array( $this, 'file_integrity_notices' ) );
	}

	/**
	 * Run integrity scan on all core files.
	 *
	 * @since 1.0.0
	 * @return array Scan results with modified, missing, and unknown files.
	 */
	public function run_integrity_scan(): array {
		global $wp_version;

		$version = $wp_version;
		$locale  = get_locale();

		// Fetch checksums from API.
		$checksums = $this->fetch_checksums( $version, $locale );

		if ( is_wp_error( $checksums ) ) {
			return array(
				'error'          => $checksums->get_error_message(),
				'modified_files' => array(),
				'missing_files'  => array(),
				'unknown_files'  => array(),
				'scan_time'      => time(),
			);
		}

		$modified_files = array();
		$missing_files  = array();

		// Check each file against its checksum.
		foreach ( $checksums as $relative_path => $expected_checksum ) {
			$absolute_path = ABSPATH . $relative_path;

			if ( ! file_exists( $absolute_path ) ) {
				$missing_files[] = $relative_path;
				continue;
			}

			$actual_checksum = $this->calculate_checksum( $absolute_path );

			if ( $actual_checksum && $actual_checksum !== $expected_checksum ) {
				$modified_files[] = array(
					'path'              => $relative_path,
					'expected_checksum' => $expected_checksum,
					'actual_checksum'   => $actual_checksum,
				);
			}
		}

		// Detect unknown files in core directories.
		$unknown_files = $this->detect_unknown_files();

		$results = array(
			'modified_files' => $modified_files,
			'missing_files'  => $missing_files,
			'unknown_files'  => $unknown_files,
			'scan_time'      => time(),
			'version'        => $version,
			'locale'         => $locale,
		);

		// Store results.
		set_transient( 'wpshadow_core_integrity_results', $results, DAY_IN_SECONDS );

		// Set flag if issues found.
		if ( ! empty( $modified_files ) || ! empty( $unknown_files ) ) {
			set_transient( 'wpshadow_core_integrity_issues', true, WEEK_IN_SECONDS );
		} else {
			delete_transient( 'wpshadow_core_integrity_issues' );
		}

		return $results;
	}

	/**
	 * Fetch checksums from WordPress.org API.
	 *
	 * @since 1.0.0
	 * @param string $version WordPress version.
	 * @param string $locale  Locale (default: en_US).
	 * @return array|\WP_Error Array of checksums or WP_Error on failure.
	 */
	private function fetch_checksums( string $version, string $locale = 'en_US' ): array|\WP_Error {
		$cache_key = 'wpshadow_checksums_' . $version . '_' . $locale;

		// Check cache first.
		$cached = get_transient( $cache_key );
		if ( false !== $cached && is_array( $cached ) ) {
			return $cached;
		}

		// Build API URL.
		$url = add_query_arg(
			array(
				'version' => $version,
				'locale'  => $locale,
			),
			self::CHECKSUMS_API
		);

		// Make API request.
		$response = wp_remote_get(
			$url,
			array(
				'timeout' => 30,
			)
		);

		if ( is_wp_error( $response ) ) {
			return $response;
		}

		$response_code = wp_remote_retrieve_response_code( $response );
		if ( 200 !== $response_code ) {
			return new \WP_Error(
				'api_error',
				sprintf(
					/* translators: %d: HTTP response code */
					__( 'Couldn\'t get file signatures from WordPress (HTTP %d)', 'wpshadow' ),
					$response_code
				)
			);
		}

		$body = wp_remote_retrieve_body( $response );
		$data = json_decode( $body, true );

		if ( ! is_array( $data ) || ! isset( $data['checksums'] ) ) {
			return new \WP_Error(
				'parse_error',
				__( 'Couldn\'t read the signatures', 'wpshadow' )
			);
		}

		$checksums = $data['checksums'];

		// Cache checksums for 24 hours.
		set_transient( $cache_key, $checksums, DAY_IN_SECONDS );

		return $checksums;
	}

	/**
	 * Calculate MD5 checksum for a file.
	 *
	 * @since 1.0.0
	 * @param string $file_path Absolute path to file.
	 * @return string|false MD5 checksum or false on failure.
	 */
	private function calculate_checksum( string $file_path ): string|false {
		if ( ! file_exists( $file_path ) || ! is_readable( $file_path ) ) {
			return false;
		}

		return md5_file( $file_path );
	}

	/**
	 * Verify a single file against official checksum.
	 *
	 * @since 1.0.0
	 * @param string $relative_path     Path relative to ABSPATH.
	 * @param string $expected_checksum Expected MD5 checksum.
	 * @return bool True if file matches, false otherwise.
	 */
	private function verify_file( string $relative_path, string $expected_checksum ): bool {
		$absolute_path = ABSPATH . $relative_path;

		if ( ! file_exists( $absolute_path ) ) {
			return false;
		}

		$actual_checksum = $this->calculate_checksum( $absolute_path );

		return $actual_checksum === $expected_checksum;
	}

	/**
	 * Repair a modified core file.
	 *
	 * @since 1.0.0
	 * @param string $relative_path Path relative to ABSPATH.
	 * @return bool|\WP_Error True on success, WP_Error on failure.
	 */
	private function repair_file( string $relative_path ): bool|\WP_Error {
		global $wp_version;

		$absolute_path = ABSPATH . $relative_path;

		if ( ! file_exists( $absolute_path ) ) {
			return new \WP_Error( 'file_not_found', __( 'File not found', 'wpshadow' ) );
		}

		// Backup current file.
		$backup_path = $absolute_path . '.wps-backup-' . time();
		if ( ! copy( $absolute_path, $backup_path ) ) {
			return new \WP_Error( 'backup_failed', __( 'Couldn\'t create a backup before repairing', 'wpshadow' ) );
		}

		// Download fresh copy from WordPress.org.
		$download_url = 'https://core.svn.wordpress.org/tags/' . $wp_version . '/' . $relative_path;
		$response     = wp_remote_get(
			$download_url,
			array(
				'timeout' => 30,
			)
		);

		if ( is_wp_error( $response ) ) {
			// Restore backup.
			copy( $backup_path, $absolute_path );
			wp_delete_file( $backup_path );
			return $response;
		}

		$response_code = wp_remote_retrieve_response_code( $response );
		if ( 200 !== $response_code ) {
			// Restore backup.
			copy( $backup_path, $absolute_path );
			wp_delete_file( $backup_path );
			return new \WP_Error(
				'download_failed',
				sprintf(
					/* translators: %d: HTTP response code */
					__( 'Couldn\'t download the file (HTTP %d)', 'wpshadow' ),
					$response_code
				)
			);
		}

		$file_contents = wp_remote_retrieve_body( $response );

		// Write new file.
		require_once ABSPATH . 'wp-admin/includes/file.php';
		global $wp_filesystem;
		if ( ! WP_Filesystem() ) {
			// Restore backup.
			copy( $backup_path, $absolute_path );
			wp_delete_file( $backup_path );
			return new \WP_Error( 'filesystem_error', __( 'Could not access filesystem', 'wpshadow' ) );
		}

		if ( ! $wp_filesystem->put_contents( $absolute_path, $file_contents, FS_CHMOD_FILE ) ) {
			// Restore backup.
			copy( $backup_path, $absolute_path );
			wp_delete_file( $backup_path );
			return new \WP_Error( 'write_failed', __( 'Couldn\'t save the repaired file', 'wpshadow' ) );
		}

		// Verify repair was successful.
		$checksums = $this->fetch_checksums( $wp_version, get_locale() );
		if ( ! is_wp_error( $checksums ) && isset( $checksums[ $relative_path ] ) ) {
			if ( ! $this->verify_file( $relative_path, $checksums[ $relative_path ] ) ) {
				// Restore backup.
				copy( $backup_path, $absolute_path );
				wp_delete_file( $backup_path );
				return new \WP_Error( 'verification_failed', __( 'Repaired file verification failed', 'wpshadow' ) );
			}
		}

		// Repair successful, delete backup after 7 days.
		wp_schedule_single_event( time() + ( 7 * DAY_IN_SECONDS ), 'wpshadow_delete_backup_file', array( $backup_path ) );

		// Log repair action.
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG && defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
			error_log( 'WPS Core Integrity: Repaired file ' . $relative_path );
		}

		return true;
	}

	/**
	 * Get list of modified core files from last scan.
	 *
	 * @since 1.0.0
	 * @return array Modified files with details.
	 */
	private function get_modified_files(): array {
		$results = get_transient( 'wpshadow_core_integrity_results' );

		if ( false === $results || ! isset( $results['modified_files'] ) ) {
			return array();
		}

		return $results['modified_files'];
	}

	/**
	 * Detect unknown files in core directories.
	 *
	 * @since 1.0.0
	 * @return array Unknown files list.
	 */
	private function detect_unknown_files(): array {
		global $wp_version;

		$unknown_files = array();

		// Get official file list.
		$checksums = $this->fetch_checksums( $wp_version, get_locale() );
		if ( is_wp_error( $checksums ) ) {
			return array();
		}

		$official_files = array_keys( $checksums );

		// Scan ONLY core directories (wp-admin and wp-includes).
		// NEVER scan wp-content, plugins, themes, or uploads.
		$wp_admin_files    = $this->scan_directory( ABSPATH . 'wp-admin', ABSPATH );
		$wp_includes_files = $this->scan_directory( ABSPATH . 'wp-includes', ABSPATH );

		$found_files = array_merge( $wp_admin_files, $wp_includes_files );

		// Filter out any files that somehow got through that are in wp-content.
		$found_files = array_filter(
			$found_files,
			function ( $file ) {
				// Ensure we never flag files in wp-content.
				return false === strpos( $file, 'wp-content' );
			}
		);

		// Find files not in official list.
		foreach ( $found_files as $file ) {
			if ( ! in_array( $file, $official_files, true ) ) {
				$unknown_files[] = $file;
			}
		}

		return $unknown_files;
	}

	/**
	 * Recursively scan a directory for files.
	 *
	 * @since 1.0.0
	 * @param string $dir       Directory path.
	 * @param string $base_path Base path to make paths relative.
	 * @return array List of relative file paths.
	 */
	private function scan_directory( string $dir, string $base_path ): array {
		$files = array();

		if ( ! is_dir( $dir ) ) {
			return $files;
		}

		// Files/directories to exclude from scanning.
		$exclude_patterns = array(
			'.git',
			'.svn',
			'.htaccess',
			'.maintenance',
			'node_modules',
			'.DS_Store',
			'Thumbs.db',
		);

		$iterator = new \RecursiveIteratorIterator(
			new \RecursiveDirectoryIterator( $dir, \RecursiveDirectoryIterator::SKIP_DOTS ),
			\RecursiveIteratorIterator::SELF_FIRST
		);

		foreach ( $iterator as $file ) {
			if ( $file->isFile() ) {
				$absolute_path = $file->getPathname();
				$relative_path = str_replace( $base_path, '', $absolute_path );
				$relative_path = ltrim( $relative_path, '/' );
				
				// Skip excluded patterns.
				$should_exclude = false;
				foreach ( $exclude_patterns as $pattern ) {
					if ( false !== strpos( $relative_path, $pattern ) ) {
						$should_exclude = true;
						break;
					}
				}
				
				if ( $should_exclude ) {
					continue;
				}
				
				// Skip backup files and temporary files.
				if ( preg_match( '/\.(bak|tmp|backup|old|orig|swp|swo)$/i', $relative_path ) ) {
					continue;
				}
				
				// Only include PHP and other core file types.
				$extension          = pathinfo( $relative_path, PATHINFO_EXTENSION );
				$allowed_extensions = array( 'php', 'js', 'css', 'png', 'jpg', 'jpeg', 'gif', 'svg', 'woff', 'woff2', 'ttf', 'eot', 'txt', 'html', 'xml', 'json', 'pot', 'mo' );
				
				if ( ! empty( $extension ) && ! in_array( strtolower( $extension ), $allowed_extensions, true ) ) {
					continue;
				}
				
				$files[] = $relative_path;
			}
		}

		return $files;
	}

	/**
	 * Run scheduled integrity scan.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function run_scheduled_scan(): void {
		$this->run_integrity_scan();
	}

	/**
	 * AJAX handler for scanning core files.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function ajax_scan_core_files(): void {
		check_ajax_referer( 'wpshadow_core_integrity', 'nonce' );

		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => __( 'Insufficient permissions.', 'wpshadow' ) ) );
		}

		$results = $this->run_integrity_scan();

		if ( isset( $results['error'] ) ) {
			wp_send_json_error(
				array(
					'message' => $results['error'],
				)
			);
		}

		wp_send_json_success(
			array(
				'modified_count' => count( $results['modified_files'] ),
				'missing_count'  => count( $results['missing_files'] ),
				'unknown_count'  => count( $results['unknown_files'] ),
				'results'        => $results,
			)
		);
	}

	/**
	 * AJAX handler for repairing a single core file.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function ajax_repair_core_file(): void {
		check_ajax_referer( 'wpshadow_core_integrity', 'nonce' );

		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => __( 'Insufficient permissions.', 'wpshadow' ) ) );
		}

		$file_path = isset( $_POST['file_path'] ) ? sanitize_text_field( wp_unslash( $_POST['file_path'] ) ) : '';

		if ( empty( $file_path ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'That file path doesn\'t work.', 'wpshadow' ),
				)
			);
		}

		$result = $this->repair_file( $file_path );

		if ( is_wp_error( $result ) ) {
			wp_send_json_error(
				array(
					'message' => $result->get_error_message(),
				)
			);
		}

		wp_send_json_success(
			array(
				'message' => __( 'File repaired successfully.', 'wpshadow' ),
			)
		);
	}

	/**
	 * AJAX handler for repairing all modified core files.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function ajax_repair_all_core_files(): void {
		check_ajax_referer( 'wpshadow_core_integrity', 'nonce' );

		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => __( 'Insufficient permissions.', 'wpshadow' ) ) );
		}

		$modified_files = $this->get_modified_files();

		if ( empty( $modified_files ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'No modified files to repair.', 'wpshadow' ),
				)
			);
		}

		$repaired = array();
		$failed   = array();

		foreach ( $modified_files as $file ) {
			$result = $this->repair_file( $file['path'] );

			if ( is_wp_error( $result ) ) {
				$failed[] = array(
					'path'    => $file['path'],
					'message' => $result->get_error_message(),
				);
			} else {
				$repaired[] = $file['path'];
			}
		}

		// Re-run scan to update results.
		$this->run_integrity_scan();

		wp_send_json_success(
			array(
				'repaired_count' => count( $repaired ),
				'failed_count'   => count( $failed ),
				'repaired'       => $repaired,
				'failed'         => $failed,
			)
		);
	}

	/**
	 * Add Site Health test for core integrity.
	 *
	 * @since 1.0.0
	 * @param array $tests Site Health tests.
	 * @return array Modified tests.
	 */
	public function add_site_health_test( array $tests ): array {
		$tests['direct']['wpshadow_core_integrity'] = array(
			'label' => __( 'WordPress core file integrity', 'wpshadow' ),
			'test'  => array( $this, 'site_health_test_callback' ),
		);

		return $tests;
	}

	/**
	 * Site Health test callback.
	 *
	 * @since 1.0.0
	 * @return array Test result.
	 */
	public function site_health_test_callback(): array {
		$results = get_transient( 'wpshadow_core_integrity_results' );

		// If no scan has been run yet, run one now.
		if ( false === $results ) {
			$results = $this->run_integrity_scan();
		}

		if ( isset( $results['error'] ) ) {
			return array(
				'label'       => __( 'Core integrity check failed', 'wpshadow' ),
				'status'      => 'recommended',
				'badge'       => array(
					'label' => __( 'Security', 'wpshadow' ),
					'color' => 'orange',
				),
				'description' => sprintf(
					'<p>%s</p>',
					esc_html( $results['error'] )
				),
				'test'        => 'wpshadow_core_integrity',
			);
		}

		$modified_count = count( $results['modified_files'] ?? array() );
		$unknown_count  = count( $results['unknown_files'] ?? array() );

		if ( $modified_count > 0 || $unknown_count > 0 ) {
			return array(
				'label'       => __( 'Modified core files detected', 'wpshadow' ),
				'status'      => 'critical',
				'badge'       => array(
					'label' => __( 'Security', 'wpshadow' ),
					'color' => 'red',
				),
				'description' => sprintf(
					'<p>%s</p>',
					sprintf(
						/* translators: 1: number of modified files, 2: number of unknown files */
						__( 'Found %1$d modified core files and %2$d unknown files. This could indicate a security compromise.', 'wpshadow' ),
						$modified_count,
						$unknown_count
					)
				),
				'test'        => 'wpshadow_core_integrity',
				'actions'     => sprintf(
					'<a href="%s">%s</a>',
					admin_url( 'tools.php?page=wps-core-integrity' ),
					__( 'View and Repair Files', 'wpshadow' )
				),
			);
		}

		return array(
			'label'       => __( 'Core files are intact', 'wpshadow' ),
			'status'      => 'good',
			'badge'       => array(
				'label' => __( 'Security', 'wpshadow' ),
				'color' => 'blue',
			),
			'description' => sprintf(
				'<p>%s</p>',
				__( 'All WordPress core files match official checksums.', 'wpshadow' )
			),
			'test'        => 'wpshadow_core_integrity',
		);
	}

	/**
	 * Show admin notices for file integrity issues.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function file_integrity_notices(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		}

		// Prevent duplicate rendering if the hook is registered more than once.
		static $rendered = false;
		if ( $rendered ) {
			return;
		}
		$rendered = true;

		if ( ! get_transient( 'wpshadow_core_integrity_issues' ) ) {
			return;
		}

		$results = get_transient( 'wpshadow_core_integrity_results' );

		if ( false === $results ) {
			return;
		}

		$modified_count = count( $results['modified_files'] ?? array() );
		$unknown_count  = count( $results['unknown_files'] ?? array() );

		if ( 0 === $modified_count && 0 === $unknown_count ) {
			return;
		}

		?>
		<div class="notice notice-error is-dismissible">
			<p>
				<strong><?php esc_html_e( 'WPShadow: Core Integrity Issue Detected', 'wpshadow' ); ?></strong>
			</p>
			<p>
				<?php
				printf(
					/* translators: 1: number of modified files, 2: number of unknown files */
					esc_html__( 'Found %1$d modified core files and %2$d unknown files. This could indicate a security issue.', 'wpshadow' ),
					$modified_count,
					$unknown_count
				);
				?>
			</p>
			<p>
				<button type="button" class="button button-primary" id="wps-repair-all-core-files">
					<?php esc_html_e( 'Repair All Modified Files', 'wpshadow' ); ?>
				</button>
				<span class="spinner" style="float: none; margin: 0 0 0 10px;"></span>
			</p>
		</div>
		<script>
		jQuery(document).ready(function($) {
			$('#wps-repair-all-core-files').on('click', function() {
				var $button = $(this);
				var $spinner = $button.next('.spinner');
				
				if (!confirm('<?php esc_html_e( 'Are you sure you want to repair all modified core files? This will overwrite the current files.', 'wpshadow' ); ?>')) {
					return;
				}
				
				$button.prop('disabled', true);
				$spinner.addClass('is-active');
				
				$.post(ajaxurl, {
					action: 'wpshadow_repair_all_core_files',
					_ajax_nonce: '<?php echo esc_js( wp_create_nonce( 'wpshadow_core_integrity' ) ); ?>'
				}, function(response) {
					if (response.success) {
						$button.closest('.notice').fadeOut(function() {
							$(this).remove();
						});
						alert('<?php esc_html_e( 'Files repaired successfully!', 'wpshadow' ); ?>\n' + 
							  '<?php esc_html_e( 'Repaired:', 'wpshadow' ); ?> ' + response.data.repaired_count);
						location.reload();
					} else {
						alert(response.data.message || '<?php esc_html_e( 'Failed to repair files.', 'wpshadow' ); ?>');
						$button.prop('disabled', false);
						$spinner.removeClass('is-active');
					}
				}).fail(function() {
					alert('<?php esc_html_e( 'An error occurred. Please try again.', 'wpshadow' ); ?>');
					$button.prop('disabled', false);
					$spinner.removeClass('is-active');
				});
			});
		});
		</script>
		<?php
	}
}
