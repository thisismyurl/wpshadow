...
final class WPSHADOW_Feature_Dark_Mode extends WPSHADOW_Abstract_Feature {
	...
	public function __construct() {
		parent::__construct(
			array(
				'id'                 => 'dark-mode',
				'name'               => __( 'Dark Mode Support', 'plugin-wpshadow' ),
				'description'        => __( 'Enable dark mode for WPShadow admin pages with automatic WordPress color scheme detection and manual toggle.', 'plugin-wpshadow' ),
				'scope'              => 'core',
	...
	public function render_dashboard_widget(): void {
		...
		<div class="postbox wpshadow-dashboard-widget">
			<div class="postbox-header">
				<h2 class="hndle">🌓 <?php esc_html_e( 'Dark Mode', 'plugin-wpshadow' ); ?></h2>
			</div>
			<div class="inside">
				<p>
					<?php esc_html_e( 'Control the appearance of WPShadow admin pages.', 'plugin-wpshadow' ); ?>
				</p>
				<p>
					<strong><?php esc_html_e( 'Current Mode:', 'plugin-wpshadow' ); ?></strong>
				...
				<p>
					<button type="button" class="button button-primary wpshadow-dark-mode-toggle-btn" data-current-preference="<?php echo esc_attr( $user_preference ); ?>">
						...
						<?php esc_html_e( 'Toggle Mode:', 'plugin-wpshadow' ); ?>
						<strong id="wpshadow-mode-label"><?php echo esc_html( ucfirst( $user_preference ) ); ?></strong>
					</button>
				</p>
				<p class="description">
					<?php esc_html_e( 'Auto mode detects your WordPress color scheme or system preferences.', 'plugin-wpshadow' ); ?>
				</p>
					...
					<p class="description">
						<em>
							...
							if ( in_array( $wp_scheme, $dark_schemes, true ) ) {
								...
								printf( esc_html__( 'Detected WordPress "%s" color scheme → Dark mode', 'plugin-wpshadow' ), esc_html( $wp_scheme ) );
							} else {
								/* translators: %s: WordPress color scheme name */
								printf( esc_html__( 'Detected WordPress "%s" color scheme → Light mode', 'plugin-wpshadow' ), esc_html( $wp_scheme ) );
							}
							?>
						</em>
					</p>
	...
	public function ajax_set_dark_mode(): void {
		...
		if ( ! in_array( $mode, array( 'auto', 'light', 'dark' ), true ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid mode.', 'plugin-wpshadow' ) ) );
		}
		...
		wp_send_json_success( array(
			'mode'    => $mode,
			'message' => __( 'Dark mode preference saved.', 'plugin-wpshadow' ),
		) );
	}
	...
	public function add_admin_bar_toggle( $wp_admin_bar ): void {
		...
		$wp_admin_bar->add_node( array(
			...
			'title'  => $icon . ' ' . __( 'Dark Mode', 'plugin-wpshadow' ),
			'href'   => '#',
			'meta'   => array(
