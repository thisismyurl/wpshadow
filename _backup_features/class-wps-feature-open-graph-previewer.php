	...
	public function __construct() {
		parent::__construct(
			array(
				...
				'priority'           => 15,
				'sub_features'       => array(
					'check_facebook'     => __( 'Check Facebook OG Tags', 'wpshadow' ),
					'check_twitter'      => __( 'Check Twitter Card Tags', 'wpshadow' ),
					'check_linkedin'     => __( 'Check LinkedIn Tags', 'wpshadow' ),
					'check_pinterest'    => __( 'Check Pinterest Tags', 'wpshadow' ),
				),
	...
	private function render_analysis_results( array $analysis, string $url ): void {
		if ( isset( $analysis['error'] ) ) {
			...
			<div class="notice notice-error">
				<p>
					<strong><?php esc_html_e( 'Error:', 'wpshadow' ); ?></strong>
		...
		<div class="wps-og-status" style="background: #fff; border: 1px solid #ccd0d4; border-radius: 4px; padding: 20px; margin-bottom: 20px;">
			<h2 style="margin-top: 0;"><?php esc_html_e( 'Analysis Summary', 'wpshadow' ); ?></h2>
			<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
				<div>
					<h3 style="margin-top: 0;">
						<span class="dashicons dashicons-share" style="color: #3b5998;"></span>
						<?php esc_html_e( 'Open Graph (LinkedIn, Facebook)', 'wpshadow' ); ?>
					</h3>
					<?php if ( $og_complete ) : ?>
						<p style="color: #46b450; font-weight: 600;">
							<span class="dashicons dashicons-yes-alt"></span>
							<?php esc_html_e( 'All required tags found', 'wpshadow' ); ?>
						</p>
					<?php else : ?>
						<p style="color: #dc3232; font-weight: 600;">
							...
							printf(
								...
								esc_html( _n( '%d required tag missing', '%d required tags missing', count( $missing ), 'wpshadow' ) ),
								count( $missing )
							);
							?>
						</p>
				...
				<div>
					<h3 style="margin-top: 0;">
						...
						<?php esc_html_e( 'Twitter Card', 'wpshadow' ); ?>
					</h3>
					...
					<?php else : ?>
						<p style="color: #dc3232; font-weight: 600;">
							...
							printf(
								/* translators: %d: number of missing tags */
								esc_html( _n( '%d tag missing', '%d tags missing', count( $missing_twitter ), 'wpshadow' ) ),
		...
		<?php if ( ! empty( $missing ) ) : ?>
			<div class="notice notice-warning" style="margin-bottom: 20px;">
				<h3><?php esc_html_e( 'Missing Open Graph Tags', 'wpshadow' ); ?></h3>
				<ul>
					<?php foreach ( $missing as $tag ) : ?>
						<li><code><?php echo esc_html( $tag ); ?></code> - <?php echo esc_html( $required_tags[ $tag ] ); ?></li>
					<?php endforeach; ?>
				</ul>
				<p><em><?php esc_html_e( 'These tags are important for how your content appears on LinkedIn, Facebook, and other platforms.', 'wpshadow' ); ?></em></p>
			</div>
		<?php endif; ?>

		<?php if ( ! empty( $missing_twitter ) ) : ?>
			<div class="notice notice-warning" style="margin-bottom: 20px;">
				<h3><?php esc_html_e( 'Missing Twitter Card Tags', 'wpshadow' ); ?></h3>
				<ul>
					<?php foreach ( $missing_twitter as $tag ) : ?>
						<li><code><?php echo esc_html( $tag ); ?></code> - <?php echo esc_html( $twitter_tags[ $tag ] ); ?></li>
					<?php endforeach; ?>
				</ul>
				<p><em><?php esc_html_e( 'These tags control how your content appears on X (Twitter).', 'wpshadow' ); ?></em></p>
			</div>
		<?php endif; ?>

		<!-- Preview Section -->
		<div style="background: #fff; border: 1px solid #ccd0d4; border-radius: 4px; padding: 20px; margin-bottom: 20px;">
			<h2 style="margin-top: 0;"><?php esc_html_e( 'Social Media Previews', 'wpshadow' ); ?></h2>
			
			<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
				<!-- LinkedIn/Facebook Preview -->
				<div>
					<h3><?php esc_html_e( 'LinkedIn / Facebook Preview', 'wpshadow' ); ?></h3>
					<div style="border: 1px solid #ddd; border-radius: 8px; overflow: hidden; background: #fff;">
						...
						if ( $og_image ) :
							...
							<div style="background-color: #f0f0f0; height: 200px; display: flex; align-items: center; justify-content: center; overflow: hidden;">
								<img src="<?php echo esc_url( $og_image ); ?>" 
									 alt="<?php esc_attr_e( 'Preview image', 'wpshadow' ); ?>" 
						...
						<div style="padding: 12px;">
							...
							<div style="font-weight: 600; font-size: 16px; margin-bottom: 4px; color: #1d2129;">
								<?php echo esc_html( $tags['og:title'] ?? __( 'No title', 'wpshadow' ) ); ?>
							</div>
							<div style="color: #606770; font-size: 14px;">
								<?php
								$description = $tags['og:description'] ?? __( 'No description', 'wpshadow' );
				...
				<!-- Twitter Preview -->
				<div>
					<h3><?php esc_html_e( 'X (Twitter) Preview', 'wpshadow' ); ?></h3>
					<div style="border: 1px solid #e1e8ed; border-radius: 14px; overflow: hidden; background: #fff;">
						...
						<div style="padding: 12px; border-top: 1px solid #e1e8ed;">
							<div style="font-size: 15px; font-weight: 600; margin-bottom: 2px; color: #0f1419;">
								<?php echo esc_html( $tags['twitter:title'] ?? $tags['og:title'] ?? __( 'No title', 'wpshadow' ) ); ?>
							</div>
							<div style="color: #536471; font-size: 15px; margin-bottom: 2px;">
								<?php
								$twitter_desc = $tags['twitter:description'] ?? $tags['og:description'] ?? __( 'No description', 'wpshadow' );
		...
		<div style="background: #fff; border: 1px solid #ccd0d4; border-radius: 4px; padding: 20px;">
			<h2 style="margin-top: 0;"><?php esc_html_e( 'All Detected Tags', 'wpshadow' ); ?></h2>
			<?php if ( empty( $tags ) ) : ?>
				<p><?php esc_html_e( 'No Open Graph or Twitter Card tags found on this page.', 'wpshadow' ); ?></p>
			<?php else : ?>
				<table class="widefat striped" style="margin-top: 10px;">
					<thead>
						<tr>
							<th><?php esc_html_e( 'Tag', 'wpshadow' ); ?></th>
							<th><?php esc_html_e( 'Content', 'wpshadow' ); ?></th>
						</tr>
					</thead>
					<tbody>
		...
		<?php if ( ! empty( $missing ) || ! empty( $missing_twitter ) ) : ?>
			<div style="background: #fff; border: 1px solid #ccd0d4; border-radius: 4px; padding: 20px; margin-top: 20px;">
				<h2 style="margin-top: 0;"><?php esc_html_e( 'Recommendations', 'wpshadow' ); ?></h2>
				<p><?php esc_html_e( 'To improve how your content appears when shared on social media:', 'wpshadow' ); ?></p>
				<ul>
					<li><?php esc_html_e( 'Install an SEO plugin like Yoast SEO or Rank Math to automatically add Open Graph tags', 'wpshadow' ); ?></li>
					<li><?php esc_html_e( 'Ensure each page has a unique title and description', 'wpshadow' ); ?></li>
					<li><?php esc_html_e( 'Use high-quality images (recommended size: 1200x630px for best results)', 'wpshadow' ); ?></li>
					<li><?php esc_html_e( 'Test your pages using Facebook\'s Sharing Debugger and Twitter\'s Card Validator', 'wpshadow' ); ?></li>
				</ul>
			</div>
		<?php endif; ?>
		<?php
