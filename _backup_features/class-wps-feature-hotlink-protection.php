...
final class WPSHADOW_Feature_Hotlink_Protection extends WPSHADOW_Abstract_Feature {
	...
	public function __construct() {
		parent::__construct(
			array(
				...
				'description'        => __( 'Stop other sites from stealing your bandwidth - protect your images and files.', 'wpshadow' ),
				'scope'              => 'core',
		...
		if ( method_exists( $this, 'register_sub_features' ) ) {
			$this->register_sub_features(
				array(
					'apache_protection'   => __( 'Apache .htaccess Protection', 'wpshadow' ),
					'nginx_guidance'      => __( 'Nginx Configuration Guidance', 'wpshadow' ),
					'cdn_recommendations' => __( 'CDN-Level Blocking', 'wpshadow' ),
					'image_protection'    => __( 'Protect Images', 'wpshadow' ),
					'media_protection'    => __( 'Protect Media Files', 'wpshadow' ),
				)
			);
	...
	private function configure_apache_protection(): void {
		...
		if ( false === $result ) {
			...
			$this->add_admin_notice(
				...
				__( 'Hotlink Protection: Could not write to uploads/.htaccess. Please check file permissions.', 'wpshadow' ),
				'error'
			);
		} else {
			...
			$this->add_admin_notice(
				...
				__( 'Hotlink Protection: Successfully configured Apache rules in uploads/.htaccess', 'wpshadow' ),
				'success'
			);
		}
	}
	...
	public function show_nginx_guidance(): void {
		...
		<div class="notice notice-info is-dismissible" data-dismissible="wpshadow-hotlink-nginx">
			<h3><?php esc_html_e( 'WPShadow Hotlink Protection - Nginx Configuration', 'wpshadow' ); ?></h3>
			<p><?php esc_html_e( 'Nginx server detected. Please add the following configuration to your server config:', 'wpshadow' ); ?></p>
   ...
   if ($invalid_referer) {
       return 403;
			...
			<p>
				<strong><?php esc_html_e( 'CDN-Level Protection (Recommended):', 'wpshadow' ); ?></strong><br>
				<?php esc_html_e( 'For even better protection, enable hotlink protection at your CDN level:', 'wpshadow' ); ?>
			</p>
			<ul style="list-style: disc; margin-left: 20px;">
				<li><strong>Cloudflare:</strong> <?php esc_html_e( 'Enable "Scrape Shield" → "Hotlink Protection" in your dashboard', 'wpshadow' ); ?></li>
				<li><strong>KeyCDN:</strong> <?php esc_html_e( 'Enable "Zone Referrer" in zone settings', 'wpshadow' ); ?></li>
				<li><strong>BunnyCDN:</strong> <?php esc_html_e( 'Use "Token Authentication" or "Allowed Referrers" in pull zone settings', 'wpshadow' ); ?></li>
			</ul>
		</div>
		<?php
	}
		...
		add_action(
			...
			function (): void {
				...
				<div class="notice notice-success is-dismissible">
					<p><?php esc_html_e( 'Hotlink protection settings saved successfully.', 'wpshadow' ); ?></p>
				</div>
				<?php
			}
		);
