...
class WPSHADOW_Feature_Favicon_Checker extends WPSHADOW_Abstract_Feature {
	...
	public function __construct() {
		parent::__construct(
			array(
				...
				'description'        => __( 'Verifies that your site icon displays correctly across all major browsers, desktop tabs, and mobile home screens. Google Search results also rely on stable, properly formatted (1:1 aspect ratio) favicons for site identity.', 'wpshadow' ),
				'scope'              => 'core',
				'version'            => '1.0.0',
	...
	private function perform_icon_check(): array {
				...
				if ( $image_meta && isset( $image_meta['width'], $image_meta['height'] ) ) {
					...
					if ( $width !== $height ) {
						$result['issues'][] = sprintf(
							...
							__( 'Site icon has non-square dimensions (%1$dx%2$d). Icons should have a 1:1 aspect ratio for best display across browsers and devices.', 'wpshadow' ),
							$width,
							$height
						);
						$result['recommendations'][] = __( 'Upload a square image (e.g., 512x512 pixels) as your site icon for optimal display.', 'wpshadow' );
					...
					if ( $width < 512 || $height < 512 ) {
						...
						$result['recommendations'][] = __( 'For best quality, use a site icon that is at least 512x512 pixels. WordPress will automatically generate smaller sizes.', 'wpshadow' );
						if ( 'ok' === $result['status'] ) {
							$result['status'] = 'warning';
						}
					}
		...
		if ( ! $site_icon_id && $favicon_exists ) {
			$result['recommendations'][] = __( 'While a legacy favicon.ico exists, setting a Site Icon in WordPress provides better cross-platform support and automatic generation of all required icon sizes.', 'wpshadow' );
		}
	...
	public function render_widget(): void {
		...
		<div class="wpshadow-favicon-checker">
			...
			<div class="wpshadow-icon-details">
				<?php
				WPSHADOW_render_widget_table_open( __( 'Icon Status:', 'wpshadow' ) );
				...
				if ( $check['icons']['site_icon']['exists'] ) {
					...
					if ( $check['icons']['site_icon']['dimensions'] ) {
						$dimensions = sprintf(
							/* translators: %1$d: width, %2$d: height */
							__( 'Dimensions: %1$dx%2$d', 'wpshadow' ),
				...
				WPSHADOW_render_widget_table_row(
					__( 'WordPress Site Icon', 'wpshadow' ),
					$site_icon_content
				);

				// Legacy Favicon.ico row.
				$favicon_content = $check['icons']['favicon']['exists']
					? WPSHADOW_render_status_indicator( 'active', __( 'Present', 'wpshadow' ) )
					: WPSHADOW_render_status_indicator( 'inactive', __( 'Not Found', 'wpshadow' ) );

				WPSHADOW_render_widget_table_row(
					__( 'Legacy Favicon.ico', 'wpshadow' ),
					$favicon_content,
				...
				if ( ! empty( $check['icons']['touch_icons'] ) ) {
					$touch_icon_content .= WPSHADOW_render_status_indicator(
						'active',
						count( $check['icons']['touch_icons'] ) . ' ' . __( 'sizes', 'wpshadow' )
					);
				...
				WPSHADOW_render_widget_table_row(
					__( 'Generated Touch Icons', 'wpshadow' ),
					$touch_icon_content
				);
			...
			WPSHADOW_render_widget_actions(
				array(
					array(
						'label' => __( 'Set Site Icon', 'wpshadow' ),
						'url'   => admin_url( 'customize.php?autofocus[section]=title_tagline' ),
						'type'  => 'primary',
					),
					array(
						'label'  => __( 'Learn More About Favicons', 'wpshadow' ),
