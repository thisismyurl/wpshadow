...
final class WPSHADOW_Feature_Skiplinks extends WPSHADOW_Abstract_Feature {
	...
	public function __construct() {
		parent::__construct(
			array(
				...
				'description'        => __( 'Help keyboard users navigate faster - add hidden skip links to main content.', 'wpshadow' ),
				'scope'              => 'core',
		...
		if ( method_exists( $this, 'register_sub_features' ) ) {
			$this->register_sub_features(
				array(
					'skip_to_content'     => __( 'Skip to Content Link', 'wpshadow' ),
					'skip_to_nav'         => __( 'Skip to Navigation Link', 'wpshadow' ),
					'skip_to_footer'      => __( 'Skip to Footer Link', 'wpshadow' ),
					'custom_styling'      => __( 'Custom Skip Link Styling', 'wpshadow' ),
				)
			);
	...
	public function inject_skip_links(): void {
		...
		if ( $options['skip_to_content'] ?? true ) {
			...
			$links[]    = sprintf(
				...
				esc_attr( $content_id ),
				esc_html( $options['content_label'] ?? __( 'Skip to content', 'wpshadow' ) )
			);
		}
		...
		if ( $options['skip_to_nav'] ?? true ) {
			...
			$links[] = sprintf(
				...
				esc_attr( $nav_id ),
				esc_html( $options['nav_label'] ?? __( 'Skip to navigation', 'wpshadow' ) )
			);
		}
	...
	public function test_skiplinks(): array {
		...
		$label  = $enabled_features >= 1 ?
			__( 'Skip links are active', 'wpshadow' ) :
			__( 'Skip links should be enabled', 'wpshadow' );
		...
		return array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					__( '%d skip link types are enabled, improving keyboard navigation accessibility.', 'wpshadow' ),
					$enabled_features
				)
			),
			'actions'     => '',
	...
	protected function get_default_options(): array {
		return array(
			...
			'content_label'    => __( 'Skip to content', 'wpshadow' ),
			'nav_label'        => __( 'Skip to navigation', 'wpshadow' ),
			'bg_color'         => '#21759b',
			'text_color'       => '#ffffff',
		);
	}
