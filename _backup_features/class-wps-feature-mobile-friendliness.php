...
final class WPSHADOW_Feature_Mobile_Friendliness extends WPSHADOW_Abstract_Feature {
	...
	public function __construct() {
		parent::__construct(
			array(
				...
				'name'               => __( 'Mobile-Friendliness Test', 'wpshadow' ),
				'description'        => __( 'Check that your site works great on phones and tablets.', 'wpshadow' ),
				'scope'              => 'core',
	...
	public function render_test_page(): void {
		...
		wp_localize_script(
			...
			array(
				...
				'strings' => array(
					'testing'    => __( 'Running tests...', 'wpshadow' ),
					'complete'   => __( 'Tests complete!', 'wpshadow' ),
					'error'      => __( 'An error occurred while running tests.', 'wpshadow' ),
					'homeUrl'    => home_url(),
				),
			)
		);
	...
	public function add_dashboard_widget(): void {
		wp_add_dashboard_widget(
			...
			__( 'Mobile-Friendliness Status', 'wpshadow' ),
	...
	public function render_dashboard_widget(): void {
		...
		if ( empty( $last_test ) ) {
			echo '<p>' . esc_html__( 'No mobile-friendliness test has been run yet.', 'wpshadow' ) . '</p>';
			echo '<p><a href="' . esc_url( admin_url( 'admin.php?page=wpshadow-mobile-test' ) ) . '" class="button button-primary">' . esc_html__( 'Run Test Now', 'wpshadow' ) . '</a></p>';
			return;
		}

		$score = $last_test['score'] ?? 0;
		$status_class = $score >= 80 ? 'good' : ( $score >= 50 ? 'warning' : 'critical' );
		$status_text = $score >= 80 ? __( 'Good', 'wpshadow' ) : ( $score >= 50 ? __( 'Needs Improvement', 'wpshadow' ) : __( 'Poor', 'wpshadow' ) );
		...
		echo '<span class="score-label">/' . esc_html__( '100', 'wpshadow' ) . '</span>';
		echo '</div>';
		...
		if ( ! empty( $last_test['issues'] ) ) {
			...
			if ( count( $last_test['issues'] ) > 3 ) {
				echo '<li><em>' . sprintf( esc_html__( 'And %d more issues...', 'wpshadow' ), count( $last_test['issues'] ) - 3 ) . '</em></li>';
			}
			echo '</ul>';
		}

		echo '<p><a href="' . esc_url( admin_url( 'admin.php?page=wpshadow-mobile-test' ) ) . '">' . esc_html__( 'View Full Report', 'wpshadow' ) . '</a></p>';
	}
	...
	public function ajax_get_mobile_report(): void {
		...
		if ( empty( $last_test ) ) {
			wp_send_json_error( array( 'message' => __( 'No test results available.', 'wpshadow' ) ) );
		}
	...
	private function check_viewport( string $url ): array {
		...
		if ( ! $has_viewport ) {
			$result = array(
				'pass'    => false,
				'message' => __( 'Viewport meta tag is missing or not properly configured.', 'wpshadow' ),
				'recommendation' => __( 'Add <meta name="viewport" content="width=device-width, initial-scale=1.0"> to your theme\'s header.php or enable HTML5 theme support.', 'wpshadow' ),
			);
		} else {
			$result = array(
				'pass'    => true,
				'message' => __( 'Viewport meta tag is properly configured.', 'wpshadow' ),
			);
		}
	...
	private function check_touch_targets( string $url, int $min_size ): array {
		...
		if ( $small_buttons > 0 ) {
			$result['issues'][] = sprintf(
				__( 'Found %d button styles with height less than %dpx (recommended minimum for touch targets).', 'wpshadow' ),
				$small_buttons,
				$min_size
			);
			$result['recommendations'][] = sprintf(
				__( 'Ensure all clickable elements (buttons, links) have a minimum size of %dx%dpx for easy tapping on mobile devices.', 'wpshadow' ),
				$min_size,
				$min_size
			);
		} else {
			$result['passes'][] = __( 'Touch target sizes appear to be adequate.', 'wpshadow' );
		}
	...
	private function check_font_sizes( string $url, int $min_size ): array {
		...
		if ( preg_match( '/body\s*\{[^}]*font-size\s*:\s*(\d+)px/i', $theme_css, $matches ) ) {
			...
			if ( $font_size < $min_size ) {
				$result['issues'][] = sprintf(
					__( 'Base font size (%dpx) is smaller than recommended minimum (%dpx) for mobile readability.', 'wpshadow' ),
					$font_size,
					$min_size
				);
				$result['recommendations'][] = sprintf(
					__( 'Increase base font size to at least %dpx for better readability on small screens.', 'wpshadow' ),
					$min_size
				);
			} else {
				$result['passes'][] = __( 'Base font size is appropriate for mobile devices.', 'wpshadow' );
			}
		} else {
			$result['passes'][] = __( 'Using browser default font size (typically 16px).', 'wpshadow' );
		}
	...
	private function check_tap_spacing( string $url ): array {
		...
		if ( preg_match( '/\.menu\s+li\s*\{[^}]*(?:padding|margin)\s*:\s*(\d+)px/i', $theme_css, $matches ) ) {
			...
			if ( $spacing < 8 ) {
				$result['warnings'][] = __( 'Navigation menu items may be too close together for comfortable tapping on mobile devices.', 'wpshadow' );
				$result['recommendations'][] = __( 'Add at least 8px padding or margin between interactive elements in navigation menus.', 'wpshadow' );
			} else {
				$result['passes'][] = __( 'Navigation spacing appears adequate for mobile interaction.', 'wpshadow' );
			}
		}
