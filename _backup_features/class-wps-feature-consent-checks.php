...
final class WPSHADOW_Feature_Consent_Checks extends WPSHADOW_Abstract_Feature {
	...
	public function __construct() {
		parent::__construct(
			array(
				...
				'description'        => __( 'Make your site compliant with privacy laws - manage cookies and tracking with consent checks.', 'wpshadow' ),
				'scope'              => 'core',
		...
		if ( method_exists( $this, 'register_sub_features' ) ) {
			$this->register_sub_features(
				array(
					...
					'customizable_banner'  => __( 'Customizable Banner Text', 'wpshadow' ),
				)
			);
	...
	public function render_settings(): void {
		...
		WPSHADOW_render_settings_heading( __( 'Cookie Consent Settings', 'wpshadow' ) );
		...
		WPSHADOW_render_textarea_field(
			...
			__( 'Banner Text', 'wpshadow' ),
			$banner_text,
			__( 'The message displayed to visitors in the consent banner.', 'wpshadow' ),
		...
		WPSHADOW_render_textarea_field(
			...
			__( 'Custom Blocked Patterns', 'wpshadow' ),
			$custom_patterns_text,
			__( 'One regex pattern per line. These patterns will be blocked until consent is given. Default patterns include Google Analytics, Facebook Pixel, and DoubleClick.', 'wpshadow' ),
			array(
				'rows'        => 8,
				'cols'        => 50,
