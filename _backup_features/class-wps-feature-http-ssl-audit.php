...
final class WPSHADOW_Feature_HTTP_SSL_Audit extends WPSHADOW_Abstract_Feature {
	...
	public function __construct() {
		parent::__construct(
			array(
				'id'                 => 'http-ssl-audit',
				'name'               => __( 'HTTP Header & SSL Audit', 'wpshadow' ),
				'description'        => __( 'Check your SSL certificate and security headers - we alert you before problems hit.', 'wpshadow' ),
				'scope'              => 'core',
	...
	public function run_scheduled_audit(): void {
		...
		if ( class_exists( '\\WPShadow\\WPSHADOW_Activity_Logger' ) ) {
			\WPShadow\WPSHADOW_Activity_Logger::log(
				'security',
				__( 'HTTP Header & SSL Audit completed', 'wpshadow' ),
				array(
	...
	private function check_ssl_certificate(): array {
		...
		if ( ! ( strpos( $site_url, 'https://' ) === 0 ) ) {
			$results['error'] = __( 'Site is not using HTTPS', 'wpshadow' );
			return $results;
		}
		...
		if ( ! isset( $parsed_url['host'] ) ) {
			$results['error'] = __( 'Could not parse site URL', 'wpshadow' );
			return $results;
		}
		...
		if ( ! $stream ) {
			$results['error'] = sprintf(
				...
				__( 'Could not connect to SSL server: %s', 'wpshadow' ),
				$errstr
			);
			return $results;
		}
		...
		if ( ! isset( $params['options']['ssl']['peer_certificate'] ) ) {
			$results['error'] = __( 'Could not retrieve SSL certificate', 'wpshadow' );
			return $results;
		}
		...
		if ( ! $cert_info ) {
			$results['error'] = __( 'Could not parse SSL certificate', 'wpshadow' );
			return $results;
		}
		...
		if ( isset( $cert_info['validTo_time_t'] ) ) {
			...
			if ( $days_until_expiry < 0 ) {
				$results['valid'] = false;
				$results['error'] = __( 'SSL certificate has expired', 'wpshadow' );
			}
		}
		...
		if ( ! empty( $results['ssl'] ) ) {
			if ( ! $results['ssl']['valid'] ) {
				$issues[] = array(
					'type'        => 'ssl_invalid',
					'severity'    => 'critical',
					'description' => $results['ssl']['error'] ?? __( 'SSL certificate isn\'t valid', 'wpshadow' ),
				);
	...
	private function send_alert_email( array $issues ): void {
		...
		$subject     = sprintf(
			...
			__( '[%s] Heads Up: We Found Some Security Things to Fix', 'wpshadow' ),
			$site_name
		);

		$message = sprintf(
			/* translators: %s: site name */
			__( 'We noticed a few security things you should fix on %s:', 'wpshadow' ),
			$site_name
		) . "\n\n";
		...
		$message .= "\n" . sprintf(
			...
			__( 'Check your dashboard to see the details: %s', 'wpshadow' ),
			admin_url( 'index.php' )
		);
	...
	public function add_dashboard_widget(): void {
		...
		wp_add_dashboard_widget(
			...
			__( 'HTTP & SSL Security Audit', 'wpshadow' ),
	...
	public function render_dashboard_widget(): void {
		...
		if ( ! $results ) {
			echo '<p>' . esc_html__( 'No audit results available. Running audit...', 'wpshadow' ) . '</p>';
			echo '<p><button type="button" class="button button-primary" onclick="wpshadowRunHttpSslAudit()">' .
				esc_html__( 'Run Audit Now', 'wpshadow' ) . '</button></p>';
			$this->add_widget_script();
			return;
		}

		$timestamp = isset( $results['timestamp'] ) ? $results['timestamp'] : 0;
		echo '<p><strong>' . esc_html__( 'Last Audit:', 'wpshadow' ) . '</strong> ' .
			esc_html( human_time_diff( $timestamp ) . ' ' . __( 'ago', 'wpshadow' ) ) . '</p>';

		// Display SSL results.
		if ( ! empty( $results['ssl'] ) ) {
			echo '<h4>' . esc_html__( 'SSL Certificate', 'wpshadow' ) . '</h4>';

			if ( ! $results['ssl']['enabled'] ) {
				echo '<p style="color: #dc3232;">⚠️ ' . esc_html__( 'HTTPS is not enabled', 'wpshadow' ) . '</p>';
			} elseif ( ! $results['ssl']['valid'] ) {
				echo '<p style="color: #dc3232;">❌ ' . esc_html( $results['ssl']['error'] ) . '</p>';
			} else {
				echo '<p style="color: #46b450;">✓ ' . esc_html__( 'SSL certificate is valid', 'wpshadow' ) . '</p>';
				...
				if ( isset( $results['ssl']['days_until_expiry'] ) ) {
					...
					echo '<p style="color: ' . esc_attr( $color ) . ';">' .
						sprintf(
							/* translators: %d: number of days */
							esc_html__( 'Expires in %d days', 'wpshadow' ),
							(int) $days
						) . '</p>';
				}
			}
		...
		if ( ! empty( $results['headers'] ) ) {
			echo '<h4>' . esc_html__( 'Security Headers', 'wpshadow' ) . '</h4>';
			...
			echo '<p>' . sprintf(
				...
				esc_html__( '%1$d of %2$d recommended headers present', 'wpshadow' ),
				(int) $present_count,
				(int) $total_checked
			) . '</p>';
			...
			if ( $missing_count > 0 ) {
				echo '<details><summary style="cursor: pointer; color: #dc3232;">' .
					sprintf(
						...
						esc_html__( 'Missing %d headers', 'wpshadow' ),
						(int) $missing_count
	...
	private function add_widget_script(): void {
		...
		function wpshadowRunHttpSslAudit(event) {
			...
			button.textContent = '<?php echo esc_js( __( 'Running...', 'wpshadow' ) ); ?>';
			...
			jQuery.post(ajaxurl, {
				...
				alert('<?php echo esc_js( __( 'Audit failed. Please try again.', 'wpshadow' ) ); ?>');
				button.disabled = false;
				button.textContent = '<?php echo esc_js( __( 'Run Audit Now', 'wpshadow' ) ); ?>';
			});
		}
		</script>
		<?php
	...
	public function display_audit_warnings(): void {
		...
		echo '<div class="notice notice-error"><p><strong>' .
			...
			sprintf(
				...
				esc_html__( '%d security issue(s) detected in HTTP/SSL audit.', 'wpshadow' ),
				count( $issues )
			) .
