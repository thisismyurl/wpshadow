...
class WPSHADOW_Feature_SEO_Validator extends WPSHADOW_Abstract_Feature {
	...
	public function __construct() {
		parent::__construct(
			array(
				...
				'sub_features'       => array(
					'validate_robots'    => __( 'Validate Robots.txt', 'wpshadow' ),
					'validate_sitemap'   => __( 'Validate Sitemap.xml', 'wpshadow' ),
					'auto_fix_issues'    => __( 'Auto-Fix Common Issues', 'wpshadow' ),
					'email_alerts'       => __( 'Send Email Alerts on Issues', 'wpshadow' ),
				),
				'aliases'            => array(
					'sitemap',
					'robots.txt',
		...
		<div class="wpshadow-seo-validator-widget">
			<div id="wpshadow-seo-results">
				...
				if ( $cached_results && is_array( $cached_results ) ) {
					...
					?>
					<p><?php esc_html_e( 'Click "Validate Now" to check your sitemap and robots.txt files.', 'wpshadow' ); ?></p>
					<?php
				}
				?>
			</div>
			...
			<p class="wpshadow-validation-actions">
				<button type="button" id="wpshadow-validate-seo-btn" class="button button-primary">
					<?php esc_html_e( 'Validate Now', 'wpshadow' ); ?>
				</button>
				<?php if ( $last_check > 0 ) : ?>
					<span class="description">
						...
						printf(
							...
							esc_html__( 'Last checked: %s', 'wpshadow' ),
							esc_html( human_time_diff( $last_check ) . ' ' . __( 'ago', 'wpshadow' ) )
						);
						?>
					</span>
				<?php endif; ?>
			...
			jQuery(document).ready(function($) {
				$('#wpshadow-validate-seo-btn').on('click', function() {
					...
					$btn.addClass('loading').text('<?php echo esc_js( __( 'Validating...', 'wpshadow' ) ); ?>');
					...
					$.ajax({
						...
						success: function(response) {
							if (response.success && response.data.html) {
								...
								alert('<?php echo esc_js( __( 'Validation failed. Please try again.', 'wpshadow' ) ); ?>');
							}
						},
						error: function() {
	...
	private function render_sitemap_results( array $result ): void {
		...
		<div class="wpshadow-validation-result <?php echo esc_attr( $status_class ); ?>">
			<h4>
				...
				<?php esc_html_e( 'Sitemap (sitemap.xml)', 'wpshadow' ); ?>
			</h4>
			...
			<?php if ( ! empty( $result['recommendations'] ) && is_array( $result['recommendations'] ) ) : ?>
				<p><strong><?php esc_html_e( 'Recommendations:', 'wpshadow' ); ?></strong></p>
				<ul>
	...
	private function render_robots_results( array $result ): void {
		...
		<div class="wpshadow-validation-result <?php echo esc_attr( $status_class ); ?>">
			<h4>
				<span class="dashicons <?php echo esc_attr( $icon ); ?>"></span>
				<?php esc_html_e( 'Robots.txt', 'wpshadow' ); ?>
			</h4>
			<?php if ( ! empty( $result['message'] ) ) : ?>
				<p><?php echo esc_html( $result['message'] ); ?></p>
			<?php endif; ?>
	...
	private function validate_sitemap(): array {
		...
		if ( is_wp_error( $response ) ) {
			...
			$result['recommendations'] = array(
				sprintf(
					...
					__( 'Ensure your sitemap is accessible at <a href="%s" target="_blank">%s</a>', 'wpshadow' ),
					esc_url( $sitemap_url ),
					esc_html( $sitemap_url )
				),
				__( 'Check if a plugin like Yoast SEO, Rank Math, or All in One SEO is installed and configured.', 'wpshadow' ),
		...
		if ( 200 !== $status_code ) {
			...
			$result['recommendations'] = array(
				__( 'Check your sitemap plugin settings.', 'wpshadow' ),
				__( 'Verify that permalinks are configured correctly.', 'wpshadow' ),
			);
			return $result;
		}
		...
		if ( ! empty( $content_type ) && false === stripos( $content_type, 'xml' ) && false === stripos( $content_type, 'text' ) ) {
			$result['issues'][] = sprintf(
				...
				__( 'Unexpected content type: %s (expected XML)', 'wpshadow' ),
		...
		if ( false === $xml || ! empty( $xml_errors ) ) {
			...
			$result['recommendations'] = array(
				__( 'Fix XML syntax errors in your sitemap.', 'wpshadow' ),
				__( 'Regenerate the sitemap using your SEO plugin.', 'wpshadow' ),
			);
			return $result;
		}
		...
		if ( ! $has_urlset && ! $has_sitemap ) {
			...
			$recommendations[] = __( 'Ensure your sitemap contains URL entries.', 'wpshadow' );
		}
		...
		if ( ! $has_sitemap_namespace ) {
			...
			$recommendations[] = sprintf(
				/* translators: %s: expected namespace URI */
				__( 'Sitemap should declare the namespace xmlns="%s"', 'wpshadow' ),
		...
		foreach ( $xml->url as $url ) {
			...
			if ( ! filter_var( $loc, FILTER_VALIDATE_URL ) ) {
				$issues[] = sprintf(
					...
					__( 'That URL doesn\'t look right: %s', 'wpshadow' ),
					esc_html( $loc )
				);
			}
	...
	private function validate_robots(): array {
		...
		if ( is_wp_error( $response ) ) {
			...
			$result['recommendations'] = array(
				sprintf(
					...
					__( 'Ensure robots.txt is accessible at <a href="%s" target="_blank">%s</a>', 'wpshadow' ),
					esc_url( $robots_url ),
					esc_html( $robots_url )
				),
				__( 'WordPress generates a virtual robots.txt by default. Check if a physical file exists that may be interfering.', 'wpshadow' ),
			);
			return $result;
		}
		...
		if ( 200 !== $status_code ) {
			...
			$result['recommendations'] = array(
				__( 'Check your server configuration and permalink settings.', 'wpshadow' ),
			);
			return $result;
		}
		...
		if ( empty( $body ) ) {
			...
			$result['recommendations'] = array(
				...
				__( 'You can customize robots.txt using your SEO plugin or a custom file in the root directory.', 'wpshadow' ),
			);
			return $result;
		}
		...
		if ( ! preg_match( '/^Sitemap:/mi', $body ) ) {
			$recommendations[] = wp_kses_post(
				sprintf(
					/* translators: %s: sitemap URL */
					__( 'Add a Sitemap directive to your robots.txt: <code>Sitemap: %s</code>', 'wpshadow' ),
		...
		if ( preg_match( '/Disallow:\s*\/$/m', $body ) ) {
			...
			$recommendations[] = __( 'Remove or modify the "Disallow: /" directive if you want search engines to index your site.', 'wpshadow' );
		}
		...
		if ( $blocks_uploads ) {
			$recommendations[] = sprintf(
				...
				__( 'Your robots.txt blocks %s/uploads. This may prevent indexing of media files. Remove this if you want images indexed.', 'wpshadow' ),
