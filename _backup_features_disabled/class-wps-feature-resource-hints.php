	...
	public function __construct() {
		parent::__construct(
			array(
				...
				'description'        => __( 'Speed up external services - tell browsers to start connecting early.', 'wpshadow' ),
				'scope'              => 'core',
		...
		if ( method_exists( $this, 'register_sub_features' ) ) {
			$this->register_sub_features(
				array(
					...
					'preconnect'        => __( 'Preconnect Resources', 'wpshadow' ),
					'preload_fonts'     => __( 'Preload Web Fonts', 'wpshadow' ),
					'preload_scripts'   => __( 'Preload Critical Scripts', 'wpshadow' ),
					'remove_s_w_org'    => __( 'Remove WordPress.org DNS Prefetch', 'wpshadow' ),
				)
			);
	...
	public function test_resource_hints(): array {
		if ( ! $this->is_enabled() ) {
			return array(
				...
				'description' => sprintf( '<p>%s</p>', __( 'Resource Hints are not enabled. Enabling resource hints can improve page load times by pre-connecting to external services.', 'wpshadow' ) ),
				'actions'     => '',
		...
		return array(
			...
			'description' => sprintf(
				...
				sprintf(
					__( 'Resource Hints are active with %1$d optimization features enabled and %2$d custom hints configured.', 'wpshadow' ),
					$enabled_features,
					$custom_count
				)
			),
