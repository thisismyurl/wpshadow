...
final class WPSHADOW_Feature_Plugin_Cleanup extends WPSHADOW_Abstract_Feature {
	...
	$this->plugin_availability = array(
		...
		if ( method_exists( $this, 'register_sub_features' ) ) {
			$this->register_sub_features(
				array(
					'jetpack_cleanup'     => __( 'Jetpack Asset Cleanup', 'wpshadow' ),
					'rankmath_cleanup'    => __( 'RankMath Asset Cleanup', 'wpshadow' ),
					'cf7_cleanup'         => __( 'Contact Form 7 Cleanup', 'wpshadow' ),
					'woocommerce_cleanup' => __( 'WooCommerce Asset Cleanup', 'wpshadow' ),
	...
	public function test_plugin_cleanup(): array {
		...
		$label  = $enabled_features >= 3 ?
			__( 'Plugin asset cleanup is active', 'wpshadow' ) :
			__( 'Plugin asset cleanup could be improved', 'wpshadow' );
