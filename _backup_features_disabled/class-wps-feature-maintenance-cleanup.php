...
final class WPSHADOW_Feature_Maintenance_Cleanup extends WPSHADOW_Abstract_Feature {
	...
	private function remove_maintenance_file(): bool|\WP_Error {
		if ( ! file_exists( self::MAINTENANCE_FILE ) ) {
			return new \WP_Error(
				'file_not_found',
				__( 'Maintenance file not found', 'wpshadow' )
			);
		}
		...
		if ( ! is_writable( self::MAINTENANCE_FILE ) ) {
			return new \WP_Error(
				'file_not_writable',
				__( 'Maintenance file is not writable', 'wpshadow' )
			);
		}
		...
		if ( ! $deleted && file_exists( self::MAINTENANCE_FILE ) ) {
			return new \WP_Error(
				'deletion_failed',
				__( 'Couldn\'t delete the maintenance file', 'wpshadow' )
			);
		}
	...
	public function site_health_test_callback(): array {
		...
		if ( false === $info ) {
			return array(
				...
				'description' => sprintf(
					'<p>%s</p>',
					__( 'No maintenance mode file detected.', 'wpshadow' )
				),
		...
		if ( $info['is_stuck'] ) {
			return array(
				...
				'description' => sprintf(
					...
					__( 'Remove Maintenance Mode', 'wpshadow' )
				),
				'test'        => 'wpshadow_maintenance_mode',
				'actions'     => sprintf(
					'<a href="%s">%s</a>',
					wp_nonce_url( admin_url( 'admin-post.php?action=WPSHADOW_cleanup_maintenance_file' ), 'wpshadow_cleanup_maintenance' ),
					__( 'Remove Maintenance File', 'wpshadow' )
				),
			);
		}
	...
	public function show_maintenance_notice(): void {
		...
		<div class="notice notice-error is-dismissible">
			<p>
				<strong><?php esc_html_e( 'WPShadow: Maintenance Mode Stuck', 'wpshadow' ); ?></strong>
			</p>
			<p>
				<?php
				printf(
			...
			$('#wps-cleanup-maintenance').on('click', function() {
				...
				$.post(ajaxurl, {
					...
					if (response.success) {
						...
						alert(response.data.message || '<?php esc_html_e( 'Failed to remove maintenance file.', 'wpshadow' ); ?>');
	...
	public function ajax_cleanup_maintenance(): void {
		...
		if ( ! current_user_can( 'update_core' ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'You do not have permission to perform this action.', 'wpshadow' ),
				)
			);
		}
		...
		wp_send_json_success(
			array(
				'message' => __( 'Maintenance mode has been removed successfully.', 'wpshadow' ),
			)
		);
	}
