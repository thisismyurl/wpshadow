...
final class WPSHADOW_Feature_External_Fonts_Disabler extends WPSHADOW_Abstract_Feature {
	...
	public function __construct() {
		parent::__construct(
			array(
				...
				'name'               => __( 'Block Google Fonts Loading', 'wpshadow' ),
				'description'        => __( 'Protect your visitors\' privacy - we block Google Fonts tracking and keep your fonts loading fast.', 'wpshadow' ),
				'scope'              => 'core',
		...
		if ( method_exists( $this, 'register_sub_features' ) ) {
			$this->register_sub_features(
				array(
					'block_googleapis'  => __( 'Block fonts.googleapis.com', 'wpshadow' ),
					'block_gstatic'     => __( 'Block fonts.gstatic.com', 'wpshadow' ),
					'remove_preconnect' => __( 'Remove Google Fonts Preconnect', 'wpshadow' ),
					'buffer_cleanup'    => __( 'Remove from HTML Output', 'wpshadow' ),
				)
			);
	...
	public function register_site_health_test( array $tests ): array {
		$tests['direct']['WPSHADOW_google_fonts_disabler'] = array(
			'label' => __( 'Google Fonts Privacy', 'wpshadow' ),
	...
	public function test_google_fonts_disabler(): array {
		if ( ! $this->is_enabled() ) {
			return array(
				...
				'description' => sprintf( '<p>%s</p>', __( 'Google Fonts blocking is not enabled. Blocking Google Fonts improves privacy by preventing third-party tracking.', 'wpshadow' ) ),
				'actions'     => '',
		...
		return array(
			...
			'description' => sprintf(
				...
				sprintf(
					__( 'Google Fonts blocking is active with %d protection layers enabled. Your site is not loading fonts from Google servers.', 'wpshadow' ),
					$enabled_features
				)
			),
			'actions'     => '',
