		...
		if ( method_exists( $this, 'register_sub_features' ) ) {
			$this->register_sub_features(
				array(
					...
					'lazy_thumbnails'    => __( 'Lazy Load Post Thumbnails', 'wpshadow' ),
					'exclude_first_image' => __( 'Exclude First Content Image', 'wpshadow' ),
				)
			);
	...
	public function test_image_lazy_loading(): array {
		...
		return array(
			...
			'description' => sprintf(
				...
				sprintf(
					__( 'Image Lazy Loading is active with %d element types being lazy loaded.', 'wpshadow' ),
					$enabled_features
				)
			),
			'actions'     => '',
